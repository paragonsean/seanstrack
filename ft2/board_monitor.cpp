...
***********************************************************************/
static bool start_chars_are_equal( const TCHAR * sorc, const char * s )
{
union {
char  c;
TCHAR t;
...
static TCHAR space_equals_space[] = TEXT( " = " );
...
if ( connect_state != new_state )
    ...
    if ( new_state & CONNECTED_STATE )
        ...
        if ( !(old_state & CONNECTED_STATE) )
            ...
            /*
            ---------------------------------------------------------------------------
            If the board is reconnecting after an absence I need to restart the op data
...
if ( do_not_upload && UploadRetractTimeout )
    {
    if ( (u & RetractWireMask) && !(input_bits & RetractWireMask) )
        {
        /*
        ------------------------------------------
...
*               This should only be called once per shot               *
***********************************************************************/
static void save_alarm_summary_counts( FTII_BOARD_ENTRY * board )
{
ALARM_SUMMARY_CLASS as;
bool                check_external_parameters;
PARAMETER_CLASS   * p;
double            * p_value;
...
p_value = board->param_value.p;
if ( !p_value )
    return;

check_external_parameters = false;
if ( board->external_parameter_status.is_enabled && board->external_parameter_status.need_to_check_alarms && !board->external_parameter_status.waiting_for_file )
    check_external_parameters = true;
...
as.set_counts( p_value, p, check_external_parameters );
...
---------------------------------------------------------------------- */
if ( need_to_check_external_parameters || !board->external_parameter_status.is_enabled  )
    {
    ...
    save_alarm_summary_counts( board );
    }
...
/***********************************************************************
*                    EXTERNAL_PARAMETER_ALARMS_INDEX                   *
* This returns the index of the ExternalParameterAlarms for the        *
* machine in question.                                                 *
***********************************************************************/
int external_parameter_alarms_index( STRING_CLASS & ma )
{
int i;

for ( i=0; i<NofExternalParameterAlarms; i++ )
    {
    if ( ExternalParameterAlarms[i].machine_name == ma )
        return i;
    }
...
mi = external_parameter_alarms_index( ma );
if ( mi == NO_INDEX )
    return;
...
if ( ExternalParameterAlarms[mi].n < 1 )
    return;

mybits  = 0;
setbits = 0;

for ( i=0; i<ExternalParameterAlarms[mi].n; i++ )
    {
    pe = p->parameter + ExternalParameterAlarms[mi].index[i];

    if ( pe->input.type == FT_EXTERNAL_PARAMETER )
        {
        mybits |= wirebit(pe->limits[ALARM_MAX].wire_number);
        mybits |= wirebit(pe->limits[WARNING_MAX].wire_number);
        parameter_value = ExternalParameterAlarms[mi].value[i];
        check_one_alarm( board->has_alarm,   setbits, parameter_value, pe->limits[ALARM_MIN].value,   pe->limits[ALARM_MIN].wire_number,   pe->limits[ALARM_MAX].value,   pe->limits[ALARM_MAX].wire_number );
        check_one_alarm( board->has_warning, setbits, parameter_value, pe->limits[WARNING_MIN].value, pe->limits[WARNING_MIN].wire_number, pe->limits[WARNING_MAX].value, pe->limits[WARNING_MAX].wire_number );
        }
    }
...
if ( eps.file_name.file_exists() )
    ...
    /*
    ---------------------------------------------------------
    It is my job to delete this file even if I can't read it.
    --------------------------------------------------------- */
...
if ( dp )
    ...
    /*
    -------------------------------------------------------------------------------
    Move the array of doubles into the param_values array in the FTII_BOARD_ENTRY.
    ------------------------------------------------------------------------------- */
    param_value.copy( MAX_PARMS, dp );
    delete[] dp;
    dp = 0;
    ...
    ------------------------------------------ */
    add_to_shotsave( has_alarm, has_warning, pn, filedata, param_value, external_parameter_status );
...
{
DSTAT_CLASS                       d;
INT_ARRAY_CLASS                   alarm_param_index;
...
int                               mi; /* ExternalParameterAlarms index of this board */
MACHINE_NAME_LIST_CLASS           mlist;
bool                              monitoring_is_on;
STRING_CLASS                      new_machine_name;
int                               nof_external_parameters_w_alarms;
...
if ( !machine_name.isempty() )
    ...
    if ( dest.m.find(ComputerName, machine_name) )
        ...
        /*
        ------------------------------------------------------------
        Count the number of the external parameters that have alarms
        and make a list of the parameter numbers.
        ------------------------------------------------------------ */
        ...
        if ( dest.external_parameter_status.is_enabled && (dest.m.monitor_flags & MA_ALARM_MONITORING_ON) )
            {
            have_external_parameters         = false;
            nof_external_parameters_w_alarms = 0;
            for ( i=0; i<MAX_PARMS; i++ )
                {
                if ( dest.param.parameter[i].input.type == FT_EXTERNAL_PARAMETER )
                    {
                    have_external_parameters = true;
                    if ( dest.param.parameter[i].has_any_wire_limits() )
                        {
                        /*
                        ------------------------------------------------------------
                        Make a list of parameters to copy to ExternalParameterAlarms
                        I can't do that yet because I haven't allocated the
                        ExternalParameterAlarms array for this machine yet.
                        ------------------------------------------------------------ */
                        alarm_param_index[nof_external_parameters_w_alarms] = i;
                        nof_external_parameters_w_alarms++;
                        }
                    }
                }
            ...
            if ( nof_external_parameters_w_alarms > 0 )
                {
                dest.external_parameter_status.need_to_check_alarms = true;
                /*
                --------------------------------------------------------------------------
                The index of the external parameters alarms is the same as the board index
                -------------------------------------------------------------------------- */
                mi = dest.index;
                if ( ExternalParameterAlarms[mi].init(nof_external_parameters_w_alarms) )
                    {
                    /*
                    ---------------------------------------------------------------
                    Copy the parameter indexes that I need to tell board_monitor to
                    check for alarms.
                    --------------------------------------------------------------- */
                    for ( i=0; i<nof_external_parameters_w_alarms; i++ )
                        ExternalParameterAlarms[mi].index[i] = alarm_param_index[i];
                    }
                /*
                -----------------------------------------------------------------
                This completes the initialization of the ExternalParameterAlarms.
                From now on shotsave will write to these and board_monitor will
                read from them.
                ----------------------------------------------------------------- */
                }
...
if ( NofBoards > 0 )
    ...
    /*
    -----------------------------------------------------------------
    Initialize the racetrack that board_monitor uses to tell shotsave
    that there is a new cycle start message for one of the machines.
    ----------------------------------------------------------------- */
    CycleStartRaceTrack.setsize( NofBoards+1 );
...
if ( cp )
    ...
    if ( findstring(TIMEOUT, cp) )
        ...
        if ( board->external_parameter_status.is_enabled )
            {
            if ( board->external_parameter_status.waiting_for_file )
                save_alarm_summary_counts( board );
