...
class STRING_ENTRY
    ...
    ~STRING_ENTRY();
    void acquire( STRING_ENTRY & sorc );
    void empty();
    };
...
class STRING_ARRAY_CLASS
    ...
    void empty();
    int  index() { return current_index; }
    ...
    bool remove();
    void remove_all();
    void rewind() { current = first; current_index = 0; rewound = true; }
    bool split( const TCHAR * separator );
    bool join( const TCHAR * separator );
    bool join() { return join(0); }
    void reverse();
    void strip( TCHAR * charlist );
    void swap( int i1, int i2 );
    void trim(  TCHAR * charlist);

    int     int_value() { if (current->s) return (int) asctoint32(current->s); else return 0; }
    float   float_value() { if (current->s) return extfloat(current->s); else return 0.0; }
    double  real_value() { if (current->s) return extdouble(current->s); else return 0.0; }
    ...
    TCHAR * text() { if (current->s) return current->s; else return emptystring; }
    void uppercase();
