...
#define MAX_TRACE_BYTES	((int)(((int)((2*MAX_DATA_SIZE-2)/stepsize))*stepsize))
...
#if defined nasys_debug_uart && defined nasys_debug_uart_irq
	if (gdb.comlink == ne_gdb_serial)
	{
...
void nios_gdb_install_ethernet (int active)
{
	...
	if (result) 
	{
		nr_uart_txstring ("nr_plugs_create failed 0x");
        nr_uart_txhex (result);
        nr_uart_txcr ();
		gdb.comlink = ne_gdb_serial;
		return;
	}
	...
	if (result)
	{
		nr_uart_txstr ("nr_plugs_connect fialed 0x");
        nr_uart_txhex (result);
        nr_uart_txcr ();
		gdb.comlink = ne_gdb_serial;
		return;
