...
#define FIELDBUS vtg.v[453]
// vtg.v[454] - vtg.v[499] available
...
#define LS7_OUTPUT 38
#define LS8_OUTPUT 39
//#define ?_OUTPUT 40
#define  BINARY_VALVE_OUTPUT 41
...
#define LS7_1_ON (OSW1 |= LS7_1_MASK)
#define LS8_1_ON (OSW1 |= LS8_1_MASK)
#define BINARY_VALVE_ON (OSW1 |= BINARY_VALVE_MASK)
...
#define LS7_1_OFF (OSW1 &= ~LS7_1_MASK)
#define LS8_1_OFF (OSW1 &= ~LS8_1_MASK)
#define BINARY_VALVE_OFF (OSW1 &= ~BINARY_VALVE_MASK)
...
#define ZSPEED_OUTPUT_ON (OSW1 |= ZSPEED_OUT_MASK)

//#define ZSPEED_OUTPUT_OFF (OSW1 &= ~0x00000001)
#define ZSPEED_OUTPUT_OFF (OSW1 &= ~ZSPEED_OUT_MASK)
...
// Actual hi-res position and velocity
unsigned int hi_res_pos; // Position counts from the hi_res sensor if present
unsigned int hi_res_vel; // Velocity in hi-res counts/sec. 
int hi_res_20P_equiv_vel;
int hi_res_20P_equiv_pos;
// 20P equiv. 
//unsigned int hi_res_latched_q1; 
//
//int hi_res_updates_per_servo_loop;
} axis_def;
...
//#define HI_RES_UPDATES_PER_SERVO_LOOP(x) (axis[x].hi_res_updates_per_servo_loop)
...
#define HI_RES_POSITION(x) (axis[x].hi_res_pos)
#define HI_RES_VELOCITY(x) (axis[x].hi_res_vel)
#define HI_RES_20P_EQUIV_VEL(x) (axis[x].hi_res_20P_equiv_vel)
#define HI_RES_20P_EQUIV_POS(x) (axis[x].hi_res_20P_equiv_pos)
//#define HI_RES_LATCHED_Q1(x) (axis[x].hi_res_latched_q1)
...
// If sensor type is hi-res, result will be a 20P equiv.
//#define RD_PV_POS(x) (SENSOR_TYPE(x)==ENCODER ? axis[x].pvu->a0.rd_current_pos : (HI_RES_POSITION(x)/HI_RES_IMPROVEMENT_FACTOR)) // div. by 16 since hires is 16 times finer res.
...
//#define RD_PV_LATCHED_Q1(x) (SENSOR_TYPE(x)==ENCODER ? axis[x].pvu->a7.rd_latched_q1 : HI_RES_LATCHED_Q1(x))
...
//#define RD_PV_VEL(x) (SENSOR_TYPE(x)==ENCODER ? axis[x].pvu->a11.rd_velocity : HI_RES_20P_EQUIV_VEL(x))
...
#define SET_PV_PLOAD(x, preload) \
						...
						if (SENSOR_TYPE(x) == HI_RES) \
							{ \
							/*if (preload == 0) {send_hi_res_reset_request(x);} /* Can only reset hi-res to zero */ \
							if (preload == 0) \
								{ \
								ctr=0; \
								send_hi_res_reset_request(x); /* Make an initial attempt at a reset */ \
								nr_delay(2); \
								while (ctr < 10 && ((int)HI_RES_POSITION(axis_num) > HIRES_RESET_COUNTS || (int)HI_RES_POSITION(axis_num) < (0-HIRES_RESET_COUNTS))) \
									{ \
									send_hi_res_reset_request(x); \
									ctr++; \
									nr_delay(2); \
									} \
								if (ctr < 10) HIRES_DID_RESET=7777; /* We did successfully reset */ \
								else HIRES_DID_RESET=9999; /* Actually we didn't reset, we'll flag it here */ \
								} \
							}
