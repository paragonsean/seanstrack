...
//////////////////////// FIRMWARE VERSION ////////////////////////
#define FIRMWARE_VERSION "emc_6_04," // Change to cyc_6_04
...
5,	When targeting an Apex 20K200E board, the configuration flash file is built with the following command:
	hexout2flash -b 0x180000 -s 0x03f000 emc_chip_11010200.hexout
	
	This will create a file called emc_chip_11010200.hexout.flash
...
#define MAX_SHOT_SAMPLES 3800
//combined_sample *data_sample; 
int num_of_shot_data_samples = MAX_SHOT_SAMPLES; // Determines the max number of data samples we can have
...
int r_get_settings_from_flash(void)
	...
	// We won't worry about the signature.  If updating from V4_14c firmware, signature will be zero.  Everything
	// else should be OK.  We'll just use what's there and not worry about the signature.
...
int initialize_axes(void)
	...
	if(HAVE_REVB_EXT_INT || (!HAVE_REVB_EXT_INT && HAVE_EXTERNAL_AMP))
		{
		ADC_CHANNEL_FOR_LVDT = 17; // WANA_CH1
		ADC_CHANNEL_FOR_COMMAND_MONITORING = 18; // WANA_CH2
		}
	else
		{
		ADC_CHANNEL_FOR_LVDT = 21; // CANA_CH1
		ADC_CHANNEL_FOR_COMMAND_MONITORING = 17; // WANA_CH1
		}
	...
	if(HAVE_REVB_EXT_INT || (!HAVE_REVB_EXT_INT && HAVE_EXTERNAL_AMP))
		{
		ADC_CHANNEL_FOR_LVDT = 19; // WANA_CH3
		ADC_CHANNEL_FOR_COMMAND_MONITORING = 20; // WANA_CH4
		}
	else
		{
		ADC_CHANNEL_FOR_LVDT = 23; // CANA_CH3
		ADC_CHANNEL_FOR_COMMAND_MONITORING = 18; // WANA_CH2
		}
...
void pos_based_monitor_isr(int context)
	...
	if (MONITOR_ENABLE && !(ZSPEED))
		...
		if (HAVE_REVB_EXT_INT) {if (RD_PV_VEL < 4100) TOGGLE_CHANNELS = 1; else TOGGLE_CHANNELS = 0;}
...
void time_based_monitor_isr(int context)
	...
	// Write OSW1 every mS, was finding outputs were changing.  
	...
	// Every 600 ms, we'll send a retrigger pulse to the watchdog timer to keep it alive - if CycloneII.
	#if CYCLONEII
...
void reset_watchdog(int context)
	...
	na_WD_rst_timer->np_timerstatus = 0; // Clear timer int signal.
	na_watchdog->np_timerperiodl = 0x55; // Write anything to periodl to reset watchdog.
...
int main(void)
	...
	load_ad_processor_program(); // Load ad processor reenable this
