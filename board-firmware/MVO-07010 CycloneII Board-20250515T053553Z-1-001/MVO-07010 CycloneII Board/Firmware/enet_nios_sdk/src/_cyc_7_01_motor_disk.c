...
#define BUILD_FOR_MOTOR_DISK 1 // Version for controlling motor disk.  Monitoring is disabled.
///////////////////////////////////////////////////////////

//////////////////////// FIRMWARE VERSION ////////////////////////
#define FIRMWARE_VERSION "cyc_7_01_motor_disk,"  
...
#if BUILD_FOR_CONTROL
	#include "combined_control_blocks7.h" // Both polled and interrupt driven control are supported in the "combined" file.
#endif
...
1, From the SOPC builder command shell
	...
	a, nios2-build -b 0x400000 emc_7_?.c<CR>
	b, nios2-srec2flash  emc_7_?.srec -flash_address=0x140000 -ram_address=0x400000 -copy_size=0x40000<CR>
	c, nios2-run emc_7_?.flash<CR>  To download the flash file.

	For the CycloneII based board:
	a, nios2-build -b 0x2010000 cyc_7_?.c<CR>
	b, nios2-srec2flash  cyc_7_?.srec -flash_address=0x80000 -ram_address=0x2010000 -copy_size=0x80000<CR>
	c, Upload the file over Ethernet using the terminal program (preferable) or nios2-run cyc_7_?.flash<CR>.  
...
3,	This firmware is to run under chip version emc_chip_12010200.hexout.flash or later (Apex 20K200E) or
	cyc_chip_14010201.flash or later (CycloneII).
...
5,	When targeting an Apex 20K200E board, the configuration flash file is built with the following command:
	hexout2flash -b 0x180000 -s 0x03f000 emc_chip_12010200.hexout
	
	This will create a file called emc_chip_12010200.hexout.flash
...
6,	When targeting the CycloneII board with EPCS serial flash, the configuration flash file is built with the following command:
	sof2flash --input=cyc_chip_14010201.sof --epcs<CR>
	
	This will create a file called cyc_chip_14010201.flash.  This file must then be manually edited and the following
...
#if CYCLONEII // Could be a different tb file based on the board we're compiling for
	#include "tb9.c"
#else
	#include "tb9.c"
#endif
...
int initialize_axes(void)
	...
	convert_uS_to_timer(SAMPLE_TIME_SETTING1, (unsigned int *)&SAMPLE_TIME);
	...
	na_ls_int_input->np_piointerruptmask = 0x3f; // Enable limit switches
...
void time_based_monitor_isr(int context)
	...
	if (PAST_MIN_STK_LEN) // Prep for time based data sample
		...
		if ((tb_ctr==(TIM_COLL_SAMPLES+1)) || ((pb_ctr+tb_ctr)==MAX_SHOT_SAMPLES)) // Shot's over, prep for end of data collection
			...
			if (!(ZSPEED))
				...
				//if (RD_PV_RAW_VEL_CTR(axis_num) > (vtg.last_pb_time + vtg.counts_to_assume_zspeed))
	...
	halfword_ptr[0] = ~na_opto_data->np_piodata; // Low half word of ISW1
	...
	halfword_ptr[1] = ~na_opto_data->np_piodata; // Upper half word of ISW1
	//printf("ISW1=0x%x\n", ISW1); // For test
...
void send_op_packets(void *context)
	...
	if (!vtg.xmitting_shot_data) // Don't send op packets if shot data is being sent
		...
		if (SENSOR_TYPE(axis_num) == ENCODER)
			{
			op_data_packet.pos = RD_PV_POS(axis_num);
			op_data_packet.vel = (axis[axis_num].vel_ctr_freq/RD_PV_VEL(axis_num)); // Vel in pps
			}
		else if (SENSOR_TYPE(axis_num) == HI_RES)
			{
			//op_data_packet.pos = HI_RES_POSITION(axis_num); // So we can see the actual count value on the realtime screen
			//op_data_packet.vel = HI_RES_VELOCITY(axis_num); // Hi-res pps
			op_data_packet.pos = HI_RES_20P_EQUIV_POS(axis_num); // Calculated by the hi-res isr
			op_data_packet.vel = HI_RES_20P_EQUIV_VEL(axis_num); //   "      "     "     "    "
			}
		//op_data_packet.pos = RD_PV_POS(axis_num); 
...
#include "combined_control_blocks7.c" // Both polled and interrupt driven control are supported in the "combined" file.
...
int parse_control_program_command(unsigned char *orig_cmd_str)
	...
	switch(*ptr)
		...
		case 'h':
			...
			if ((int)g.spc!=9999) stop_control(); // Only if not serial mode
			//printf(".H command \n"); // For test
		...
		case 'g':
			...
			if ((int)g.spc!=9999) reset_control_program(CLEAR_SAVED_WARNING); // Only if not serial mode
			break;
			
		case 'R':
		case 'r':
			// Reload control program from flash
			if ((int)g.spc!=9999) reload_cprog_from_flash(); // Only if not serial mode
			break;
		...
		default: // Probably a line of control code to compile
			...
			if (line_num == IMMED_EXECUTE_BLOCK)
				{
				if ((int)g.spc!=9999) // Make sure we're not in serial mode
					{
...
void update_control_parameters(void)
	...
	VEL_LOOP_INT_GAIN = VEL_LOOP_INTEGRAL_GN_SETTING1; 
	...
	VEL_LOOP_INT_GAIN = VEL_LOOP_INTEGRAL_GN_SETTING2;
	...
	convert_uS_to_timer(SAMPLE_TIME_SETTING2, (unsigned int *)&SAMPLE_TIME);
...
int null_valve(int axis_num, int action)
	...
	if (axis_num < 1 || axis_num > 2)
		...
		out_dac(DAC_OUTPUT_CHANNEL(axis_num), 0-LVDT_OFFSET, VOLTAGE_X_1000, ABSOLUTE);
	//printf("outputting %d volts, dac channel = %d\n", 0-LVDT_OFFSET, DAC_OUTPUT_CHANNEL(axis_num));
		// Then get null value
	...
	//printf("Null voltage = %d (0x%x)\n", null_voltage, null_voltage);	
		...
		if (null_voltage > 500 || null_voltage < -500) // Too far gone
			...
			for (ctr=0; ctr<200; ctr++) 
				...
				out_dac(DAC_OUTPUT_CHANNEL(axis_num), NULL_OFFSET-LVDT_OFFSET, VOLTAGE_X_1000, ABSOLUTE);
...
int main(void)
	...
	unsigned int chk_ethernet_io_time=0, servo_here_time=0;
	...
	#else
		else if(!HAVE_REVB_EXT_INT && (AD_STATUS==!0x9999))
	#endif
		{
...
if (no_ext_int)
	...
	while (1)
		...
		if (SW8) {display_ip(); seven_seg_interface(seven_seg_E,seven_seg_o);} // Scroll IP address if button pressed
		}
	}
...
////////// Periodic non-precision events based on mS counter cur_time can be placed here
	...
	//	else out_dac(1, 0, VOLTAGE_X_1000, ABSOLUTE);
		...
		//printf("%d ", HI_RES_POSITION(1)); // For test
		//}
