...
2-25-04 emc_4_0 Made A/D setups in on-chip shared memory.
  ...
  6-1-07 emc_6_55 / emc_7_0.c
	...
	5-17-10 cyc_7_53.c / cyc_8_01.c
	Added fieldbus / DeviceNet support.  Elminated hires support.
	*/
...
//////////////////////// FIRMWARE VERSION ////////////////////////
#define FIRMWARE_VERSION "cyc_8_01,"  
...
#if BUILD_FOR_CONTROL
	#include "combined_control_blocks11.h" // Both polled and interrupt driven control are supported in the "combined" file.
#endif
...
int fieldbus_xmit_ctr;
int devicenet_connection;
int pending_fieldbus_write;
unsigned char fieldbus_arr[10]; // For transmitting OSW2 and OSW3
} vtw_globals;
...
int wr_word_to_uart2_fieldbus(unsigned char message, unsigned int *word);
...
///////////////////////////////// Fieldbus defines and globals /////////////////////////
unsigned int prev_osw2, prev_osw3;
...
int parse_operational_command(unsigned char *orig_cmd_str)
	...
	int ch_num, val, return_val=0, incremental=0, axis_num=1, x, result_type, tempi=0, ctr=0;
		...
		case 'o': // Set or get OSW1 - OSW3
			...
			if (*ptr == '=') // We're going to set the OSW
				...
				if (*ptr == '~') // Clear bits
					...
					else if (ch_num == 2) OSW2 &= ~uval; 
					else if (ch_num == 3) OSW3 &= ~uval; 
...
int initialize_axes(void)
	...
	if (SENSOR_TYPE(1) == HI_RES) { } // Do nothing
...
void time_based_monitor_isr(int context)
	...
	// Either read Opto inputs and update ISW1 or map ISW2/3 to ISW1.  ISW2/3 are not read here as is ISW1.  
	if (UART2_FIELDBUS)
		{
		// A priority scheme is set up here to prepare items for transmit to the fieldbus.
		// No more than one item will be sent per 1mS loop.  Highest priority begins with OSW2 and goes down from there.
		// Only the highest priority item will be sent.
		if (vtg.pending_fieldbus_write != 0x5555) // Proceed if there are no pending writes, otherwise we'll keep coming back here until the previous write clears
			{
			if (OSW3 == prev_osw3) // 1st. priority, control outputs
				{
				if (OSW2 == prev_osw2) // 2nd. priority, monitor outputs
					{
					// Can add more lower priority items here for future expansion
					} 
				else {prev_osw2 = OSW2; wr_word_to_uart2_fieldbus(0x80, &OSW2);}  // OSW2 changed, xmit new value to the fieldbus, this is the 2nd. highest priority 
				}
			else {prev_osw3 = OSW3; wr_word_to_uart2_fieldbus(0x81, &OSW3);}  // OSW3 changed, xmit new value to the fieldbus, this is the highest priority 
			}
			
		if (MAP_TO_ISW1)
			{
			ISW1 = ISW2 & 0x0000ffff; // Lower 16 bits of OSW2 are mapped into the lower 16 bits of OSW1, these are the monitor inputs.
			ISW1 = ISW1 | (ISW3 << 16); // Now move the lower 16 bits of OSW3 to the upper 16 bits of OSW1, these are the control inputs.
			}
		else {READ_ISW1;} // We have an Opto I/O board and an independent fieldbus operating side by side at the same time, we'll now read the Opto board
		
		// We can copy OSW1 values to OSW2 and OSW3 or let them stand on their own.
		if (MAP_FROM_OSW1)
			{
			OSW2 = OSW1 & 0x0000ffff;         // Monitor outputs, copy lower 16 bits OSW1, the monitor outputs to OSW2
			OSW3 = (OSW1 & 0xffff0000) >> 16;   // Control outputs, copy upper 16 bits of OSW1, the control outputs to OSW3
			if (ECHO_OSW1_TO_OPTO) {WRITE_OSW1;}
			}
		else {WRITE_OSW1;}	// We have an Opto I/O board and an independent fieldbus operating side by side at the same time, we'll now write the Opto board
		}
	else {WRITE_OSW1; READ_ISW1;} // No fieldbus, then read the Opto I/O board	
...
#include "combined_control_blocks11.c" // Both polled and interrupt driven control are supported in the "combined" file.
...
unsigned field1:7;
unsigned garbage_bit1:1; // MSBs not used	
unsigned field2:7;
unsigned garbage_bit2:1;
unsigned field3:7;
unsigned garbage_bit3:1;
} isw_receive_bit_fields;


// Uart2 recieve isr, eventually make part of fieldbus.c above
void uart2_isr(int context)
	{
	static int ctr, length_is_correct, run;
	static unsigned char rec_record[6]; 
	unsigned char byte, *byteptr;
	fieldbus_bit_fields *isw_bit_fields;
	isw_receive_bit_fields *received_message;
	PL;
	...
	if (na_uart2->np_uartstatus & np_uartstatus_rrdy_mask) // Receive character
		...
		if (byte == 13) // CR
			{
			//printf("byte = 13\n");
			if (ctr == 5) // Only a CR in the 5th. position will be considered valid
				{
				received_message = (isw_receive_bit_fields *)&rec_record;
				if (received_message->message_type == 0x80) isw_bit_fields = (fieldbus_bit_fields *)&ISW2;      // Message type == 0x80 == ISW2
				else if (received_message->message_type == 0x81) isw_bit_fields = (fieldbus_bit_fields *)&ISW3;  // Message type == 0x81 == ISW3
				else goto chk_xmit;  // Bad message type, ignore
				
				isw_bit_fields->field1 = received_message->field1; //printf("F1=0x%x\n", isw_bit_fields->field1);
				isw_bit_fields->field2 = received_message->field2; //printf("F2=0x%x\n", isw_bit_fields->field2);
				isw_bit_fields->field3 = received_message->field3; //printf("F3=0x%x\n", isw_bit_fields->field3);
				isw_bit_fields->leftover = 0;
				
				if (vtg.devicenet_connection == 0)
					{
					vtg.devicenet_connection = 1; // Flag that we've recieved a poll message and we're talking to the scanner
					
					// Set these to an unlikely pattern to force an OSW2, OSW3 update onto the fieldbus
					// now that we have a connection.
					prev_osw2 = 0x55555555; 
					prev_osw3 = 0x55555555; 
					}
					
				// For test
				//if (received_message->message_type == 0x80) printf("ISW2 = 0x%x\n", ISW2);
				//else if (received_message->message_type == 0x81) printf("ISW3 = 0x%x\n", ISW3);
				}
			ctr = 0;	
			}
		else 
			{
			//printf("byte in the else = %d\n", byte);
			if (ctr >= 6) ctr = 0; // If we get to a count of 6 without a CR, start over
			else {rec_record[ctr-1] = byte;}
			}			
		}	
	
chk_xmit: ;	
	// Transmit routing.  The first 2 bytes of vtg.fieldbus_arr[] are transmitted from wr_osw2_osw3_to_devicenet() below.
	// The rest will be transmitted here when the xmit ready interrupt goes active.
	if (na_uart2->np_uartstatus & np_uartstatus_trdy_mask) // Transmit character
		{
		if (na_uart2->np_uartcontrol & np_uartstatus_trdy_mask) // Transmit ready interrupt enabled? 
			{
			if (vtg.fieldbus_xmit_ctr < 5) {PRINT_CHAR_UART2(vtg.fieldbus_arr[vtg.fieldbus_xmit_ctr++]);}
			else if (vtg.fieldbus_xmit_ctr == 5)
				{
				if (vtg.fieldbus_arr[vtg.fieldbus_xmit_ctr] == 0x81) // Keep going, we need to xmit osw3
					{
					PRINT_CHAR_UART2(vtg.fieldbus_arr[vtg.fieldbus_xmit_ctr++]);
					}
				else  // Nothing there, Xmit complete
					{
					na_uart2->np_uartcontrol = na_uart2->np_uartcontrol & ~np_uartstatus_trdy_mask; // Disable the xmit interrupt
					vtg.pending_fieldbus_write = 0;
					}
				}
			else if (vtg.fieldbus_xmit_ctr < 10) {PRINT_CHAR_UART2(vtg.fieldbus_arr[vtg.fieldbus_xmit_ctr++]);}
			else  // Xmit complete
				{
				na_uart2->np_uartcontrol = na_uart2->np_uartcontrol & ~np_uartstatus_trdy_mask; // Disable the xmit interrupt
				vtg.pending_fieldbus_write = 0;
...
// Function to initiate the transmission of OSW2 and OSW3.  Only 21 bits out of the 32 bit word are usable for both OSW2 and OSW3.	
int wr_word_to_uart2_fieldbus(unsigned char message, unsigned int *word)
	{
	//static unsigned int prev_osw2, prev_osw3;
	fieldbus_bit_fields *word_fields;
	int ctr;
	PL;
	
	//printf("About to service OSW2 and 3, prnding write = 0x%x \n", vtg.pending_fieldbus_write);
	if (vtg.pending_fieldbus_write != 0x5555) // No pending write operations
		{
		vtg.pending_fieldbus_write = 0x5555; // Flag function as in operation to avoid recursion
		for (ctr=0; ctr <10; ctr++) vtg.fieldbus_arr[ctr] = 0; // Blank out fieldbus string	
		ctr = 0;
		//printf("*"); // For test
		
		word_fields = (fieldbus_bit_fields *)word;
		
		word_fields->leftover = 0; // Only 21 bits are used, clear upper 11 bits 
		vtg.fieldbus_arr[ctr++] = message;  // Message type, 0x80 == OSW2, 0x81 == OSW3
		vtg.fieldbus_arr[ctr++] = word_fields->field1 | 0x80; // Load 7 lowest bits and set the MSB
		vtg.fieldbus_arr[ctr++] = word_fields->field2 | 0x80; // Load 7 middle bits and set the MSB
		vtg.fieldbus_arr[ctr++] = word_fields->field3 | 0x80; // Load 7 highest bits and set the MSB
		vtg.fieldbus_arr[ctr++] = 13; // Add in the terminating character
		
		// For test
		//printf("vtg.fieldbus_arr[0] = 0x%x\n", vtg.fieldbus_arr[0]);
		//printf("vtg.fieldbus_arr[1] = 0x%x\n", vtg.fieldbus_arr[1]);
		//printf("vtg.fieldbus_arr[2] = 0x%x\n", vtg.fieldbus_arr[2]);
		//printf("vtg.fieldbus_arr[3] = 0x%x\n", vtg.fieldbus_arr[3]);
		//printf("vtg.fieldbus_arr[4] = 0x%x\n", vtg.fieldbus_arr[4]);
					
		if (vtg.devicenet_connection)  // Don't transmit unless we've received something from the DeviceNet master
			{
			// Begin by quickly sending 2 bytes to the transmit buffer.  After this, we'll have to use the xmit ready interrupt, 
			// otherwise we'll eat gobs of time waiting for the bytes to transmit.
			if (ctr != 0) // We've got something to xmit
				{
				PRINT_CHAR_UART2(vtg.fieldbus_arr[0]); // Loads the xmit buffer
				PRINT_CHAR_UART2(vtg.fieldbus_arr[1]); // Loads the xmit holding buffer, after this, we can't load anything else until the xmit buffer clears.
				
				vtg.fieldbus_xmit_ctr = 2; // For xmit interrupt routine.  Bytes 0 and 1 have been sent above thus the preset to 2.
				
				////////////// Enable UART2 xmit ready interrupt  ///////////////
				na_uart2->np_uartcontrol = na_uart2->np_uartcontrol | np_uartstatus_trdy_mask; 
				}
			else vtg.pending_fieldbus_write = 0;	
			}
		else vtg.pending_fieldbus_write = 0;	
		}
	}	
...
int main(void)
	...
	unsigned int look_for_at_home_time=0, service_samp_time=0, test_time=0, uart2_test_time=0;
	...
	static unsigned char test_message[5] = {0x55, 0x11, 0x22, 0x33, 0x44}; // For test
	//net_48 base_enet, cur_enet;			// For test
	...
	if (UART2_FIELDBUS) // Are we using a UART2 based fieldbus connection?
		{
		//na_uart2->np_uartdivisor = 3471; // Clk freq/baudrate - 1 == (33.3333MHz/9600)-1 == 3471  9600 baud
		na_uart2->np_uartdivisor = UART2_BAUD_RATE; // Clk freq/baudrate - 1 == (33.3333MHz/38.4K)-1 == 867 for 38.4K, 3471 for 9600
		nr_installuserisr(na_uart2_irq, uart2_isr, 0);
		vtg.devicenet_connection = 0;
		}
...
#define XMIT_UART2_TEST_TIME 1000
...
while (1)
	...
	if(first_loop) // Set up the various times to check stuff
		...
		uart2_test_time=cur_time+XMIT_UART2_TEST_TIME;
...
////////// Periodic non-precision events based on mS counter cur_time can be placed here
	...
	#if 0	
	// For testing UART transmitting	
	if(cur_time>=uart2_test_time)
		{
		uart2_test_time=cur_time+XMIT_UART2_TEST_TIME;
		uctr++;
		if(uctr==4) uctr=0;
