...
4-18-06 combined_control_blocks4.c
...
int dwell_block_compile(char *str_to_compile)
	...
	switch (*ptr) 
		...
		case 'V': //Variable
			ptr++;
			if ((tempi = atoi((char *)ptr)) > MAX_VAR) {return_val = 1; goto clr_id;}
	...
	if (zone == 1) // We're executing the block from the beginning.
		...
		if (cond_result < 10 && cond_result > 0) // Some sort of executive error took place.
			...
			if (block->db_indicator_bits & db_const_mask)
				...
				{
				return_val = BLOCK_EXECUTION_ERROR; // dwell_time_mS already defaulted to zero
				}	
...
///////////////////////////////// Pressure control ////////////////////////////////////////////
int pressure_block_compile(char *str_to_compile)
	{
	xb *pressure_block=0;
	int return_val = 0, stepnum, x, tempi, item_being_scanned, immed_exe=0;
	volatile char *ptr=0;
	...
	if ((ptr = (char *)strchr(str_to_compile, '#')) != NULL) *ptr = 0; 
		...
		printf("Comment has been stripped - see %s\n", str_to_compile);
	#endif	
	if (str_to_compile[0] == 'X') //Command for immediate execution, first compile.
		{
		immed_exe=1;	
		pressure_block = (xb *)&vtg.control_program[stepnum=IMMED_EXECUTE_BLOCK][0]; 
		}
	else 
		//Compile and save step to SRAM, get step number
		{
		...
		stepnum = atoi(str_to_compile); 
		if (stepnum > MAX_STEP || stepnum < 1) {return_val = 1; goto ext;} //Error 
		pressure_block = (xb *)&vtg.control_program[stepnum][0];
		}
	...
	// Set the block ID, after this, any aborts must goto clr_id
	// to clear the block ID before exiting.
	pressure_block->blk_id = PRESSURE;
	...
	if ((ptr = (char *)strchr(str_to_compile, '_')) == NULL); // None found
	else 
	   {
		...
		// same.  The update block structure in this case is being used as a 
		// template that is good for all block types.
		return_val = block_conditional_compile((c_block)pressure_block, (char *)ptr+1);
		if (return_val != 0) goto clr_id; // Error in conditional string
		*ptr = 0; 
		} 
	...
	// A for loop will loop between the 3 possible choices of items in
	// a pressure block, ramp, pressure and ending time.
	// The cases in the switch statement are the possible data
	// types the scanned items can be, only constants and variables
	// in this case.  
	for (item_being_scanned=XB_PRESS; item_being_scanned<=XB_ET; item_being_scanned++)
		{
		#if PRESS_COMPILE_DEBUG	
			printf("item_being_scanned = %d\n", item_being_scanned);
		#endif	
		// Move ptr to point to correct part of string based on item being scanned
		if (item_being_scanned == XB_PRESS)
			{
			if ((ptr = (char *)strchr(str_to_compile, 'X')) == NULL) {return_val = 1; goto clr_id;}	
			else
				{
				ptr+=2;	
				if ((*ptr > NUM_OF_AXES+48) || (*ptr < 49)) {return_val = 1; goto clr_id;} // Axis# out of range	
				else if (*ptr == '1') pressure_block->xb_indicator_bits |= xb_axis1_mask;
				else if (*ptr == '2') pressure_block->xb_indicator_bits |= xb_axis2_mask;
				else if (*ptr == '3') pressure_block->xb_indicator_bits |= xb_axis3_mask;
				else if (*ptr == '4') pressure_block->xb_indicator_bits |= xb_axis4_mask;
				#if PRESS_COMPILE_DEBUG
					printf("Axis number is %c\n", *ptr);	
				#endif	
				ptr++; // Move past the axis specifier
		...
		else if (item_being_scanned==XB_RAMP)
			{
			if ((ptr = (char *)strstr(str_to_compile, "R")) == NULL) continue; // No ramp to process, loop again 
			else ptr++;
			#if PRESS_COMPILE_DEBUG	
				printf("found a R %s\n", ptr-1);
			#endif	
			}
		else if (item_being_scanned==XB_ET)
			{
			if ((ptr = (char *)strchr(str_to_compile, 'T')) == NULL) break; // No ending time to process 
			else ptr++;
			#if PRESS_COMPILE_DEBUG	
				printf("found a T %s\n", ptr-1);
			#endif	
		...
		switch (*ptr) 
			...
			case 'H':
				...
				if (item_being_scanned == XB_PRESS || item_being_scanned == XB_RAMP)
					{
					// Pressure and ramp cannot be neg. 
					if (*ptr == '-') {return_val = 1; goto clr_id;}
					else
						{
						if (item_being_scanned == XB_PRESS)
							{
							pressure_block->xb_indicator_bits |= xb_press_const_mask;	
							pressure_block->press_df = tempi;
							}
						else
							{
							pressure_block->xb_indicator_bits |= xb_ramp_const_mask;	
							pressure_block->ramp_df = tempi;	
							}
						}
					}
				else if (item_being_scanned == XB_ET)
					{
					if (tempi > 8388607) tempi = 8388607;
					else if (tempi < 0) tempi = 0;
					pressure_block->xb_indicator_bits |= xb_endtime_const_mask;
					pressure_block->endtime_df = tempi;	
					}	
				break;
			...
			case 'V': //Variable
				...
				if (item_being_scanned == XB_PRESS)
					{
					pressure_block->xb_indicator_bits |= xb_press_var_mask;
					pressure_block->press_df = tempi;
					}
				else if (item_being_scanned == XB_RAMP)
					{
					pressure_block->xb_indicator_bits |= xb_ramp_var_mask;
					pressure_block->ramp_df = tempi; 	
					}
				else if (item_being_scanned == XB_ET)
					{
					pressure_block->xb_indicator_bits |= xb_endtime_var_mask;
					pressure_block->endtime_df = tempi; 	
					}		
				break;
	...
	if (immed_exe && return_val==0)
		...
		if (!PROGRAM_RUNNING)
			{
			// Execute the block we just compiled. 
			pressure_block_exe((c_block)pressure_block, IMMED, 1); 
			}
		}
...
int pressure_block_exe(c_block control_block, int type_of_execution, int zone)
	{
	...
	int next_time;
	//unsigned int initial_timer_mS, send_op_time;
	unsigned int tempu=0, loop_time, temp_pressure_setting;
	unsigned int initial_timer_mS=0; // For polled control
	volatile unsigned short int *halfword_ptr;
	np_pio *control_int = na_control_int;
	np_pio *cont_int_output = na_cont_int_output;
	...
	if (zone == 1) // Initial entry of block
		...
		//#if PRESS_EXE_DEBUG
		//	printf("press blk exe-entry to zone 1, block #%d\n", vtg.prog_ctr);
		//#endif
		if (PRESS_CONTROL_ENABLED)
			...
			if (cond_result < 10 && cond_result > 0) // Some sort of executive error took place.
				...
				// Check if closed loop or open loop
				if (block->xb_indicator_bits & xb_press_var_mask)
					temp_pressure_setting = vtg.v[block->press_df];
				else if (block->xb_indicator_bits & xb_press_const_mask)
					temp_pressure_setting = block->press_df;
				else temp_pressure_setting=0x7fff; // Skip code
				...
				if (commanded_press == 0x7fff) goto ext; // Skip entire block without any test inputs/set outputs.
				...
				if (block->xb_indicator_bits & xb_ramp_var_mask)
					tempu = vtg.v[block->ramp_df];
				else if (block->xb_indicator_bits & xb_ramp_const_mask)
					tempu = block->ramp_df;
				else tempu=0;
				...
				if (tempu>0)
					...
					if (tempu & 0x80000000) 
						{
						#if PRESS_EXE_DEBUG
							printf("Pressure goose detected!!!\n");
						#endif
   						goose = tempu & 0x0000ffff; // Goose value in volts X 1000, always assumed to be positive.
						}
					else
				...
				// Get ending time
				if (block->xb_indicator_bits & xb_endtime_var_mask)
					tempu = vtg.v[block->endtime_df];
				else if (block->xb_indicator_bits & xb_endtime_const_mask)
					tempu = block->endtime_df;
				else tempu=10000; // No ending time specified, default it to 10 seconds
				time = tempu;

				if (block->xb_indicator_bits & xb_axis2_mask) axis_num = 2;
				else axis_num = 1;
				///////////////////////////////////////////////////////	
	...
	#if 0
	if (zone == 2) // Wait for pressure to rise to the point where control will take over
		{
		if (first_loop)
			{
			#if PRESS_EXE_DEBUG
				printf("press blk exe-zone 2, waiting for initial pressure to rise\n");
			#endif
			first_loop=0;
	...
	chk_pressure: ;
		vtg.timer2_timeout = 0;
		convert_ana_ch_from_dynamic(1, VOLTAGE_X_1000, &act_press); // Pressure in volts x 1000
		if (act_press >= PRESS_TO_BEGIN_CONTROL)
			{
			...
			first_loop = 1;
			// Pressure for control reached, determine what zone to go to next
			if (closed_loop)
				{
				if (goose) zone = 3;
				else if (ramp) zone = 4; 
	...
	if (zone == 3) // Goose
		...
		// Old goose code
			#if PRESS_EXE_DEBUG
				printf("press blk exe-zone 2, Goose\n");
			#endif
		
			#if BUILD_FOR_POLLED_CONTROL
				#include "PRESSURE_LOOP_EXIT_CONDITIONS.H" // Check if we need to abort for some reason if polled operation
			#endif
		
     		out_dac(DAC_OUTPUT_CHANNEL, goose, PRESS_CMD, ABSOLUTE); // Goose it here
	...
	ramp_loop: ;
		...
		out_dac(DAC_OUTPUT_CHANNEL, temp_dac_command>>10, PRESS_RAW, INCREMENTAL);
	...
	press_loop: ;
		...
		out_dac(DAC_OUTPUT_CHANNEL, temp_dac_command, PRESS_RAW, INCREMENTAL);
	...
	ol_ramp_loop: ;
		vtg.timer2_timeout = 0;
		//out_dac(DAC_OUTPUT_CHANNEL, dac_increment, PRESS_RAW, INCREMENTAL);
		current_raw_dac += dac_increment;
		out_dac(DAC_OUTPUT_CHANNEL, current_raw_dac>>10, PRESS_RAW, ABSOLUTE);
	...
	if (zone == 7) // Open loop control
		...
		servo_amp_interface(CLOSE_PRESSURE_FEEDBACK2);
		out_dac(DAC_OUTPUT_CHANNEL, commanded_raw_dac, PRESS_RAW, ABSOLUTE);
...
int jump_block_compile(char *str_to_compile)
	...
	if ((ptr = (char *)strchr(str_to_compile, 'J')) == NULL) {return_val = 1; goto clr_id;}	
		...
		switch (*ptr) 
			...
			case '9':
				// First get integer and save to tempi
				if ((tempi = atoi((char *)ptr)) > MAX_STEP) {return_val = 1; goto clr_id;}
		...
		variable: ;
			...
			case 'C': //Command
				...
				if (item_being_scanned == OP1 || item_being_scanned == OP2)
					...
					{
					ptr++;
					if ((tempi = atoi((char *)ptr)) > MAX_CMD) {return_val = 1; goto clr_id;}	
					...
					if (item_being_scanned == ITU)
						...
						if (*ptr == 'A') //Accel specified
							...
							if (*ptr == 'V') //Variable
								...
								if (*ptr == '+' || (*ptr >= '0' && *ptr <= '9')) 
									update_block->vramp_df = atoi((char *)ptr);
			...
			case 'N': //Analog input channel
				...
				if (item_being_scanned == ITU || item_being_scanned == OP2)
					...
					// Dyn. channels 1 - 8 or 17 - 23 are acceptable
					if (tempi<1 || (tempi>8 && tempi<17) || tempi>MAX_ADC_CH) {return_val = 1; goto clr_id;}
	...
	if (*ptr == 'I' || *ptr == 'W')
		...
		if (*ptr == '1' || *ptr == '0') //Basic form conditional check
			...
			if (*ptr == 'V')
				...
				x = atoi((char *)ptr); if (x<0 || x>MAX_VAR) {return_val = 1; goto ext;} //Variable number out of range
				blkptr->cond_op1_df = (unsigned short int)x;
				}
			else 
				{
				// discrete_io_indicator already set
				x = atoi((char *)ptr); if (x<0 || x>MAX_DISCRETE_IO_NUM) {return_val = 1; goto ext;} //I/O number out of range
				blkptr->cond_op1_df = (unsigned short int)x;	
				}
			}
			...
			switch (*ptr) //1st. operand
				...
				case 'N': //Analog input channel
					...
					// Dyn. channels 1 - 8 or channels 17 -23 are acceptable
					if (x<1 || (x>8 && x<17) || x>MAX_ADC_CH) {return_val = 1; goto ext;}
			...
			if (*ptr == '+' || *ptr == '-' || (*ptr >= '0' && *ptr <= '9')) 
				...
				blkptr->cond_op2_df = atoi((char *)ptr);
				#if COND_COMPILE_DEBUG
