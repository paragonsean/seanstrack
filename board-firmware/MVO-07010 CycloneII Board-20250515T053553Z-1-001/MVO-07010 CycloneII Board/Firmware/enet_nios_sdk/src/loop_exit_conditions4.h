// loop_exit_conditions4.h  For use with combined polled/interrupt control code
	    ...
	    temp_pos = RD_PV_POS(axis_num);   // Freeze current position

		if (temp_pos >= MIN_STROKE_LEN)
			{
			if (ZSPEED)
			//if ((temp_pos-prev_pos) <= delta_pos_zspeed) // Moving slower than EOS velocity?
				{ 
		...
		prev_pos = temp_pos;
			...
			if (input_state == OFF)
				{
				#if EXT_LOOP_DEBUG
					printf("[loop_exit_conditions4] Exiting, master enable dropped out\n");
				#endif		
			...
			if (cond_result == KEEP_GOING_CALL_BACK_DURING)
				...
				if (vac_wait && !vac_on); // Skip conditional checking
					...
					if (block_conditional_exe(control_block) == KEEP_GOING_CALL_BACK_DURING);		
						...
						{
						#if EXT_LOOP_DEBUG
							printf("[loop_exit_conditions4] Exiting, cond call back during failed\n");
						#endif		
						do_set_output=1;
			...
			if (position)
				...
				if (vac_wait && vac_on)  // Vacuum shot 
					...
					if (temp_pos > position+240)
						{
						#if EXT_LOOP_DEBUG
							printf("[loop_exit_conditions4] Exiting, cyl. exceeded EP+240\n");
						#endif	
					...
					if (vac_wait && fill_test && !vac_on)
						{
						#if EXT_LOOP_DEBUG
							printf("[loop_exit_conditions4] Exiting, fill time abort\n");
						#endif		
						...
						{
						#if EXT_LOOP_DEBUG
							printf("[loop_exit_conditions4] Exiting, position met, actpos=%d, EP setpt = %d\n", RD_PV_POS(axis_num), position);
						#endif	
						do_set_output=1;	
						goto ext;
						}
