...
2-25-04 emc_4_0 Made A/D setups in on-chip shared memory.
  ...
  6-1-07 emc_6_55 / emc_7_0.c
	...
	3-4-08 emc_7_03.c  Added the ability to reverse both osw1 and iswi polarity.  V314 config word2 bits 10 and 11 control this.
...
//////////////////////// FIRMWARE VERSION ////////////////////////
#define FIRMWARE_VERSION "cyc_7_03,"  
...
#if BUILD_FOR_CONTROL
	#include "combined_control_blocks8.h" // Both polled and interrupt driven control are supported in the "combined" file.
#endif
...
#if 0 // New version to allow any LS to arm pressure control.
void ls_service_isr(int context)
	{
	//int axis_num=2;
	unsigned char ls_status, ls_edge_capture;
	//static unsigned char prev_ls_status;
	static int ls1_set, ls2_set, ls3_set, ls4_set, ls5_set, ls6_set, x;
	int arm_press=0, ok_to_enbl=0;
	PL;

	ls_edge_capture = na_ls_int_input->np_pioedgecapture;
...
void ls_service_isr(int context)
	{
	//int axis_num=2;
	unsigned char ls_edge_capture;
	//static unsigned char prev_ls_status;
	static int ls1_set, ls2_set, ls3_set, ls4_set, ls5_set, ls6_set, x;
	int arm_press=0, ok_to_enbl=0, cur_pos;
	PL;
...
void time_based_monitor_isr(int context)
	...
	#if 0
	// Service limit switches
	pos = RD_PV_POS(1);
	if ((PRESS_CONTROL_ENABLED) && (SHOT_IN_MOTION)) ok_to_enbl=1;
	
	if (pos>=LS1_1_SETPOINT)
		{
		LS1_1_ON;
		if (ok_to_enbl && LS1_ARM_PRESS) arm_press=1;
		}
...
#include "combined_control_blocks8.c" // Both polled and interrupt driven control are supported in the "combined" file.
