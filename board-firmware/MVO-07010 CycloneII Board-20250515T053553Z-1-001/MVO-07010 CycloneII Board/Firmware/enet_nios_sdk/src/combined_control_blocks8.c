...
int velocity_block(c_block control_block, int type_of_execution, int zone)
	...
	if (zone == 6) // Open loop control
		...
		if (first_loop)  // There's only one loop here
			...
			//printf("[*%d]", DAC_OUTPUT_CHANNEL(1));
			//printf("[%d, %d] ", RD_PV_POS(axis_num), commanded_raw_dac);
			}
	...
	if (zone == 8) // Ending position reached
		...
		//printf("[EP %d]", DAC_OUTPUT_CHANNEL(1));	
		}
...
int pressure_block(c_block control_block, int type_of_execution, int zone)
	...
	if (zone == 1) // Initial entry of block
		...
		if (PRESS_CONTROL_ENABLED)
			...
			if (cond_result < 10 && cond_result > 0) // Some sort of executive error took place.
				...
				#if PRESS_EXE_DEBUG
					printf("temp_pressure_setting & 0x80000000==TRUE_this is an hardware software closed loop step\n");
				#endif	
				...
				if (tempu)
					...
					if (tempu & 0x80000000) 
						{
						if (tempu & 0x40000000) open_loop=1; // An open loop percentage was entered, open loop_may already be set
						goose = tempu & 0x0000ffff; // Goose value in volts X 1000, always assumed to be positive.
						#if PRESS_EXE_DEBUG
			...
			chk_pressure_goose: ;
				...
				if ((going_higher && (act_press >= (int)commanded_press)) || (!going_higher && (act_press <= (int)commanded_press)))
					...
					if (soft_cl)
						{
						#if PRESS_EXE_DEBUG
							printf("Goose exit condition met, moving on to software closed loop zone 5\n");
						#endif
						zone = 5;
						}
					else  // Hardware closed loop
						{
						#if PRESS_EXE_DEBUG
							printf("Goose exit condition met, moving on to hardware software closed loop zone 7\n");
						#endif
						zone = 7;
						}
					}
	...
	if (zone == 6) // Hardware closed loop ramp
		...
		if (first_loop)
			{
			#if PRESS_EXE_DEBUG
				printf("press blk exe-zone 6, Hardware closed loop ramp, first time through\n");
			#endif
	...
	if (zone == 7) // Hardware closed loop control / open loop
		{
		#if PRESS_EXE_DEBUG
			printf("press blk exe-zone 7, Outputting hardware software closed loop DAC setting = %d\n\n", commanded_raw_dac);
		#endif
