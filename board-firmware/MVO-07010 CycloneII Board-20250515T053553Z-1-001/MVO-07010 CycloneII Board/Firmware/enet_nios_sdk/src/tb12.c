...
/*
 ...
 1-16-07 still tb7.c
  ...
  1-18-08 tb10.c
	Version to work with vtw_global14.h.  

  3-15-08
    Added dither code to out_dac().  
	
  7-12-10 tb12.c
	Added discrete I/O numbers for ISW2/OSW2 and ISW3/OSW3.  OSW2 = 101 - 132.  ISW2 = 133 - 164.  OSW3 = 201 - 322.  ISW3 = 233 - 264.
	Changed OSW1 control outputs to 17 - 32, Monitor inputs to 33 - 48.  A new control_all program is required to work with these changes.
*/
...
int set_clr_discrete_output(int output_num, int action)
	...
	if (output_num != 0) // Zero == do nothing
		{
		if (output_num < 100) // Opto discrete I/O
			{
			// Discrete Opto outputs are 1-16 (monitor) and 17-32 (control), on ISW1.	
			if (output_num > 0 && output_num < 33) // ISW1 outputs
				{
				if (action >= TURN_ON)			
		...
		else if (output_num < 200) // Fieldbus monitoring outputs
			{
			// 101-132 outputs numbers correspond to OSW2 bits 0-31.
			if (output_num > 100 && output_num < 133)
				{
				if (action >= TURN_ON)			
					OSW2 |= 1<<(output_num-101); // OSW2 bits 0-31
				else
					OSW2 &= ~(1<<(output_num-101)); 	
				}
			else return_val=1; // I/O number out of range.	
			}	
		else if (output_num < 300) // Fieldbus control outputs
			{
			// 201-232 output numbers correspond to OSW3 bits 0-31.
			if (output_num > 200 && output_num < 233)
				{
				if (action >= TURN_ON)			
					OSW3 |= 1<<(output_num-201); // OSW3 bits 0-31
				else
					OSW3 &= ~(1<<(output_num-201)); 		
				}
			else return_val=1; // I/O number out of range.
			}
		else if (output_num < 500) // "400" flags.   
			{
			if (output_num > 400 && output_num < 500) // Within range
				{
				switch (output_num)
					{
					case 401: // Turn axis#1 dither on/off.
						if (action >= TURN_ON)
							{
							flags_400.dither1_enbl=1;
							if (!(na_dither_timer->np_timerstatus & np_timerstatus_run_mask)) {START_DITHER_TIMER;} // Timer's not running
							}
						else 
							{
							flags_400.dither1_enbl=0;
							if (!flags_400.dither2_enbl) {STOP_DITHER_TIMER;} // If axis #2 dither active, we won't stop the timer.
							}
						break;
					case 402: // Turn axis#2 dither on/off.
						if (action >= TURN_ON)
							{
							flags_400.dither2_enbl=1;
							if (!(na_dither_timer->np_timerstatus & np_timerstatus_run_mask)) {START_DITHER_TIMER;} // Timer's not running
							}
						else 
							{
							flags_400.dither2_enbl=0;
							if (!flags_400.dither1_enbl) {STOP_DITHER_TIMER;} // If axis #1 dither active, we won't stop the timer.
							}
						break;	
					default:
						break;
...
// 1 (ON) or 0 (OFF).  Output numbers and the internal 400 flags can also be checked as well.
int chk_discrete_input(int input_num, int *input_state)
	...
	if (input_num < 100) // Opto discrete I/O
		{
		if (input_num > 32 && input_num < 65) // 33 - 48 Monitor inputs 49 - 64 control inputs
			{
			if (ISW1 & (1<<(input_num-33))) *input_state=ON; // ISW1 bits 0-15
		...
		// We can check for outputs here and make conditional decisions based 	
		else if (input_num > 0 && input_num < 33) // 1 - 16 Monitor outputs, 17 - 32 control inputs 
			{
			if (OSW1 & (1<<(input_num-1))) *input_state=ON; // OSW1 bits 0-15, actually an output in this case
			else *input_state=OFF;
			}		
			
		// We can check for outputs here and make conditional decisions based 	
		//else if (input_num > 0 && input_num < 17) // Monitor outputs
		//	{
		//	if (OSW1 & (1<<(input_num-1))) *input_state=ON; // OSW1 bits 0-15, actually an output in this case
		//	else *input_state=OFF;
		//	}	
		//else if (input_num > 32 && input_num < 49) // Control outputs
		//	{
		//	//printf("ION=%d OSW1=0x%x input_num-17=%d", input_num, OSW1, input_num-17);
		//	if (OSW1 & (1<<(input_num-17))) *input_state=ON; // OSW1 bits 16-31, actually an output in this case
		//	else *input_state=OFF; 
		//	}	
		else return_val=1; // Input number out of range.
		}
	else if (input_num < 200) // ISW2 Fieldbus inputs
		{
		if (input_num > 132 && input_num < 165) // Monitor inputs
			{
			if (ISW2 & (1<<(input_num-133))) *input_state=ON; // ISW2 bits 0-31
			else *input_state=OFF;
			}	
		// We can check for outputs here and make conditional decisions based 	
		else if (input_num > 100 && input_num < 133) // Monitor outputs
			{
			if (OSW2 & (1<<(input_num-101))) *input_state=ON; // OSW2 bits 0-31, actually an output in this case
			else *input_state=OFF;
			}		
		}
	else if (input_num < 300) // ISW3 Fieldbus inputs
		{
		if (input_num > 232 && input_num < 265) // Control inputs
			{
			if (ISW3 & (1<<(input_num-233))) *input_state=ON; // ISW2 bits 0-31
			else *input_state=OFF;
			}
		// We can check for outputs here and make conditional decisions based 	
		else if (input_num > 200 && input_num < 233) // Monitor outputs
			{
			if (OSW3 & (1<<(input_num-201))) *input_state=ON; // OSW3 bits 0-31, actually an output in this case
			else *input_state=OFF;
			}		
		}
	...
	else if (input_num < 500) // 400 internal flags			
		{
		if (input_num > 400 && input_num < 500) // 401-499 acceptable range
			{
			if (input_num == 401)
				{
				if (flags_400.dither1_enbl) *input_state=ON; else *input_state=OFF;
				}
			else if (input_num == 402)
				{
				if (flags_400.dither2_enbl) *input_state=ON; else *input_state=OFF;
				}
			else return_val=1; // Out of range	
			}
		else return_val=1; // Out of range	
		}		
	else return_val=1; // Input number out of range.	
...
// Another attempt at memory allocation.  Previous attempts at using malloc() and free() worked but were 
// unstable.  It's possible free() was being called more than once.  Calling free() again after a pointer
// was already freed, could result in unstable behavior.  The C language doesn't define what to do in those
// cases, it's up to the implementation.  We'll try to make it more stable here.
// Five allocations are allowed numbered 0 - 4.  Each attempt at memory allocation must be given an allocation number.
// That number will reference an array of void pointers.  The array entry for a given Malloc must be NULL before
// we can allocate memory.  It must be not NULL before we can free it.  This will prevent us from allocating
// something that has already been allocated and from freeing something that has already been freed.

// void *malloc_ptr[5]; // Must be declared globally in program using these functions.

// Define malloc alloc_num's in case we go back to using malloc.
// These defines are also in vtg_global14.h or later.
...
void init_Malloc(void)
	...
	for (x=0; x<5; x++) malloc_ptr[x]=NULL;
	}
	
void *Malloc(size_t size, int alloc_num)
	{
	if (alloc_num<0 || alloc_num>5) return (NULL); // Allocation number not valid
	if (malloc_ptr[alloc_num] == NULL) malloc_ptr[alloc_num] = malloc(size); // Nothing presently allocated, call malloc()
	
	// Either return he pointer we got from malloc() - or - if malloc_ptr[alloc_num] did not == NULL, 
	// return what was already there.
	return (malloc_ptr[alloc_num]);
	}

void Free(int alloc_num)
	{
	if (alloc_num>=0 && alloc_num<=4) // Within allowable limits
		{
		if (malloc_ptr[alloc_num] != NULL)
			{
			free(malloc_ptr[alloc_num]);
			malloc_ptr[alloc_num] = NULL;
			}
		}
	}
	
	
//In numerical analysis, the Kahan summation algorithm (also known as compensated summation) significantly
//reduces the numerical error in the total obtained by adding a sequence of finite precision floating point 
//numbers, compared to the obvious approach. This algorithm is attributed to William Kahan.
//
...
int out_dac(int ch_num, int value, int value_type, int abs_inc)
	{
	int ctr, error = 0, null_offset, lvdt_offset, axis_num=99, value_to_save;
	...
	if (!vtg.dac_write_in_progress) // Don't allow recursion of this function
		...
		if (ch_num > 0 && ch_num < 5) // Good channel specification
			...
			if (value_type == PRESS_CMD || value_type == PRESS_RAW) 
				{
			...
			value_to_save = value; // the value we'll archive.  Does not include the NULL, LVDT offsets or dither.	
			...
			if (axis_num != 99)
				...
				null_offset = (819 * NULL_OFFSET(axis_num))/1000;
				lvdt_offset = (819 * LVDT_OFFSET(axis_num))/1000;	
				//printf("[V=%d ", value);
				// Then add to value.  
				value += (null_offset+lvdt_offset);
				//printf("NO=%d LO=%d V=%d ", null_offset, lvdt_offset, value);
				//We must process dither here so that dac commands written by control blocks will also have dither
				// applied.  Otherwise, we could just process dither in the isr and not mess with it here.
				// If dither is disabled, the value will be zero so it will have no effect.
				if (axis_num==1) value+=vtg.dither_raw_val1;
				else if (axis_num==2) value+=vtg.dither_raw_val2;
				//printf("VAD=%d]\n", value);
				}
				
			// This macro will set error to zero if no fault and to one if there is a fault.
			// Another sanity check will also be performed.
			// value_to_save will be archived to vtg.dac_word[ch_num].
			//printf("Before OUT_DAC, V=%d VTS=%d CN=%d\n", value, value_to_save, ch_num);
			OUT_DAC(value, value_to_save, ch_num, ctr, error); 
