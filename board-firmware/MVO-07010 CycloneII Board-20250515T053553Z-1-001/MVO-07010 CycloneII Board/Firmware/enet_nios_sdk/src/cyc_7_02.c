...
//////////////////////// FIRMWARE VERSION ////////////////////////
#define FIRMWARE_VERSION "cyc_7_02,"  
...
void ls_service_isr(int context)
	...
	ls_status = na_ls_int_input->np_piodata;
	//ls_status = na_ls_int_input->np_pioedgecapture;
	ls_cap = na_ls_int_input->np_pioedgecapture;
	na_ls_int_input->np_pioedgecapture = 0;

	//printf("*C0x%x S0x%x*", ls_cap, ls_status); // For test
	if (ls_cap & 0x01)
		{
		if (ls_status & 0x01)
			{
		...
		else {LS1_1_OFF; printf("LS1_1_OFF");}	
		}
	

	if (ls_cap & 0x02) {if(ls_status & 0x02) {LS2_1_ON; printf("LS2_1_ON");} else {LS2_1_OFF; printf("LS2_1_OFF");}} 
	if (ls_cap & 0x04) {if(ls_status & 0x04) {LS3_1_ON; printf("LS3_1_ON");} else {LS3_1_OFF; printf("LS3_1_OFF");}} 
	if (ls_cap & 0x08) {if(ls_status & 0x08) {LS4_1_ON; printf("LS4_1_ON");} else {LS4_1_OFF; printf("LS4_1_OFF");}} 
	if (ls_cap & 0x10) {if(ls_status & 0x10) {LS5_1_ON; printf("LS5_1_ON");} else {LS5_1_OFF; printf("LS5_1_OFF");}}  
	if (ls_cap & 0x20) {if(ls_status & 0x20) {LS6_1_ON; printf("LS6_1_ON");} else {LS6_1_OFF; printf("LS6_1_OFF");}} 
	}	
...
////////// Periodic non-precision events based on mS counter cur_time can be placed here
	...
	//	else out_dac(1, 0, VOLTAGE_X_1000, ABSOLUTE);
		printf("ls=0x%x OSW1=0x%x\n", na_ls_int_input->np_piodata, OSW1);
