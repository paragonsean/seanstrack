...
6-4-10 	combined_control_blocks9_tmp.c
		Special test version for troubleshooting a problem at Zitai.  In the velocity closed loop section,
		additional information will be sent to the serial port when in debug mode 1 or 2.
*/
...
int velocity_block(c_block control_block, int type_of_execution, int zone)
	{
	static int accel, loop_num, cl_num, starting_pos, delta_pos_zspeed, prev_pos, regular_dac_channel;
	...
	if (zone == 3) // Closed loop accel
		...
		if (first_loop)
			...
			//act_vel=FIXED4_DIV((axis[axis_num].vel_ctr_freq*10000LL), (SOFT_VELOCITY(axis_num)*10000LL)); // Doesn't require reading the PV unit
	...
	vel_loop: ;
		...
		// Output specific information on the velocity calculation if in debug mode 1
		if (vtg.control_debug_level==1)
			{
			// Print every 5th. loop
			if (!(cl_num % 5)) printf("DBG V vcf%d pvv%d av%d\n", axis[axis_num].vel_ctr_freq, RD_PV_VEL(axis_num), act_vel);
			}
		...
		// If enabled, this will output gain information in debug mode 1
		//if (vtg.control_debug_level==1)
		//	{
		//	// Print every 5th. loop
		//	if (!(cl_num % 5)) printf("DBG V pg%d ig%d dg%d rig%d\n", PID_P_GAIN(axis_num), PID_I_GAIN(axis_num), PID_D_GAIN(axis_num), PID_RECURSIVE_INTEGRAL(axis_num));
		//	}
		
		// Added test function, output servo information when in debug mode 2
		if (vtg.control_debug_level==2)
			{
			// Print every 5th. loop
			if (!(cl_num % 5)) printf("DBG V cv%d av%d dv%d cp%d do%d\n", commanded_vel_fixed4, act_vel, dv, temp_pos, PID_OUTPUT(axis_num));
			}
