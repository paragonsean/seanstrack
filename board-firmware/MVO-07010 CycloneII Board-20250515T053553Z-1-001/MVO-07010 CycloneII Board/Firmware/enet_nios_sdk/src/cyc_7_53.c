...
7-14-03 enet_mon_cont12.c  This supports 2 position axes. This version works with 
		...
		vtw_global2.h.		This version supports 2 transducer axes.
...
//////////////////////// FIRMWARE VERSION ////////////////////////
#define FIRMWARE_VERSION "cyc_7_53,"
...
3,	This firmware is to run under chip version emc_chip_16010200.hexout.flash or later (Apex 20K200E) or
...
6,	When targeting the CycloneII board with EPCS serial flash, the configuration flash file is built with the following command:
	sof2flash --input=cyc_chip_18010201.sof --epcs<CR>
	
	This will create a file called cyc_chip_18010201.flash.  This file must then be manually edited and the following
		...
		case 'c': // Commands
			...
			//else if (*ptr=='12') // Open/close pressure feedback on amp2
				...
				if(tempi) servo_amp_interface(OPEN_PRESSURE_FEEDBACK2);
                            ...
                            /* Enable end-of-shot position reporting. */
                            ptr += 3;
                            pos_eos_en = atoi(ptr) ? 1:0;
                        }
	...
	if (proceed) 
		{
                ...
                /* Re-initialize end-of-shot position state. */
                pos_eos_valid = 0;
                pos_eos_tx = 0;
...
////////// Periodic non-precision events based on mS counter cur_time can be placed here
        ...
        /* Tx end-of-shot position if event is registered, data is valid, and not previously Tx'ed. */
        if (pos_eos_en && pos_eos_valid && !pos_eos_tx) {
            sprintf(pos_eos_msg,"R_POS1EOS:11 #%d\n", pos_eos);
            send_string((void *)&local_net_settings, pos_eos_msg);
            pos_eos_tx = 1;
        }
