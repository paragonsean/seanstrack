...
// This program is compiled for location 0x20f000
// nios2-build -b 0x20f000 germs_monitor_epcs.c

// To create a hex file
// elf2hex -b 0x20f000 -e 0x20f800 -i germs_monitor_epcs.elf -o germs_monitor_epcs.hex -w 32
...
static int r_in_flash_range(unsigned int addr)
	...
	int result = 0;
		...
		result = addr < (unsigned int)GM_FlashTop; // Since flash base is zero, we'll only check the top if Nios II
	#else
...
static int r_stash_byte(unsigned int addr,unsigned char value)
	...
	if(r_in_flash_range(addr))
		...
		if((addr & 1) == 0)
			...
			#if nm_cpu_architecture == nios2
				result = nr_epcs_write_buffer((unsigned long)(addr & ~1), 2, (unsigned char *)&halfword);
			#else
	...
	while( (((int)addr_hi) - ((int)addr_lo)) > 0 )
		...
		for(i = 0; i < word_count; i++)
		{
			if (r_in_flash_range(addr_lo+i))
				{
				#if nm_cpu_architecture == nios2
					nr_epcs_read_buffer ((unsigned long)(addr_lo + i * k_word_size), k_word_size, (unsigned char *)&v);
				#endif
				}
	...
	if(r_in_flash_range(addr))
          {
            if (do_entire_chip == 1)
              ...
              {
				#if nm_cpu_architecture == nios2
					nr_epcs_erase_sector((unsigned long)addr);
				#else
...
int main(void)
	...
	#if nm_cpu_architecture == nios2
		// Unprotect EPCS
		if (nr_epcs_protect_region(na_epcs_protect_none)) r_send_string("Unable to unprotect EPCS!!!\n");
	#endif
