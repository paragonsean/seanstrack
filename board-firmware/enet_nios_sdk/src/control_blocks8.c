...
int dwell_block_exe(c_block control_block, int type_of_execution, int zone)
	...
	if (zone == 1) // We're executing the block from the beginning.
		...
		else if (cond_result < 20) // OK to proceed, WHILE interrupt may have been armed. 
			{
			#if DWELL_EXE_DEBUG
	...
	if (zone == 2)
		{
		#if DWELL_EXE_DEBUG
			printf("Dwell blk exe-Zone 2, timeout interupt has occured.\n");
		#endif
...
int dwell_block_exe(c_block control_block, int type_of_execution, int zone)
	...
	if (zone == 1) // We're executing the block from the beginning.
		...
		if(type_of_execution != NULL) saved_execution_type = type_of_execution;
	...
	if (zone == 2)
		...
		//when_cont_isr[TIMER2_INT_BIT].zone_to_goto=2;
		//printf("!");
...
int velocity_block_exe(c_block control_block, int type_of_execution, int zone)
	...
	if (zone == 1) // Initial entry of block
		...
		if (cond_result < 10 && cond_result > 0) // Some sort of executive error took place.
			...
			{
			#if VEL_EXE_DEBUG
				printf("vel blk exe-zone 1, OK to proceed\n");
			#endif
			...
			if (position)
				{
				if (vac_wait && vac_on) {ARM_GP_POS1_CONT_INTERRUPT(position+240, 9);} // If vacuum shot, don't allow cylinder to mave more than 3" past the ending position.
	...
	// Now check for zspeed, if so, no point in going any further.
	temp_pos = RD_PV_POS;
	if (temp_pos >= MIN_STROKE_LEN)
		...
		if (ZSPEED)
			...
			if (vac_wait && vac_on)
				{
				#if EXT_LOOP_DEBUG
					printf("vel blk exe-Exiting, vacuum abort zspeed\n");
				#endif		
				WARNING_WORD = VAC_ABORT_ZSPEED;
				zone=999; // Exit
				} 
			else 
				{
				#if EXT_LOOP_DEBUG
					printf("vel blk exe-Exiting, zspeed, pos=%d\n", RD_PV_POS);
				#endif
	...
	if (zone == 6) // Open loop control
		...
		if (first_loop)
			{
			#if VEL_EXE_DEBUG
				printf("vel blk exe, zone 6, open loop velocity, first time through\n");
			#endif
	...
	if (zone == 9) // Vac abort, moved 3" past ending position.
		{
		#if VEL_EXE_DEBUG
			printf("vel blk exe-zone 9, VACUUM ERROR, Exiting, cyl. exceeded EP+240\n");
		#endif	
		WARNING_WORD = VAC_ABORT_POS_TOO_LONG;
		}
	...
	// Handle any outputs to set/clr now. 
	if (!WARNING_WORD && !FATAL_ERROR_WORD && do_set_output) 
		{
		if (SET_OUT_DUR) INVERT_CONT_OUTPUT_NOW; // Set output back to its original state.
		else if (SET_OUT_AT_END) SET_CLR_CONT_OUTPUT_NOW;
		}
	...
	if (zone == 1)
		...
		if (cond_result < 10 && cond_result > 0) // Some sort of executive error took place.
			...
			if (block->ub_indicator_bits & ub_op1_const_mask)
				...
				while (convert_ana_ch(block->op1_df, VOLTAGE_X_1000, &op1)); // Keep trying until we get it
				}
...
int block_conditional_exe(c_block control_block)
	...
	if ((block->cond_indicator_bits & while_mask) ||
		...
		if (block->cond_indicator_bits & op1_discrete_io_mask)
			...
			while (convert_ana_ch(block->cond_op1_df, VOLTAGE_X_1000, &cond_op1));
			}
