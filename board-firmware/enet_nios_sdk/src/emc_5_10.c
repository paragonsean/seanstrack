...
//////////////////////// FIRMWARE VERSION ////////////////////////
#define FIRMWARE_VERSION "emc_5_10,"
...
#if BUILD_FOR_CONTROL
	#include "combined_control_blocks4.h" // Both polled and interrupt driven control are supported in the "combined" file.
#endif
...
inline void monitor_supervisor(void)
	{
	int y, axis_num=1, shot_data_size, cs_present, ss_present, proceed, pos=0;
	volatile unsigned short int *halfword_ptr;
	//vp_pvu *pv1 = va_pv1;
	np_pio *misc_ins_pio = na_misc_ins;
...
//	#endif
		{
		// Check if no cycle start until at home.  If so, we'll see if an Opto I/O board is installed.
		// If not, we'll check fast input 304 (fast at home).
		proceed=1;
		if (!CYCLE_START_NO_MATTER_WHAT) // We're placing conditions on when we'll accept a CS signal
			{
			if (OPTO_IO_BOARD_NOT_INSTALLED) // Must be monitor only
				{
				if (NO_CYCLE_START_UNTIL_AT_HOME)  // We'll wait for the fast at home input before proceeding
					{
					chk_discrete_input(FAST_AT_HOME_INPUT, &proceed);
					}
				else  // We won't allow a cycle start until cylinder position is less than 1".
					{
					pos = RD_PV_POS;
					if (pos > 80) proceed=0;
					}
				}
			else  // This is probably a control system.
				{
				if (NO_CYCLE_START_UNTIL_AT_HOME)  // We'll wait for the at home input before proceeding
					{
					chk_discrete_input(AT_HOME_INPUT, &proceed);
					}
				}
			}
		...
		if (proceed)
			...
			if (data_sample != NULL)  // Success in allocating memory
				...
				// Write OSW1 here as a precaution against something not set or cleared as it should.
				// This is more of an issue with monitor only systems.  Some customers have found that
				// an alarm bit is set when there is no alarm resulting in scrapped castings.
				WRITE_OSW1;
...
#include "combined_control_blocks4.c" // Both polled and interrupt driven control are supported in the "combined" file.
