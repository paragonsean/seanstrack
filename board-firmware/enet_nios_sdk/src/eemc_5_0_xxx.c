...
//////////////////////// FIRMWARE VERSION ////////////////////////
#define FIRMWARE_VERSION "emc_5_0,"
...
1, From the SOPC builder V2.7 command shell
	...
	a, nios-build -b 0x400000 emc_5_0.c<CR>
	b, srec2flash  emc_5_0.srec -flash_address=0x140000 -ram_address=0x400000 -copy_size=0x40000<CR>
	c, nios-run emc_5_0.flash<CR>  To download the flash file.
...
3,	This firmware is to run under chip version emc_chip_08000000.hexout.flash
...
// computer.  This is inform the remote computer that it is saft to load the control program at this time.
...
void r_get_settings_from_flash(void)
	...
	remote_io_net_settings = *PTR_TO_REMOTE_IO_NET_SETTINGS_IN_FLASH;
...
int r_save_settings(int x)
	...
	nr_flash_write_buffer((void *)-1,(void *)PTR_TO_REMOTE_IO_NET_SETTINGS_IN_FLASH,(void *)&remote_io_net_settings,sizeof(remote_io_net_settings));
	}
...
void send_op_packets(void *context)
	...
	if (!vtg.xmitting_shot_data) // Don't send op packets if shot data is being sent
		...
		// For now, force upper channels to zero volts until the speed problem is solved with the upper channels.
		op_data_packet.channels.cana_ch1 = 0xf800;
		op_data_packet.channels.cana_ch2 = 0xf800;
		op_data_packet.channels.cana_ch3 = 0xf800;
		op_data_packet.channels.cana_ch4 = 0xf800;
		op_data_packet.channels.wana_ch1 = 0xf800;
		op_data_packet.channels.wana_ch2 = 0xf800;
		op_data_packet.channels.wana_ch3 = 0xf800;
		op_data_packet.channels.wana_ch4 = 0xf800;
...
inline void monitor_supervisor(void)
	{
	int y, axis_num=1, shot_data_size, cs_present, ss_present;
	volatile unsigned short int *halfword_ptr;
	//vp_pvu *pv1 = va_pv1;
	np_pio *misc_ins_pio = na_misc_ins;
	...
	chk_discrete_input(CYCLE_START_INPUT, &cs_present);
	#if BUILD_FOR_CONTROL
		chk_discrete_input(START_SHOT_INPUT, &ss_present);
	#endif
	...
	else if (!(MONITOR_ENABLE) && ((CS_DETECTED) || CS_PRESENT))
//#if BUILD_FOR_CONTROL
//  else if (!(MONITOR_ENABLE) && (cs_present || ss_present))
//	#else
//	  else if (!(MONITOR_ENABLE) && cs_present)		
//	#endif
		{
...
void set_analog_mode(int mode)
	...
	if (mode==1 || mode==6) 
		...
		RESULT5_SETUP = ad_setup_words[CANA_CH1]; // LVDT feedback amp1 
		RESULT6_SETUP = ad_setup_words[CANA_CH3]; // LVDT feedback amp2
		...
		RESULT3_SETUP = ad_setup_words[ANA_CH3]; 
		RESULT4_SETUP = ad_setup_words[ANA_CH4]; 
		RESULT5_SETUP = ad_setup_words[CANA_CH1]; 
		RESULT6_SETUP = ad_setup_words[CANA_CH2]; 
		RESULT7_SETUP = ad_setup_words[CANA_CH3]; 
		RESULT8_SETUP = ad_setup_words[CANA_CH4]; 
		}
	}
		...
		if (!(MONITOR_ENABLE)) // No shot in progress, keep going
			...
			if (sample_ctr == 2000) // Transmit data
				...
				for (x=0; x<4; x++) 
					{
					if(AMP_TYPE == ONE_AXIS_INTEGRAL_AMP || AMP_TYPE == TWO_AXIS_INTEGRAL_AMP)
						adata = shared_memory[x+16]; // CANA_CH1 - CANA_CH4
					else
						adata = shared_memory[x+20]; // WANA_CH1 - WANA_CH4
					...
					scope_data_packet->control_channels[sample_ctr][x] = ((adata-0x800)*10000)/2048;
					}
						...
						if (O_SCOPE_TRIG_V > -10001 && O_SCOPE_TRIG_V < 10001) // Tigger spec within a +/-10V range?
							...
							if (vtg.been_below_trig_voltage) // Capture on rising edge, we've been below the trig point.
								...
								if (scope_data_packet->control_channels[0][O_SCOPE_TRIG_CH<21 ? O_SCOPE_TRIG_CH-17 : O_SCOPE_TRIG_CH-21] >= O_SCOPE_TRIG_V)
									{
									printf("Trigger voltage met, %d volts\n", scope_data_packet->control_channels[0][O_SCOPE_TRIG_CH<21 ? O_SCOPE_TRIG_CH-17 : O_SCOPE_TRIG_CH-21]); // For test
...
// Good working V5 o_scope_isr before adding analog reporting stuff
void o_scope_isr(int context)
	{
	int x;
	static int sample_ctr;
...
void o_scope_isr(int context)
	{
	int x;
	static int sample_ctr, analog_reporting_sample_ctr;
	short int adata;
	volatile unsigned short int upper_channels[8];
	np_timer *o_scope_timer = na_o_scope_timer;
	...
	if (local_net_settings.send_o_scope_data || plant_net_settings.send_o_scope_data)
		...
		if (!(MONITOR_ENABLE)) // No shot in progress, keep going
			...
			else  // Analog reporting in progress
				{
				sample_ctr=0; // Reset sample counter, don't send o-scope data if analog repoting in progress.
				}
			}
		else  // Reset sample_ctr, don't send o_scope data during a shot
			{
	...
	if (vtg.block_ana_report)  // Analog reporting
		{
		if (!SHOT_IN_PROGRESS)
			{
			if (analog_reporting_sample_ctr == 2000) // Sampling complete, send report
				{
				o_scope_timer->np_timercontrol = 8;
				sample_ctr=0;

				// Clear out vtg.block_ana_report flag that prompted this report, others may be pending

				// Transmit report

				// Reenable timer, o_scope function may be pending.
				o_scope_timer->np_timercontrol = 7; // Start/restart timer, continuous mode
				}
			else if (analog_reporting_sample_ctr == 0) // Start another report
				{
				analog_reporting_sample_ctr++;
		...
		else  // Reset analog_reporting_sample_ctr and don't report analog if shot in progress.
			{
			//o_scope_timer->np_timercontrol=8; // Stop timer
			//analog_reporting_sample_ctr=0;
			vtg.block_ana_report=0;
			}
		}

	if (!local_net_settings.send_o_scope_data && !plant_net_settings.send_o_scope_data && !vtg.block_ana_report)   // Stop timer and reset sample_ctr, nobody wants o_scope data
		{
		//printf("Stopping o_scope timer and clearing sample_ctr, nobody wants scope data\n");
		o_scope_timer->np_timercontrol = 8;
		sample_ctr=0;
...
int analog_reporting_single(int report_num)
	...
	convert_all_channels((volatile unsigned short int *)&single_analog_report.channels.ana_ch1, RAW_VAL);
					
	single_analog_report.report_number = report_num;
		
	send_binary((void *)&local_net_settings, SINGLE_SAMPLE_ANALOG_REPORTING, (void *)&single_analog_report, sizeof(input_based_analog_single), NO_ACK);
	send_binary((void *)&plant_net_settings, SINGLE_SAMPLE_ANALOG_REPORTING, (void *)&single_analog_report, sizeof(input_based_analog_single), NO_ACK);
...
int main(void)
	...
	unsigned int chk_ethernet_io_time=0, isw1_bits_to_ignore=0, isw4_bits_to_ignore=0;
...
// Set the io reporting bits to ignore here.  These are isw1 bits that when they change, we won't
// send a report.  The analog report bits need not generate an io change report as well.
isw1_bits_to_ignore |= ISW1_MASK_ANALOG_REPORT1;
isw1_bits_to_ignore |= ISW1_MASK_ANALOG_REPORT2;
isw1_bits_to_ignore |= ISW1_MASK_ANALOG_REPORT3;
isw1_bits_to_ignore |= ~ISW1_BITS_TO_WATCH;

isw4_bits_to_ignore |= 0x00000003; // Ignore CS and master enable
...
////////// Periodic non-precision events based on mS counter cur_time can be placed here
	...
	//	}
		...
		if(cur_time>=chk_ana_io_reporting_time)
			...
			chk_ana_io_reporting_time=cur_time+CHK_ANA_IO_REPORTING_TIME;
			//cur_bits = ISW1 & ISW1_BITS_TO_WATCH;
			...
			if (!SHOT_IN_PROGRESS) // First motion to end of timed collection
				...
				if (shot_was_in_progress) // Skip any input changes for 1 loop after a shot
					...
					if ((isw1_bits_that_have_set & ISW1_MASK_ANALOG_REPORT1) || (isw4_bits_that_have_set & ISW4_MASK_ANALOG_REPORT1))
						{
						if (SAMPLE_INTERVAL_REPORT1 == 0)  // Single sample
							{
							convert_all_channels((volatile unsigned short int *)&single_analog_report.channels.ana_ch1, RAW_VAL);
					
							single_analog_report.report_number = 1;
							...
							{
							vtg.block_ana_report |= 0x00000001; // Flag for report #1, o_scope isr will process.
							o_scope_timer->np_timercontrol = 7; // Start/restart timer, continuous mode
							}	
						}
					...
					if ((isw1_bits_that_have_set & ISW1_MASK_ANALOG_REPORT2) || (isw4_bits_that_have_set & ISW4_MASK_ANALOG_REPORT2))
						{
						if (SAMPLE_INTERVAL_REPORT2 == 0)  // Single sample
							{
							convert_all_channels((volatile unsigned short int *)&single_analog_report.channels.ana_ch1, RAW_VAL);
					
							single_analog_report.report_number = 2;
							...
							{
							vtg.block_ana_report |= 0x00000002; // Flag for report #2, o_scope isr will process.
							o_scope_timer->np_timercontrol = 7; // Start/restart timer, continuous mode
							}
						}
					...
					if ((isw1_bits_that_have_set & ISW1_MASK_ANALOG_REPORT3) || (isw4_bits_that_have_set & ISW4_MASK_ANALOG_REPORT3))
						{
						if (SAMPLE_INTERVAL_REPORT3 == 0)  // Single sample
							{
							convert_all_channels((volatile unsigned short int *)&single_analog_report.channels.ana_ch1, RAW_VAL);
					
							single_analog_report.report_number = 3;
							...
							{
							vtg.block_ana_report |= 0x00000004; // Flag for report #3, o_scope isr will process. 
							o_scope_timer->np_timercontrol = 7; // Start/restart timer, continuous mode
							}
						}

					// Check and see if any bits outside of the bits to ignore have changed
					//if ((~isw1_bits_to_ignore & isw1_bits_that_have_changed) || (~isw4_bits_to_ignore & isw4_bits_that_have_changed))
					if ((ISW1_BITS_TO_WATCH & isw1_bits_that_have_changed) || (ISW4_BITS_TO_WATCH & isw4_bits_that_have_changed))
						{
						io_report.isw1 = cur_isw1_bits;
						io_report.isw4 = cur_isw4_bits;
						io_report.one_ms_counter = ONE_MS_COUNTER;
						send_binary((void *)&local_net_settings, IO_CHANGE_REPORTING, (void *)&io_report, sizeof(input_change_report), NO_ACK);
						send_binary((void *)&plant_net_settings, IO_CHANGE_REPORTING, (void *)&io_report, sizeof(input_change_report), NO_ACK);
						}
					}
				}
