...
2-25-04 emc_4_0 Made A/D setups in on-chip shared memory.
	...
	10-11-04 emc_4_12.c
		Added new bit to MON_STATUS_WORD, V312, SHOT_IN_MOTION.  This sets at first position based
		motion and clears at ZSPEED.  Local and remote Ethernet commands will not be serviced if a 
		shot is in motion and we're running control.  Jay and Al had found that if the control text
		file is downloaded while a shot is taking place, bad things can happen.  This is an attempt
		to correct that.  I/O Ethernet commands will still be serviced.

	11-11-04 emc_4_14.c
		Problem found with the time based isr.  If a PB update came during the time based isr,
		missing data would occur in the current update as the pb_ctr was incremented.  Added code
		to save the current pb_ctr and to compare the saved with the current at the end of the
		routine.  If different, the tb_ctr is not updated, in effect nullifying the current time based
		isr.
...
#define BUILD_WITH_DIAGNOSTIC_ROUTINES 0
///////////////////////////////////////////////////////////

//////////////////////// FIRMWARE VERSION ////////////////////////
#define FIRMWARE_VERSION "emc_4_14,"
...
#if BUILD_FOR_CONTROL
	#include "combined_control_blocks1.h" // Both polled and interrupt driven control are supported in the "combined" file.
#endif
...
1, From the SOPC builder V2.7 command shell

	a, nios-build -b 0x400000 emc_4_11.c<CR>
	b, srec2flash  emc_4_11.srec -flash_address=0x140000 -ram_address=0x400000 -copy_size=0x40000<CR>
	c, nios-run emc_4_11.flash<CR>  To download the flash file.

2, This file is built using:

	a, Nios V2.2 
	b, SOPC builder V2.7
	c, Quartus II V2.2, SP1 installed

3,	This firmware is to run under chip version emc_chip_06000000.hexout.flash
...
unsigned short int analog[20]; // Ch 1-20.
int pos;
int vel;
unsigned int isw1;
unsigned int isw4;
...
volatile unsigned short int *ad_setup = na_ad_setup_ram; // 18 bytes of shared memory
...
void loadBuffer(s_connection_status *u, char* payload, int payload_length);
...
int send_binary(void *context, unsigned char data_type, void *data, int length, int ack)
	{
	static unsigned short int data_set_num, shot_num; // This routine will assign and increment shot/data_set numbers.
...
int plugs_init(void)
	...
	// Now attempt to send a hello message if UDP, not a big deal if we fail to make contact 
	// but at least we tried.  We'll attempt 3 trys.
	#if BUILD_FOR_UDP
		//nr_delay(60000);
		// Try local first
		if (((net_32)LAST_IP_CONNECTED_TO_LOCAL & 0x00ffffff)==(OUR_IP_ADDRESS & 0x00ffffff))
			{
			//printf("At power up, for local plug, saved ip=0x%x and saved port = %d\n", nr_n2h32((net_32)LAST_IP_CONNECTED_TO_LOCAL), nr_n2h16((net_16)LAST_PORT_CONNECTED_TO_LOCAL)); // For test
			for (ctr=1; ctr<4; ctr++)
				{
				network_xmit((void *)local, (void *)hello_str, strlen(hello_str), NO_ACK, (net_32)LAST_IP_CONNECTED_TO_LOCAL, (net_16)LAST_PORT_CONNECTED_TO_LOCAL);
				nr_delay(UDP_WAIT_TIME);
				if (local->connected) break; // We made contact!!!
				}
			}
		
		// Then plant-wide.
		if (((net_32)LAST_IP_CONNECTED_TO_PLANT & 0x00ffffff)==(OUR_IP_ADDRESS & 0x00ffffff)) // We have saved a valid address
			{
			//printf("At power up, for plant plug, saved ip=0x%x and saved port = %d\n", nr_n2h32((net_32)LAST_IP_CONNECTED_TO_PLANT), nr_n2h16((net_16)LAST_PORT_CONNECTED_TO_PLANT)); // For test
			for (ctr=1; ctr<4; ctr++)
				{
				network_xmit((void *)plant, (void *)hello_str, strlen(hello_str), NO_ACK, (net_32)LAST_IP_CONNECTED_TO_PLANT, (net_16)LAST_PORT_CONNECTED_TO_PLANT);
				nr_delay(UDP_WAIT_TIME);
				if (plant->connected) break; // We made contact!!!
				}
			}
	#endif
	return (result);
...
int transmit_monitor_data(void *context)
	...
	if (!rval) // Keep going
		...
		// 
		#if BUILD_WITH_DIAGNOSTIC_ROUTINES
			if (data_sample[comp_shot_parameters.eos_pos+1].position == 0)
				{
				send_string((void *)&local_net_settings, "# First time based point is bad!!!\n");
				print_one_data_sample(data_sample[comp_shot_parameters.eos_pos+1]);
				} 
		#endif
		...
		//send_string((void *)&local_net_settings, "# About to xmit shot data\n"); // For test
...
void r_get_settings_from_flash(void)
	...
	// For test
	//printf("g.pns.settings_index=%d and nk_plugs_settings_count=%d\n",g.pns.settings_index, nk_plugs_settings_count);
...
void show_net_settings(void)
	...
	sprintf(tempstr, "\n# Network Settings %d (of %d), normally only 1 is used\n\n",g.pns.settings_index+1,nk_plugs_settings_count);
	send_string((void *)g.spc, tempstr);

	nr_plugs_ethernet_to_string(&g.pns.settings[g.pns.settings_index].ethernet_address, tempstr1);
	sprintf(tempstr, "# N1	ethernet address: %s\n", tempstr1);
	send_string((void *)g.spc, tempstr);

	nr_plugs_ip_to_string(g.pns.settings[g.pns.settings_index].ip_address, tempstr1);
	sprintf(tempstr, "# N2	ip address: %s\n", tempstr1);
	send_string((void *)g.spc, tempstr);

	nr_plugs_ip_to_string(g.pns.settings[g.pns.settings_index].nameserver_ip_address, tempstr1);
	sprintf(tempstr, "# N3	nameserver ip address: %s\n", tempstr1);
	send_string((void *)g.spc, tempstr);

	nr_plugs_ip_to_string(g.pns.settings[g.pns.settings_index].subnet_mask, tempstr1);
	sprintf(tempstr, "# N4	subnet mask: %s\n", tempstr1);
	send_string((void *)g.spc, tempstr);

	nr_plugs_ip_to_string(g.pns.settings[g.pns.settings_index].gateway_ip_address, tempstr1);
	sprintf(tempstr, "# N5	gateway ip address: %s\n", tempstr1);
	send_string((void *)g.spc, tempstr);

	sprintf(tempstr, "# N6	Setup index: %d  (0 - 3 are legal values)\n#\n", g.pns.settings_index);
	send_string((void *)g.spc, tempstr);
...
int parse_variable_command(unsigned char *orig_cmd_str)
	...
	unsigned char *ptr, *p_range, *p_set, *p_set_multiple;
...
parse_net_setup_command(unsigned char *orig_cmd_str)
	...
	switch(*ptr)
		...
		case '1': // Set the Ethernet address
			ptr+=2;
			nr_plugs_string_to_ethernet(ptr, &g.pns.settings[g.pns.settings_index].ethernet_address);
			break;
		
		case '2': // Set the IP address
			ptr+=2;
			nr_plugs_string_to_ip(ptr, &g.pns.settings[g.pns.settings_index].ip_address);
			break;

		case '3': // Set the nameserver IP address
			ptr+=2;
			nr_plugs_string_to_ip(ptr, &g.pns.settings[g.pns.settings_index].nameserver_ip_address);
			break;

		case '4': // Set the subnet mask
			ptr+=2;
			nr_plugs_string_to_ip(ptr, &g.pns.settings[g.pns.settings_index].subnet_mask);
			break;

		case '5': // Set the gateway IP address
			ptr+=2;
			nr_plugs_string_to_ip(ptr, &g.pns.settings[g.pns.settings_index].gateway_ip_address);
			break;

		case '6': // Set the setup index, valid entries are between 0 and 3
			ptr+=2;
			if (atoi(ptr)<0 || atoi(ptr)>3) break; // Invalid entry
			else g.pns.settings_index = atoi(ptr);
			break;
...
int parse_operational_command(unsigned char *orig_cmd_str)
	...
	unsigned char *ptr, tempstr[15];
	int ch_num, val, return_val=0, incremental=0, axis_num=1, x;
	...
	switch (*ptr)
		...
		case 'c': // Commands
			...
			if (*ptr=='1') // Command to set / clear O-scope mode
				...
				if (val != 0)  // Someone commanded o_scope mode, start timer.
					{
					// Set analog mode 4, we need to read channels 17-20.
					set_analog_mode(4); // Mode 4 allows reading channels 17-20.
					TOGGLE_ANALOG=0; // Force A/D processor to convert all 8 channels, no alternating.

					// Allocate the memory for the scope data packet.  If not NULL
					// another plug may be getting scope data, memory is already allocated.
		...
		sprintf(g.b, "R:6#%s%s%s%s%s%s\n", FIRMWARE_VERSION, FIRMWARE_TYPE, CONTROL_TYPE, ETHERNET_TYPE, PROTOCOL, HANDSHAKE);
		send_string((void *)g.spc, g.b); // Transmit the result.
		break; 
...
void pos_based_monitor_isr(int context)
	...
	if (MONITOR_ENABLE && !(ZSPEED))
		...
		// If we're moving faster than about 200 IPS, Let the A/D processor know.
		// It will begin to alternate channels to allow for higher speeds.
		if (TOGGLE_MODE_ENABLED) {if (RD_PV_VEL < 4100) TOGGLE_ANALOG = 1; else TOGGLE_ANALOG = 0;}
...
void send_op_packets(void *context)
	...
	if (!vtg.xmitting_shot_data) // Don't send op packets if shot data is being sent
		...
		// We'll only send the 8 dynamic channels.  If other channels, they will have to
		// be selected as dynamic before selecting op mode. 
		...
		// Channels 8 - 19, the floating channels were handled above	
...
inline void monitor_supervisor(void)
	{
	int y, axis_num=1, shot_data_size;
	volatile unsigned short int *halfword_ptr;
	//vp_pvu *pv1 = va_pv1;
	np_pio *misc_ins_pio = na_misc_ins;
	...
	if (SHOT_COMPLETE)
		...
		#endif
		free(data_sample); // Deallocate memory
		}
		...
		if (PAST_MIN_STK_LEN)
			...
			bounceback_pio->np_piointerruptmask = 0;
			axis1_interrupt->np_piointerruptmask = 0;
			free(data_sample); // Deallocate shot data memory	
	...
	// to think that a CS has triggered.  The control program will set SHOT_IN_PROGRESS
	...
	//else if ((!(MONITOR_ENABLE) && CS_PRESENT) || (!(MONITOR_ENABLE) && SHOT_IN_PROGRESS)) 
	else if (!(MONITOR_ENABLE) && (CS_PRESENT || SHOT_IN_PROGRESS))
		{
		...
		// Force the A/D processor out of toggle mode to begin with.  If the toggle mode turn
		// off bit is cleared in V313, analog chanels will toggle above 200 IPS. 
		TOGGLE_ANALOG=0; 
		set_analog_mode(ANALOG_MODE);
...
void xmit_fake_shots(void)
	...
	free(data_sample); // Make sure memory freed up before reassigning.
...
void set_analog_mode(int mode)
	...
	if (mode==1)
		{
		RESULT1_SETUP = ad_setup_words.ch1; 
		RESULT2_SETUP = ad_setup_words.ch2; 
		RESULT3_SETUP = ad_setup_words.ch3; 
		RESULT4_SETUP = ad_setup_words.ch4; 
		RESULT5_SETUP = ad_setup_words.ch5; 
		RESULT6_SETUP = ad_setup_words.ch6; 
		RESULT7_SETUP = ad_setup_words.ch7; 
		RESULT8_SETUP = ad_setup_words.ch8; 
		sel_hi_channels->np_piodata = 0; 
		}
	else if (mode==2)
		{
		RESULT1_SETUP = ad_setup_words.ch1; 
		...
		RESULT3_SETUP = ad_setup_words.ch3; 
		RESULT4_SETUP = ad_setup_words.ch4; 
		RESULT5_SETUP = ad_setup_words.ch9; 
		RESULT6_SETUP = ad_setup_words.ch10; 
		RESULT7_SETUP = ad_setup_words.ch11; 
		RESULT8_SETUP = ad_setup_words.ch12; 
		sel_hi_channels->np_piodata = 0; 
		}
	else if (mode==3)
		{
		...
		RESULT3_SETUP = ad_setup_words.ch3; 
		RESULT4_SETUP = ad_setup_words.ch4; 
		RESULT5_SETUP = ad_setup_words.ch13; 
		RESULT6_SETUP = ad_setup_words.ch14; 
		RESULT7_SETUP = ad_setup_words.ch15; 
		RESULT8_SETUP = ad_setup_words.ch16; 
		sel_hi_channels->np_piodata = 0; 
		}
		...
		RESULT3_SETUP = ad_setup_words.ch3; 
		RESULT4_SETUP = ad_setup_words.ch4; 
		RESULT5_SETUP = ad_setup_words.ch17; 
		RESULT6_SETUP = ad_setup_words.ch18; 
		RESULT7_SETUP = ad_setup_words.ch19; 
		RESULT8_SETUP = ad_setup_words.ch20; 
		sel_hi_channels->np_piodata = 1; 
		}
	else if (mode==5)  // Default control mode, monitors upper channels 17 and 18 only.
		{
		RESULT1_SETUP = ad_setup_words.ch1; 
		RESULT2_SETUP = ad_setup_words.ch2; 
		RESULT3_SETUP = ad_setup_words.ch3; 
		...
		RESULT5_SETUP = ad_setup_words.ch5; 
		RESULT6_SETUP = ad_setup_words.ch6; 
		RESULT7_SETUP = ad_setup_words.ch17; 
		RESULT8_SETUP = ad_setup_words.ch18; 
		sel_hi_channels->np_piodata = 1; 
		}
	}
...
#include "combined_control_blocks1.c" // Both polled and interrupt driven control are supported in the "combined" file.
...
void o_scope_isr(int context)
	...
	// Must be in analog mode 4 for this to work.
	if (local_net_settings.send_o_scope_data || plant_net_settings.send_o_scope_data)
		{
		if (!(SHOT_IN_PROGRESS)) // No shot in progress, keep going
			...
			if (sample_ctr == 2000) // Transmit data
				...
				for (x=0; x<4; x++)
					scope_data_packet->ch_17_20[sample_ctr][x] = shared_memory[x+4];
				#endif
				...
				for (x=0; x<4; x++) 
					{
					adata = shared_memory[x+4];
					adata &= 0x0fff; // Get rid of channel ID
					...
					scope_data_packet->ch_17_20[sample_ctr][x] = ((adata-0x800)*10000)/2048;
					}
					...
					if (O_SCOPE_TRIG_CH > 16 && O_SCOPE_TRIG_CH < 21) // Only channels 17-20 are valid
						...
						if (O_SCOPE_TRIG_V > -10001 && O_SCOPE_TRIG_V < 10001) // Tigger spec within a +/-10V range?
							...
							if (vtg.been_below_trig_voltage) // Capture on rising edge, we've been below the trig point.
								...
								if (scope_data_packet->ch_17_20[0][O_SCOPE_TRIG_CH-17] < O_SCOPE_TRIG_V)
									{
									printf("Went below trigger voltage, %d volts\n", scope_data_packet->ch_17_20[0][O_SCOPE_TRIG_CH-17]); // For test
...
void update_control_parameters(void)
	...
	POS_CHECK_ZSPEED = POS_CHECK_ZSPEED_SETTING; // Not used for anything, min stk len is now used			
...
int print_one_data_sample(combined_sample *data_sample)
	{
	char tempstr[81];

	sprintf(tempstr, "n# Analog ch.1 0x%x \n", data_sample->ana_ch1);
	send_string((void *)&local_net_settings, tempstr);

	sprintf(tempstr, "n# Analog ch.2 0x%x \n", data_sample->ana_ch2);
	send_string((void *)&local_net_settings, tempstr);

	sprintf(tempstr, "n# Analog ch.3 0x%x \n", data_sample->ana_ch3);
	send_string((void *)&local_net_settings, tempstr);

	sprintf(tempstr, "n# Analog ch.4 0x%x \n", data_sample->ana_ch4);
	send_string((void *)&local_net_settings, tempstr);

	sprintf(tempstr, "n# Analog ch.5 0x%x \n", data_sample->ana_ch5);
	send_string((void *)&local_net_settings, tempstr);

	sprintf(tempstr, "n# Analog ch.6 0x%x \n", data_sample->ana_ch6);
	send_string((void *)&local_net_settings, tempstr);

	sprintf(tempstr, "n# Analog ch.7 0x%x \n", data_sample->ana_ch7);
	send_string((void *)&local_net_settings, tempstr);

	sprintf(tempstr, "n# Analog ch.8 0x%x \n", data_sample->ana_ch8);
	send_string((void *)&local_net_settings, tempstr);

	sprintf(tempstr, "n# Velocity count Q1 0x%x \n", data_sample->vel_count_q1);
	send_string((void *)&local_net_settings, tempstr);

	sprintf(tempstr, "n# Velocity count Q2 0x%x \n", data_sample->vel_count_q2);
	send_string((void *)&local_net_settings, tempstr);

	sprintf(tempstr, "n# Velocity count Q3 0x%x \n", data_sample->vel_count_q3);
	send_string((void *)&local_net_settings, tempstr);

	sprintf(tempstr, "n# Velocity count Q4 0x%x \n", data_sample->vel_count_q4);
	send_string((void *)&local_net_settings, tempstr);

	sprintf(tempstr, "n# ISW1 0x%x \n", data_sample->isw1);
	send_string((void *)&local_net_settings, tempstr);

	sprintf(tempstr, "n# ISW4 0x%x \n", data_sample->isw4);
	send_string((void *)&local_net_settings, tempstr);

	sprintf(tempstr, "n# OSW1 0x%x \n", data_sample->osw1);
	send_string((void *)&local_net_settings, tempstr);

	sprintf(tempstr, "n# One Ms timer 0x%x \n", data_sample->one_ms_timer);
	send_string((void *)&local_net_settings, tempstr);

	sprintf(tempstr, "n# Position %d \n", data_sample->position);
	send_string((void *)&local_net_settings, tempstr);

	sprintf(tempstr, "n# Sample number %d \n", data_sample->sample_num);
	send_string((void *)&local_net_settings, tempstr);
...
int main(void)
	...
	int axis_num=1, payload_length; // Used for any interactions with PV1.
...
//#if BUILD_FOR_CONTROL
//	FLOATING_CHANNEL_SETUP = ad_setup_words.ch9; // Initially just set to something.
//	ANALOG_MODE1(1); // Set up dynamic channels to 1-8;
//#else
//	FLOATING_CHANNEL_SETUP = ad_setup_words.ch9; // Initially just set to something.
//	ANALOG_MODE1(0); // Set up dynamic channels to 1-8;
//#endif
...
if(SW7)
	{
	(int)g.spc=9999; // Set to serial mode
...
#define CHK_ISW1_BITS_FOR_CHANGE_TIME 100
...
while (1)
	...
	if(first_loop) // Set up the various times to check stuff
		...
		chk_isw1_bits_for_change_time=cur_time+CHK_ISW1_BITS_FOR_CHANGE_TIME;
		}
...
////////// Periodic non-precision events based on mS counter cur_time can be placed here
	...
	#if BUILD_FOR_CONTROL
		// Here we'll check for any change of states in ISW1. ISW_BITS_TO_WATCH are the only
		// bits we care about.  If any of those bits change, we'll xmit ISW1's state.
		if(cur_time>=chk_isw1_bits_for_change_time)
			{
			chk_isw1_bits_for_change_time+=CHK_ISW1_BITS_FOR_CHANGE_TIME;
			cur_bits = ISW1 & ISW_BITS_TO_WATCH;
			if(cur_bits != prev_bits)
				{
				prev_bits = cur_bits;

				// Now xmit ISW1 status
				sprintf(g.b, "O_ISW1=%x\n", ISW1);

				// Send string with handshaking, any not connected will be ignored
				SEND_STRING((void *)&local_net_settings, STRING, g.b, strlen(g.b), ACK);
				SEND_STRING((void *)&plant_net_settings, STRING, g.b, strlen(g.b), ACK);
				}
			}
	#endif
///////////////////////////////////////////////////////////////////
	
	// Process any packets that may be present.  We won't process local or remote Ethernet
	// commands if a shot is in motion and we're running control.
#if BUILD_FOR_CONTROL
	if (!SHOT_IN_MOTION)    
#endif
		{
