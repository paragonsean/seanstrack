// loop_exit_conditions1.h
		...
		if (temp_pos >= MIN_STROKE_LEN)
			...
			if (ZSPEED)
				...
				if (vac_wait && vac_on)
					...
					{
					#if EXT_LOOP_DEBUG
						printf("Exiting, zspeed, pos=%d, blk_no=%d\n", RD_PV_POS, vtg.prog_ctr);
					#endif		
					goto outputs;
					}
				}
		...
		if (nr_timer_milliseconds() > print_again)
			...
			printf("dac_voltage = %d\n", (((vtg.dac_word[axis_num]-8192)*10000)/8192));
			}
		#endif		
		...
		if (cond_result == KEEP_GOING_CALL_BACK_DURING)
			...
			if (vac_wait && !vac_on); // Skip conditional checking
				...
				{ 
				if (block_conditional_exe((ub *)velocity_block, DURING) == KEEP_GOING_CALL_BACK_DURING);		
				else 
					{
					#if EXT_LOOP_DEBUG
