...
#define VEL_EXE_DEBUG 1
#define EXT_LOOP_DEBUG 1
...
int dwell_block_exe(unsigned int (*control_program)[10], int type_of_execution)
	...
	if (cond_result < 10 && cond_result > 0) // Some sort of executive error took place.
		...
		if (dwell_time_mS > 0)
			...
			while (!vtg.timer2_timeout)
				...
				// For test	
				chk_discrete_input(INPUT_303, &input_state);
				if (input_state) {vtg.warning = TEST_WARNING; goto ext;} 
				chk_discrete_input(INPUT_304, &input_state);
				if (input_state) {vtg.fatal_error = TEST_FATAL_ERROR; goto ext;} 	
				}
			#if DWELL_EXE_DEBUG
	...
	#endif
	if (type_of_execution==PROGRAM) vtg.prog_ctr++; // Increment prog ctr, NULL indicates an immed. exe. block.			
	return(return_val);
	}
...
int velocity_block_exe(unsigned int (*control_program)[10], int type_of_execution)
	...
	int vac_wait=0, closed_loop=1, input_state, dv, loop_num=0, vac_on;
	int accel=0, act_vel, int_vel_cmd, x, starting_pos, next_pos;
	int current_dac_voltage, temp_dac_command, slow, dac_increment;
	int position, fill_test, commanded_raw_dac, current_raw_dac;
	...
	chk_discrete_input(FILL_TEST, &input_state);	
	...
	if (tempu & 0x80000000) closed_loop=0; // Open loop
	...
	cond_result = block_conditional_exe(velocity_block, BEGINNING); 
	...
	if (cond_result < 10 && cond_result > 0) // Some sort of executive error took place.
		...
		if (closed_loop)  
   			...
   			if (accel) // Closed loop acceleration in X1 counts
				...
				starting_pos = RD_PV1_POS;
      			act_vel = VEL1_CLK_FREQ/RD_PV1_VEL; // Vel in pps
      			...
      			for (x = 1; x <= accel; x++)
					...
					if (((VEL1_CLK_FREQ / int_vel_cmd) > 40000) && (accel_increments > 0)) // less than about 20 IPS
					...
					do
						...
						#include "LOOP_EXIT_CONDITIONS.H"

						}while (RD_PV1_POS < next_pos);  // Wait for next sample 
					} // for loop 
	  			}  // Closed loop accel 
			...
			timer2->np_timercontrol = 7; // Start timer continuously.
   			...
   			while (1)  // Loop until forced out.
				...
				act_vel = VEL1_CLK_FREQ/RD_PV1_VEL; // Vel in pps 
      			...
      			// Now check gain breaks.
				temp_pos = RD_PV1_POS; // Get current position
		...
		// be serviced here.   
		if (cond_result != KEEP_GOING_DONT_CALL_BACK) // May be something to set or clear
			block_conditional_exe(velocity_block, AT_END); // Checking back at end per block_conditional()'s request.	
		}
	else if (cond_result == COND_NOT_MET); // Nothing more to do 
...
int jump_block_exe(unsigned int (*control_program)[10], int type_of_execution)
	...
	cond_result = block_conditional_exe(jump_block, BEGINNING); 
	...
	if (cond_result < 10 && cond_result > 0) // Some sort of executive error took place.
   			...
   			if (jump_block->jb_indicator_bits & jb_type_normal_mask)
				...
				if (jump_block->jb_indicator_bits & jb_rel_abs_specifier_mask) // Relative jump forward
					...
					if (where_to_jump==0) // Jump to current block
						...
						if (cond_result == KEEP_GOING_CALL_BACK_DURING)
							{ 
							while (block_conditional_exe(jump_block, DURING) == KEEP_GOING_CALL_BACK_DURING)
								{
		...
		// be serviced here.   
		if (cond_result != KEEP_GOING_DONT_CALL_BACK) // May be something to set or clear
			block_conditional_exe(jump_block, AT_END); // Checking back at end per block_conditional()'s request.	
		}
	else if (cond_result == COND_NOT_MET) // Nothing more to do 
		{
		if (type_of_execution==IMMED) dont_increment_prog_ctr=1; // vtg.prog_ctr will be unaffected
...
int update_block_exe(unsigned int (*control_program)[10], int type_of_execution)
	{
	int return_val=0, cond_result, op1, op2, ctr=0, op2_present=1, temp_itu=0;
	...
	if (cond_result < 10 && cond_result > 0) // Some sort of executive error took place.
		...
		if (update_block->ub_indicator_bits & ub_op1_const_mask)
			...
			if (convert_ana_ch(update_block->op1_df, VOLTAGE_X_1000, &op1))
		...
		if (update_block->ub_indicator_bits & ub_itu_var_mask)
			...
			if (ramp)
				...
				if (ramp) // Check again, ramp could now be zero.
					{
					// Now get ramp time.
					if (vtg.v[393] < 100) ramp_time = (CLK_FREQ/1000000)*100; // Default to 100uS.
					else ramp_time = (CLK_FREQ/1000000)*vtg.v[393]; // Crude 
					...
					do
						...
						while (!vtg.timer2_timeout)
							...
							if (cond_result == KEEP_GOING_CALL_BACK_DURING)
								{ 
								if (block_conditional_exe(update_block, DURING) == KEEP_GOING_CALL_BACK_DURING);		
								else goto ext; // Condition was not met.  Any outputs to
											  // set_clr will have already been taken care
											 // of by block_conditional() therfore we jump
											// past the AT_END checking.
			...
			{
			if (op2_present) WR_PV1_PRELOAD = temp_itu;	
			else WR_PV1_PRELOAD = op1;
			PULSE_PRELOAD1;
			}	
