//START_MODULE_HEADER////////////////////////////////////////////////////////
//
// Filename:    fastrak2a.c
//
// Description: code downloader for Nios development board
//
// Copyright (c) Visi-Trak Worldwide 2003
// 
//
//END_MODULE_HEADER//////////////////////////////////////////////////////////
...
/* This program is built with the following command line
	...
	gcc fastrak2a.c -w -DF_TARGET_WINDOWS=1 -o fastrak2a
	...
	11-13-03
	Fixed problems associated with downloading files over the ethernet port.  Added
	capability to select UDP with the -u switch.
...
int main(int argc, char *argv[])
{
	...
	BOOL bExit = FALSE;
	BOOL bNetwork = FALSE;
	BOOL udp = FALSE; // TCP is the default protocol
	...
	int charpos = 0, x;						// position on line
	...
	if (!bTerminal)
	{
		if (argc < 1)
		{
			printf("### FasTrak2: please specify a filename on the command line\n");
		...
		if (f == NULL)
		{
			printf("### FasTrak2: could not open %s.\n", srecName);
			return -1;
		}
	}
	...
	if(bNetwork)
	{
	  ...
	  if (udp) gPortInfoPtr->socket_id = socket(PF_INET, SOCK_DGRAM, PF_UNSPEC);	
	  else gPortInfoPtr->socket_id = socket(PF_INET, SOCK_STREAM, PF_UNSPEC); // TCP
	...
	{	char c, lineBuf[LINEBUF_SIZE];
		...
		{	struct timeval curTime, lastTime;
			...
			if(bNetwork)
				{
				// First stop control program, if monitor only, this will be ignored.
				lineBuf[0] = '.'; 
				lineBuf[1] = 'H';
				lineBuf[2] = 13;
				lineBuf[3] = 10;
				...
				fflush(stdout); // Dump the response, we don't need it.
			...
			else  // GERMS mode over the serial port
				{
				// ---------
...
gotMonitorChar:
			...
			{	int len;
				...
				do
					...
					} while(c != 10 && c != 13); // wait for CR or LF
		...
		if(bNetwork)
			{
			// Reenable control program, if monitor only, this will be ignored.
			lineBuf[0] = '.'; 
			lineBuf[1] = 'G';
			lineBuf[2] = 13;
			lineBuf[3] = 10;
...
ExecutegotMonitorChar:
			...
			{	int len;
				...
				do
				{
					...
					recv(portHandle, lineBuf, LINEBUF_SIZE, 0);
...
//						if(!isPromptChar(c))
		...
		// enter terminal mode
		printf("\nFasTrak2: Terminal mode (Control-C exits)");
		printf("\n-----------------------------------------\n");
		fflush(stdout);
		bTerminalDone = FALSE;
		if (bTimeout) {
...
//zzzz
			...
			//if(recvfrom(portHandle, lineBuf, LINEBUF_SIZE, 0, 0, 0))
			x=recvfrom(portHandle, lineBuf, LINEBUF_SIZE, 0, 0, 0);
			//printf("x=%d\n", x);
			if (x>0)
			{
			printf("x=%d\n", x);
			for (x=0; x<LINEBUF_SIZE; x++)
				{
				c=lineBuf[x]; printf("c=[%d]\n", c); 
				if ((c == 0x0A || c == 0x0D))
					{
					// 2nd if causes the LF after a CR or CR after LF to do nothing.
					if( (c == 0xa && cLast != 0xd)
...
show_usage:
	printf("Usage: fastrak2 [OPTION(S)] [FILENAME]\n");
	printf("Download code to VTW Ethernet monitor and control board and perform terminal I/O.\n");
	printf("Download through either RS232 serial or network (TCP) connection.\n");
	printf("\n");
	...
	printf("  -u                selects UDP as the network connection protocol, TCP is the default\n");
...
int network_port_read(int portHandle, char *pData, int nCount)
{
  ...
  // check to make sure that there is actually something there
    ...
    if((nready=select(portHandle + 1, &(gPortInfoPtr->set), NULL, NULL, &(gPortInfoPtr->time)))>0)
    {
 	  ...
 	  //b[end]=0;
	  //printf("Receive Bytes: %c, nCount=%d\n", *pData, nCount);
  	  //sleep(1);
          //b[0] = (b[0] & 0x1f) % 26 + 'A';
