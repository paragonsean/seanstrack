...
int convert_ana_ch_from_dynamic(int ch_num, int result_type, int *result)
	...
	unsigned int spi_data;
		...
		// Reenable this when speed problems with upper channels are resolved
		//if (ch_num<0) ch_num=0;
		//else if (ch_num>7)
		//	{
		...
		#if 0 // Also reenable this when speed problems are resolved with upper channels
			if (ch_num > 15) // Upper channels
				{
				// Upper channels are always -10 to +10V = -2048 to +2048 input range	
				//if (adata & 0x00000800) adata |= 0xfffff000; // Negative?
...
void convert_upper_channels(volatile unsigned short *ptr_result, int result_type)
	...
	AD_CMD1=1;
	while (AD_CMD1);
	for (x=4; x<8; x++) ptr_result[x] = ptr_source[x]; // Copy over upper channels
...
void convert_all_channels(volatile unsigned short *ptr_result, int result_type)
	...
	AD_CMD1=1;
	while (AD_CMD1);
	for (x=0; x<4; x++) ptr_result[x+16] = ptr_source[x+4]; // Copy over channels 17 - 20

	set_analog_mode(4);
	AD_CMD1=1;
	while (AD_CMD1);
	for (x=0; x<4; x++) ptr_result[x+20] = ptr_source[x+4]; // Copy over channels 21 - 24
