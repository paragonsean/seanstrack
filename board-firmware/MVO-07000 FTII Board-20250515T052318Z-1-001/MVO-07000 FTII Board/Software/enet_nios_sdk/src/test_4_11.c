...
//////////////////////// FIRMWARE VERSION ////////////////////////
#define FIRMWARE_VERSION "test_4_11,"
...
void pos_based_monitor_isr(int context)
	...
	if (MONITOR_ENABLE && !(ZSPEED))
		...
		pos = RD_PV_POS;
		vel = RD_PV_VEL;
		//printf("[$%d %d]", pos, RD_PV_RAW_VEL_CTR); // For test
...
void time_based_monitor_isr(int context)
	...
	if (PAST_MIN_STK_LEN) // Prep for time based data sample
		{
		//printf("!"); // For test
		//printf("*%d %d*",ONE_MS_COUNTER, RD_PV_RAW_VEL_CTR); // For test
...
void reset_control_program(int saved_warning_action)
	...
	#if BUILD_FOR_INTERRUPT_CONTROL
		//printf("rst interrupt control\n");

		SET_START_PROGRAM_INTERRUPT; // Set the interrupt bit
		//cont_int_output->np_piodata = (vtg.cont_int_out_status |= START_PROG_OUT_BIT_MASK);

	#else // Polled control, we'll set up everything here
		// Also need to clear the watchdog output and the warning output here
		//printf("rst0\n");	
		SET_PROGRAM_RUNNING; //printf("rst1\n");
		CLR_FATAL_ERROR; //printf("rst2\n");
		CLR_WARNING; //printf("rst3\n");
		//OSW1 &= TURN_OFF_CONT_OUTPUTS_EXCEPT_LS_WARN_MASK; 
		TURN_OFF_CONT_OUTPUTS_EXCEPT_LS_WARN; //printf("rst4\n");
		WARNING_WORD=0; //printf("rst5\n");
		FATAL_ERROR_WORD=0;	//printf("rst6\n"); 
	...
	//printf("rst10\n");
		PULSE_WATCHDOG_RESET; //printf("rst11\n");
		update_control_parameters(); //printf("rst12\n");
	#endif
...
int main(void)
	...
	unsigned int timer_load_value, cur_time, check_idle_time=0, mon_supervisor_time=0, test_time=0;
