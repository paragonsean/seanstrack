...
11-22-04 combined_control_blocks2.c
	...
	conditional checking this will be generated.  Also fix a problem with BLOCK_EXECUTION_ERROR 104
	so that it now functions correctly.

4-12-05 combined_control_blocks4.c
...
int pressure_block_exe(c_block control_block, int type_of_execution, int zone)
	...
	static int return_val, goose, cond_result, closed_loop, dp, time;
	...
	if (zone == 1) // Initial entry of block
		...
		if (PRESS_CONTROL_ENABLED)
			...
			if (cond_result < 10 && cond_result > 0) // Some sort of executive error took place.
				...
				commanded_press = (short int)tempu; // Pressure cmd is the lower half word, can be pos. or neg.
	...
	if (zone == 3) // Goose
		...
		zone=5; // Move to zone 5 and begin closing the pressure loop
		}	
