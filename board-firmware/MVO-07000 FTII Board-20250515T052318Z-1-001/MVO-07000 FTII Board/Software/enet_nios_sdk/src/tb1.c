...
// Version that uses the floating channel for channels 9 - 16.  Channels 1-8 and 17-20
// are always converted by the A/D loop on the A/D Nios processor.  Channels 9 -16
// must be converted one at a time using a "floating channel" within the A/D loop.
int convert_ana_ch(int ch_num, int result_type, int *result)
	{
	volatile unsigned short int *shared_memory = (unsigned short int *)0x2000;
	...
	unsigned int spi_data;
		...
		if (ch_num > 7 && ch_num < 16) // Floating channel?
			{
...
#if 0
// Version used when converting all 20 channels
int convert_ana_ch(int ch_num, int result_type, int *result)
	{
	volatile unsigned short int *shared_memory = (unsigned short int *)0x2000;
	int adata=0;
...
int convert_ana_ch(int sample_ctr, int ch_num, int result_type, int *result)
	...
	if (vtg.ad_loop_in_progress)
		{
		//printf("~"); // For test
		goto ext; // Do not allow recursion
		}

	//vtg.ad_loop_in_progress++;  // Do not allow recursion

	if (sample_ctr) // Must be pos or time based data collection
		{
		vtg.ad_loop_in_progress = 1;  // Do not allow recursion

		if (CONV_HI_CHANNELS) sel_hi_channels->np_piodata = 1;
		else sel_hi_channels->np_piodata = 0;
		
		// 1st. 8 halfwords of the data sample struct are the ana. ch. data.
		halfword_ptr = (unsigned short int *)&data_sample[sample_ctr];
			
		adc_spi->np_spitxdata = dynamic_adc_setup.first_ch; // Write setup word to ADC.
		while (!(adc_spi->np_spistatus & np_spistatus_rrdy_mask));
		junk = adc_spi->np_spirxdata;

		adc_spi->np_spitxdata = dynamic_adc_setup.second_ch; // Write setup word to ADC.
		while (!(adc_spi->np_spistatus & np_spistatus_rrdy_mask));
		halfword_ptr[0] = adc_spi->np_spirxdata;

		adc_spi->np_spitxdata = dynamic_adc_setup.third_ch; // Write setup word to ADC.
		while (!(adc_spi->np_spistatus & np_spistatus_rrdy_mask));
		halfword_ptr[1] = adc_spi->np_spirxdata;

		adc_spi->np_spitxdata = dynamic_adc_setup.fourth_ch; // Write setup word to ADC.
		while (!(adc_spi->np_spistatus & np_spistatus_rrdy_mask));
		halfword_ptr[2] = adc_spi->np_spirxdata;

		adc_spi->np_spitxdata = dynamic_adc_setup.fifth_ch; // Write setup word to ADC.
		while (!(adc_spi->np_spistatus & np_spistatus_rrdy_mask));
		halfword_ptr[3] = adc_spi->np_spirxdata;

		adc_spi->np_spitxdata = dynamic_adc_setup.sixth_ch; // Write setup word to ADC.
		while (!(adc_spi->np_spistatus & np_spistatus_rrdy_mask));
		halfword_ptr[4] = adc_spi->np_spirxdata;

		adc_spi->np_spitxdata = dynamic_adc_setup.seventh_ch; // Write setup word to ADC.
		while (!(adc_spi->np_spistatus & np_spistatus_rrdy_mask));
		halfword_ptr[5] = adc_spi->np_spirxdata;

		adc_spi->np_spitxdata = dynamic_adc_setup.eighth_ch; // Write setup word to ADC.
		while (!(adc_spi->np_spistatus & np_spistatus_rrdy_mask));
		halfword_ptr[6] = adc_spi->np_spirxdata;

		adc_spi->np_spitxdata = dynamic_adc_setup.first_ch; // Write setup word to ADC.
		while (!(adc_spi->np_spistatus & np_spistatus_rrdy_mask));
		halfword_ptr[7] = adc_spi->np_spirxdata;

		vtg.ad_loop_in_progress = 0;  
		}
	else  // A general purpose request for analog data
		{
		vtg.ad_loop_in_progress = 2;  // Do not allow recursion
		...
		// Do it twice to get the correct result.
		adc_spi->np_spitxdata = halfword_ptr[ch_num]; // Send it to the SPI
		while (!(adc_spi->np_spistatus & np_spistatus_rrdy_mask));
		spi_data = adc_spi->np_spirxdata;
	...
	//vtg.ad_loop_in_progress--;

	ext: ;
	return (vtg.ad_loop_in_progress);
	}
#endif
...
int convert_ana_ch(int ch_num, int result_type, int *result)
	...
	axis1_interrupt->np_piointerruptmask = 0; // Disable pv1 interrupt.

	if (ch_num > 15) sel_hi_channels->np_piodata = 1; // Select hi channels 
	else sel_hi_channels->np_piodata = 0;
	halfword_ptr = (unsigned short int *)&ad_setup_words;
	for (y=0; y<2; y++) // Do it twice to get the correct result.
		{
		adc_spi->np_spitxdata = halfword_ptr[ch_num]; // Send it to the SPI
		while (!(adc_spi->np_spistatus & np_spistatus_rrdy_mask));
		spi_data = adc_spi->np_spirxdata;
	...
	axis1_interrupt->np_piointerruptmask = 1; // Enable pv1 interrupt.
