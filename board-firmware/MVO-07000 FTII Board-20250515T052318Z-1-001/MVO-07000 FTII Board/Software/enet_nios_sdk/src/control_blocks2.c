...
// Debug enables
#define COND_EXE_DEBUG 1

#define COND_COMPILE_DEBUG 1
#define DWELL_COMPILE_DEBUG 1
#define POS_COMPILE_DEGUG 1
#define VEL_COMPILE_DEBUG 1
#define JUMP_COMPILE_DEBUG 1
#define UPDATE_COMPILE_DEBUG 1
...
int ??????_block_exe(unsigned int (*control_program)[10], unsigned int *prog_ctr)
	{
	...
	if (prog_ctr==NULL) // NULL ptr == an immed exe block.  prog_ctr will not be incremented if immed exe.
		?????_block = (?b *)&control_program[IMMED_EXECUTE_BLOCK][0];
	else
		?????_block = (?b *)&control_program[*prog_ctr][0];
		
	cond_result = block_conditional_exe(?????_block, BEGINNING); 
	...
	if (cond_result < 10 && cond_result > 0) // Some sort of executive error took place.
		...
		if (cond_result != KEEP_GOING_DONT_CALL_BACK) // May be something to set or clear
			block_conditional_exe(?????_block, AT_END); // Checking back at end per block_conditional()'s request.	
		}
	...
	ext: ;	
	if (prog_ctr != NULL) *prog_ctr++; // Increment prog ctr, NULL indicates an immed. exe. block.			
	return(return_val);
	}
#endif
...
int dwell_block_compile(char *str_to_compile, unsigned int (*control_program)[10])
	...
	if ((ptr = (char *)strchr(str_to_compile, '_')) == NULL); // None found
	else 
	   {
		...
		return_val = block_conditional_compile((ub *)&control_program[stepnum][0], ptr+1);
	...
	if (immed_exe)
		...
		dwell_block_exe(control_program, NULL); 
		}
...
// Execute dwell block
int dwell_block_exe(unsigned int (*control_program)[10], unsigned int *prog_ctr)
	{
	int return_val=0, cond_result;
	unsigned int dwell_time_mS, timer_load_value;
	unsigned short int *halfword_ptr;
	...
	if (prog_ctr==NULL) // NULL ptr == an immed exe block.  prog_ctr will not be incremented if immed exe.
		...
		dwell_block = (db *)&control_program[*prog_ctr][0];
		
	cond_result = block_conditional_exe(dwell_block, BEGINNING); 
	...
	// Timing done using nr_timer_milliseconds().
		...
		while((nr_timer_milliseconds() - initial_timer_mS) < dwell_time_mS)
			...
			if (cond_result == KEEP_GOING_CALL_BACK_DURING)
				{ 
				if (block_conditional_exe(dwell_block, DURING) == KEEP_GOING_CALL_BACK_DURING);		
	...
	// Timing done with a seperate timer, timer2.
		...
		timer_load_value = dwell_time_mS*(vtg.v[300]/1000); // Clk. freq./1000 == counts/mS.
		...
		// be serviced here.   
		if (cond_result != KEEP_GOING_DONT_CALL_BACK) // May be something to set or clear
			block_conditional_exe(dwell_block, AT_END); // Checking back at end per block_conditional()'s request.	
		}
	else if (cond_result == COND_NOT_MET); // Nothing more to do 
...
int position_block_exe(unsigned int (*control_program)[10], unsigned int *prog_ctr)
	{
...
int velocity_block_exe(unsigned int (*control_program)[10], unsigned int *prog_ctr)
	{
...
// Execute jump block
int jump_block_exe(unsigned int (*control_program)[10], unsigned int *prog_ctr)
	{
...
int update_block_compile(char *str_to_compile, unsigned int (*control_program)[10])
	...
	if (immed_exe)
		...
		// Execute the block we just compiled.  Send a NULL ptr for the prog_ctr
		// to indicate this is an immed. exe. block.
		update_block_exe(control_program, NULL); 
		}
...
int update_block_exe(unsigned int (*control_program)[10], unsigned int *prog_ctr)
	{
	int return_val=0, cond_result, op1, op2, ctr=0, op2_present=1;
	ub *update_block=0;
	vp_pvu *pv1 = va_pv1;
	...
	if (prog_ctr==NULL) // NULL ptr == an immed exe block.  prog_ctr will not be incremented if immed exe.
		...
		update_block = (ub *)&control_program[*prog_ctr][0];
		
	cond_result = block_conditional_exe(update_block, BEGINNING); 
...
typedef volatile struct
{
	...
	unsigned int cond_op2_df; 				//Word9
		...
		if (update_block->ub_indicator_bits & ub_op1_const_mask)
			...
			if (get_analog_input_voltage(update_block->op1_df, &op1))	
				return(CANT_CONVERT_ANA_CH);
			}
		else if (update_block->ub_indicator_bits & ub_op1_dac_mask)
			{
			get_dac_output_voltage(update_block->op1_df, &op1)
			}
		else if (update_block->ub_indicator_bits & ub_op1_pos_mask)
			{
			// Assume 1 axis for now. blkptr->cond_op1_df will contain the
			// axis number.  May have to support more than 1 axis in the
			// future. 
			op1 = RD_PV1_POS; 
			}	
			...
			op1 = 0xffffffff/(RD_PV1_VEL*VEL1_MUL);
			}
		else 
			{
			return_val = EXE_FAULT_IND_BITS;
		...
		if (!op2_present)
			...
			if (update_block->ub_indicator_bits & ub_itu_var_mask)
				{
				vtg.v[update_block->itu_df] = op1;
				}
			else if (update_block->ub_indicator_bits & ub_itu_dac_out_mask)
				{
				out_dac(update_block->itu_df, 0xffffffff, op1); // op1 should be voltage * 1000	
				}
			else if (update_block->ub_indicator_bits & ub_itu_pos_mask)
				{
				// For now assume axis 1.  The itu_df contains the axis number.
				// May support more axes in the future.
				WR_PV1_PRELOAD = op1;	
				}
			...
			else  // Gotta have something to update or this can't be an update block.
				{
				return_val = EXE_FAULT_IND_BITS;
				goto ext;
				}			
		...
		// be serviced here.   
		if (cond_result != KEEP_GOING_DONT_CALL_BACK) // May be something to set or clear
			block_conditional_exe(update_block, AT_END); // Checking back at end per block_conditional()'s request.	
		}
	else if (cond_result == COND_NOT_MET); // Nothing more to do 
...
inline int set_clr_discrete_output(int output_num, int action)
	{
	int return_val=0;
...
// 1 (ON) or 0 (OFF).
inline int chk_discrete_input(int input_num, int *input_state)
	{
	int return_val=0;	
...
int block_conditional_exe(ub *blkptr, int when_called)
	{
	int cond_op1, cond_op2, ctr=0, condition=0, condition_met=0;
	int while_ = 0;
	...
	if (when_called == AT_END)
		...
		if ((blkptr->cond_indicator_bits & while_mask) ||
			...
			if (blkptr->cond_indicator_bits & op1_discrete_io_mask)
				...
				//
				// Convert velocity to X1 pulses/sec	 
				cond_op1 = 0xffffffff/(RD_PV1_VEL*VEL1_MUL);
				...
				{
				COMMAND_FROM_MAIN = 0x80000000 | blkptr->cond_op1_df;
					while (COMMAND_FROM_MAIN)
						{
						ctr++;
						if (ctr == 100000) return(CANT_CONVERT_ANA_CH);
						}
					cond_op1 = DATA_FROM_MON & 0x00000fff; 
						
				if (blkptr->cond_op1_df > 16) // Upper channels
					{
					// Upper channels are always -10 to +10V = -2048 to +2048 input range	
					if (cond_op1 & 0x00000800) cond_op1 |= 0xfffff000; // Negative?
					// Now convert to voltage * 1000.
					cond_op1 = (cond_op1*10000)/2048;
					}
				else // Lower channels, these can only be positive.
					{
					// First look at V313 the config word to determine the range.
					if (vtg.v[313] & 1<<(blkptr->cond_op1_df-1)) // 0-5V input range.
						// Convert to voltage * 1000.
						cond_op1 = (cond_op1*5000)/4095; // 0-5V=0-4095.	
					else // 0-2.5V range. 
						// Convert to voltage * 1000.
						cond_op1 = (cond_op1*2500)/4095; // 0-2.5V=0-4095.	
					}		
				}
			...
			if (blkptr->cond_indicator_bits & input_on_mask)
				...
				chk_discrete_input(cond_op1, &condition_met); // condition_met will == 1 if input on
				} 
...
int get_analog_input_voltage(int ch_num, int *voltage_x_1000)
	{
	int ctr=0, adata;
	
	if (ch_num<0 || ch_num>20) return(1); // Invalid channel number
	else
		{	
		COMMAND_FROM_MAIN = 0x80000000 | ch_num;
		while (COMMAND_FROM_MAIN)
			{
			ctr++;
			if (ctr == 100000) return(1);
			}
		adata = DATA_FROM_MON & 0x00000fff; 
			
		if (ch_num > 16) // Upper channels
			{
			// Upper channels are always -10 to +10V = -2048 to +2048 input range	
			if (adata & 0x00000800) adata |= 0xfffff000; // Negative?
			// Now convert to voltage * 1000.
			*voltage_x_1000 = (adata*10000)/2048;
			}
		else // Lower channels, these can only be positive.
			{
			// First look at V313 the config word to determine the range.
			if (vtg.v[313] & 1<<(ch_num-1)) // 0-5V input range.
				// Convert to voltage * 1000.
				*voltage_x_1000 = (adata*5000)/4095; // 0-5V=0-4095.	
			else // 0-2.5V range. 
				// Convert to voltage * 1000.
				*voltage_x_1000 = (adata*2500)/4095; // 0-2.5V=0-4095.	
			}
		}		
	return(0);		
	}
...
int get_dac_output_voltage(int ch_num, int *voltage_x_1000)
	{
	if (ch_num<1 || ch_num>4) return(1);
	else	
...
|	  |
|     discrete output number indicator, if set, data field represents
|	  a discrete I/O number to set/clear.
|	
Unused at this time, reserved.
...
Low 16 bits, 1st. operand, advanced conditional check
   ...
   | | | | | | Extended variable indicator, 32 bit value.
   | | | | | |
   | | | | | discrete I/O number indicator, data field indicates I/O number.
   | | | | |
