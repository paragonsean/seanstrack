//#include "excalibur.h"
//#include "nios.h"
//#include "control_program_blocks.h"
...
printf("word%d %x\n", x, control_program[stepnum][x]);
	...
	dwell_block->word0.blk_id = DWELL;
...
printf("Got past block_conditional()\n");
	...
	switch (*ptr) 
		...
		case 'H':
			...
			dwell_block->word1.dwell_const_indicator = 1;
		...
		case 'V': //Variable
			...
			dwell_block->word1.dwell_var_indicator = 1;
			dwell_block->dwell_df = tempi;
			break;
...
printf("word%d %x\n", x, control_program[stepnum][x]);
	...
	// Set the block ID, after this, any aborts must goto clr_id
	// to clear the block ID before exiting.
	position_block->word0.blk_id = POSITION;
	...
	printf("item_being_scanned = %d\n", item_being_scanned);
		...
		if (item_being_scanned == PB_POS)
			...
			if ((ptr = (char *)strchr(str_to_compile, 'L')) == NULL) {return_val = 1; goto clr_id;}	
				...
				else if (*ptr == '1') position_block->word1.axis1_indicator = 1;
				else if (*ptr == '2') position_block->word1.axis2_indicator = 1;
				else if (*ptr == '3') position_block->word1.axis3_indicator = 1;
				else if (*ptr == '4') position_block->word1.axis4_indicator = 1;
		...
		switch (*ptr) 
			...
			case 'H':
				...
				if (item_being_scanned == PB_POS)
					{
					position_block->word1.pos_const_indicator = 1;	
					position_block->pos_df = tempi;
					}
				else if (item_being_scanned == PB_VEL)
					{
					position_block->word1.vel_const_indicator = 1;	
			...
			case 'V': //Variable
				...
				if (item_being_scanned == PB_POS)
					{
					position_block->word1.pos_var_indicator = 1;
					position_block->pos_df = tempi;
					}
				else if (item_being_scanned == PB_VEL)
					{
					position_block->word1.vel_var_indicator = 1;
					position_block->vel_df = tempi; 	
					}
				break;
...
clr_id: ;
if (return_val != 0) // Error of some kind, clear the block ID
	position_block->word0.blk_id = NULL;
// Now print the block for debug
for (x=0; x<10; x++)
	printf("Word%d %x\n", x, control_program[stepnum][x]);	
...
printf("word%d %x\n", x, control_program[stepnum][x]);
	...
	// Set the block ID, after this, any aborts must goto clr_id
	// to clear the block ID before exiting.
	velocity_block->word0.blk_id = VELOCITY;
	...
	printf("item_being_scanned = %d\n", item_being_scanned);
		...
		if (item_being_scanned == VB_VEL)
			...
			if ((ptr = (char *)strchr(str_to_compile, 'V')) == NULL) {return_val = 1; goto clr_id;}	
				...
				ptr+=2;	
				if ((*ptr > NUM_OF_AXES+48) || (*ptr < 49)) {return_val = 1; goto clr_id;} // Axis# out of range	
				else if (*ptr == '1') velocity_block->word1.axis1_indicator = 1;
				else if (*ptr == '2') velocity_block->word1.axis2_indicator = 1;
				else if (*ptr == '3') velocity_block->word1.axis3_indicator = 1;
				else if (*ptr == '4') velocity_block->word1.axis4_indicator = 1;
			printf("Axis number is %c\n", *ptr);	
				ptr++; // Move past the axis specifier
				}
			printf("found a V %s\n", ptr-3);
		...
		switch (*ptr) 
			...
			case 'H':
				...
				if (item_being_scanned == VB_VEL || item_being_scanned == VB_ACCEL)
					...
					if (*ptr == '-') {return_val = 1; goto clr_id;}
						...
						if (item_being_scanned == VB_VEL)
							{
							velocity_block->word1.vel_const_indicator = 1;	
							velocity_block->vel_df = tempi;
							}
						else
							{
							velocity_block->word1.accel_const_indicator = 1;	
					...
					velocity_block->word1.endpos_const_indicator = 1;
			...
			case 'V': //Variable
				...
				if (item_being_scanned == VB_VEL)
					{
					velocity_block->word1.vel_var_indicator = 1;
					velocity_block->vel_df = tempi;
					}
				else if (item_being_scanned == VB_ACCEL)
					{
					velocity_block->word1.accel_var_indicator = 1;
					velocity_block->accel_df = tempi; 	
					}
				else if (item_being_scanned == VB_EP)
					{
					velocity_block->word1.endpos_var_indicator = 1;
					velocity_block->endpos_df = tempi; 	
					}		
				break;
...
clr_id: ;
if (return_val != 0) // Error of some kind, clear the block ID
	velocity_block->word0.blk_id = NULL;
// Now print the block for debug
for (x=0; x<10; x++)
	printf("Word%d %x\n", x, control_program[stepnum][x]);	
...
printf("word%d %x\n", x, control_program[stepnum][x]);
	...
	// Set the block ID, after this, any aborts must goto clr_id
	// to clear the block ID before exiting.
	jump_block->word0.blk_id = JUMP;
	
printf("ID should be there now - let's see\n");
for (x = 0; x<10; x++)
printf("word%d %x\n, ID = %d", x, control_program[stepnum][x], JUMP);
...
printf("Got past block_conditional()\n");
	...
	if ((ptr = (char *)strchr(str_to_compile, 'J')) == NULL) {return_val = 1; goto clr_id;}	
		...
		{
		if (*(ptr-1) == '-') {jump_block->word1.rel_abs_specifier = 1; rel = 1;}
	printf("Found rel jump %s\n", ptr-1);
		ptr++; // Now point to the jump type
		tempi = atoi(ptr);
		if (tempi < 0 || tempi > 2) {return_val = 1; goto clr_id;} // Jump type out of range
		else if (tempi == 0) jump_block->word1.type_normal = 1;
		else if (tempi == 1) jump_block->word1.type_sub = 1;
		else if (tempi == 2) jump_block->word1.type_return = 1;
		ptr+=2;
		if (rel)
			{
			if (*ptr == '-') {jump_block->word1.rel_dir = 0; ptr++;}
			else if (*ptr == '+') {jump_block->word1.rel_dir = 1; ptr++;}
			else jump_block->word1.rel_dir = 1; // No sign, must be a forward jump, don't increment ptr
			}
		...
		switch (*ptr) 
			...
			case '9':
				...
				jump_block->word1.jump_const_indicator = 1;	
			...
			case 'V': //Variable
				ptr++;
				if ((tempi = atoi(ptr)) > MAX_VAR) {return_val = 1; goto clr_id;}
				jump_block->word1.jump_var_indicator = 1;	
				jump_block->jump_df = tempi;
				break;
...
printf("word%d %x\n", x, control_program[stepnum][x]);
	...
	// Set the block ID, after this, any aborts must goto clr_id
	// to clear the block ID before exiting.
	update_block->word0.blk_id = UPDATE;
	...
	for (item_being_scanned=ITU; item_being_scanned<=OP2; item_being_scanned++)
		...
		switch (*ptr) 
			...
			case 'H':
				if (item_being_scanned == ITU) {return_val = 1; goto clr_id;} // Item to update cannot be a constant	
					...
					if (item_being_scanned == OP1) 
						{
						update_block->word1.op1_const_indicator = 1;
						update_block->op1_df = tempi;
						}
					else if (item_being_scanned == OP2) 
						{
						update_block->word1.op2_const_indicator = 1;
			...
			case 'V': //Variable
				...
				if (item_being_scanned == ITU)
					{
					update_block->word1.itu_var_indicator = 1;
					update_block->itu_df = tempi;
					}
				else if (item_being_scanned == OP1)
					{
					update_block->word1.op1_var_indicator = 1;
					update_block->op1_df = tempi; 	
					}
				else if (item_being_scanned == OP2)
					{
					update_block->word1.op2_var_indicator = 1;
					update_block->op2_df = tempi; 	
					}		
				break;
			...
			case 'A': //Axis position
				...
				if (item_being_scanned == OP2) {return_val = 1; goto clr_id;}
					...
					if (item_being_scanned == ITU)
						{
						update_block->word1.itu_pos_indicator = 1;
						update_block->itu_df = tempi;	
						}
					else if (item_being_scanned == OP1)
						{
						update_block->word1.op1_pos_indicator = 1;
						update_block->op1_df = tempi; 	
						}
					}
				break;
			...
			case 'C': //Command
				...
				if (item_being_scanned == OP1 || item_being_scanned == OP2)
					...
					update_block->word1.itu_command = 1;
					update_block->itu_df = tempi; 
					}
				break;
			...
			case 'D': //DAC output channel
				...
				if (item_being_scanned == OP2) {return_val = 1; goto clr_id;}	
					...
					if (item_being_scanned == ITU)
						{
						update_block->word1.itu_dac_out_indicator = 1;
						update_block->itu_df = tempi;
						// Now check for accel
						ptr++; 
						if (*ptr == 'A') //Accel specified
							...
							if (*ptr == 'V') //Variable
								...
								if (tempi<0 || tempi>MAX_VAR) {return_val = 1; goto clr_id;}
									...
									{
									update_block->word1.vramp_var_indicator = 1;
									update_block->vramp_df = tempi;
									}
								}
							else //Constant
								{
								update_block->word1.vramp_const_indicator = 1;
								if (*ptr == '+' || (*ptr >= '0' && *ptr <= '9')) 
						...
						{
						update_block->word1.op1_dac_indicator = 1;
						update_block->op1_df = tempi; 	
						}	
					}
				break;
			...
			case 'N': //Analog input channel
				...
				if (item_being_scanned == ITU || item_being_scanned == OP2)
					...
					if (item_being_scanned == OP1)
						{
						update_block->word1.op1_adc_indicator = 1;
						update_block->op1_df = tempi; 	
						}
					}
				break;
			...
			case 'F': //Velocity
				...
				if (item_being_scanned == ITU || item_being_scanned == OP2)
					...
					if (item_being_scanned == OP1)
						{
						update_block->word1.op1_vel_indicator = 1;
						update_block->op1_df = tempi;	
						}
					}
				break;
		...
		if (item_being_scanned == OP1)
			...
			do
				{
				if (*ptr == NULL) break;
				else if (*ptr == '+') {update_block->word1.operator_ = PLUS; break;}
				else if (*ptr == '-') {update_block->word1.operator_ = MINUS; break;}
				else if (*ptr == '*') {update_block->word1.operator_ = SUB; break;}
				else if (*ptr == '/') {update_block->word1.operator_ = DIV; break;}
				else if (*ptr == '&') {update_block->word1.operator_ = AND; break;}
				else if (*ptr == '|') {update_block->word1.operator_ = OR; break;}
				else if (*ptr == '^') {update_block->word1.operator_ = XOR; break;}
				ptr++;
				} while (1);

			if (*ptr == NULL)
				{
				update_block->word1.operator_ = NULL; // No operator	
				break; // Quit for loop
				}
...
clr_id: ;
if (return_val != 0) // Error of some kind, clear the block ID
	update_block->word0.blk_id = NULL;
// Now print the block for debug
for (x=0; x<10; x++)
	printf("Word%d %x\n", x, control_program[stepnum][x]);	
...
printf("condstr is %s\n", condstr);	
	if (*ptr == 'I' || *ptr == 'W')
		{
		if (*ptr == 'I') blkptr->word7.if_indicator = 1; 
		else blkptr->word7.while_indicator = 1;
		ptr++;
		if (*ptr == '1' || *ptr == '0') //Basic form conditional check
			{
			blkptr->word7.op1_descrete_io_indicator = 1;
			if (*ptr == '1') blkptr->word7.input_on_indicator = 1;
			else blkptr->word7.input_off_indicator = 1;	
			
			ptr += 2;
			if (*ptr == 'V')
			 	{
				ptr++;
				blkptr->word7.op1_var_indicator = 1;
			...
			{
			blkptr->word7.advanced_testing_indicator = 1;
			...
			switch (*ptr) //1st. operand
				...
				case 'A': //Axis position
					blkptr->word7.op1_pos_indicator = 1;
					ptr++;
					x = atoi(ptr);
				...
				case 'N': //Analog input channel
					blkptr->word7.op1_adc_indicator = 1;
					ptr++;
					x = atoi(ptr);
				...
				case 'D': //DAC output channel
					blkptr->word7.op1_dac_indicator = 1;
					ptr++;
					x = atoi(ptr);
				...
				case 'F': //Velocity
					blkptr->word7.op1_vel_indicator = 1;
					ptr++;
					x = atoi(ptr);
					if (x<1 || x>NUM_OF_AXES) {return_val = 1; goto ext;} //Axis #
					blkptr->word8.cond_op1_df = x; 
			...
			do
				...
				if (*ptr == NULL) {return_val = 1; goto ext; break;}
					...
					if (*ptr = '=') // >= operator 
						...
						blkptr->word7.operator_ = GTR_THAN_EQU_TO;
						break;
						}
					else {blkptr->word7.operator_ = GTR_THAN; break;} // > operator	
					} 
				else if (*ptr == '<')
					{
					ptr++; 
					if (*ptr = '=') // <= operator 
						{
						ptr++; //Move beyond the operator;
						blkptr->word7.operator_ = LESS_THAN_EQU_TO;
						break;
						}
					else {blkptr->word7.operator_ = LESS_THAN; break;} // < operator	
					} 
				else if (*ptr == '=') {blkptr->word7.operator_ = EQUAL; ptr++; break;}
				else if (*ptr == '!') {blkptr->word7.operator_ = NOT_EQUAL; ptr+=2; break;}
				ptr++;
				} while (1);
			...
			if (*ptr == '+' || *ptr == '-' || (*ptr >= '0' && *ptr <= '9')) 
				{
				blkptr->word7.op2_const_indicator = 1;
				...
				switch (*ptr) //1st. operand
					...
					case 'V': //Variable
						blkptr->word7.op2_var_indicator = 1;
						ptr++;
						x = atoi(ptr);
						if (x<0 || x>MAX_VAR) {return_val = 1; goto ext;}
	...
	if (*ptr == 'B' || *ptr == 'D' || *ptr == 'E')
	   {
		if (*ptr == 'B') blkptr->word7.set_out_beginning = 1; 
		else if (*ptr == 'D') blkptr->word7.set_out_during = 1; 
		else if (*ptr == 'E') blkptr->word7.set_out_at_end = 1;
			
		ptr++;
		if (*ptr == '1') blkptr->word7.turn_on_turn_off = 1;
		else if (*ptr == '0') blkptr->word7.turn_on_turn_off = 0; 
		...
		if (*ptr >= '0' && *ptr <= '9')
			...
			blkptr->word8.set_out_df = x;	
			}
		else if (*ptr = 'V')
			{
			blkptr->word7.set_out_var_indicator = 1;	
			ptr++;
			x = atoi(ptr);
			if (x<0 || x>MAX_VAR) {return_val = 1; goto ext;}
			blkptr->word8.set_out_df = x;	
