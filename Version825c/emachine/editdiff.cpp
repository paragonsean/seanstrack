...
if ( t.open(s, DCURVE_RECLEN, PFL) )
    ...
    cell.SetText( StringTable.find(TEXT("GRID_NUMBER_STRING")) );
    Grid.SetCell( 0,-1, &cell );

    cell.SetText( StringTable.find(TEXT("GRID_NAME_STRING")) );
    Grid.SetCell( 1,-1, &cell );

    cell.SetText( StringTable.find(TEXT("GRID_HEAD_COEF_STRING")) );
    Grid.SetCell( 2,-1, &cell );

    cell.SetText( StringTable.find(TEXT("GRID_ROD_COEF_STRING")) );
...
/***********************************************************************
*                    SET_EDIT_DCURVE_SETUP_STRINGS                     *
***********************************************************************/
static void set_edit_dcurve_setup_strings( HWND hWnd )
{
STRING_CLASS s;
WINDOW_CLASS w;
int          i;
...
static RESOURCE_TEXT_ENTRY rsn[] = {
    { EDITDIFF_CANCEL_BUTTON,       TEXT("CANCEL") },
    { EDITDIFF_SAVE_CHANGES_BUTTON, TEXT("SAVE_CHANGES") },
    { DELETE_EQUATION_BUTTON,       TEXT("DELETE_STRING") },
    { INSERT_EQUATION_BUTTON,       TEXT("INSERT") }
    };
...
w = hWnd;
w.set_title( StringTable.find(TEXT("EDIT_DCURVE")) );
w.refresh();
}
...
switch (msg)
    ...
    case WM_INITDIALOG:
        ...
        set_edit_dcurve_setup_strings( hWnd );
