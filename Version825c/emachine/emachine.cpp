...
static int nof_rsn = sizeof( rsn)/sizeof(RESOURCE_TEXT_ENTRY);
...
for ( i=0; i<nof_rsn; i++ )
    ...
    if ( !have_message )
        ...
        if ( msg_id == rsn[i].id )
            {
            message = StringTable.find( rsn[i].s );
            have_message = true;
            }
        }
    if ( !have_title )
        ...
        if ( title_id == rsn[i].id )
            {
            title = StringTable.find( rsn[i].s );
            have_title = true;
            }
        }
...
w = hWnd;
w.set_title( StringTable.find(TEXT("CHOOSE_SERVO")) );
w.refresh();
}
    ...
    case WM_COMMAND:
        ...
        switch ( id )
            ...
            case ENABLE_MULTIPART_RUNLIST_XB:
                m->multipart_runlist_settings.enable = tobool( is_checked(MachSetupWindow, ENABLE_MULTIPART_RUNLIST_XB) );
                Machines[CurrentMachine].has_changed = TRUE;
                break;
