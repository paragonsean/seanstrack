...
struct TEXT_PARAMETER_ENTRY
    {
    int          parameter_number;
    STRING_CLASS value;

    TEXT_PARAMETER_ENTRY() { parameter_number = NO_PARAMETER_NUMBER; }
    void operator=(TEXT_PARAMETER_ENTRY & sorc) { parameter_number=sorc.parameter_number; value=sorc.value; }
...
class FTII_FILE_DATA {
    ...
    int                  nof_text_parameters;  // Added in file version 4
    TEXT_PARAMETER_ENTRY * text_parameter;
    ...
    BOOLEAN write_csv( STRING_CLASS & dest_file_path );
    BOOLEAN write_raw_positions_csv( STRING_CLASS & dest_file_path );
    BOOLEAN resize_text_parameters( int new_count, BOOLEAN copy_old_recs );
    };
...
class FTII_ARRAY_DATA {
    ...
    double   max_average_velocity( int & max_index, int start_index, int end_index, int nof_points_on_each_side, double multiplier );
