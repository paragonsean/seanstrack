...
static NAME_CLASS   LocalIniFile;
...
if ( is_checked(CopyLogWindow.handle(), SAVE_TO_NETWORK_XBOX) )
    ...
    while ( c.next() )
        ...
        if ( !c.is_this_computer() )
            ...
            StatusListbox.add( s.text() );
            WritePrivateProfileString( ConfigSection, filekey,   file.text(),   s.text() );
            WritePrivateProfileString( ConfigSection, formatkey, format.text(), s.text() );
            }
        }
    }
...
{
static STRING_CLASS m( TEXT("M01") );
static STRING_CLASS old_part( TEXT("OLD_PART") );
static STRING_CLASS new_part( TEXT("NEW_PART") );
...
LocalIniFile.get_exe_dir_file_name( VisiTrakIniFile );
