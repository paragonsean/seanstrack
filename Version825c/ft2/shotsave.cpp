...
static bool    ShotsaveLogIsEnabled = false;;
...
static NAME_CLASS ShotsaveLogFile;

static TCHAR ShotparmLogFile[]           = TEXT( "shotparm-shotsave.log" );
...
static TCHAR ShotsaveLogEnableKey[]      = TEXT( "ShotsaveLogEnable" );
...
/*
--------------------------------------------------------------------------------------------------
The following is in board_monitor. It gets the external parameters and copies the values into dest
-------------------------------------------------------------------------------------------------- */
...
{
static TCHAR FileName[] = SAVE_CSV_WITH_SHOT_NAME;

FILE_CLASS f;
NAME_CLASS s;

s.get_part_dir_file_name( pn.computer, pn.machine, pn.part, FileName );

if ( !s.file_exists() )
    s.get_exe_dir_file_name( pn.computer, FileName );
...
*                           SHOT_BUFFER_ENTRY                          *
***********************************************************************/
SHOT_BUFFER_ENTRY::SHOT_BUFFER_ENTRY()
{
flags       = 0;
has_alarm   = false;
has_warning = false;
param_value.init( MAX_PARMS );
save_failed = false;
}
...
*                            LOG_SHOTSAVE_ERROR                        *
***********************************************************************/
static void log_shotsave_error( STRING_CLASS & message )
{
STRING_CLASS s;
FILE_CLASS   f;
TIME_CLASS   t;
...
s = t.text();
s += message;
f.open_for_append( ShotsaveLogFile );
f.writeline( s );
f.close();
}
...
/*
-----------------------------------------------------------------------------------
Get the name of the shotsave error log file. This is for logging errors when trying
to save to or update the SHOTPARM.CSV file.
----------------------------------------------------------------------------------- */
ShotsaveLogFile.get_log_dir_file_name( TEXT("ShotsaveLog.txt") );

cp = get_ini_string( MonallIniFile, ConfigSection, ShotsaveLogEnableKey );
if ( *cp == YChar )
    ShotsaveLogIsEnabled = true;
...
/***********************************************************************
*        UPDATE_FTII_PARAMETER_RECORD_WITH_EXTERNAL_PARAMETERS         *
***********************************************************************/
static bool update_ftii_parameter_record_with_external_parameters( SHOT_BUFFER_ENTRY * b, PARAMETER_CLASS & pc )
{
...
if ( !s.file_exists() )
    ...
    /*
    ------------------------------------------------------------------
    This should never happen but if it does return true to stop trying
    ------------------------------------------------------------------ */
    return true;
    }
...
vdb.set_nof_cols( SHOTPARM_NOF_COLS );
if ( vdb.open_for_read(s) )
    {
    openok = true;
    opentime.get_local_time();
    ...
    write_shotparm_log( b->pn.machine, b->pn.part, TEXT("update_ftii_parameter_record_with_external_parameters"), openok, opentime, closetime, ShotparmLogFile );

    if ( status )
        {
        for ( i=0; i<b->f.nof_text_parameters; i++ )
            {
            pi = b->f.text_parameter[i].parameter_number - 1;
            if ( pc.parameter[pi].input.type == FT_EXTERNAL_PARAMETER )
                {
                si = SHOTPARM_PARAM_1_INDEX + pi;
                vdb.ra.goto_index( si );
                vdb.ra = b->f.text_parameter[i].value;
                }
            }
        openok = false;
        opentime.get_local_time();
        closetime = opentime;
        if ( vdb.open_for_rw(s) )
            {
            openok = true;
            opentime.get_local_time();
            vdb.goto_last_record();
...
***********************************************************************/
static BOOLEAN write_ftii_parameters( SHOT_BUFFER_ENTRY * b, SYSTEMTIME & st, BITSETYPE save_flags )
{
TIME_CLASS  closetime;
int         i;
DWORD       n;
...
if ( t.open_for_rw(s) )
    ...
    t.close();
    closetime.get_local_time();
    write_shotparm_log( b->pn.machine, b->pn.part, TEXT("write_ftii_parameters"), openok, opentime, closetime, ShotparmLogFile );
    return TRUE;
    }
...
* set the BufPtr, I should no longer make any changes to it from       *
* board_monitor. This is the only routine that is known globally.      *
***********************************************************************/
...
for ( i=0; i<MAX_SHOTSAVE_BUFFERS; i++ )
    ...
    if ( BufPtr[i] == 0 )
        ...
        if ( b )
            ...
            b->f.move( fsorc );
            b->param_value = psorc;
            b->external_parameter_status      = external_parameter_status;
            b->has_alarm                      = new_has_alarm;
            b->has_warning                    = new_has_warning;
            b->pn                             = partname;
            b->save_failed                    = false;
            b->flags                          = 0;
            if ( !b->external_parameter_status.is_enabled )
                b->flags = SHOTPARM_HAS_BEEN_UPDATED | SHOT_FILE_HAS_BEEN_UPDATED;
...
for ( i=0; i<MAX_PARMS; i++ )
    ...
    if ( ShowParam[i] )
        {
        s.cat_w_char( ascii_double(b->param_value[i]), DelimitChar );
        }
    }
    ...
    if ( is_numeric(*cp) )
        ...
        if ( n >= 0 && n <= MAX_PARMS )
            ...
            if ( pc.parameter[n].vartype == TEXT_VAR )
                ...
                {
                rounded_double_to_tchar( bp, b->param_value[n] );
                if ( EosFixedWidth > 0 )
        ...
        *bp = NullChar;
        ...
        /*
        ------------------------------------------------------------------------------
        Put an * if there are no alarms. This will be overwritten if there is an alarm
        ------------------------------------------------------------------------------ */
        *bp     = AsterixChar;
        *(bp+1) = NullChar;
                ...
                if ( pc.parameter[j].vartype != TEXT_VAR )
                    ...
                    if ( pc.parameter[j].is_alarm(x) )
                        ...
                        if ( *(cp+1) == IChar )
                            ...
                            lstrcpy( bp, pc.parameter[j].name );
                        break;
                        }
                    }
                }
...
/***********************************************************************
*                         POST_NEWSHOT_MESSAGE                         *
***********************************************************************/
static void post_newshot_message( PROFILE_NAME_ENTRY & pn )
{
TCHAR      * cp;
int          n;
STRING_CLASS s;
...
s.empty();
n = pn.machine.len() + pn.part.len() + 1;
if ( s.init(n) )
    {
    s = pn.machine;
    s += CommaChar;
    s += pn.part;
    cp = maketext( pn.shot.text() );
    if ( cp )
...
FIELDOFFSET                       fo;
int                               n;
bool                              need_to_update_graphlst;
PARAMETER_CLASS                   pc;
PART_NAME_ENTRY                   part_name;
short                             record_length;
NAME_CLASS                        s;
...
if ( !(b->flags & SHOT_FILE_HAS_BEEN_SAVED) )
    {
    if ( !nextshot(b) )
        return;
...
need_to_update_graphlst = false;

/*
------------------------------------------------------------------------------
Write the profile. If this is the second time this shot is being saved it will
...
if ( !(b->flags & SHOT_FILE_HAS_BEEN_SAVED) )
    {
    b->f.put( s );
    b->flags |= SHOT_FILE_HAS_BEEN_SAVED;
    if ( !b->external_parameter_status.waiting_for_file )
        b->flags |= SHOT_FILE_HAS_BEEN_UPDATED;
    need_to_update_graphlst = true;
    }
else if ( b->external_parameter_status.is_enabled && !b->external_parameter_status.waiting_for_file )
    {
    if ( !(b->flags & SHOT_FILE_HAS_BEEN_UPDATED) )
        {
        b->f.put( s );
        b->flags |= SHOT_FILE_HAS_BEEN_UPDATED;
        }
...
/*
-----------------------------------
Save a csv version of the shot file
----------------------------------- */
if ( file_attributes == FILE_ATTRIBUTE_ARCHIVE && !(b->flags & CSV_SHOT_FILE_HAS_BEEN_SAVED) )
    {
    if ( csv_shot_file.get( b->pn ) )
        ...
        if ( csv_shot_file.is_enabled && !csv_shot_file.dest_file_dir.isempty() )
            ...
            if ( s.contains(LeftCurlyBracket) )
                {
                tc.set( b->f.shot_time );
                part_name.set( b->pn.computer, b->pn.machine, b->pn.part );
                s = dynamic_file_name( tc, part_name, s.text() );
                }
    ...
    b->flags |= CSV_SHOT_FILE_HAS_BEEN_SAVED;
    }

/*
----------------------------------------
I need to have the parameter definitions
---------------------------------------- */
...
if ( !(b->flags & SHOTPARM_HAS_BEEN_SAVED) )
    {
    if ( write_ftii_parameters(b, st, save_flags) )
        {
        b->flags |= SHOTPARM_HAS_BEEN_SAVED;
        if ( !b->external_parameter_status.waiting_for_file )
            {
            b->flags |= SHOTPARM_HAS_BEEN_UPDATED;
            b->flags |= NEED_TO_SEND_PARAMETER_EVENT_STRING;
            }
        b->flags |= NEED_TO_POST_NEWSHOT_MESSAGE;
        }
    }
else if ( !b->external_parameter_status.waiting_for_file  )
    {
    if ( !(b->flags & SHOTPARM_HAS_BEEN_UPDATED) )
        {
        if ( update_ftii_parameter_record_with_external_parameters(b, pc) )
            {
            b->flags |= SHOTPARM_HAS_BEEN_UPDATED;
            }
        else
            {
            if ( ShotsaveLogIsEnabled )
                {
                s = TEXT(" Unable to update SHOTPARM.CSV ");
                s += b->pn.machine;
                s += SpaceChar;
                s += b->pn.shot;
                log_shotsave_error( s );
                }
            }
        }
    /*
    ---------------------------------------------------------
    I need to set these if I save the external parameters but
    also if I do not get the external parameters at all.
    --------------------------------------------------------- */
    if ( b->flags & SHOTPARM_HAS_BEEN_UPDATED )
        {
        b->flags |= NEED_TO_SEND_PARAMETER_EVENT_STRING;
...
/*
-------------------------------------------------------------------------------------------
I only write these files if there are no external parameters or if I have read them already
If b->shot_file_has_been_updateded then I know I have read the external parameters and I
can write the output files now.
------------------------------------------------------------------------------------------- */
if ( !(b->flags & OUTPUT_FILES_HAVE_BEEN_CREATED) )
    {
    u = SHOT_FILE_HAS_BEEN_SAVED | SHOT_FILE_HAS_BEEN_UPDATED;
    if ( (b->flags & u) == u )
        {
        if ( EosOutputFile )
        ...
        b->flags |= OUTPUT_FILES_HAVE_BEEN_CREATED;
        }
     }
}
...
SHOT_BUFFER_ENTRY * b;
TCHAR             * cp;
bool                have_new_external_parameters;
int                 i;
FILETIME            i_time;
int                 i_to_save;
int                 j;
...
for ( i=0; i<MAX_SHOTSAVE_BUFFERS; i++ )
    ...
    if ( BufPtr[i] )
        ...
        if ( b->pn.machine == machine_to_save )
            {
            i_time = b->f.shot_time;
            if ( newest_i == NO_INDEX )
                {
                newest_i    = i;
                newest_time = i_time;
                oldest_i    = i;
            ...
            else if ( seconds_difference(i_time, newest_time) < 0 )
                {
                /*
                ---------------------------------------------------------------
                i is older than newest_i.
                Make sure the older shot is not waiting for external parameters
                --------------------------------------------------------------- */
                if ( b->external_parameter_status.waiting_for_file )
                    {
                    b->external_parameter_status.waiting_for_file = false;
                    b->flags |= SHOTPARM_HAS_BEEN_UPDATED;
                    b->flags |= SHOT_FILE_HAS_BEEN_UPDATED;
                    u = SHOT_FILE_HAS_BEEN_SAVED | SHOTPARM_HAS_BEEN_SAVED;
                    if ( seconds_difference(i_time, oldest_time) < 0 )
                        {
                        /*
                        ----------------------------------------------
                        i is older than oldest. Make it the new oldest
                        ---------------------------------------------- */
                        oldest_i = i;
                        oldest_time = i_time;
                        }
                ...
                /*
                -------------------------------------------------------------
                Either the oldest time is the same as the newest time or
                it is older. In any case I don't need to remember the current
                newest time.
                ------------------------------------------------------------- */
                newest_i    = i;
                newest_time = i_time;
...
if ( i_to_save != NO_INDEX )
    ...
    if ( b->external_parameter_status.waiting_for_file )
        {
        if ( b->external_parameter_status.file_name.file_exists() )
            {
            pc.find( b->pn.computer, b->pn.machine, b->pn.part );

            have_new_external_parameters = get_external_parameters( b->f, pc, b->external_parameter_status );
            b->external_parameter_status.waiting_for_file = false;

            /*
            ----------------------------------------------------------------------
            I've added the external parameters to the file but I need to check for
            any alarms that need to be set or cleared. This has to be done by
            board_monitor so I need to put the external_parameters in the
            ExternalParameterAlarms and then tell board_monitor to read them.
            ---------------------------------------------------------------------- */
            if ( have_new_external_parameters )
                {
                n = 0;
                for ( mi=0; mi<NofExternalParameterAlarms; mi++ )
                    {
                    if ( ExternalParameterAlarms[mi].machine_name == machine_to_save )
                        {
                        for ( i=0; i<b->f.nof_text_parameters; i++ )
                            {
                            pi = b->f.text_parameter[i].parameter_number - 1;
                            if ( pc.parameter[pi].input.type == FT_EXTERNAL_PARAMETER )
                                {
                                for ( j=0; j<ExternalParameterAlarms[mi].n; j++ )
                                    {
                                    if ( pi == ExternalParameterAlarms[mi].index[j] )
                                        {
                                        ExternalParameterAlarms[mi].value[j] = b->f.text_parameter[i].value.float_value();
                                        n++;
                                        break;
                                        }
                                    }
                ...
                if ( n > 0 )
                    ExternalAlarmsRaceTrack.put( machine_to_save );
                }
            }
        }
    ...
    if ( b->flags & NEED_TO_POST_NEWSHOT_MESSAGE )
        {
        post_newshot_message( b->pn );
        u = ~NEED_TO_POST_NEWSHOT_MESSAGE;
        b->flags &= u;
        }

    if ( b->flags & NEED_TO_SEND_PARAMETER_EVENT_STRING )
        {
        cp = make_parameter_event_string( b );
        if ( cp )
            MainWindow.post( WM_POKEMON, (WPARAM) PARAM_DATA_INDEX, (LPARAM) cp );
        u = ~NEED_TO_SEND_PARAMETER_EVENT_STRING;
        b->flags &= u;
        }

    u = SHOT_FILE_HAS_BEEN_SAVED | SHOT_FILE_HAS_BEEN_UPDATED | SHOTPARM_HAS_BEEN_SAVED | SHOTPARM_HAS_BEEN_UPDATED;
    if ( (b->flags & u) == u )
        {
        /*
        ---------------------------------------------
        ...
        --------------------------------------------- */
        if ( !(b->flags & SHOT_SAVE_COMPLETE_MESSAGE_HAS_BEEN_SENT) )
            {
            s = ShotSaveCompleteMessage;
            s += CommaChar;
            s += b->pn.machine;
            post_new_data_message( s );
            b->flags |= SHOT_SAVE_COMPLETE_MESSAGE_HAS_BEEN_SENT;
            }
...
while ( true )
    ...
    /*
    ------------------------------------------------------------------
    When I get a cycle start, stop looking for the external parameters
    ------------------------------------------------------------------ */
    while ( CycleStartRaceTrack.get(s) )
        {
        for ( i=0; i<MAX_SHOTSAVE_BUFFERS; i++ )
            {
            bp = BufPtr[i];
            if ( bp )
                {
                if ( s == bp->pn.machine && bp->external_parameter_status.waiting_for_file )
                    {
                    if ( ShotsaveLogIsEnabled )
                        {
                        error_msg = TEXT( " Cycle Start before external_parameter_file - " );
                        error_msg += s;
                        log_shotsave_error( error_msg );
                        }
                    bp->external_parameter_status.waiting_for_file = false;
                    bp->flags |= SHOT_FILE_HAS_BEEN_UPDATED;
                    bp->flags |= SHOTPARM_HAS_BEEN_UPDATED;
                    }
                }
            }
        }
