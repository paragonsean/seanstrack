...
*                        HAS_ANY_WIRE_LIMITS                           *
***********************************************************************/
BOOLEAN PARAMETER_ENTRY::has_any_wire_limits()
{

if ( has_warning_limits() )
    if ( limits[WARNING_MAX].wire_number != NO_WIRE || limits[WARNING_MIN].wire_number != NO_WIRE )
        return TRUE;

if ( has_alarm_limits() )
    if ( limits[ALARM_MAX].wire_number != NO_WIRE || limits[ALARM_MIN].wire_number != NO_WIRE )
        return TRUE;
