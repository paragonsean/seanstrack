...
BOOLEAN STRING_CLASS::operator==( const STRING_CLASS & sorc )
{
return operator==( sorc.s );
}

/***********************************************************************
*                              OPERATOR==                              *
  This now returns TRUE even if one string is NULL and the other
  begins with a NullChar. Before 8.25 it returned FALSE if either
  string was a null pointer.
***********************************************************************/
BOOLEAN STRING_CLASS::operator==( const TCHAR * sorc )
{
if ( !s )
