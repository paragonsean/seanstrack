...
/***********************************************************************
*                       ENABLE_SHOT_NUMBER_RESET                       *
***********************************************************************/
static void enable_shot_number_reset()
{
static TCHAR yh[]  = TEXT("{yh}");
static TCHAR yy[]  = TEXT("{yy}");
static TCHAR ys[]  = TEXT("{ys}");
static TCHAR yhs[] = TEXT("{yhs}");
static TCHAR mx[]  = TEXT("{mx}");
static TCHAR mm[]  = TEXT("{mm}");
static TCHAR mxs[] = TEXT("{mxs}");
static TCHAR ms[]  = TEXT("{ms}");
static TCHAR j[]   = TEXT("{j}");
static TCHAR js[]  = TEXT("{js}");
static TCHAR dd[]  = TEXT("{dd}");
static TCHAR ds[]  = TEXT("{ds}");
static TCHAR hh[]  = TEXT("{hh}");
...
w  = WindowInfo[ADVANCED_SETUP_TAB].dialogwindow;
wc = GetDlgItem( w, RESET_SHOT_COUNT_ENABLE_XB );
...
Year
---- */
if ( s.contains(yh) || s.contains(yy) || s.contains(ys) || s.contains(yhs) )
    has_year = true;
...
if ( s.contains(j) || s.contains(js) )
    ...
    {
    if ( s.contains(mx) || s.contains(mm) || s.contains(mxs) || s.contains(ms) )
        has_month = true;
    }
...
if ( !has_day )
    {
    if ( s.contains(dd) || s.contains(ds) )
        has_day = true;
    }
...
Hour
---- */
if ( midnight_only || s.contains(hh) )
    has_hour = true;

ok_to_reset = false;
if ( has_year && has_month && has_day && has_hour )
    ok_to_reset = true;
...
{
static int id[] = { RESET_TIME_EB, PURGE_ON_RESET_XB };
...
{
static int button[]  = { COPYTO_BASIC_CHECKBOX, COPYTO_ADVANCED_CHECKBOX, COPYTO_SHOT_NAME_XBOX, COPYTO_SHOT_NAME_RESET_XBOX,
