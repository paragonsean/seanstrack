...
static TCHAR DowntimeResourceFile[] = TEXT( "downtime" );

FONT_LIST_CLASS   FontList;
HFONT             StaticsFont = INVALID_FONT_HANDLE;
...
void init(){ *cat.number = NullChar; *cat.name = NullChar; *subcat.number=NullChar; *subcat.name=NullChar; color=0; }
...
TCHAR  * cat_number()    { return cat.number; }
TCHAR  * cat_name()      { return cat.name; }
TCHAR  * subcat_number() { return subcat.number; }
TCHAR  * subcat_name()   { return subcat.name; }

void    inc_subcat_number();
void    fill_editboxes();
TCHAR * lbline( void);
...
static int tabs[] = {
    CAT_LBTITLE_STATIC,
    SUBINDEX_LBTITLE_STATIC,
    SUBCAT_LBTITLE_STATIC
    };

int NOF_TABS  = sizeof(tabs)/sizeof(int);
...
*/
set_listbox_tabs_from_title_positions( DownTimeSetupWindow, SUBCAT_LISTBOX, tabs, NOF_TABS );
}
...
*                            READ_EDITBOXES                            *
***********************************************************************/
BOOLEAN DOWNTIME_CLASS::read_editboxes()
{
init();
...
*                            FILL_EDITBOXES                            *
***********************************************************************/
void DOWNTIME_CLASS::fill_editboxes()
{
...
***********************************************************************/
void DOWNTIME_CLASS::inc_subcat_number()
{
int32  x;
...
***********************************************************************/
BOOLEAN DOWNTIME_CLASS::saveall()
{
DB_TABLE       t;
INT            i;
INT            nof_rows;
...
while ( c.next() )
    ...
    if ( !c.is_this_computer() )
        {
        sorc.get_data_dir_file_name( DOWNCAT_DB );
        dest.get_data_dir_file_name( c.name(), DOWNCAT_DB );
        sorc.copyto( dest );

        sorc.get_data_dir_file_name( DOWNSCATC_DB );
        dest.get_data_dir_file_name( c.name(), DOWNSCATC_DB );
        sorc.copyto( dest );
        }
    }
}
...
/***********************************************************************
*                    SET_DOWNTIME_SETUP_STRINGS                        *
***********************************************************************/
static void set_downtime_setup_strings( HWND hWnd )
{
STRING_CLASS s;
WINDOW_CLASS w;
int          i;
...
static RESOURCE_TEXT_ENTRY rsn[] = {
    { RED_STATIC,              TEXT("RED") },
    { GREEN_STATIC,            TEXT("GREEN") },
    { BLUE_STATIC,             TEXT("BLUE") },
    { ALL_STATIC,              TEXT("ALL_STRING") },
    { SAVE_BUTTON,             TEXT("SAVE_CHANGES") },
    { SUBTRACT_TIMEOUT_XBOX,   TEXT("SUBTRACT_TIMEOUT") },
    { INSERT_SUBCAT_BUTTON,    TEXT("NEW_STRING") },
    { REMOVE_SUBCAT_BUTTON,    TEXT("REMOVE") },
    { INDEX_LBTITLE_STATIC,    TEXT("INDEX") },
    { CAT_LBTITLE_STATIC,      TEXT("CATEGORY") },
    { SUBINDEX_LBTITLE_STATIC, TEXT("SUB_INDEX") },
    { SUBCAT_LBTITLE_STATIC,   TEXT("SUB_CAT") },
    { INDEX_STATIC,            TEXT("INDEX") },
    { DESC_STATIC,             TEXT("DESCRIPTION") },
    { CATEGORY_STATIC,         TEXT("CATEGORY") },
    { SUBCATEGORY_STATIC,      TEXT("SUB_CAT") },
    { COPY_TO_NETWORK_XBOX,    TEXT("COPY_TO_NET") },
    { PLOT_COLOR_STATIC,       TEXT("PLOT_COLOR") }
    };
...
switch (msg)
    ...
    case WM_INITDIALOG:
        ...
        set_tab_stops();
        set_downtime_setup_strings( hWnd );
...
----------------------------------------------------- */
s.get_stringtable_name( DowntimeResourceFile );
StringTable.read( s );
...
***********************************************************************/
static bool my_previous_instance()
{
...
MyWindowTitle = StringTable.find( TEXT("MY_TITLE") );
