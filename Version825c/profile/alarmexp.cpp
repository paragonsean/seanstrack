...
*                         FILL_EXPORT_LISTBOX                          *
***********************************************************************/
static void fill_export_listbox()
{
...
s.get_alarmsum_dbname( CurrentPart.computer, CurrentPart.machine, CurrentPart.name );
if ( t.open( s, ALARMSUM_RECLEN, PFL) )
    ...
    while ( t.get_next_record(NO_LOCK) )
        ...
        if ( pn == TOTALS_PARAMETER_NUMBER )
            {
            cp = copy_w_char( cp, StringTable.find(TEXT("TOTALS_STRING")), TabChar );
...
***********************************************************************/
static void set_export_alarms_tab_stops()
{
static int id[] = {
...
if ( n > 0 )
    {
    s = StringTable.find( TEXT("ALARMSUM_EXPORT_HEADER_STRING") );
    slen = LB_BUFLEN + 2;
    slen *= n;
    slen += CurrentPart.machine.len() + 1;
    slen += (DWORD) CurrentPart.name.len() + 2;
    slen += s.len() + 2;
    slen++;
    slen *= sizeof( TCHAR );
    ...
    cp = copy_w_char( cp, CurrentPart.machine.text(), TabChar );
    cp = copystring( cp, CurrentPart.name.text() );
...
*                          KILL_ALARMSUM_DATA                          *
***********************************************************************/
static void kill_alarmsum_data()
{
NAME_CLASS s;
DB_TABLE   t;
...
if ( t.open(s, ALARMSUM_RECLEN, FL) )
    {
    ntokill   = 0;
    first_rec = 0;
    n = lb.get_select_list( &sindex );
...
***********************************************************************/
static void close_export_alarms_dialog()
{
HWND w;
...
/***********************************************************************
*                  SET_EXPORT_ALARMS_SETUP_STRINGS                     *
***********************************************************************/
static void set_export_alarms_setup_strings( HWND hWnd )
{

static RESOURCE_TEXT_ENTRY rsn[] = {
    { DEL_ALARM_RANGE_BUTTON,    TEXT("DEL_RANGE") },
    { EXPORT_ALARM_RANGE_BUTTON, TEXT("EXPORT_TO_CLIP") },
    { EA_DATE_STATIC,            TEXT("DATE_STRING") },
    { ALARMSUM_SHIFT_STATIC,     TEXT("SHIFT_STRING") },
    { ALARMSUM_PARAMETER_STATIC, TEXT("PARAMETER_STRING") },
    { LOW_WARNING_COUNT_STATIC,  TEXT("LOW_STRING") },
    { HIGH_WARNING_COUNT_STATIC, TEXT("HIGH_STRING") },
    { LOW_ALARM_COUNT_STATIC,    TEXT("LOW_STRING") },
    { HIGH_ALARM_COUNT_STATIC,   TEXT("HIGH_STRING") },
    { TOTAL_WARNINGS_STATIC,     TEXT("TOTAL_WARNINGS") },
    { TOTAL_ALARMS_STATIC,       TEXT("TOTAL_ALARMS") },
    { TOTAL_ALARM_COUNT_STATIC,  TEXT("TOTAL_COUNT") },
    { EA_WARN_COUNTS_STATIC,     TEXT("WARNING_COUNTS") },
    { EA_ALARM_COUNTS_STATIC,    TEXT("ALARM_COUNTS") }
    };

const int nof_rsn = sizeof(rsn)/sizeof(RESOURCE_TEXT_ENTRY);

static UINT box[] = {
    EXPORT_ALARMS_LISTBOX
    };

const int nof_boxes = sizeof(box)/sizeof(UINT);

set_setup_strings( hWnd, rsn, nof_rsn, box, nof_boxes, TEXT("EA_TITLE") );
}
...
switch ( msg )
    ...
    case WM_INITDIALOG:
        ...
        gray_buttons();
        set_export_alarms_tab_stops();
        set_export_alarms_setup_strings( hWnd );
