    ...
    if ( s )
        ...
        if( ReadFile(mailslot_handle, s, slen, &bytes_read, NULL) )
            ...
            *(s+bytes_read) = NullChar;
            ...
            /*
            --------------------------------------------------------------------------------
            Put a Null after the topic and then move cp to point at the start of the message
            -------------------------------------------------------------------------------- */
            cp = findchar( CommaChar, s );
            if ( cp )
                {
                *cp = NullChar;
                if ( strings_are_equal(s, DDE_CONFIG_TOPIC) )
                    {
                    c.load();
                    BList.load();
                    }
                    ...
                    /*
                    ---------------------------------------------------
                    Move past the item to the start of the machine name
                    --------------------------------------------------- */
                    cp++;
                    cp = findchar( CommaChar, cp );
                    cp++;
                    if ( !DiskIsFull )
                        BList.process_mailslot_message( cp );
                    }
                }
            }
        delete[] s;
