...
if ( BackupAllTextFiles )
    ...
    if ( BackupList && SaveList )
        {
        set_text( ProgressWindow, StringTable.find(TEXT("BACKING_UP")) );
        backup_data();
        }
    ...
    if ( NtoSave > 0 )
        ...
        if ( BackupType & DO_RESTORE )
            {
            set_text( ProgressWindow, StringTable.find(TEXT("RESTORING")) );
            restore_data();
            }

        if ( BackupType & DO_DELETE )
            {
            set_text( ProgressWindow, StringTable.find(TEXT("DELETING...")) );
            delete_data();
            }
        }
    }
