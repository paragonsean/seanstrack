...
*                             READ_EDITBOXES                           *
***********************************************************************/
BOOLEAN DENSITY_CLASS::read_editboxes()
{
...
*                            FILL_EDITBOXES                            *
***********************************************************************/
BOOLEAN DENSITY_CLASS::fill_editboxes()
{
...
*                          END_DENSITY_DIALOG                          *
***********************************************************************/
static void end_density_dialog()
{
HWND w;
...
s.get_data_dir_file_name( CurrentPart.computer, DENSITY_DB );
if ( t.open(s, DENSITY_RECLEN, FL ) )
    {
    t.empty();
    for ( i=0; i<n; i++ )
        {
...
***********************************************************************/
static void get_currently_selected_density()
{
LISTBOX_CLASS lb;
...
*                         INIT_DENSITY_DIALOG                          *
***********************************************************************/
static void init_density_dialog()
{
LISTBOX_CLASS lb;
NAME_CLASS    s;
DB_TABLE      t;
...
s.get_data_dir_file_name( CurrentPart.computer, DENSITY_DB );
if ( t.open(s, DENSITY_RECLEN, PFL ) )
    {
...
lstrcpy( density.name, StringTable.find(TEXT("NEW_STRING")) );
...
***********************************************************************/
static void delete_density_entry()
{
LISTBOX_CLASS lb;
DENSITY_CLASS density;
int           i;
...
***********************************************************************/
static void refresh_density_listbox_entry()
{
LISTBOX_CLASS lb;
DENSITY_CLASS density;
...
***********************************************************************/
static void update_weight_unit_labels()
{
TCHAR * cp;
...
***********************************************************************/
static void update_velocity_unit_labels()
{
TCHAR * cp;
TCHAR  buf[MAX_DOUBLE_LEN+1];
...
***********************************************************************/
static void update_distance_unit_labels()
{
TCHAR * cp;
...
*                                                                      *
***********************************************************************/
static void do_filltime_change()
{
double x;
double v;
double t;
...
*                                                                      *
***********************************************************************/
static void do_gate_velocity_change()
{
double a;
double x;
double v;
...
fn.get_calc_file_name( CurrentPart.computer, CurrentPart.machine, CurrentPart.name );
f.open_for_write( fn.text() );
...
if ( is_checked(CalcWindow, COPY_P1_P2_CHECKBOX) )
    ...
    /*
    ---------------------------------------------------------------------------------------------
    What I called the runner_fill_distance in the part class is actually the sweep to fill cavity
    --------------------------------------------------------------------------------------------- */
    CurrentPart.sp[PARTSET_RUNNER_FILL_DIST_INDEX].value.get_text( CalcWindow, SWEEP_TO_FILL_CAVITY_TBOX );
    CurrentPart.runner_fill_distance = CurrentPart.sp[PARTSET_RUNNER_FILL_DIST_INDEX].value.real_value();

    /*
    -----------------------------------------------------------------------------------------------
    Get the actual distance to fill the runner. I'm just using the value string to convert the text
    to a double. I will replace the setpoint after I add the sweep to fill cavity.
    ----------------------------------------------------------------------------------------------- */
    CurrentPart.sp[PARTSET_SLEEVE_FILL_DIST_INDEX].value.get_text( CalcWindow, DIST_TO_FILL_RUNNER_TBOX );

    /*
    --------------------------------------------
    Add the sweep to fill cavity to get P3 - P1.
    -------------------------------------------- */
    CurrentPart.sleeve_fill_distance = CurrentPart.sp[PARTSET_SLEEVE_FILL_DIST_INDEX].value.real_value();
    CurrentPart.sleeve_fill_distance += CurrentPart.runner_fill_distance;

    /*
    ------------------------------------------------------------------------------------------------
    I just used the setpoint to convert the text to a double. Replace that with the actual distance,
    which I wrongly call sleeve_fill_distance. It should have been p3_minus_p1.
    ------------------------------------------------------------------------------------------------ */
    CurrentPart.sp[PARTSET_SLEEVE_FILL_DIST_INDEX].value = CurrentPart.sleeve_fill_distance;
    }
...
* Check to see if the message telling the operator to save the part    *
* setup should be displayed or not. Display or hide as necessary.      *
***********************************************************************/
...
if ( (tbox_is_visible != need_save_message) || firstime )
...
/*
----------------------------------------------------------------------
See if I am to use the plunger diameter from the advanced setup screen
---------------------------------------------------------------------- */
s.get_local_ini_file_name( EditPartIniFile );
ini.set_file( s.text() );
SyncPlungerDiameter = ini.get_boolean( ConfigSection, SyncPlungerDiameterKey );
set_checkbox( CalcWindow, SYNC_PLUNGER_DIAMETER_XBOX, SyncPlungerDiameter );
check_save_part_setup_tbox();

s.get_part_dir_file_name( CurrentPart.computer, CurrentPart.machine, CurrentPart.name, CALC_FILENAME );
...
*                          GET_TON_CONVERSION                          *
***********************************************************************/
static void get_ton_conversion()
{
STRING_CLASS s;
...
*                           SHUTDOWN_CALCULATOR                        *
***********************************************************************/
static void shutdown_calculator()
{
HWND w;
...
/**********************************************************************
*                       FILL_CALCULATOR_STATICS                       *
**********************************************************************/
static void fill_calculator_statics( HWND hWnd )
{
static RESOURCE_TEXT_ENTRY rlist[] =
    {
    { IN_UNITS_RADIO,                  TEXT("IN_UNITS") },
    { CM_UNITS_RADIO,                  TEXT("CM_UNITS") },
    { MM_UNITS_RADIO,                  TEXT("MM_UNITS") },
    { IPS_UNITS_RADIO,                 TEXT("IPS") },
    { FPM_UNITS_RADIO,                 TEXT("FPM") },
    { CMPS_UNITS_RADIO,                TEXT("CMPS") },
    { MPS_UNITS_RADIO,                 TEXT("MPS") },
    { KG_UNITS_RADIO,                  TEXT("KG_UNITS") },
    { LBS_UNITS_RADIO,                 TEXT("LBS_UNITS") },
    { EDIT_DENSITIES_BUTTON,           TEXT("EDIT_STRING") },
    { CALC_SAVE_CHANGES_BUTTON,        TEXT("SAVE_CHANGES") },
    { COPY_P1_P2_CHECKBOX,             TEXT("COPY_P1_P2") },
    { SYNC_PLUNGER_DIAMETER_XBOX,      TEXT("USE_PLUNGER_DIA") },
    { SAVE_PART_SETUP_TBOX,            TEXT("YOU_MUST_SAVE") },
    { CALC_PLUNGER_DIAMETER_STATIC,    TEXT("PLUNGER_DIAMETER") },
    { PLUNGER_AREA_STATIC,             TEXT("PLUNGER_AREA") },
    { DRY_SHOT_LENGTH_STATIC,          TEXT("DRY_SHOT_LENGTH") },
    { SHOT_SLEEVE_VOL_STATIC,          TEXT("SHOT_SLEEVE_VOL") },
    { CASTING_WEIGHT_STATIC,           TEXT("CASTING_WEIGHT") },
    { OVERFLOW_WEIGHT_STATIC,          TEXT("OVERFLOW_WEIGHT") },
    { CAVITY_WEIGHT_STATIC,            TEXT("CAVITY_WEIGHT") },
    { WEIGHT_R_B_STATIC,               TEXT("WEIGHT_R_B") },
    { TOTAL_SHOT_WEIGHT_STATIC,        TEXT("TOTAL_SHOT_WEIGHT") },
    { DENSITY_STATIC,                  TEXT("DENSITY") },
    { VOL_OF_SHOT_STATIC,              TEXT("VOL_OF_SHOT") },
    { PERCENT_SSF_STATIC,              TEXT("PERCENT_SSF") },
    { SSF_POS_STATIC,                  TEXT("SSF_POS") },
    { BISCUIT_LEN_STATIC,              TEXT("BISCUIT_LEN") },
    { BISCUIT_VOL_STATIC,              TEXT("BISCUIT_VOL") },
    { BISCUIT_WT_STATIC,               TEXT("BISCUIT_WT") },
    { RUNNER_WT_STATIC,                TEXT("RUNNER_WT") },
    { RUNNER_VOL_STATIC,               TEXT("RUNNER_VOL") },
    { DIST_TO_FILL_RUNNER_STATIC,      TEXT("RUNNER_FILL_DIST") },
    { METAL_AT_GATE_STATIC,            TEXT("METAL_AT_GATE") },
    { FILL_VEL_STATIC,                 TEXT("FILL_VEL") },
    { GATE_AREA_STATIC,                TEXT("GATE_AREA") },
    { GATE_VEL_STATIC,                 TEXT("GATE_VEL") },
    { CAVITY_VOL_STATIC,               TEXT("CAVITY_VOL") },
    { CAVITY_SWEEP_STATIC,             TEXT("CAVITY_SWEEP") },
    { FILL_TIME_STATIC,                TEXT("FILL_TIME") },
    { INTENS_PRES_STATIC,              TEXT("INTENS_PRES") },
    { PLUNGER_GATE_RATIO_STATIC,       TEXT("PLUNGER_GATE_RATIO") },
    { CYL_AREA_STATIC,                 TEXT("CYL_AREA") },
    { CYL_DIA_STATIC,                  TEXT("CYL_DIA") },
    { METAL_PRES_STATIC,               TEXT("METAL_PRES") },
    { AREA_CAST_STATIC,                TEXT("AREA_CAST") },
    { OPENING_FORCE_STATIC,            TEXT("OPENING_FORCE") },
    { CALC_DIST_UNITS_STATIC,          TEXT("DISTANCE_UNITS") },
    { CALC_VEL_UNITS_STATIC,           TEXT("VELOCITY_UNITS") },
    { WT_UNITS_STATIC,                 TEXT("WT_UNITS") },
    { ALLOY_STATIC,                    TEXT("ALLOY") }
...
set_statics_font( hWnd, VEL_DURING_FILL_UNITS_TBOX      );
set_statics_font( hWnd, GATE_AREA_UNITS_TBOX            );
set_statics_font( hWnd, GATE_VELOCITY_UNITS_TBOX        );
set_statics_font( hWnd, CAVITY_VOLUME_UNITS_TBOX        );
set_statics_font( hWnd, SWEEP_TO_FILL_CAVITY_UNITS_TBOX );
set_statics_font( hWnd, FILLTIME_UNITS_TBOX             );
set_statics_font( hWnd, INTENS_PRESSURE_UNITS_TBOX      );
set_statics_font( hWnd, CYLINDER_DIAMETER_UNITS_TBOX    );
set_statics_font( hWnd, CYLINDER_AREA_UNITS_TBOX        );
set_statics_font( hWnd, METAL_PRESSURE_UNITS_TBOX       );
set_statics_font( hWnd, AREA_CAST_OVRFLW_UNITS_TBOX     );
set_statics_font( hWnd, OPENING_FORCE_UNITS_TBOX        );
set_statics_font( hWnd, PLUNGER_DIAMETER_UNITS_TBOX     );
set_statics_font( hWnd, PLUNGER_AREA_UNITS_TBOX         );
set_statics_font( hWnd, DRY_SHOT_LENGTH_UNITS_TBOX      );
set_statics_font( hWnd, SHOT_SLEEVE_VOL_UNITS_TBOX      );
set_statics_font( hWnd, CASTING_WEIGHT_UNITS_TBOX       );
set_statics_font( hWnd, OVERFLOW_WEIGHT_UNITS_TBOX      );
set_statics_font( hWnd, CAVITY_WEIGHT_UNITS_TBOX        );
set_statics_font( hWnd, WEIGHT_R_AND_B_UNITS_TBOX       );
set_statics_font( hWnd, TOTAL_SHOT_WEIGHT_UNITS_TBOX    );
set_statics_font( hWnd, DENSITY_UNITS_TBOX              );
set_statics_font( hWnd, VOLUME_OF_SHOT_UNITS_TBOX       );
set_statics_font( hWnd, P1_POSITION_UNITS_TBOX          );
set_statics_font( hWnd, BISCUIT_LEN_UNITS_TBOX          );
set_statics_font( hWnd, BISCUIT_VOLUME_UNITS_TBOX       );
set_statics_font( hWnd, BISCUIT_WEIGHT_UNITS_TBOX       );
set_statics_font( hWnd, RUNNER_WEIGHT_UNITS_TBOX        );
set_statics_font( hWnd, RUNNER_VOLUME_UNITS_TBOX        );
set_statics_font( hWnd, DIST_TO_FILL_RUNNER_UNITS_TBOX  );
set_statics_font( hWnd, METAL_AT_GATE_UNITS_TBOX        );
set_statics_font( hWnd, DENSITY_UNITS_CBOX              );

w = hWnd;
w.set_title( StringTable.find(TEXT("CALCULATOR")) );
w.refresh();
}
