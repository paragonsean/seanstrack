...
std::map <STRING, EVENT_CLIENT> RemoteClients;
...
for ( c=0; c<NofPlotCurves; c++ )
    ...
    if ( channel > NO_FT_CHANNEL || i == DIFFERENTIAL_CURVE_INDEX )
        ...
        if ( curvetype & DIFF_PRESSURE_CURVE )
            {
            StringTable.find( s, TEXT("DIF_PRES_ZOOM_STRING") );
            }
        else if ( IsPressureDisplay )
            {
            if ( curvetype & HEAD_PRESSURE_CURVE )
                StringTable.find( s, TEXT("HEAD_PRES_ZOOM_STRING") );
            else if ( curvetype & ROD_PRESSURE_CURVE )
                StringTable.find( s, TEXT("ROD_PRES_ZOOM_STRING") );
            else if ( curvetype & DIFF_PRESSURE_CURVE )
                StringTable.find( s, TEXT("DIFF_PRES_ZOOM_STRING") );
            }
        if ( s.isempty() )
            {
...
StringTable.find( s, TEXT("FIXED_AXIS_STRING") );
...
if ( current_record >= 0 && current_record == last_record )
    ...
    if ( !pn.part.isempty() )
        ...
        if ( cp )
            {
            EventClient.poke_data( DOWN_DATA_INDEX, cp );
            delete[] cp;
            }
        }
    }
...
c.find( c.whoami() );
MachList.add_computer( c.name(), c.hostname(), c.ip_address() );
if ( MachList.count() > 0 )
    {
    MachList.rewind();
    while ( MachList.next() )
...
if ( CurrentPart.find(ComputerName, MachineName, PartName) )
    ...
    CurrentFtCalc.load( CurrentParam, ComputerName, MachineName, PartName );
...
/*
-----------------------------------
HORRIFIC KLUDGE FROM HELL VARIABLES
----------------------------------- */
int          ch3_index;
int          diff_index;
int          kludge_index;
...
for ( channel=0; channel<nof_channels; channel++ )
    ...
    if ( curve_type == DIFF_PRESSURE_CURVE )
        curve_type <<= 1;  //skip over DIFF_PRESSURE_CURVE
    }

/*
---------------------------------------------------------------------------------
HORRIFIC KLUDGE FROM HELL
This kludge masks the bug where ch3 and diff plots are swapped
by adding logic that swaps their curve_indexs.
TODO: Replace this kludge by finding and fixing the root cause of the index swap.
--------------------------------------------------------------------------------- */
if ( my_curve_types & DIFF_PRESSURE_CURVE && my_curve_types & CHAN_3_ANALOG_CURVE )
{
    for (i = 0; i<MAX_CONTINUOUS_CHANNELS; i++)
        {
        s = PlotCurve[i].label;
        if ( s == resource_string(DIFF_PRESSURE_LABEL_STRING) )
            diff_index = i;
        else if ( s == resource_string(CHANNEL_3_LABEL_STRING) )
            ch3_index = i;
        }

        kludge_index = PlotCurve[diff_index].curve_index;
        PlotCurve[diff_index].curve_index = PlotCurve[ch3_index].curve_index;
        PlotCurve[ch3_index].curve_index = kludge_index;
}
//END KLUDGE
...
for ( c=2; c<4; c++ )
    if ( NofPlotCurves > c )
        ...
        if ( i == DIFFERENTIAL_CURVE_INDEX )
            {
            StringTable.find( s, TEXT("DIF_AXIS_LABEL_STRING") );
            }
        else
            {
...
if ( state == ALARM_STATE )
    ...
    StringTable.find( s, TEXT("ALARM_STRING") );
    }
else
    {
    old_background_color = SetBkColor( MyDc, WarningColor );
    StringTable.find( s, TEXT("WARNING_STRING") );
    }
...
StringTable.find( s, TEXT("TIME_NAME_STRING") );
TextOut( dc, 2, y, s.text(), s.len() );

StringTable.find( s, TEXT("SHOT_NUMBER_STRING") );
...
void event_callback( SOCKET_MSG * msg )
{
    ...
    *sm = *msg;
    ...
    Log event msg
    ------------- */
    log_msg = TEXT( "event_callback cmd: " );
    log_msg += msg->cmd;
    log_msg += TEXT(" item: ");
    log_msg += msg->item;
    log_msg += TEXT( " data: " );
    log_msg += msg->data;
    ServiceLog.add_entry( log_msg, LOG_LEVEL_DEBUG );
}

/***********************************************************************
*                           PROCESS_NEW_SHOT                           *
***********************************************************************/
void process_new_shot( SOCKET_MSG * msg )
{
    STRING_CLASS   s;
    COMPUTER_CLASS c;
...
/***********************************************************************
*                       PROCESS_NEW_MACHINE_SETUP                      *
***********************************************************************/
void process_new_machine_setup( SOCKET_MSG * msg )
{
    COMPUTER_CLASS c;
    STRING_CLASS   s;

    NewSetupMachine = msg->data;
...
***********************************************************************/
void process_monitor_setup( SOCKET_MSG * msg )
{
    STRING_CLASS   s;
    COMPUTER_CLASS c;
...
/***********************************************************************
*                       PROCESS_NEW_MACHINE_STATE                      *
***********************************************************************/
void process_new_machine_state( SOCKET_MSG * msg )
{
...
*                           PROCESS_DOWN_DATA                          *
***********************************************************************/
void process_down_data( SOCKET_MSG * msg )
{
    DOWN_DATA_CLASS dd;
    COMPUTER_CLASS  c;
    STRING_CLASS    s;
...
/***********************************************************************
*                          PROCESS_NEW_PART_SETUP                      *
***********************************************************************/
void process_new_part_setup( SOCKET_MSG * msg )
{
    COMPUTER_CLASS c;
    STRING_CLASS   s;
    STRING_CLASS   cs;
...
/***********************************************************************
*                       PROCESS_PROFILE_SHUTDOWN                       *
***********************************************************************/
void process_profile_shutdown( SOCKET_MSG *msg )
{
    int i;
    void * fifo_entry;
    std::map<STRING, EVENT_CLIENT>::iterator remote_itr;

    EventClient.stop();

    for ( remote_itr = RemoteClients.begin(); remote_itr != RemoteClients.end(); remote_itr++ )
    {
        remote_itr->second.stop();
    }

    for ( i = 0; i < EventFifo.count(); ++i )
    {
        fifo_entry = EventFifo.pop();
        delete fifo_entry;
    }
...
/***********************************************************************
*                          PROCESS_PASSWORD_LEVEL                      *
***********************************************************************/
void process_password_level( SOCKET_MSG *msg )
{
...
/***********************************************************************
*                    REGISTER_FOR_ALL_REMOTE_EVENTS                    *
***********************************************************************/
static void connect_and_register_for_all_remote_events()
{
    COMPUTER_CLASS c;
    STRING_CLASS s;
    ...
    while (c.next())
    {
        if (!c.is_this_computer())
        {
            s = c.name();
            root_from_path(s);
            RemoteClients[s.text()].start_remote(s.text());
            RemoteClients[s.text()].register_for_event( MACH_SETUP_INDEX, event_callback );
            RemoteClients[s.text()].register_for_event( NEW_MACH_STATE_INDEX, event_callback );
            RemoteClients[s.text()].register_for_event( DOWN_DATA_INDEX, event_callback );
            RemoteClients[s.text()].register_for_event( MONITOR_SETUP_INDEX, event_callback );
            MachList.rewind();
            while (MachList.next())
            {
                RemoteClients[s.text()].register_for_event( SHOT_NUMBER_INDEX, MachList.name().text(), event_callback );
                RemoteClients[s.text()].register_for_event( PART_SETUP_INDEX, MachList.name().text(), event_callback );
            }
        }
    }
}
...
/***********************************************************************
*                         PROCESS_MONITOR_STARTUP                      *
***********************************************************************/
void process_monitor_startup( SOCKET_MSG *msg )
{
    if (!msg->data)
    {
        return;
    ...
    connect_and_register_for_all_remote_events();
}

/***********************************************************************
*                    REGISTER_FOR_ALL_LOCAL_EVENTS                     *
***********************************************************************/
static void register_for_all_local_events()
{
    EventClient.register_for_event( MACH_SETUP_INDEX, event_callback );
    EventClient.register_for_event( NEW_MACH_STATE_INDEX, event_callback );
    ...
    //EventClient.register_for_event( SHUTDOWN_INDEX, process_profile_shutdown );
    EventClient.register_for_event( SHUTDOWN_INDEX, event_callback );
    MachList.rewind();
    while ( MachList.next() )
    {
        EventClient.register_for_event( SHOT_NUMBER_INDEX, MachList.name().text(), event_callback );
        EventClient.register_for_event( PART_SETUP_INDEX, MachList.name().text(), event_callback );
    }

    if ( network_get_computer_type() == WORKSTATION_COMPUTER_TYPE )
    {
        EventClient.register_for_event( MONITOR_STARTUP_INDEX, event_callback );
    }
}
...
static void unregister_for_current_machine()
{
    ...
    EventClient.unregister_for_event( PART_SETUP_INDEX, MachList.name().text() );
    EventClient.unregister_for_event( SHOT_NUMBER_INDEX, MachList.name().text() );
    ...
    if ( network_get_computer_type() == WORKSTATION_COMPUTER_TYPE )
    {
        ...
        while ( c.next() )
        {
            if ( !c.is_this_computer() )
            {
                s = c.name();
                root_from_path( s );
                RemoteClients[s.text()].unregister_for_event( PART_SETUP_INDEX, MachList.name().text() );
                RemoteClients[s.text()].unregister_for_event( SHOT_NUMBER_INDEX, MachList.name().text() );
            }
        }
    }
}
...
static void unregister_all()
{
    ...
    EventClient.unregister_for_event( MACH_SETUP_INDEX );
    EventClient.unregister_for_event( NEW_MACH_STATE_INDEX );
    EventClient.unregister_for_event( DOWN_DATA_INDEX );
    EventClient.unregister_for_event( MONITOR_SETUP_INDEX );
    EventClient.unregister_for_event( PW_LEVEL_INDEX );
    ...
    if ( network_get_computer_type() == WORKSTATION_COMPUTER_TYPE )
    {
        ...
        while ( c.next() )
        {
            if ( !c.is_this_computer() )
            {
                s = c.name();
                root_from_path( s );
                RemoteClients[s.text()].unregister_for_event( MACH_SETUP_INDEX );
                RemoteClients[s.text()].unregister_for_event( NEW_MACH_STATE_INDEX );
                RemoteClients[s.text()].unregister_for_event( DOWN_DATA_INDEX );
                RemoteClients[s.text()].unregister_for_event( MONITOR_SETUP_INDEX );
            }
        }
...
*                            fill_languages                           *
**********************************************************************/
static void fill_languages( HWND hWnd )
{
LISTBOX_CLASS   lb;
NAME_CLASS      s;
TEXT_LIST_CLASS t;
...
s.get_exe_dir_file_name( RESOURCES_DIRECTORY_NAME );
...
**********************************************************************/
static void do_language_selection( HWND hWnd )
{
INI_CLASS     ini;
NAME_CLASS    s;
LISTBOX_CLASS lb;
...
static RESOURCE_TEXT_ENTRY rsn[] = {
    ...
    { COMPUTER_IP_STATIC ,  TEXT("COMPUTER_IP_ADDRESS") },
    { PATH_STATIC,          TEXT("PATH") },
...
for ( c=0; c<NOF_FTII_ANALOGS; c++ )
    {
    s += CommaChar;
    s += units_name(CurrentPart.analog_sensor[c].units_index());
    }
f.writeline( s.text() );
...
for ( i=0; i<fp.n; i++ )
    ...
    for ( c=0; c<NOF_FTII_ANALOGS; c++ )
        {
        s += CommaChar;
        s += CurrentPart.analog_sensor[c].converted_value(fp.analog[c][i]);
        }
    f.writeline( s.text() );
    }
...
p.get_last_shot_name( ShotName, ComputerName, MachineName, PartName );
    ...
    case WM_VTW_MENU_CHOICE:
        ...
        switch ( id )
            ...
            case EXIT_MENU:
                ...
                ----------------------------------------------------------------------- */
                EventClient.poke_data( NEW_MACH_STATE_INDEX, UNKNOWN );
                EventClient.poke_data( SHUTDOWN_INDEX, TEXT("0") );
                return 0;

            case MINIMIZE_MENU:
                EventClient.poke_data( PLOT_MIN_INDEX, TEXT("0") );
...
void log_startup()
{
    ...
    log_msg = TEXT( "True-Trak Version: ");
    log_msg += get_ini_string( VisiTrakIniFile, ConfigSection, TEXT("Version") );
    ServiceLog.add_entry( log_msg );

    log_msg = TEXT( "Serial Number: " );
    log_msg += get_ini_string( VisiTrakIniFile, ConfigSection, SerialNumberKey );
    ServiceLog.add_entry( log_msg );

    log_msg = TEXT( "Computer Type: ");
    log_msg += get_ini_string( VisiTrakIniFile, ConfigSection, TEXT("ComputerType") );
    ServiceLog.add_entry( log_msg );

    log_msg = TEXT("Computer Name: ");
    log_msg += ComputerName;
    ServiceLog.add_entry( log_msg );

    log_msg = TEXT( "Local IP Address: " );
    log_msg += EventClient.get_local_ip_addr();
    ServiceLog.add_entry( log_msg );

    log_msg = TEXT( "Local Port: " );
    log_msg += EventClient.get_local_port();
    ServiceLog.add_entry( log_msg );

    RealtimeMachineList.rewind();
    while ( RealtimeMachineList.next() )
    {
        log_msg = TEXT( "Realtime Machine: " );
        log_msg += RealtimeMachineList.text();
        ServiceLog.add_entry(log_msg);

        s = RealtimeMachineList.text();
        fn.get_machine_dir_file_name( ComputerName, s, TEXT("ftii_editor_settings.ini") );
        log_msg = TEXT("Board Flash Version: ");
        GetPrivateProfileString( ConfigSection, TEXT("FlashVersion"), UnknownString, Buf, MAX_LONG_INI_STRING_LEN, fn.text() );
        log_msg += Buf;
        ServiceLog.add_entry( log_msg );
    }
}
...
DWORD event_thread()
{
    ...
    while ( ProcessEvents )
    {
        if ( EventFifo.count() > 0 )
        {
            ...
            switch ( msg->item )
            {
                case SHOT_NUMBER_INDEX:
                    process_new_shot( msg );
                    break;

                case MACH_SETUP_INDEX:
                    process_new_machine_setup( msg );
                    break;

                case MONITOR_SETUP_INDEX:
                    process_monitor_setup( msg );
                    break;

                case MONITOR_STARTUP_INDEX:
                    process_monitor_startup( msg );
                    break;

                case NEW_MACH_STATE_INDEX:
                    process_new_machine_state( msg );
                    break;

                case PART_SETUP_INDEX:
                    process_new_part_setup( msg );
                    break;

                case PW_LEVEL_INDEX:
                    process_password_level( msg );
                    break;

                case SHUTDOWN_INDEX:
                    process_profile_shutdown( msg );
                    break;
        ...
        {
            log_entry = TEXT( "ERROR: event_thread() unable to continue." );
