...
static ANALOG_SENSOR AnalogSensor; /* Used only by the ftii routines */

static const float MaxFloatShort = float( 0xFFFF );
double * calculate_ftii_parameters( FTII_PROFILE & f, MACHINE_CLASS & machine, PART_CLASS & part, PARAMETER_CLASS & param, ANALOG_SENSOR & asensor, FTANALOG_CLASS & ftanalog, FTCALC_CLASS & ftclass, LIMIT_SWITCH_CLASS &limits );
...
dp = calculate_ftii_parameters( fp, CurrentMachine, CurrentPart, CurrentParam, AnalogSensor, CurrentFtAnalog, CurrentFtCalc );
if ( dp )
    {
    for ( i=0; i<MAX_PARMS; i++ )
        {
        if ( CurrentParam.parameter[i].input.type == ASCII_PARAMETER )
            Parms[i].null();
        else
            Parms[i] = dp[i];
        }
