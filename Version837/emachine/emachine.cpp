...
BOOLEAN EventmanShutdown = FALSE;
...
for ( i=0; i<NofMachines; i++ )
    ...
    if ( Machines[i].has_changed && ( (m->computer == c.whoami()) || (CurrentPasswordLevel >= EditMaOnOtherCoLevel) ) )
        ...
        EventClient.poke_data( MACH_SETUP_INDEX, m->name.text() );
        if (Machines[i].machine.monitor_flags & MA_MONITORING_ON)
            EventClient.poke_data( MONITOR_SETUP_INDEX, m->name.text() );
...
EventClient.poke_data( MACH_SETUP_INDEX, machine.text() );
}
...
if ( lb.index(s.text()) == NO_INDEX )
    ...
    add_machine( s );
    ShowWindow( NewMachineWindow, SW_HIDE );
    EventClient.poke_data( MACH_SETUP_INDEX, s.text() );
    }
else
...
/***********************************************************************
*                       EMACHINE_SHUTDOWN_CALLBACK                     *
***********************************************************************/
void emachine_shutdown_callback( SOCKET_MSG *msg )
{
...
switch (msg)
    ...
    case WM_DBINIT:
        ...
        EventClient.register_for_event( SHUTDOWN_INDEX, emachine_shutdown_callback );
