...
if ( s.get_save_csv_w_shot_setup_file_name(pn.computer, pn.machine, pn.part) )
    {
    f.open_for_read( s );
    s = f.readline();
    s.next_field();
...
if ( t.open_for_rw(s) )
    ...
    {
    MessageBox( 0, TEXT("write_ftii_parameters"), TEXT("Could not open rw"), MB_OK | MB_SYSTEMMODAL );
    }
return FALSE;
}
...
for ( c=0; c<NOF_FTII_ANALOGS; c++ )
    {
    s += CommaChar;
    s += units_name( pa.analog_sensor[c].units );
    }
f.writeline( s.text() );
...
for ( i=0; i<fp.n; i++ )
    ...
    for ( c=0; c<NOF_FTII_ANALOGS; c++ )
        {
        s += CommaChar;
        s += pa.analog_sensor[c].converted_value( fp.analog[c][i] );
        }
    f.writeline( s.text() );
    }
