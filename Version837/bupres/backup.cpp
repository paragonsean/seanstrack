...
if ( BackupAllTextFiles )
    ...
    if ( dest.directory_exists() )
        ...
        dest += BackupZipFile;
        dest += TEXT( " data\\*.ini data\\*.txt data\\*.dat data\\*.csv data\\*.msg data\\*.prg data\\*.log exes\\*.ini exes\\*.txt display\\*.*" );
    ...
    if ( dir_from_filename(dest.text()) )
        ...
        if ( !dest.directory_exists() )
            ...
            if ( !dest.create_directory() )
                {
                StringTable.find( sorc,  TEXT("NO_ZIP_DIR") );
...
if ( backing_up && !path_is_drive_only(BackupDir.text()) )
    ...
    if ( !dest.create_directory() )
        {
        StringTable.find( sorc, TEXT("DIR_CREATE_FAIL") );
...
if ( backing_up )
    ...
    if ( !execute_and_wait(dest.text(), NULL, TEN_MINUTES) )
        {
        StringTable.find( dest, TEXT("NO_BUP") );
    ...
    if ( !BackupZipFile.file_exists() )
        {
        StringTable.find( dest, TEXT("ZIP_CREATE_FAIL") );
        MessageBox( 0, dest.text(), BackupZipFile.text(), MB_OK | MB_SYSTEMMODAL );
        backup_ok = FALSE;
        }
    }
...
StringTable.find( sorc, TEXT("PURGING...") );
