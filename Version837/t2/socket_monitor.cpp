...
/***********************************************************************
*                       BOOL  MAKE_START_MESSAGE                       *
*    The caller is responsible for deleting this string when done.     *
***********************************************************************/
static TCHAR * make_start_message( char * s )
{
static TCHAR StartMessage[] = TEXT( "Begin Upload... " );
int n;
int ns;
TCHAR * dest;
TCHAR * cp;
...
n = strlen( s );
ns = strlen( START_OF_UPLOAD );
if ( n <= ns )
    s = 0;
else
    s += ns;
n += lstrlen( StartMessage );
dest = maketext( n );          // This is longer than I need by ns-1.
if ( dest )
    {
    cp = copystring( dest, StartMessage );
    if ( s )
        char_to_tchar( cp, s );
    }
