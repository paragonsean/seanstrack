...
HANDLE  ZipThreadEvent;

BOOLEAN NeedToZipAll     = FALSE;
BOOLEAN UpdatingListbox  = FALSE;
BOOLEAN UpdatingBoxes    = FALSE;
BOOLEAN ShuttingDown;
BOOLEAN ZipThreadRunning = FALSE;

TCHAR MyClassName[]   = TEXT( "DataServer" );
TCHAR MyWindowTitle[] = TEXT( "Data Archiver" );
...
*                          NEW_SHOT_CALLBACK                           *
***********************************************************************/
void new_shot_callback( SOCKET_MSG & msg )
{   
    SOCKET_MSG *sm;

    sm = new SOCKET_MSG( msg );
    EventFifo.push( sm );
}

/***********************************************************************
*                       PROCESS_NEW_SHOT_EVENT                         *
***********************************************************************/
void process_new_shot_event()
{
    STRING_CLASS part;
    STRING_CLASS s;
    SOCKET_MSG * msg;
    ...
    /*
    -----------------------
    Log event msg for debug
    ----------------------- */
    ServiceLog.add_entry( s, LOG_LEVEL_DEBUG );
    ...
    if (s.next_field())
    {
        ...
        if ( BList.find(s.text()) )
        {
            if ( s.next_field() )
            {
                ...
                {
                    if ( s == TEXT("NS") )
                        BList.current_entry->check_for_new_shot( part.text() );
                    else
                        BList.current_entry->check_for_new_downtime();
                }
            }
        }
    }
...
void accept_connection( void * socket )
{
    ...
    {
        if (msg.cmd == POKE_DATA_ITEM && msg.item == DS_NOTIFY_INDEX)
        {
            new_shot_callback( msg );
        }
    ...
    {
        e = TEXT("ERROR: accept_connection recv failed with winsock error: ");
        e += WSAGetLastError();
...
/***********************************************************************
*                              ZIP_THREAD                              *
*                    Checks to see if any zipping or                   * 
*               processing of new shot events are needed.              *
***********************************************************************/
void *zip_thread( void* )
{
    ...
    while ( TRUE )
    {
        ...
        if ( WaitForSingleObject(ZipThreadEvent, MS_TO_WAIT) == WAIT_FAILED )
            resource_message_box( UNABLE_TO_CONTINUE_STRING, MAILSLOT_WAIT_FAIL_STRING );
        ResetEvent( ZipThreadEvent );
    }   
...
switch ( msg )
    ...
    case WM_COMMAND:
        switch ( id )
            ...
            case ZIP_NOW_BUTTON:
                if ( ZipThreadEvent && !NeedToZipAll )
                    {
                    NeedToZipAll = TRUE;
                    SetEvent( ZipThreadEvent );
                    }
                return TRUE;
    ...
    case WM_COMMAND:
        ...
        switch ( id )
            ...
            case GLOBAL_FILE_SETTINGS_MENU:
                DialogBox(
                    MainInstance,
                    TEXT("GLOBAL_FILE_SETTINGS_DIALOG"),
                    hWnd,
...
void log_startup()
{
    ...
    log_msg += get_ini_string( VisiTrakIniFile, ConfigSection, TEXT("SerialNumber") );
    ServiceLog.add_entry( log_msg );
...
ZipThreadEvent = CreateEvent( NULL, FALSE, FALSE, NULL );

h = CreateThread(
    NULL,                                       /* no security attributes        */
    0,                                          /* use default stack size        */
    ( LPTHREAD_START_ROUTINE )zip_thread,       /* thread function               */
...
Start socket server
------------------- */
DAServer.callback = accept_connection;
network_get_ip_addr( ip_addr, IP_TYPE_DATA_ARCHIVER );
ip_port = network_get_ip_port( IP_TYPE_DATA_ARCHIVER );
DAServer.start( ip_addr, ip_port );

log_msg = TEXT( "DataServer IP Address: " );
log_msg += DAServer.ip_addr;
ServiceLog.add_entry( log_msg );

log_msg = TEXT( "DataServer IP Port: " );
log_msg += DAServer.ip_port;
ServiceLog.add_entry( log_msg );
