...
static TCHAR V5dsDataPath[]   = TEXT( "c:\\v5ds\\data\\" );
...
static TCHAR NetworkCsvFile[]  = TEXT( "network.csv" );
...
ini.set_file( s.text() );
s = ini.get_string( ConfigSection, VersionKey );
s.replace( PeriodChar, NullChar );
if ( s.int_value() < 8 )
    {
    Lb.add( TEXT("V5DS directory is not Version 8") );
    is_old_version = true;
    }
else
    {
    ...
    if ( s.int_value() < 8 )
        {
        Lb.add( TEXT("V5 directory is not Version 8") );
        is_old_version = true;
        }
    }
...
if ( is_old_version )
    {
    Lb.add( TEXT("Nothing was installed, press [ESC] to exit") );
    scroll_listbox_to_last_entry();
    }
...
if ( copy_one_file(V5dsExesPath, BackupExeFile) )
    ...
    /*
    -------------------------------------------------
    Only copy the network csv file if there isn't one
    ------------------------------------------------- */
    copystring( sorc, V5dsDataPath );
    append_filename_to_path( sorc, NetworkCsvFile );
    if ( !file_exists(sorc) )
        copy_one_file( V5dsDataPath, NetworkCsvFile );
    }
