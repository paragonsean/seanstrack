...
class FILE_CLASS
    ...
    BOOLEAN get_file_size( DWORD & dest );
    DWORD   getpos() { return current_position; }
    BOOLEAN get_creation_time( FILETIME * dest );
    HANDLE  file_handle() { return fh; }
    BOOLEAN insertbinary( void * sorc, DWORD nof_bytes );
    BOOLEAN insertline( TCHAR * sorc );
    ...
    TCHAR * readline();
    BOOLEAN read_unicode_file( STRING_CLASS & dest );
    BOOLEAN set_eof();
    BOOLEAN setpos( int32 byte_position );
