...
/*
--------------------------------------------
Set the size of the setpoint class in ftcalc
-------------------------------------------- */
...
/*
-------------------------------------------------------------------------------
                 Get the Coefficients for the Pre-Impact Points
...
/*
--------------------------------------------
Calculate pressure for the pre-impact points
-------------------------------------------- */
j = 0;
for ( i=0; i<f.np; i++ )
    {
    ...
    if ( need_head_pressure )
        pressure[j] += ch * part.analog_sensor[hi].converted_value( f.analog[hi][j]  );

    if ( need_rod_pressure )
        pressure[j] += cr * part.analog_sensor[ri].converted_value( f.analog[ri][j] );
    j++;
    }

/*
-------------------------------------------------------------------------------
                Get the Coefficients for the Post-Impact Points
------------------------------------------------------------------------------- */
if ( parameter_pressure_type & HEAD_PRESSURE_CURVE )
    {
    if ( hi != NO_INDEX )
...
/*
---------------------------------------------
Calculate pressure for the post-impact points
--------------------------------------------- */
for ( i=0; i<f.nt; i++ )
    {
    pressure[j] = 0.0;
...
if ( differential_curve_number != NO_DIFF_CURVE_NUMBER )
    ...
    if ( !(parameter_pressure_type & DIFF_PRESSURE_CURVE) && (ri != NO_INDEX) && (hi != NO_INDEX) )
        ...
        while ( i<f.n )
            {
            pressure[i] =  ch * part.analog_sensor[hi].converted_value( f.analog[hi][i]  );
            pressure[i] += cr * part.analog_sensor[ri].converted_value( f.analog[ri][i] );
            i++;
            }
        }
    }
...
for ( p=0; p<MAX_PARMS; p++ )
    ...
    if ( param.parameter[p].input.type == FT_ANALOG_INPUT || param.parameter[p].input.type == FT_TARGET_INPUT )
        ...
        x = ftcalc.value( ftanalog.array[p].ind_value, dest, part, limits, f );
        y = ftcalc.value( ftanalog.array[p].end_value, dest, part, limits, f );
        ...
        if ( param.parameter[p].input.type == FT_ANALOG_INPUT )
            ...
            if ( i >= 0 )
                ...
                if ( ftanalog.array[p].result_type & SUB_RESULT_TYPE )
                    ...
                    else if ( 0 <= c && c < NOF_DYNAMIC_CHANNELS )
                        {
                        x = part.analog_sensor[c].converted_value( f.analog[c][i] );
                        y = part.analog_sensor[c].converted_value( f.analog[c][j] );
                        }
                    ...
                    if ( c == NO_INDEX )
                        ...
                        {
                        max_analog_value = part.analog_sensor[c].converted_value( f.analog[c][i]  );
                        }
                    else if ( c == FT_ANALOG_VELOCITY_CHANNEL )
                        {
            ...
            -------------------------------------- */
            target = part.analog_sensor[c].raw_count( x );
            x = ftcalc.value( ftanalog.array[p].end_value, dest, part, limits, f );
            x = part.x4_from_dist( x );
            i = f.index_from_position( x );
        ...
        if ( wire == 0 )
            ...
            ------------------------------------------------------------------------------------ */
            if ( c >= NOF_DYNAMIC_CHANNELS )
                {
                i = c - NOF_DYNAMIC_CHANNELS + FIRST_STATIC_ANALOG_INDEX;
        ...
        u &= 0xFFF;
        dest[p] = part.analog_sensor[c].converted_value( u );
        }
    else if ( param.parameter[p].input.type == FT_CALC_INPUT )
        {
        dest[p] = ftcalc.value( p+1, dest, part, limits, f );
        }
