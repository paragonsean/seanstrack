...
{
int i;
static const ANALOG_SENSOR asensor;
...
nof_steps                 = 0;
analog_sensor             = asensor.DefaultSensor;
...
digital_pot_gain           = sorc.digital_pot_gain;
...
*                              FIND                                    *
***********************************************************************/
BOOLEAN PRESSURE_CONTROL_CLASS::find( STRING_CLASS & computer, STRING_CLASS & machine, STRING_CLASS & part )
{
FILE_CLASS   f;
STRING_CLASS s;
NAME_CLASS   tn;
...
tn.get_part_dir_file_name( computer, machine, part, STPRES_DATA );

if ( tn.file_exists() )
    {
    if ( f.open_for_read(tn) )
        {
        is_enabled = FALSE;
        use_only_positive_voltages = FALSE;
        retctl_is_enabled  = FALSE;
        ...
        for ( i=0; i<nof_steps; i++ )
            ...
            cp = step[i].pressure.find( IS_OPEN_STRING );
            if ( cp )
                {
        ...
        if ( cp )
            {
            an.get_asensor_csvname();
            if ( !an.file_exists() )
                an.get_asensor_dbname();
            if ( an.file_exists() )
                {
                s = cp;
                analog_sensor.find( an, s );
                }
            }
...
*                              SAVE                                    *
***********************************************************************/
BOOLEAN PRESSURE_CONTROL_CLASS::save( STRING_CLASS & computer, STRING_CLASS & machine, STRING_CLASS & part )
{
TCHAR      bs[5];
FILE_CLASS f;
int32      i;
...
s.get_stpres_datname( computer, machine, part );
if ( f.open_for_write(s) )
    ...
    f.writeline( analog_sensor.desc );

    s = (int) voltage_output_range;
    f.writeline( s );

    s = (int) arm_intensifier_ls_number;
    f.writeline( s );
