...
*                           NEXT_SHOT_NAME                             *
***********************************************************************/
BOOLEAN next_shot_name( STRING_CLASS & dest, STRING_CLASS & co, STRING_CLASS & ma, STRING_CLASS & pa )
{
PLOOKUP_CLASS p;
int           shot_number;

shot_number = p.get_last_shot_number( co, ma, pa );
shot_number++;
p.set_last_shot_number( shot_number );
if ( shot_name(dest, co, ma, pa, shot_number) )
    {
    p.set_last_shot_name( dest );
    p.put();
    return TRUE;
    }
...
if ( shot_name.len() < shot_number_length )
    {
    shot_number = shot_name.int_value();
    shot_number++;
    shot_name = shot_number;
    return TRUE;
    }

cp = shot_name.last_char();
if ( !cp )
    return FALSE;

has_check_digit = false;
old_check_digit = NullChar;
if ( (*cp>=AChar) && (*cp<=ZChar) )
    {
...
insalph( cp, shot_number, shot_number_length, ZeroChar, DECIMAL_RADIX );

if ( has_check_digit )
    append_check_digit( shot_name );
