...
while ( TRUE )
    ...
    if ( !th )
        ...
        if ( name.find(GRAPHLST_DB) || name.find(SHOTPARM_CSV) )
            {
            file_flags = FILE_ATTRIBUTE_HIDDEN | FILE_FLAG_RANDOM_ACCESS;
            }
        else
            {
...
/***********************************************************************
*                   DB_TABLE::REC_INSERT_OR_APPEND                     *
***********************************************************************/
BOOLEAN DB_TABLE::rec_insert_or_append( int nof_key_fields )
{
    TCHAR *buf;
    ...
    if ( !goto_first_greater_than_record(1) )
    {
        copychars( recbuf, buf, recbuflen + 1 );
        delete[] buf;
        return rec_append();
