...
*                           STRING_ENTRY                               *
*                             acquire                                  *
* Be Careful! This moves the string to the new entry                   *
* It does NOT change the next pointer.                                 *
***********************************************************************/
void STRING_ENTRY::acquire( STRING_ENTRY & sorc )
{
maxlen = sorc.maxlen;
s      = sorc.s;
sorc.maxlen = 0;
...
* Swap two entries                                                     *
***********************************************************************/
void STRING_ARRAY_CLASS::swap( int i1, int i2 )
{
STRING_ENTRY copy1;
STRING_ENTRY copy2;
int          original_index;
...
if ( !find(i1) )
    return;
copy1.acquire( *current );

if ( !find(i2) )
    return;
copy2.acquire( *current );

find( i1 );
current->acquire( copy2 );
find( i2 );
current->acquire( copy1 );
...
*                               REMOVE                                 *
***********************************************************************/
bool STRING_ARRAY_CLASS::remove()
{
STRING_ENTRY * se;
STRING_ENTRY * prev;
...
*                                REVERSE                               *
***********************************************************************/
void STRING_ARRAY_CLASS::reverse()
{
TCHAR   c;
TCHAR * p1;
TCHAR * p2;
