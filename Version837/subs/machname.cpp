...
{
    return add_computer( computer, EmptyStr, EmptyStr );
}
...
*   mean that there were any machines on the computer.                 *
***********************************************************************/
BOOLEAN MACHINE_NAME_LIST_CLASS::add_computer( STRING_CLASS & computer, STRING_CLASS & hostname, STRING_CLASS & ip_addr )
{
NAME_CLASS      s;
TEXT_LIST_CLASS t;
...
while ( t.next() )
    ...
    if ( mne )
        ...
        mne->hostname = hostname;
        mne->ip_address = ip_addr;
        mne->machine  = t.text();
...
***********************************************************************/
STRING_CLASS & MACHINE_NAME_LIST_CLASS::hostname()
{
    if ( current_entry )
        return current_entry->hostname;
...
***********************************************************************/
STRING_CLASS & MACHINE_NAME_LIST_CLASS::ip_address()
{
    if ( current_entry )
        return current_entry->ip_address;
