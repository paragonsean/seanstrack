...
p.init( dest.computer, dest.machine, dest.part );
p.get();
p.set_last_shot_number( 0 );
p.put();
return TRUE;
}
...
PART_NAME_ENTRY sorcname;
PART_NAME_ENTRY destname;

if ( !p.find(sorc_computer, sorc_machine, sorc_part) )
    return FALSE;

if ( !part_exists(dest_computer, dest_machine, dest_part) )
    {
    sorcname.set( sorc_computer, sorc_machine, sorc_part );
    destname.set( dest_computer, dest_machine, dest_part );
    return copy_part_by_files( destname, sorcname );
    }
...
if ( flags & COPY_PART_CHANNELS )
    {
    destfile.get_ftchan_csvname( dest_computer, dest_machine, dest_part );
    if ( !destfile.file_exists() )
        {
        destfile.get_ftchan_dbname( dest_computer, dest_machine, dest_part );
        if ( !destfile.file_exists() )
            {
            sorcfile.get_ftchan_csvname( sorc_computer, sorc_machine, sorc_part );
            if ( sorcfile.file_exists() )
                destfile.get_ftchan_csvname( dest_computer, dest_machine, dest_part );
            else
...
destfile.get_ft2_part_settings_file_name( dest_computer, dest_machine, dest_part );
if ( destfile.file_exists() )
    {
...
if ( flags & COPY_PART_CHANNELS )
    ...
    d.analog_sensor         = p.analog_sensor;
    }
...
if ( flags & COPY_PART_ADVANCED )
    ...
    sorcfile.get_impact_param_file_csvname( sorc_computer, sorc_machine, sorc_part );
    if ( sorcfile.file_exists() )
        {
        destfile.get_impact_param_file_csvname( dest_computer, dest_machine, dest_part );
        sorcfile.copyto( destfile );
        }
    }
