...
thirty_second_timeout_wire = NO_WIRE;
st2_program_abort_wire     = NO_WIRE;
shot_complete_wire         = NO_WIRE;
shot_save_complete_wire    = NO_WIRE;
warning_shot_wire          = NO_WIRE;
alarm_shot_wire            = NO_WIRE;
...
*                             OPERATOR=                                *
***********************************************************************/
void PART_CLASS::operator=( PART_CLASS & sorc )
{
int i;
...
multipart_runlist_wires = sorc.multipart_runlist_wires;
...
analog_sensor = sorc.analog_sensor;

x4_divisor                     = sorc.x4_divisor;
velocity_multiplier            = sorc.velocity_multiplier;

thirty_second_timeout_wire     = sorc.thirty_second_timeout_wire;
st2_program_abort_wire         = sorc.st2_program_abort_wire;
shot_complete_wire             = sorc.shot_complete_wire;
shot_save_complete_wire        = sorc.shot_save_complete_wire;
warning_shot_wire              = sorc.warning_shot_wire;
alarm_shot_wire                = sorc.alarm_shot_wire;
...
if ( sp.get(fname) )
    ...
    /*
    ------------------------------------------------------------------------
    If this is an old file (pre-v8.04) I need to add the zero speed settings
    ------------------------------------------------------------------------ */
    if ( sp.count() < PARTSET_SETPOINT_COUNT )
        {
        sp.setsize( PARTSET_SETPOINT_COUNT );
        sp[PARTSET_ZERO_SPEED_OUTPUT_WIRE_INDEX].value = (int) NO_WIRE;
        sp[PARTSET_ZERO_SPEED_OUTPUT_ENABLE_INDEX].value = FALSE;
        }
    update();
    return TRUE;
    }
...
*                        LOAD_ANALOG_SENSORS                           *
***********************************************************************/
BOOLEAN PART_CLASS::load_analog_sensors()
{
    FILE_CLASS f;
    int        i;
    short      j;
    NAME_CLASS s;
    DB_TABLE   t;
    ANALOG_SENSOR current_analog;

    /*
    -----------------------------------------
    Load full set of available analog sensors
    ----------------------------------------- */
    s.get_asensor_csvname( computer );
    if ( !current_analog.get(s) )
        return FALSE;

    analog_sensor.kill_sensor_array();
    s.get_ftchan_csvname( computer, machine, name ); //ftchan.csv
    if ( s.file_exists() )
    {
        /*
        -----------------
        ...
        {
            analog_sensor.SensorArray = new ANALOG_SENSOR_ENTRY[MAX_FT2_CHANNELS];

            if ( analog_sensor.SensorArray )
            {
                for ( i=0; i < MAX_FT2_CHANNELS; ++i )
                {
                    ...
                    s.next_field();
                    s.next_field();
                    if (current_analog.find(s.text()))
                    {
                        analog_sensor.SensorArray[i] = current_analog.current();
                    }
                    else
                    {
                        /*
                        -----------------------------------------------------
                        Could not find the analog sensor. Use default sensor.
                        ----------------------------------------------------- */
                        analog_sensor.SensorArray[i] = analog_sensor.DefaultSensor;
                    }
                }
                analog_sensor.NofSensors = i;
            }
            f.close();
        }
        else
    ...
    {
        s.get_ftchan_dbname( computer, machine, name ); //ftchan.txt
        if ( s.file_exists() )
        {
            /*
            -----------------
            ...
            {
                analog_sensor.setsize( MAX_FT2_CHANNELS );
                for (i = 0; i < MAX_FT2_CHANNELS; ++i)
                {
                    if ( t.get_next_record(NO_LOCK) )
                    {
                        j = t.get_short( FTCHAN_SENSOR_NUMBER_OFFSET );
                        analog_sensor[i] = current_analog[j];
                    }
                    else
                    {
                        return FALSE;
    ...
    if ( analog_sensor.NofSensors == MAX_FT2_CHANNELS )
        return TRUE;
    else
        return FALSE;
}
...
*                        SAVE_ANALOG_SENSORS                           *
***********************************************************************/
BOOLEAN PART_CLASS::save_analog_sensors()
{
    FILE_CLASS f;
    STRING_CLASS s;
    NAME_CLASS fname;
    ...
    fname.get_ftchan_csvname( computer, machine, name );

    if ( f.open_for_write(fname) )
    {
        for (i = 0; i<MAX_FT2_CHANNELS; i++)
        {
            s = i + 1;
            s += CommaChar;
            s += analog_sensor.SensorArray[i].desc;
            f.writeline(s);
        }
...
if ( status )
    ...
    name     = name_to_find;
    load_analog_sensors(); //loads asensor.csv and ftchan.csv
    read_x4_divisor();
    multipart_runlist_wires.get( computer, machine, name );
    ...
    good_shot_wire             = NO_WIRE;
    good_shot_requires_no_warnings = FALSE;
    shot_complete_timeout_seconds  = 0;

    s.get_shot_events_setup_file_name( computer, machine, name );
    if ( s.file_exists() )
        {
        n = sp.get( s );
        if ( n > GOOD_SHOT_REQUIRES_NO_WARNINGS_INDEX )
            {
            thirty_second_timeout_wire = sp[0].value.uint_value();
            if ( thirty_second_timeout_wire > HIGH_FT2_OUTPUT_WIRE )
                thirty_second_timeout_wire = NO_WIRE;
            st2_program_abort_wire = sp[1].value.uint_value();
            if ( st2_program_abort_wire > HIGH_FT2_OUTPUT_WIRE )
                st2_program_abort_wire = NO_WIRE;
            shot_complete_wire = sp[2].value.uint_value();
            if ( shot_complete_wire > HIGH_FT2_OUTPUT_WIRE )
                shot_complete_wire = NO_WIRE;
            shot_save_complete_wire = sp[3].value.uint_value();
            if ( shot_save_complete_wire > HIGH_FT2_OUTPUT_WIRE )
                shot_save_complete_wire = NO_WIRE;
            warning_shot_wire = sp[4].value.uint_value();
            if ( warning_shot_wire > HIGH_FT2_OUTPUT_WIRE )
                warning_shot_wire = NO_WIRE;
            alarm_shot_wire = sp[5].value.uint_value();
            if ( alarm_shot_wire > HIGH_FT2_OUTPUT_WIRE )
                alarm_shot_wire = NO_WIRE;
            good_shot_wire = sp[6].value.uint_value();
            if ( good_shot_wire > HIGH_FT2_OUTPUT_WIRE )
                good_shot_wire = NO_WIRE;
            good_shot_requires_no_warnings = sp[7].value.boolean_value();

            if ( n > SHOT_COMPLETE_TIMEOUT_SECONDS_INDEX )
                shot_complete_timeout_seconds = sp[8].value.int_value();
            }
        }
    else
        {
        ...
        if ( sp.get(s) > 1 )
            ...
            if ( good_shot_wire > HIGH_FT2_OUTPUT_WIRE )
                good_shot_wire = NO_WIRE;
            good_shot_requires_no_warnings = sp[1].value.boolean_value();
            }
        }
    }
...
if ( status )
    ...
    save_analog_sensors();
    s.get_shot_events_setup_file_name( computer, machine, name );
    gsp.setsize( SHOT_EVENTS_COUNT );
    gsp[0].desc  = THIRTY_SECOND_TIMEOUT_WIRE_DESC;
    gsp[0].value = thirty_second_timeout_wire;
    gsp[1].desc  = ST2_PROGRAM_ABORT_WIRE_DESC;
    gsp[1].value = st2_program_abort_wire;
    gsp[2].desc  = SHOT_COMPLETE_WIRE_DESC;
    gsp[2].value = shot_complete_wire;
    gsp[3].desc  = SHOT_SAVE_COMPLETE_WIRE_DESC;
    gsp[3].value = shot_save_complete_wire;
    gsp[4].desc  = WARNING_SHOT_WIRE_DESC;
    gsp[4].value = warning_shot_wire;
    gsp[5].desc  = ALARM_SHOT_WIRE_DESC;
    gsp[5].value = alarm_shot_wire;
    gsp[6].desc  = GOOD_SHOT_WIRE_DESC;
    gsp[6].value = good_shot_wire;
    gsp[7].desc  = GOOD_SHOT_IF_NO_WARNINGS_DESC;
    gsp[7].value = boolean_to_ascii( good_shot_requires_no_warnings );
    gsp[8].desc  = SHOT_COMPLETE_TIMEOUT_SECONDS_DESC;
    gsp[8].value = shot_complete_timeout_seconds;
    gsp.put( s );
    }

multipart_runlist_wires.put( computer, machine, name );
...
*                           VELOCITY_FROM_X4                           *
***********************************************************************/
double PART_CLASS::velocity_from_x4( double x4_value )
{
double x;
x = x4_value * velocity_multiplier;
x /= x4_divisor;
return x;
}
...
***********************************************************************/
void PART_CLASS::set_zero_speed_output_wire( short new_zero_speed_output_wire )
{
zero_speed_output_wire = new_zero_speed_output_wire;
...
*                      set_zero_speed_output_wire                      *
***********************************************************************/
void PART_CLASS::set_zero_speed_output_wire( STRING_CLASS & new_zero_speed_output_wire )
{
zero_speed_output_wire = (short) new_zero_speed_output_wire.int_value();
sp[PARTSET_ZERO_SPEED_OUTPUT_WIRE_INDEX].value = new_zero_speed_output_wire;
}
...
velocity_multiplier        = dups_to_vu( velocity_units, distance_units );
zero_speed_output_wire     = (short) sp[PARTSET_ZERO_SPEED_OUTPUT_WIRE_INDEX].value.int_value();
