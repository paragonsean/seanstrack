...
/***********************************************************************
*                            READ_UNICODE_FILE                         *
* This reads the entire file into one string. It is assumed the file   *
* is UNICODE and has a unicode introducer at the beginning. The        *
* unicode introducer is NOT copied.                                    *
* Dest should have the path of the file to be read. This will be       *
* replaced with the contents of the file.                              *
***********************************************************************/
BOOLEAN FILE_CLASS::read_unicode_file( STRING_CLASS & dest )
{
const int bytes_per_wchar = (int) sizeof( wchar_t );;
...
/*
---------------------------------------------
Dest contains the name of the file to be read
--------------------------------------------- */
if ( open_for_read(dest) )
    {
    /*
    ---------------------------------------------------------
    The file size includes the unicode introducer, which I am
    not going to copy so there is room for a NullChar.
    --------------------------------------------------------- */
    ...
    /*
    -------------------------------------------------
    I'm not going to read the first unicode character
    ------------------------------------------------- */
    n = file_size - bytes_per_wchar;;
    slen = n;
    if ( is_unicode )
        slen /= bytes_per_wchar;

    /*
    -------------------------------------------------
    Upsize the dest to make sure there is enough room
    for all the characters in the file. STRING_CLASS
    will put room for a null at the end.
    ------------------------------------------------- */
    dest.upsize( slen );

    /*
    ---------------------------
    Skip the unicode introducer
    --------------------------- */
    setpos( bytes_per_wchar );

    cp = dest.text();
    readbinary( (void *)cp, n );
    close();

    /*
    -------------------------------------------------------
    Put a null at the end of the string if this is unicode.
    ------------------------------------------------------- */
    if ( is_unicode )
        *(cp+slen) = NullChar;
    else
        wchar_to_char( (char *)cp, (wchar_t *)cp, slen );
