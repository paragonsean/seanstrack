...
*                            STRING_CLASS                              *
***********************************************************************/
STRING_CLASS::STRING_CLASS( int sorc )
{
    s = 0;
    maxlen = 0;
    operator=( sorc );
...
if ( *cp == separator )
    ...
    *ep = NullChar;
    ...
    if ( cp < ep )
        ...
        /*
        ---------------------------------------------------
        This only happens when this is the the first call
        for this string and there is no separator.
        Increase the length so it won't be equal next time.
        --------------------------------------------------- */
        upsize( maxlen+1 );
        return true;
        }
