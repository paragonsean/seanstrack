...
static TCHAR       MainMenuWindowName[]  = TEXT( "StdDownMainMenu" );
static TCHAR       MainMenuFile[]        = TEXT( "stddown.mnu" );
TCHAR              MyClassName[]         = TEXT( "StdDown" );
...
if ( dc != NULL )
    {
    old_font = (HFONT) SelectObject( dc, hf );
...
TEXT_LEN_CLASS tmax;
NAME_CLASS     s;
const int START_TIME_COL_NAME_INDEX = 2;
...
{
NAME_CLASS s;
s.get_exe_dir_file_name( MainMenuFile );
...
StringTable.find( title, TEXT("STD_DOWNTIME_SETUP") );
