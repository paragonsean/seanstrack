...
TCHAR MyClassName[] = TEXT( "ShiftReport" );
...
s.get_local_ini_file_name( MyIniFile );
ini.set_file( s.text() );
...
while ( SelectedMachines.next() )
    ...
    if ( !Machine.find(machine_name) )
        break;
    s.get_downtime_dbname( Machine.computer_name(), machine_name );
    if ( s.file_exists() )
        {
...
*                             PLOT_MACHINE                             *
***********************************************************************/
void plot_machine( STRING_CLASS & machine_name, int ytop, int ybottom )
{
SYSTEMTIME      st;
NAME_CLASS      s;
DB_TABLE        t;
...
while ( SelectedMachines.next() )
    ...
    s = SelectedMachines.text();
    plot_machine( s, y, y+interval );
    y -= interval;
    }
...
while ( SelectedMachines.next() )
    ...
    s.get_downtime_dbname( Machine.computer_name(), Machine.name() );
    if ( !s.file_exists() )
        return;
...
*                          LOAD_REPORT_LIST                            *
***********************************************************************/
static void load_report_list( HWND listbox_handle )
{
TCHAR         * cp;
STRING_CLASS    s;
TEXT_LIST_CLASS t;
...
s.get_exe_dir_file_name( ConfigFilePrefix );
if ( is_empty(name) )
...
while ( Machine.next() )
    {
    i = lb.add( Machine.name().text() );
    if ( SelectedMachines.find( Machine.name().text() ) )
        lb.setcursel( i );
    }
}
...
void get_help()
{
static char buf[] = "name=shiftrpt.swf";
const int SLEN = sizeof( buf );

NAME_CLASS s;
FILE_CLASS f;

if ( s.get_exe_dir_file_name(TEXT("ft2help.txt")) )
    {
    f.open_for_write( s.text() );
    f.writebinary( buf, SLEN );
    f.close();

    if ( s.get_exe_dir_file_name(TEXT("ft2help.exe")) )
        execute( s.text() );
    }
}
...
if ( CommandLine.find(TEXT("-p")) )
    {
    PrintOnly = true;
    cmd_show = SW_SHOWMINIMIZED;
    }

if ( CommandLine.find(TEXT("-csv")) )
    {
    CsvOnly = true;
    cmd_show = SW_SHOWMINIMIZED;
    }
