...
FIFO_CLASS   EventFifo;
HANDLE       EventThreadEvent;
static const DWORD MS_TO_WAIT = 505;
static BOOLEAN EventmanShutdown = FALSE;
...
static FIFO_CLASS ExternalParameterFifo;
...
const UINT          EXTERNAL_PARAMETER_TIMER_ID = 5;
...
/*
-----------------------------
EXTERNAL_PARAMETER_FILE_CLASS
----------------------------- */
class EXTERNAL_PARAMETER_FILE_CLASS
{
public:
BOOLEAN         is_enabled;
PART_NAME_ENTRY part;
NAME_CLASS      external_parameters_file_name;

EXTERNAL_PARAMETER_FILE_CLASS();
~EXTERNAL_PARAMETER_FILE_CLASS();
void operator=( const EXTERNAL_PARAMETER_FILE_CLASS & sorc );
};

/*
-------------------------------------------------
List of External Parameter Files for each Machine
------------------------------------------------- */
EXTERNAL_PARAMETER_FILE_CLASS * Epf      = 0;
int                             NofEpf   = 0;
...
/***********************************************************************
*     EXTERNAL_PARAMETER_FILE_CLASS::EXTERNAL_PARAMETER_FILE_CLASS     *
***********************************************************************/
EXTERNAL_PARAMETER_FILE_CLASS::EXTERNAL_PARAMETER_FILE_CLASS()
{
is_enabled = FALSE;
...
/***********************************************************************
*               EXTERNAL_PARAMETER_FILE_CLASS::OPERATOR=               *
***********************************************************************/
void EXTERNAL_PARAMETER_FILE_CLASS::operator=( const EXTERNAL_PARAMETER_FILE_CLASS & sorc )
{
is_enabled                    = sorc.is_enabled;
part                          = sorc.part;
external_parameters_file_name = sorc.external_parameters_file_name;
}
...
***********************************************************************/
void process_new_downtime_event( SOCKET_MSG * msg )
{
    DOWN_DATA_CLASS dd;
...
if ( LoggingSentCommands )
    LoggingSentCommands = FALSE;
else
    LoggingSentCommands = TRUE;

MainMenu.set_checkmark( (UINT) FTII_LOG_MESSAGES_CHOICE, LoggingSentCommands );
...
StringTable.find( s, TEXT("STATUS_TITLE") );
...
StringTable.find( s, TEXT("TERMINAL_TITLE") );
CreateWindow(
    TerminalClassName,
    s.text(),
...
/*
------------------------------------------------------
The following is commented out because I realized that
the opto display is helpful for monitor only systems
and not just st2 controls.
------------------------------------------------------ */
...
***********************************************************************/
void event_callback( SOCKET_MSG * msg )
{
    SOCKET_MSG *sm;
    ...
    *sm = *msg;
    EventFifo.push( sm );
    SetEvent( EventThreadEvent );
}

/***********************************************************************
*                       PROCESS_NEW_MONITOR_SETUP                      *
***********************************************************************/
void process_new_monitor_setup_event( SOCKET_MSG * msg )
{
    MACHINE_CLASS m;
    STRING_CLASS  s;
    HWND          w;
    ...
    if ( s.len() == 0 || MyMonitorSetupCount>0 )
    {
    ...
    /*
    -----------------------------------------------------
    Savepart may have changed the ft or jog valve setting
    ----------------------------------------------------- */
#ifdef UNICODE
    GlobalParameterDialog.post(WM_NEW_DATA);
#endif
}
...
static UINT box[] = {
    NULL_DAC_EDITBOX,
    LVDT_DAC_EDITBOX,
    JOG_SHOT_VALVE_EDITBOX,
    RETRACT_VALVE_EDITBOX,
    VC_DITHER_FREQ_EB,
    VC_DITHER_AMPL_EB,
    FT_VALVE_EDITBOX,
    MIN_LI_EDITBOX,
    GLOBAL_DIST_UNITS_XBOX,
    ZERO_SPEED_CHECK_EDITBOX,
    MAX_VELOCITY_EDITBOX,
    PC_DITHER_FREQ_EB,
    PC_DITHER_AMPL_EB,
    NULL_DAC_2_EB,
    LVDT_DAC_2_EB
...
static UINT box[] = {
    VOLTAGE_STEP_TBOX,
    VOLTAGE_EBOX,
    PARK_VOLTAGE_STEP_TBOX,
    PARK_VOLTAGE_EBOX,
    ALT_STEP_TBOX,
    SLOW_SHOT_VELOCITY_EBOX,
    ACCELERATION_EBOX,
    ACCUMULATOR_ON_POSITION_EBOX
    };
...
static FILE_CLASS     f;
static NAME_CLASS     filename;
static STRING_CLASS   logs;
static STRING_CLASS   ns;
static STRING_CLASS   os;
static NAME_CLASS     s;
static SETPOINT_CLASS sp;
static HWND           w;
static WINDOW_CLASS   wc;
...
if ( Monitor_Status != CurrentOpStatus.monitor_status || NeedRefresh )
    ...
    /*
    -----------------------------------------------------
    Check to see if the ZSpeed bit changed from off to on
    I changed this to when the shot in progress goes off
    ----------------------------------------------------- */
    x = MonitorStatusControl[2].bit;
    if ( (x & CurrentOpStatus.monitor_status) && !(x & Monitor_Status) )
        {
        filename.get_impact_param_file_csvname( CurrentPart.computer, CurrentPart.machine, CurrentPart.name );
        if ( filename.file_exists() )
            {
            sp.get( filename );
            if ( sp[IMPACT_PARAM_FILE_ENABLED_INDEX].boolean_value() )
                {
                filename = sp[IMPACT_PARAM_FILE_NAME_INDEX].value;
                if ( !filename.isempty() )
                    {
                    s = CurrentPart.total_stroke_length - CurrentPart.dist_from_x4( (double) CurrentOpStatus.pos );
...
if ( Lindex >= 0 )
    ...
    logs += TEXT( ", Ch 1= " );
    x = AnalogValue[0];
    logs += ascii_double( CurrentPart.analog_sensor[0].converted_value(x) );
    ...
    logs += TEXT( ", Ch 2= " );
    x = AnalogValue[1];
    logs += ascii_double( CurrentPart.analog_sensor[1].converted_value(x) );
...
if ( OptoMode )
    StringTable.find( s, TEXT("VERT_TEXT") );
else
    StringTable.find( s, TEXT("VERT_OPTO") );
    ...
    case WM_COMMAND:
        switch ( id )
            ...
            case LOG_FILE_SETUP_PB:
                /*
                ----------------------------------------------------------------
                NeedErrorLog is FALSE at program start. It is only changed here.
                ---------------------------------------------------------------- */
                if ( NeedErrorLog )
                    {
                    NeedErrorLog = FALSE;
...
if ( WireString.isempty() )
    StringTable.find( WireString, TEXT("WIRE_STRING") );
...
StringTable.find( s, TEXT("OUTPUT_LIST_TITLE") );
w = hWnd;
...
s += ns;
s += shot;
EventClient.poke_data( DS_NOTIFY_INDEX, s.text() );
}
...
EventClient.poke_data( SHOT_NUMBER_INDEX, shot, buf );
...
if ( cp )
    {
    lstrcpy( cp, s );
    EventClient.poke_data( DS_NOTIFY_INDEX, sorc );
    }
}
...
/***********************************************************************
*                           FT2_SHUTDOWN_CALLBACK                      *
***********************************************************************/
void process_ft2_shutdown_event( SOCKET_MSG *msg )
{
    int i;
    void * entry;

    EventClient.is_reading = FALSE;
    EventmanShutdown = TRUE;
    ProcessEvents = FALSE;
    SetEvent( EventThreadEvent );

    for ( i = 0; i < EventFifo.count(); i++ )
    {
        entry = EventFifo.pop();
        delete entry;
    }

    SendMessage( MainWindow.handle(), WM_EV_SHUTDOWN, 0, 0L );
}

/***********************************************************************
*                          PASSWORD_LEVEL_CALLBACK                     *
***********************************************************************/
void process_password_level_event( SOCKET_MSG *msg )
{
    INI_CLASS    ini;
    NAME_CLASS   s;
...
*                              PLOT_MIN_CALLBACK                       *
***********************************************************************/
void process_plot_min_event( SOCKET_MSG *msg )
{
...
/***********************************************************************
*                     DO_EXTERNAL_PARAMETER_TIMER                      *
***********************************************************************/
static void do_external_parameter_timer()
{
    const UINT EXTERNAL_PARAMETER_TIMEOUT_MS = 1000;
    static BOOLEAN firstime = TRUE;
    double         dp[MAX_PARMS];
    int            i;
    int            j;
    ...
    int            pi;
    int            shotparm_index;
    bool           have_shotparm_rec;
    bool           have_shotparm_changes;
    bool           need_to_free_semaphor;
    DWORD          file_attributes;
    FTII_FILE_DATA fd;
    FILE_CLASS     f;
    NAME_CLASS     s;
    NAME_CLASS     fn;
    STRING_CLASS   external_parameter_name;
    STRING_CLASS   mapa;
    STRING_CLASS   shot_name;
    EXTERNAL_PARAMETER_FILE_CLASS * new_epf;
    ...
    if ( firstime )
    {
        ...
        if ( n > 0 )
        {
            Epf = new EXTERNAL_PARAMETER_FILE_CLASS[n];
            if ( Epf )
            {
                NofEpf = n;
                mlist.rewind();
                ...
                {
                    mlist.next();
                    Epf[i].part.computer = ComputerName;
                    Epf[i].part.machine = mlist.name();
                }
            }
        }
    }

    n = ExternalParameterFifo.count();

    for ( i = 0; i<n; i++ )
    {
        have_shotparm_changes = false;
        new_epf = (EXTERNAL_PARAMETER_FILE_CLASS *)ExternalParameterFifo.pop();
        for ( j = 0; j<NofEpf; j++ )
        {
            if ( Epf[j].part.machine == new_epf->part.machine )
            {
                if ( new_epf->is_enabled )
                    Epf[j] = *new_epf;
                else
                    Epf[j].is_enabled = FALSE;
                break;
            }
        }
        delete new_epf;
    ...
    /*
    --------------------------------------
    See if anyone is looking for files now
    -------------------------------------- */
    n = 0;
    for ( i = 0; i<NofEpf; i++ )
    {
    ...
    need_to_free_semaphor = false;

    for ( i = 0; i<NofEpf; i++ )
    {
        if ( Epf[i].is_enabled && Epf[i].external_parameters_file_name.file_exists() )
        {
            if ( get_shotsave_semaphore(Epf[i].part.computer, Epf[i].part.machine) )
            {
                need_to_free_semaphor = true;

                /*
                --------------------------------------------------------------------
                Disable this part until it is re-enabled at the end of the next shot
                -------------------------------------------------------------------- */
                Epf[i].is_enabled = FALSE;
                ...
                /*
                -------------------------------------------
                Put the external variables in the shot file
                ------------------------------------------- */
                plookup.get_last_shot_name( shot_name, Epf[i].part.computer, Epf[i].part.machine, Epf[i].part.part );
                fn.get_profile_ii_name( Epf[i].part.computer, Epf[i].part.machine, Epf[i].part.part, shot_name );
                if ( fd.get(fn) )
                {
                    if ( p.find(Epf[i].part.computer, Epf[i].part.machine, Epf[i].part.part) )
                    {
                        s.get_shotparm_csvname( Epf[i].part.computer, Epf[i].part.machine, Epf[i].part.part );
                        if ( s.file_exists() )
                        {
                            vdb.open_for_read( s );
                            vdb.goto_last_record();
                            have_shotparm_rec = vdb.get_rec();
                            vdb.close();
                        }

                        if ( f.open_for_read(Epf[i].external_parameters_file_name) )
                        {
                            while (true)
                            {
                                external_parameter_name = f.readline();
                                if ( external_parameter_name.isempty() )
                                    break;
                                external_parameter_name.next_field();
                                shotparm_index = SHOTPARM_PARAM_1_INDEX;
                                for ( pi = 0; pi<MAX_PARMS; pi++ )
                                {
                                    if ( p.parameter[pi].input.type == FT_EXTERNAL_PARAMETER )
                                    {
                                        if ( external_parameter_name == p.parameter[pi].name )
                                        {
                                            /*
                                            ------------------------------------------
                                            Move to the next field, which is the value
                                            ------------------------------------------ */
                                            external_parameter_name.next_field();

                                            new_count = fd.nof_text_parameters + 1;
                                            fd.resize_text_parameters( new_count, TRUE );
                                            fd.text_parameter[new_count - 1].parameter_number = pi + 1;
                                            fd.text_parameter[new_count - 1].value = external_parameter_name;
                                            if ( have_shotparm_rec )
                                            {
                                                vdb.ra.goto_index( shotparm_index );
                                                vdb.ra = external_parameter_name; //This is the value now
                                                have_shotparm_changes = true;
                                            }
                                            break;
                                        }
                                    }
                            ...
                            f.close();
                            Epf[i].external_parameters_file_name.delete_file();
                            fd.put( fn );

                            /*
                            ----------------------------------------------------------------
                            Mark the shot file so I will send a message to the data archiver
                            ---------------------------------------------------------------- */
                            file_attributes = GetFileAttributes( fn.text() );
                            file_attributes |= FILE_ATTRIBUTE_ARCHIVE;
                            SetFileAttributes( fn.text(), file_attributes );

                            mapa.null();
                            mapa.cat_w_char( Epf[i].part.machine, CommaChar );
                            mapa += Epf[i].part.part;
                            poke_newshot_message( (WPARAM)shot_name.release(), (LPARAM)mapa.release() );
                        }
                    }

                    if ( have_shotparm_changes )
                    {
                        vdb.open_for_rw( s );
                        vdb.goto_last_record();
                        vdb.rec_delete();
                    ...
                    /*
                    ----------------------------------------------------------
                    Check the alarms and add the counts to the alarm summaries
                    ---------------------------------------------------------- */
                    shotparm_index = SHOTPARM_PARAM_1_INDEX;
                    for ( pi = 0; pi<MAX_PARMS; pi++ )
                    {
                        if ( have_shotparm_rec )
                        {
                            /*
                            ----------------------------------------------------------------
                            I have to fill the parameter value array for the alarm summaries
                            If the vartype is TEXT_VAR only then there is no numeric value.
                            I used to do it this way with the following statement
                            if ( p.parameter[pi].vartype != TEXT_VAR )
                            but users don't want to specify vartype so now I just try to
                            convert them all. real_value() checks for numbers so it's ok.
                            ---------------------------------------------------------------- */
                            vdb.ra.goto_index( shotparm_index );
                            dp[pi] = vdb.ra.real_value();
                        }
                        shotparm_index++;
                    }
                    check_external_alarms( Epf[i].part.machine, dp );
                }
                if ( need_to_free_semaphor )
                    free_shotsave_semaphore();
            }
        }
    }
    ...
    if ( n > 0 )
        SetTimer( MainWindow.handle(), EXTERNAL_PARAMETER_TIMER_ID, EXTERNAL_PARAMETER_TIMEOUT_MS, NULL );
}

/***********************************************************************
*              BEGIN_CHECKING_FOR_EXTERNAL_PARAMETER_FILE              *
*                    mapa contains the machine,part                    *
***********************************************************************/
void begin_checking_for_external_parameter_file( TCHAR * mapa )
{
const UINT EXTERNAL_PARAMETER_TIMEOUT_MS = USER_TIMER_MINIMUM;
STRING_CLASS s;
STRING_CLASS machine;
STRING_CLASS part;
EXTERNAL_PARAMETER_FILE_CLASS   * epf;
...
eps.get( ComputerName, machine, part );
if ( eps.enable && eps.available_at_eos )
    {
    epf = new EXTERNAL_PARAMETER_FILE_CLASS;
    epf->is_enabled    = TRUE;
    epf->part.computer = ComputerName;
    epf->part.machine  = machine;
    epf->part.part     = part;
    epf->external_parameters_file_name = eps.file_name;
    ExternalParameterFifo.push( (void *) epf );
...
/***********************************************************************
*              STOP_CHECKING_FOR_EXTERNAL_PARAMETER_FILE               *
* The string passed has been allocated by someone else. It is my       *
* responsibility to delete it.                                         *
***********************************************************************/
void stop_checking_for_external_parameter_file( TCHAR * mapa )
{
const UINT EXTERNAL_PARAMETER_TIMEOUT_MS = USER_TIMER_MINIMUM;
...
switch (msg)
    ...
    case WM_DBINIT:
        ...
        EventClient.register_for_event( MONITOR_SETUP_INDEX, event_callback );
        EventClient.register_for_event( DOWN_DATA_INDEX, event_callback );
        EventClient.register_for_event( PLOT_MIN_INDEX, event_callback );
        EventClient.register_for_event( PW_LEVEL_INDEX, event_callback );
        EventClient.register_for_event( SHUTDOWN_INDEX, process_ft2_shutdown_event );
    ...
    case WM_TIMER:
        ...
        else if ( EXTERNAL_PARAMETER_TIMER_ID == (UINT) wParam )
            {
            KillTimer( hWnd, EXTERNAL_PARAMETER_TIMER_ID );
            do_external_parameter_timer();
            }
        return 0;

    case WM_NEW_DATA:
        if ( wParam == NEW_CYCLE_START )
            stop_checking_for_external_parameter_file( (TCHAR *) lParam );
        return 0;
    case WM_NO_DATA:
        /*
        ----------------------------------------------------------------------------------------
    ...
    case WM_NEWSHOT:
        if ( lParam && wParam )
            {
            begin_checking_for_external_parameter_file( (TCHAR *)lParam );
    ...
    case WM_POKEMON:
        ...
        if ( lParam )
            {
            s = (TCHAR *) lParam;
            EventClient.poke_data( wParam, s );
...
/***********************************************************************
*                              EVENT_THREAD                            *
*               Checks to see if any events need processing.           *
***********************************************************************/
DWORD event_thread()
{
    SOCKET_MSG * msg;
    ...
    while ( ProcessEvents )
    {
        if ( EventFifo.count() > 0 )
        {
            msg = ( SOCKET_MSG *)EventFifo.pop();

            switch ( msg->item )
            {
            case MONITOR_SETUP_INDEX:
                process_new_monitor_setup_event( msg );
                break;

            case DOWN_DATA_INDEX:
                process_new_downtime_event( msg );
                break;

            case PLOT_MIN_INDEX:
                process_plot_min_event( msg );
                break;

            case PW_LEVEL_INDEX:
                process_password_level_event( msg );
                break;
        ...
        if ( WaitForSingleObject(EventThreadEvent, MS_TO_WAIT) == WAIT_FAILED )
        {
            resource_message_box( UNABLE_TO_CONTINUE_STRING, TEST_FATAL_ERROR_STRING );
            log_entry = TEXT( "ERROR: da_notify_thread() unable to continue." );
            ServiceLog.add_entry( log_entry );
        }

        ResetEvent( EventThreadEvent) ;
    }
...
----------------------------------------------- */
StringTable.find( CmdString,  TEXT("CMD_STRING") );
StringTable.find( LvdtString, TEXT("LVDT") );
StringTable.find( MeasString, TEXT("MEAS_STRING") );
...
NetworkLogFileEnabled       = ini.get_boolean( NetworkLogFileEnabledKey );
NetworkLogFile              = ini.get_string(  NetworkLogFileKey );
...
------------------------------ */
EventThreadEvent = CreateEvent(NULL, FALSE, FALSE, NULL);

CreateThread(
    NULL,                                       /* no security attributes        */
    0,                                          /* use default stack size        */
    ( LPTHREAD_START_ROUTINE )event_thread,     /* thread function               */
...
/*
-------------------------------------------
Delete the list of External Parameter Files
------------------------------------------- */
if ( Epf )
    {
    delete[] Epf;
...
if ( (nSize > 3000) && (nSize < 5000) )
    fprintf( logFile, "Hi you guys\n" );
