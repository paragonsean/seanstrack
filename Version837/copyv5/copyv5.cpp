...
/*
----------------------------------------------------------
Copy the last ShotsToSave parameters from the SHOTPARM.CSV
---------------------------------------------------------- */
s = sorcdir;
d = destdir;
s.cat_path( SHOTPARM_CSV );
d.cat_path( SHOTPARM_CSV );

dest.ensure_existance( d );
if ( ShotsToSave > 0 )
    {
    if ( sorc.open_for_read(s) )
        {
        i = sorc.nof_recs();
        if ( i > ShotsToSave )
            {
            ...
            i--;
            sorc.get_rec( i );
            dest.open_for_write( d );
            while ( sorc.get_next_record() )
                {
                dest.copy_rec( sorc );
                dest.rec_append();
                }
...
if ( ShotsToSave > 0 )
    ...
    if ( s.file_exists() )
        ...
        td.open( d, AUTO_RECLEN, PFL  );
        while ( td.get_next_record(FALSE) )
            {
            td.get_alpha( shot_name, GRAPHLST_SHOT_NAME_INDEX );
            shot_name.replace_all( SpaceChar, ZeroChar );
            s = sorcdir;
            s.cat_path( shot_name.text() );
            s += PROFILE_II_SUFFIX;
            if ( s.file_exists() )
                {
                d = s;
                ...
                {
                s.replace( PROFILE_II_SUFFIX, PROFILE_NAME_SUFFIX );
                if ( s.file_exists() )
                    {
                    d = s;
                    d.replace( sorcdir.text(), destdir.text() );
