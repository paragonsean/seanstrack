...
if ( is_checked(w, TIME_SAMPLE_BUTTON) )
    StringTable.find( s, TEXT("POSITION_STRING") );
else
...
if ( channel >= 0 && channel < MAX_FT2_CHANNELS )
    {
    s = CurrentPart.analog_sensor[channel].desc;
    }
else if ( channel == FT_ANALOG_VELOCITY_CHANNEL )
    {
    StringTable.find( s, TEXT("VELOCITY_STRING") );
    }
else
    {
    StringTable.find( s, TEXT("QUESTION_MARKS") );
    }
...
if ( CurrentDialogNumber == FT_TARGET_TAB )
    {
    set_text( w, ANALOG_UNITS_TEXTBOX,     EmptyString );
    set_text( w, END_ANALOG_UNITS_TEXTBOX, units_name(CurrentPart.distance_units) );
    }
else
    {
    ...
    set_text( w, ANALOG_UNITS_TEXTBOX,     units_name(i) );
    set_text( w, END_ANALOG_UNITS_TEXTBOX, units_name(j) );
    }
}
...
i = CurrentPart.distance_units;
set_text( w, PFSV_POS_UNITS_TBOX, units_name(i) );
...
for ( i=0; i<MAX_FT2_CHANNELS; i++ )
    ...
    if ( is_checked(w, CHANNEL_1_PB+i) )
        ...
        CurrentParam.parameter[p].units   = CurrentPart.analog_sensor[i].units;
        CurrentParam.parameter[p].vartype = CurrentPart.analog_sensor[i].vartype;
        break;
        }
    }
...
if ( CurrentDialogNumber == FT_ANALOG_TAB )
    ...
    nof_buttons = NOF_DYNAMIC_CHANNELS;
    }
...
for ( i=0; i<nof_buttons; i++ )
    ...
    if ( is_checked(w, CHANNEL_1_BUTTON+i) )
        ...
        --------------------------------------------------------------------------------- */
        if ( i < NOF_DYNAMIC_CHANNELS || CurrentDialogNumber == FT_EXTENDED_ANALOG_TAB )
...
static RESOURCE_TEXT_ENTRY rlist[] =
    ...
    { FT_CALC_RESULT_LABEL,   TEXT( "RESULT_STRING" ) },
    { FT_CALC_EVAL_BTN,       TEXT( "EVALUATE_STRING" ) }
    };
const int nr = sizeof(rlist)/sizeof(RESOURCE_TEXT_ENTRY);
...
set_statics_font( hWnd, FT_CALC_RESULT_TB );
set_statics_font( hWnd, FT_CALC_COMMAND_EBOX );
}

/***********************************************************************
*                           EVALUATE_FTCALC                            *
***********************************************************************/
static void evaluate_ftcalc( HWND hWnd )
{
    int n;
    STRING_CLASS  s;
    NAME_CLASS    fn;
    double      * parameters;
    DB_TABLE      db;
    FTII_PROFILE  profile;

    /*
    ----------------------
    Get list of shot files
    ---------------------- */
    fn.get_graphlst_dbname( CurrentPart.computer, CurrentPart.machine, CurrentPart.name );
    if ( fn.file_exists() )
    {
        if ( db.open(fn, 0, PFL) )
        {
            n = db.nof_recs() - 1;
            db.goto_record_number ( n );
            db.get_next_record( NO_LOCK );
            db.get_alpha( s, GRAPHLST_SHOT_NAME_INDEX );
        }
    }
    ...
    ------------------------------------- */
    fn.get_profile_name( CurrentPart.computer, CurrentPart.machine, CurrentPart.name, s.text(), TEXT(".pr2") );

    if  ( !profile.get(fn) )
    {
        resource_message_box( TEXT("NO_SHOT_FILE"), TEXT("WARNING_STRING") );
        return;
    }

    /*
    -----------------------------------------------
    Evaluate all parameters and populate result box
    ----------------------------------------------- */
    parameters = calculate_ftii_parameters( profile, CurrentMachine, CurrentPart, CurrentParam, CurrentFtAnalog, CurrentFtCalc, CurrentLimitSwitch );
    s = parameters[CurrentParameterIndex];
    s.set_text( hWnd, FT_CALC_RESULT_TB );
    validate_ftcalc_cmd( CurrentParameterIndex );
}
...
switch (msg)
    ...
    case WM_COMMAND:
        switch ( id )
            ...
            case FT_CALC_EVAL_BTN:
                evaluate_ftcalc( hWnd );
                break;
...
ft_calc_hwnd = WindowInfo[FT_CALC_TAB].dialogwindow;
...
switch ( input_type )
    ...
    default:
        ...
        set_text( ft_calc_hwnd, FT_CALC_RESULT_TB, EmptyString );
        break;
    }
...
resource_string( s, x );
s.set_text( ParamDialogWindow, PARAMETER_TYPE_TEXTBOX );

s = new_parameter_index+1;
s.set_text( ParamDialogWindow, PARAMETER_NUMBER_TBOX );
...
set_text( ParamDialogWindow, PARAMETER_DESC_EDITBOX, CurrentParam.parameter[new_parameter_index].name );
