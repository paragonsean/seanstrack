...
#define SHOT_NAME_VALID         1  // Shot name template is valid
#define SHOT_NAME_INVALID_DATE  2  // Shot name template is missing date/time
#define SHOT_NAME_INVLAID_NUM   3  // Shot name template is missing shot number
...
ANALOG_SENSOR      OriginalAnalogSensor;    /* I fill this when I load the part, check when I save */
...
PARAMETER_CLASS OriginalParam;      /* I fill this when I load the part, check when I save */
...
Since FT is no longer supported, I set this to 8 by default now.
------------------------------------------------------------------------- */
int Nof_Editable_Channels = MAX_EDITABLE_FTII_CHANNELS;
...
*                          VALIDATE_FTCALC_CMD                         *
***********************************************************************/
BOOLEAN validate_ftcalc_cmd()
{
    NAME_CLASS name;
    FILE_CLASS file;
    int i;
    STRING cmd;
    static const REGEX gate_area_ex( TEXT(".*Gate_Area.*|.*Gate Area.*") );
    static const REGEX hydraulic_cyl_area_ex( TEXT(".*Hydraulic_Cyl_Area.*|.*Hydraulic Cyl Area.*") );
    static const REGEX hydraulic_cyl_dia_ex( TEXT(".*Hydraulic_Cyl_Dia.*|.*Hydraulic Cyl Dia.*") );
    static const REGEX eol_replace_ex( TEXT("(\\r\\n)") );

    name.get_calc_file_name( CurrentPart.computer, CurrentPart.machine, CurrentPart.name );

    if ( !file.open_for_read(name) )
    {
        /*
        -----------------------------------------------------
        Could not open calc file so check for calc parameters
        ----------------------------------------------------- */
        for ( i = 0; i < MAX_PARMS; i++ )
        {
            if ( !CurrentFtCalc.cmd[i].isempty() )
            {
                cmd = CurrentFtCalc.cmd[i].text();
                cmd = regex_replace( cmd, eol_replace_ex, TEXT(";") );
                if ( regex_match(cmd, gate_area_ex) ||
                     regex_match(cmd, hydraulic_cyl_area_ex) ||
                     regex_match(cmd, hydraulic_cyl_dia_ex) )
                {
                    resource_message_box( TEXT("NO_CALCULATOR_FILE"), TEXT("WARNING_STRING") );
                    return FALSE;
                }
            }
        }
...
*                          VALIDATE_FTCALC_CMD                         *
***********************************************************************/
BOOLEAN validate_ftcalc_cmd( int index )
{
    NAME_CLASS name;
    FILE_CLASS file;
    STRING cmd;
    ...
    if ( !file.open_for_read(name) )
    {
        ...
        Could not open calc file so check for calc parameters
        ----------------------------------------------------- */
        if ( !CurrentFtCalc.cmd[index].isempty() )
        {
            cmd = CurrentFtCalc.cmd[index].text();
            cmd = regex_replace( cmd, eol_replace_ex, TEXT(";") );
            if ( regex_match(cmd, gate_area_ex) ||
                 regex_match(cmd, hydraulic_cyl_area_ex) ||
                 regex_match(cmd, hydraulic_cyl_dia_ex) )
...
StringTable.find( s, TEXT("ST_PARAMS") );
text_out( dc, r.left, y, s.text() );
...
StringTable.find( s, TEXT("VEL_LOOP_GAIN") );
x1 = x + pixel_width(dc, s.text()) + 2 * dx;
text_out( dc, x,  y, s.text() );
text_out( dc, x1, y, SureTrakSetup.velocity_loop_gain().text() );

velocity_x = x + pixel_width(dc, StringTable.find(TEXT("BREAK_1"))) + 2*dx;

s.get_text( WindowInfo[SURETRAK_PROFILE_TAB].dialogwindow, VELOCITY_STATIC );
gain_x = velocity_x + pixel_width(dc, s.text()) + 2*dx;

y += dy;
y += dy/2;

text_out( dc, velocity_x, y, s.text() );
...
if ( new_tab_number == FASTRAK_CHANNELS_TAB )
    {
    w = GetDlgItem( MainWindow, PART_LISTBOX_ID );
    lb.init( w );
    lb.setcursel( 0 );
    get_current_part();
    }
...
if ( type == PARAMETER_MARK )
    ...
    sc += (int) number;
    copystring( s, StringTable.find(sc.text()), PARAMETER_NAME_LEN );
    }
...
for ( i=0; i<CurrentAnalog.count(); i++ )
    ...
    for ( bi=0; bi<nof_boxes; bi++ )
        SendDlgItemMessage(w, box + bi, CB_ADDSTRING, 0, (LPARAM) CurrentAnalog[i].desc.text());
    }
}
...
StringTable.find( s, TEXT("TIME_STRING") );
s += SpaceString;
...
SureTrakPlot.add_label( Y_AXIS_INDEX, units_name(PressureControl.analog_sensor.units), BlackColor );
}
...
ymin = (float) PressureControl.analog_sensor.low_value();
ymax = (float) PressureControl.analog_sensor.high_value();
...
if ( is_checked( w ) )
    ...
    u = PressureControl.analog_sensor.units;
    }
else
    {
    u = MS_UNITS;
...
if ( is_proportional_valve )
    {
    is_visible = false;
    StringTable.find( s, TEXT("PERCENT_STRING") );
    }
else
    {
    is_visible = true;
    StringTable.find( s, TEXT("PRESSURE_STRING") );
    }
...
if ( is_proportional_valve )
    StringTable.find( s, TEXT("RETRACT_PERCENT") );
else
    StringTable.find( s, TEXT("RETRACT_VOLTS") );
s.set_text( parent, RETRACT_VOLTS_STATIC );

if ( is_proportional_valve )
    StringTable.find( s, TEXT("VALVE_PARK_PERCENT") );
else
    StringTable.find( s, TEXT("VALVE_PARK_VOLTS") );
...
range = PressureControl.analog_sensor.high_value() - PressureControl.analog_sensor.low_value();
percent = s.real_value();
percent *= 2.0;
percent /= max_percent;
x = PressureControl.analog_sensor.low_value() + percent * range;
...
percent = pressure - PressureControl.analog_sensor.values[0];
percent *= max_percent;
percent /= range;
percent /= 2.0;
...
lb.setcursel(PressureControl.analog_sensor.desc.text());
...
if ( CurrentMachine.monitor_flags & MA_USE_TIME_TO_MEASURE_BISCUIT )
    StringTable.find( s, TEXT("TIME_TO_MEASURE_BISCUIT") );
else
    StringTable.find( s, TEXT("BISCUIT_TIME_DELAY") );
s.set_text( w, BISCUIT_TIME_DELAY_STATIC );
...
if ( new_parameter_type == FT_DIGITAL_INPUT )
    ...
    CurrentParam.parameter[i].units        = CurrentPart.analog_sensor[0].units;
    CurrentParam.parameter[i].vartype      = CurrentPart.analog_sensor[0].vartype;
...
s.get_ftchan_csvname( CurrentMachine.computer, CurrentMachine.name, mypart );
if ( !s.file_exists() )
    {
...
--------------------------------------------------------------------------------------- */
OriginalAnalogSensor = CurrentPart.analog_sensor;

HaveCurrentPart = TRUE;

refresh_basic_setup();

s.get_asensor_csvname( CurrentPart.computer );
CurrentAnalog.get( s );
...
if ( CurrentParam.find(CurrentPart.computer, CurrentPart.machine, CurrentPart.name) )
    {
    refresh_parameter_limits();
    OriginalParam = CurrentParam;
    CurrentFtCalc.load( CurrentParam, CurrentPart.computer, CurrentPart.machine, CurrentPart.name );
    }
...
for ( i=0; i<Nof_Editable_Channels; i++ )
    {
    cp = CurrentPart.analog_sensor[i].desc.text();
    if ( cp )
        {
        lb.init( w, FT_CHANNEL_1_LISTBOX+i );
        lb.setcursel( cp );
        ...
        if ( lstrlen(label[i]) > 0 )
            ...
            resource_string( s, CHANNEL_1_LABEL_STRING+i );
        s.set_text( w, CHAN_1_AXIS_LABEL_EBOX+i );
        }
    }
...
CurrentAnalog.find( lb.selected_text() );
PressureControl.analog_sensor = CurrentAnalog.current();
...
for ( i=0; i<Nof_Editable_Channels; i++ )
    ...
    cp = CurrentPart.analog_sensor[c].desc.text();
    if ( cp )
        {
...
for ( i=0; i<Nof_Editable_Channels; i++ )
    ...
    lb.init( ExtendedChannelsDialog, FT_CHANNEL_1_LISTBOX+i );
    CurrentAnalog.find( lb.selected_text() );
    CurrentPart.analog_sensor[c] = CurrentAnalog.current();
    }
}
...
max_pressure = PressureControl.analog_sensor.high_value();
min_pressure = - max_pressure;
if ( max_pressure <= min_pressure )
    max_pressure = min_pressure + 1.0;
max_volts = PressureControl.analog_sensor.max_volts;
...
*                            VALIDATE_SHOT_NAME                        *
***********************************************************************/
int validate_shot_name()
{
    STRING_CLASS s;
    HWND w;
    static const REGEX date_time( TEXT(".*\\{(yy|yh)\\}.*\\{(mm|mx)\\}.*\\{dd\\}.*\\{hh\\}.*\\{s[0-9]+\\}.*") );
    static const REGEX date_shift( TEXT(".*\\{(ys|yhs)\\}.*\\{(ms|mxs)\\}.*\\{ds\\}.*\\{hs\\}.*\\{s[0-9]+\\}.*") );
    static const REGEX julian_time( TEXT(".*\\{(yy|yh)\\}.*\\{j\\}.*\\{hh\\}.*\\{s[0-9]+\\}.*") );
    static const REGEX julian_shift( TEXT(".*\\{(ys|yhs)\\}.*\\{js\\}.*\\{hh\\}.*\\{s[0-9]+\\}.*") );
    static const REGEX shot_number( TEXT(".*\\{s[0-9]+\\}.*") );

    w = WindowInfo[ADVANCED_SETUP_TAB].dialogwindow;

    if ( !is_checked(w, RESET_SHOT_COUNT_ENABLE_XB) )
    {
        return SHOT_NAME_VALID;
    }

    if ( is_checked(w, PURGE_ON_RESET_XB) )
    {
        return SHOT_NAME_VALID;
    }
    ...
    /*
    ---------------------------------------------------------
    Validate the shot name template contains date/time string
    --------------------------------------------------------- */
    s.get_text( w, SN_TEMPLATE_EDITBOX );

    /*
    ----------------------------------------------------
    Template must contain date (or julian date) and time
    ---------------------------------------------------- */
    if ( !((regex_match(s.text(), date_time) || regex_match(s.text(), julian_time)) ||
            regex_match(s.text(), date_shift) || regex_match(s.text(), julian_shift)) )
    {
        if ( !regex_match(s.text(), shot_number) )
        {
            return SHOT_NAME_INVLAID_NUM;
        }
        else
        {
            return SHOT_NAME_INVALID_DATE;
        }
    }
...
int nof_orig;
NAME_CLASS p;
int shot_name_status;

shot_name_status = validate_shot_name();

switch ( shot_name_status )
{
    case SHOT_NAME_INVALID_DATE:
        s = StringTable.find( TEXT("INVALID_SHOT_NAME_DATE_DLG") );
        break;
    case SHOT_NAME_INVLAID_NUM:
        s = StringTable.find( TEXT("INVALID_SHOT_NAME_NUM_DLG") );
        break;
}

if ( shot_name_status != SHOT_NAME_VALID )
{
    if ( IDCANCEL == MessageBox( NULL, s.text(), StringTable.find(TEXT("INVALID_SHOT_NAME") ), MB_OKCANCEL) )
    {
        return;
    }
    else
    {
        s = TEXT( "{yy}-{mm}-{dd}-{hh}-{s6}" );
        s.set_text( WindowInfo[ADVANCED_SETUP_TAB].dialogwindow, SN_TEMPLATE_EDITBOX );
    }
}
...
sr.init( CurrentPart.computer, CurrentPart.machine, CurrentPart.name );
sr.get();
sr.set_enable( is_checked(w, RESET_SHOT_COUNT_ENABLE_XB) );
...
eps.enable = is_checked( w, EXTERNAL_PARAMETER_FILE_ENABLE_XB );
eps.available_at_eos = is_checked( w, EXTERNAL_PARAMETER_FILE_IS_EOS_XB );
...
for ( i=0; i<Nof_Editable_Channels; i++ )
    ...
    if ( NeedToCheckChannels5and7 && (i==4 || i==6) )
        {
        CurrentPart.analog_sensor[i] = CurrentAnalog.CMDSensor;
        }
    else
        {
        ...
        if ( sensor_index != CB_ERR )
            {
            lb.init( w, FT_CHANNEL_1_LISTBOX+i );
            x = CurrentAnalog.index(lb.selected_text());
            if ( x == NO_INDEX )
                x = CurrentAnalog.index(CurrentAnalog.DefaultSensor.desc.text());
            CurrentPart.analog_sensor[i] = CurrentAnalog[x];
            }
        }
    }
...
new_units = CurrentPart.analog_sensor[i].units ;
if ( new_units != OriginalAnalogSensor[i].units )
...
/*
--------------------------------------------
Check for removed or changed parameter names
-------------------------------------------- */
nof_orig = OriginalParam.count();

for (i = 0; i < nof_orig; ++i)
{
    if (OriginalParam.parameter[i].input.type != FT_CALC_INPUT)
        continue;

    if (CurrentParam.get_parameter_index(OriginalParam.parameter[i].name) == -1)
    {
        s.empty();
        s += OriginalParam.parameter[i].name;
        s += TEXT(".txt");
        p.get_part_dir_file_name(CurrentPart.computer, CurrentPart.machine, CurrentPart.name, s.text());
...
for ( i=0; i<MAX_FT2_CHANNELS; i++ )
    ...
    if ( OriginalAnalogSensor[i] != CurrentPart.analog_sensor[i] )
        {
        old_units = OriginalAnalogSensor[i].units;
        new_units = CurrentPart.analog_sensor[i].units;
        old_type  = OriginalAnalogSensor[i].vartype;
        new_type  = CurrentPart.analog_sensor[i].vartype;
...
validate_ftcalc_cmd();
CurrentFtCalc.save( CurrentParam, CurrentPart.computer, CurrentPart.machine, CurrentPart.name );
...
answer_button = stringtable_message_box( TEXT("WISH_TO_CONTINUE"), TEXT("ABOUT_TO_DEL") );
...
***********************************************************************/
void new_machine_setup_callback( SOCKET_MSG * msg )
{
...
/***********************************************************************
*                       EDITPART_SHUTDOWN_CALLBACK                     *
***********************************************************************/
void editpart_shutdown_callback( SOCKET_MSG *msg )
{
    EventClient.is_reading = FALSE;
    EventmanShutdown = TRUE;
    SendMessage( MainWindow, WM_EV_SHUTDOWN, 0, 0L );
}
...
*                              PLOT_MIN_CALLBACK                       *
***********************************************************************/
void plot_min_callback( SOCKET_MSG* )
{
...
switch (msg)
    ...
    case WM_DBINIT:
        EventClient.register_for_event( MACH_SETUP_INDEX, new_machine_setup_callback );
        EventClient.register_for_event( PW_LEVEL_INDEX, password_level_callback );
        EventClient.register_for_event( PLOT_MIN_INDEX, plot_min_callback );
        EventClient.register_for_event( SHUTDOWN_INDEX, editpart_shutdown_callback );
...
resource_string( s, SURETRAK_PLOT_WINDOW_TITLE_STRING );
