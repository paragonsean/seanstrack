...
TCHAR MyClassName[]        = TEXT( "Install" );
...
TCHAR ComputerTypeKey[]        = TEXT( "ComputerType" );
...
TCHAR LocalIpPortKey[]         = TEXT( "LocalIpPort" );
TCHAR RemoteIpPortKey[]        = TEXT( "RemoteIpPort" );
...
/*
--------------------------------
Timeout for extracting exe files
-------------------------------- */
const DWORD EXES_MS_TO_WAIT = 300000L;
...
*                            WAIT_FOR_FILE                             *
***********************************************************************/
BOOLEAN wait_for_file( STRING_CLASS & filename )
{
...
static void set_global_computer_type_from_buttons( HWND w )
{
    if ( is_checked(w, MONITORING_COMPUTER_RADIO) )
        ...
        ComputerType = DATA_ARCHIVER_COMPUTER_TYPE;
    else if ( is_checked(w, OFFICE_WORKSTATION_RADIO) )
        ComputerType = WORKSTATION_COMPUTER_TYPE;
    else if ( is_checked(w, NO_NETWORK_RADIO) )
        ComputerType = LOCAL_INSTALL_TYPE;
    else
...
if ( f.open_for_write(s) )
    ...
    if ( NeedMonitorStartup && (ComputerType != WORKSTATION_COMPUTER_TYPE) )
        {
        /*
        -------------------------------
...
--------------------------------------------------------------------------------------------- */
cp = NString;
if ( ComputerType == LOCAL_INSTALL_TYPE )
    cp = YString;
...
execute_and_wait( s.text(), dest.text(), EXES_MS_TO_WAIT );
...
/*
-------------------------
Copy the network.csv file
------------------------- */
s = ExeDirectoryName;
s.add_ending_backslash();
s += TEXT( "pkzip25 -ext -dir -over=never " );
s += FloppyDirectoryName;
s += TEXT( "v5data network.csv" );

dest = DataDirectoryName;
dest.remove_ending_backslash();
execute_and_wait( s.text(), dest.text() );

/*
----------------------------
Convert the asensor.csv file
---------------------------- */
if ( s.get_asensor_csvname() )
    {
    CurrentAnalog.get( s );
    CurrentAnalog.put( s );
    }
...
if ( ComputerType != DATA_ARCHIVER_COMPUTER_TYPE )
    {
    CoInitialize(0);
    /*
    -----------------------------------------
...
execute_and_wait( sorc, ExeDirectoryName, EXES_MS_TO_WAIT );
...
/*
---------------------------------------------
Insert the ComputerType key into VISITRAK.INI
--------------------------------------------- */
s.get_exe_dir_file_name( NewComputerName, VisiTrakIniFileName );
...
switch ( ComputerType )
    ...
    case WORKSTATION_COMPUTER_TYPE:
        ini.put_string( ComputerTypeKey, OFFICE_WORKSTATION_KEY );
        break;
    case MONITORING_COMPUTER_TYPE:
        ini.put_string( ComputerTypeKey, MONITORING_COMPUTER_KEY );
        break;
    case DATA_ARCHIVER_COMPUTER_TYPE:
        ini.put_string( ComputerTypeKey, DATA_ARCHIVER_KEY );
        break;
    }

ini.put_int( LocalIpPortKey,  (int) DEFAULT_PORT );
ini.put_int( RemoteIpPortKey, (int) DEFAULT_PORT );

/*
-------------------------------------------------------------------------
The LOCAL_INSTALL_TYPE is the local install on the data archiver. Neither
of these is a monitoring computer and so they don't need monitor menus
------------------------------------------------------------------------- */
if ( ComputerType==WORKSTATION_COMPUTER_TYPE || ComputerType==LOCAL_INSTALL_TYPE )
    {
    s = ExeDirectoryName;
...
if ( (ComputerType == WORKSTATION_COMPUTER_TYPE) || (ComputerType==DATA_ARCHIVER_COMPUTER_TYPE) )
    {
    /*
    ----------------------------------
    ...
    if ( !UsingRestorePoint )
        ...
        for ( i=0; i<NofStSorcFiles; i++ )
            {
            if ( i != StSorcIndex || (ComputerType == WORKSTATION_COMPUTER_TYPE) )
                {
                lstrcpy( sorc_file, SorcDirNames[i] );
                kill_directory( sorc );
                }
...
if ( computer.get_text(w, EXE_DIR_EDITBOX) )
    ...
    if ( Ft2ControlAllFileIsOld )
       StringTable.find( s, TEXT("NEW_STRING") );
    else
       s.null();
...
StringTable.find( s, TEXT("OLD_DIR_RENAME") );
s += dest;
...
set_setup_strings( hWnd, rsn, nof_rsn, NULL, 0, NULL );
StringTable.find( s, TEXT("IU_TITLE") );
