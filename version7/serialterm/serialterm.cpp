...
static TCHAR MyIniFileName[]     = TEXT( "serialterm.ini" );
static TCHAR ConfigSection[]     = TEXT( "Config" );
static TCHAR CommPortKey[]       = TEXT( "CommPort" );
...
static TCHAR MyClassName[] = "SerialTerm";
...
bool set_current_comm_port( char * new_address, u_short new_port );
void set_comm_send_event();
...
if ( f.open_for_read(filename) )
    {
    send_comm_message( START_OF_UPLOAD, false );
    send_comm_message( Turn_On_Plusses, false );
    while ( TRUE )
        {
        cp = f.readline();
        if ( !cp )
        ...
        if ( *cp != NullChar )
            ...
            if ( s.len() > 0 )
                {
                s += CrString;
                send_comm_message( s.text(), false );
                }
            }
        }
    send_comm_message( END_OF_UPLOAD, false );
    send_comm_message( Turn_Off_Plusses, false );
...
if ( ini.exists() )
    ...
    if ( ini.find(CommPortKey) )
        CommPort = ini.get_string();
...
ini.put_string( CommPortKey, CommPort );
}
...
/***********************************************************************
*                         SERIAL_SETUP_DIALOG_PROC                     *
***********************************************************************/
BOOL CALLBACK serial_setup_dialog_proc( HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam )
{
int  id;
int  cmd;
...
switch (msg)
    {
    case WM_INITDIALOG:
        set_text( hWnd, COMM_PORT_EBOX, CommPort );
        return FALSE;
    ...
    case WM_COMMAND:
        ...
        if ( cmd == BN_CLICKED )
            ...
            if ( id == IDOK )
                {
                STRING_CLASS s;
                s.get_text( hWnd, COMM_PORT_EBOX );
                copystring( CommPort, s.text() );
                set_window_title();
                set_current_comm_port( CommPort );
                save_comm_port();
                }
...
*       Set IsCyclone if SELECT_7_RADIO is pressed                    *
**********************************************************************/
...
if ( get_dir_from_radio(s, hWnd) )
    {
    t.get_file_list( s.text(), StarDotFlash );
    t.rewind();
    while ( t.next() )
        {
        s = t.text();
...
*                           UPLOAD_ALL_FILES                           *
***********************************************************************/
static void upload_all_files( HWND hWnd )
{
int             i;
int             n;
LISTBOX_CLASS  lb;
...
if ( get_dir_from_radio(root, hWnd) )
    {
    FilesSentToCyclone = n;
    if ( IsCyclone )
        {
        send_comm_message( UnlockCycloneCmd, true );
        }
    else
        {
        /*
        ...
        for ( i=0; i<n; i++ )
            {
            if ( strings_are_equal(V650FileName, lb.item_text(i)) )
                {
                send_comm_message( EraseApexCmd, true );
                break;
                }
            }
        }
...
*                           UPLOAD_ONE_FILE                            *
***********************************************************************/
static void upload_one_file( HWND hWnd )
{
NAME_CLASS filename;
NAME_CLASS s;
        ...
        if ( cmd == BN_CLICKED )
            ...
            if ( id == IDOK )
                ...
                if ( is_checked(hWnd, UPLOAD_ALL_FILES_XBOX) )
                    upload_all_files( hWnd );
                else
                    upload_one_file( hWnd );
                }
...
***********************************************************************/
void upload_all_files_in_flash_dir( TCHAR * sorcdir )
{
static TCHAR file_name_pattern[] = TEXT( "*.flash" );

TEXT_LIST_CLASS t;
STRING_CLASS    s;

t.get_file_list( sorcdir, file_name_pattern );
MainListBox.add( TEXT("Files to Upload...") );

t.rewind();
while ( t.next() )
    MainListBox.add( t.text() );
...
{
static TCHAR filter[] = TEXT( "Flash Files\0*.flash\0" );
...
if ( GetOpenFileName( &fh ) )
    ...
    if ( dir_from_filename(fname) )
        upload_all_files_in_flash_dir( fname );
            ...
            if ( FilesSentToCyclone == 0 )
                ...
                if ( IsCyclone )
                    ...
                    if ( IDYES == MessageBox(hWnd, "Reboot the FasTrak2 Board?", "Upload Complete", MB_YESNO | MB_SYSTEMMODAL) )
                        send_comm_message( RebootCycloneCmd, true );
                    }
                else
                    {
                    DialogBox(
    ...
    case WM_COMMAND:
        switch (id)
            ...
            case SET_ADDRESS_CHOICE:
                DialogBox(
                    MainInstance,
                    TEXT( "SERIAL_SETUP_DIALOG" ),
                    MainWindow.handle(),
                    (DLGPROC) serial_setup_dialog_proc
                     );
                return 0;
