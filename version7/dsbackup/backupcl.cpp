...
last_examined_shot_number = ini.get_int( LastExaminedShotNumber );

profile.last_saved_shot.number = ini.get_int( LastSavedProfileNumber );
profile.last_saved_shot.time   = ini.get_string( LastSavedProfileTime );
profile.number_acquired        = ini.get_int( NumberProfilesAcquired );
profile.acquiring              = ini.get_boolean( AcquiringProfiles );

params.last_saved_shot.number  = ini.get_int( LastSavedParamsNumber );
params.last_saved_shot.time    = ini.get_string( LastSavedParamsTime   );
params.number_acquired         = ini.get_int( NumberParamsAcquired );
...
ini.put_string( LastExaminedShotNumber, int32toasc(last_examined_shot_number) );

ini.put_string(  LastSavedParamsTime,   params.last_saved_shot.time.text() );
ini.put_string(  LastSavedParamsNumber, int32toasc(params.last_saved_shot.number) );
ini.put_int(     NumberParamsAcquired, params.number_acquired );
ini.put_boolean( AcquiringParams, params.acquiring );

ini.put_string(  LastSavedProfileTime,   profile.last_saved_shot.time.text() );
ini.put_int(     LastSavedProfileNumber, profile.last_saved_shot.number );
ini.put_int(     NumberProfilesAcquired, profile.number_acquired );
...
s.get_shotparm_dbname( c.whoami(), name.machine, name.part );
db.create( s);
...
if ( !s.open(tn, GRAPHLST_RECLEN, PFL) )
    return;
...
if ( d.open(tn, GRAPHLST_RECLEN, WL) )
    {
...
--------------------- */
tn.get_shotparm_dbname( name.computer, name.machine, name.part );
if ( !tn.file_exists() )
    return;

if ( !s.open(tn, SHOTPARM_RECLEN, PFL) )
    return;

tn.get_shotparm_dbname( dest.computer, dest.machine, dest.part );
if ( !tn.file_exists() )
    d.create( tn );

if ( d.open(tn, SHOTPARM_RECLEN, WL) )
    {
