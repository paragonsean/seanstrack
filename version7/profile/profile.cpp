...
/*
----------------------------------
Print to clipboard (PrintToClip) variables
---------------------------------- */
static BOOLEAN       HaveNewShotFromFt2    = FALSE;    /* I'm only showing newly acqired shots */
static BOOLEAN       PrintingToClipboard   = FALSE;
static STRING_CLASS  PrintToClipCommand;
static HFONT         PrintToClipFontHandle = INVALID_FONT_HANDLE;
static FONT_CLASS    PrintToClipFont;
static int           PrintToClipFontHeight = 0;
static int           PrintToClipFontWidth  = 0;
REGION_ENTRY         PrintToClipRegion[NOF_REGIONS];
static int           PrintToClipHeight     = 0;
static int           PrintToClipWidth      = 0;
static TCHAR         PrintingToClipboardKey[] = TEXT( "PrintingToClipboard" );
static TCHAR         PrintToClipCommandKey[]  = TEXT( "PrintToClipCommand" );
static TCHAR         PrintToClipHeightKey[]   = TEXT( "PrintToClipHeight" );
static TCHAR         PrintToClipWidthKey[]    = TEXT( "PrintToClipWidth" );

/*
------------------------------------
Print to clipboard profile variables
------------------------------------ */
static int           PtoCProfileHeight;
static int           PtoCProfileWidth;

/*
---------------------------------------------
Print to clipboard corner parameter variables
--------------------------------------------- */
static int             PToCNofParameters = 0;
static int             PtoCParamFontHeight;
static HFONT           PToCParamFontHandle = 0;
static int             PToCParamHeight;
static int             PToCParamWidth;

/*
------------------------------------------
Print to clipboard zabish screen variables
------------------------------------------ */
static RECTANGLE_CLASS PToCHistHeight;;
static RECTANGLE_CLASS PToCHistWidth;;
...
PartName     = p->part;
copystring( ShotName, p->shot.text() );
...
* This used to show a II in the lower left corner if this was an ft2   *
* but that is no longer needed so now I put a "warning" or "alarm"     *
* there if any of the parameters are out.                              *
***********************************************************************/
...
if ( state == ALARM_STATE )
    ...
    {
    old_background_color = GetBkColor( MyDc );
    }
...
DayParamLb.empty();
name.get_shotparm_dbname( ComputerName, MachineName, PartName );
if ( db.open(name, SHOTPARM_RECLEN, PFL) )
    {
    if ( db.goto_first_equal_or_greater_date(SHOTPARM_DATE_OFFSET, st) )
        {
        if ( db.get_rec() )
            {
            while ( true )
                ...
                ------------------------------------ */
                db.get_date( nt, SHOTPARM_DATE_OFFSET );
                if ( nt.wDay != st.wDay )
                    break;
                ...
                db.get_time( nt, SHOTPARM_TIME_OFFSET );
                s = alphatime( nt );
                s += TabChar;

                s += (int) db.get_long( SHOTPARM_SHOT_NUMBER_OFFSET );
                s += TabChar;
                ...
                for ( j=0; j<NofCornerParameters; j++ )
                    ...
                    if ( i > 0 && i <= MAX_PARMS )
                        {
                        i--;
                        s += db.get_double( SHOTPARM_PARAM_1_OFFSET + i*(SHOTPARM_FLOAT_LEN+1) );
                        }
                    if ( j < NofCornerParameters-1 )
                        s += TabChar;
                    }
                DayParamLb.add( s.text() );
                if ( !db.get_next_record(NO_LOCK) )
                    break;
                }
            }
        }
...
/***********************************************************************
*                          PAINT_THE_PROFILE                           *
*  This paints the profile on the screen. It works for the normal      *
*  screen and the memory bitmap for the copy_to_clipboard.             *
***********************************************************************/
static void paint_the_profile()
{
int i;
...
if ( viewing_reference() )
    ...
    if ( load_reference_trace() )
        {
        if ( viewing_reference() )   //Lawyer Code, check viewing_ref again for an old ref trace
            {
...
REGION_ENTRY current_region[NOF_REGIONS];

/*
-----------------------------------------
Make a copy of the current screen globals
----------------------------------------- */
current_screen_rect = PlotWindowRect;
for ( i=0; i<NOF_REGIONS; i++ )
    current_region[i] = Region[i];

/*
------------------------
Create the memory bitmap
------------------------ */
screen_dc = GetDC( MainPlotWindow );
memory_dc = CreateCompatibleDC( screen_dc );

hBitmap = CreateCompatibleBitmap( screen_dc, PrintToClipWidth, PrintToClipHeight );

/*
--------------------------------------
If I couldn't ceate the bitmap go home
-------------------------------------- */
if ( hBitmap == NULL )
    return;
...
/*
------------------------------
See if I have to create a font
------------------------------ */
if( PrintToClipFontHandle == INVALID_FONT_HANDLE )
    {
    if ( PrintToClipHeight < 1000 )
        lines_per_page = 25L;
    else
        lines_per_page = 35L;
    PrintToClipFont          = StringTable.fontclass();
    PrintToClipFont.lfHeight = PrintToClipHeight / lines_per_page;
    PrintToClipFont.lfWidth  = 0;
    PrintToClipFontHandle    = FontList.get_handle( PrintToClipFont );
    /*
    ---------------------------------------------------------------------
    Zero the font sizes so I will set them once I have selected this font
    --------------------------------------------------------------------- */
    PrintToClipFontHeight    = 0;
    PrintToClipFontWidth     = 0;
    }

if ( PrintToClipFontHandle != INVALID_FONT_HANDLE )
    SelectObject( memory_dc, PrintToClipFontHandle );

/*
-------------------------------------------------------------
Get the character height and width if I just created the font
------------------------------------------------------------- */
if ( PrintToClipFontHeight == 0 )
    {
    GetTextMetrics( memory_dc, &tm );
    PrintToClipFontHeight = tm.tmHeight - tm.tmInternalLeading;
    PrintToClipFontWidth  = tm.tmAveCharWidth;
    }
...
r.left   = 0;
r.top    = 0;
r.right  = PrintToClipWidth;
r.bottom = PrintToClipHeight;

size_profile_window( r );
paint_the_profile();

MyDc = NULL;
SelectObject( memory_dc, GetStockObject(SYSTEM_FIXED_FONT) ) ;

DeleteDC( memory_dc );
ReleaseDC(MainPlotWindow, screen_dc );

/*
--------------------------
Restore the screen globals
-------------------------- */
PlotWindowRect = current_screen_rect;
for ( i=0; i<NOF_REGIONS; i++ )
    Region[i] = current_region[i];

if (hBitmap)
    {
    OpenClipboard( MainPlotWindow );
    EmptyClipboard();
    SetClipboardData( CF_BITMAP, hBitmap );
    CloseClipboard();
    }

if ( !PrintToClipCommand.isempty() )
    execute( PrintToClipCommand.text() );
...
EndPaint( MainPlotWindow, &ps );
if ( HaveNewShotFromFt2 && PrintingToClipboard )
    {
    HaveNewShotFromFt2 = FALSE;
    print_to_clipboard();
    }
set_realtime_menu_string();
}
...
line.init( ComputerName.len() + MachineName.len() + PartName.len() + SHOT_LEN + 3 );
line = ComputerName;
line += CommaChar;
line += MachineName;
line += CommaChar;
...
/***********************************************************************
*                       KILL_PRINT_TO_CLIP_FONTS                       *
***********************************************************************/
void kill_print_to_clip_fonts()
{
if ( PrintToClipFontHandle != INVALID_FONT_HANDLE )
    FontList.free_handle( PrintToClipFontHandle );

PrintToClipFontHandle = INVALID_FONT_HANDLE;
PrintToClipFont.lfFaceName[0] = NullChar;
PrintToClipFontHeight = 0;
PrintToClipFontWidth  = 0;

if ( PToCParamFontHandle != INVALID_FONT_HANDLE )
    FontList.free_handle( PToCParamFontHandle );

PToCParamFontHandle = INVALID_FONT_HANDLE;
}

/***********************************************************************
*                      STOP_PRINTING_TO_CLIPBOARD                      *
***********************************************************************/
void stop_printing_to_clipboard()
{
PrintToClipHeight = 0;
PrintToClipWidth  = 0;
PrintingToClipboard = FALSE;
HaveNewShotFromFt2  = FALSE;
kill_print_to_clip_fonts();
}

/***********************************************************************
*                    CHECK_PRINT_TO_CLIPBOARD_SETUP                    *
***********************************************************************/
void check_print_to_clipboard_setup()
{
INI_CLASS ini;
NAME_CLASS s;
BOOLEAN    b;
...
b = ini.get_boolean( PrintingToClipboardKey );
if ( b != PrintingToClipboard )
    {
    PrintingToClipboard = b;
    if ( !PrintingToClipboard )
        {
        stop_printing_to_clipboard();
        return;
        }
...
if ( PrintingToClipboard )
    {
    new_height = ini.get_int( PrintToClipHeightKey );
    new_width  = ini.get_int( PrintToClipWidthKey );
    if ( new_height != PrintToClipHeight || new_width != PrintToClipWidth || NofCornerParameters != PToCNofParameters )
        {
        PrintToClipHeight = new_height;
        PrintToClipWidth  = new_width;
        PToCNofParameters = NofCornerParameters;
        kill_print_to_clip_fonts();
        }
    }

if ( ini.find(PrintToClipCommandKey) )
    PrintToClipCommand = ini.get_string();
else
    PrintToClipCommand.null();
}
...
lstrcpyn( ShotName, ini.get_string(), SHOT_LEN+1 );
return TRUE;
}
...
shot_number = p.get_last_shot_number( ComputerName, MachineName, PartName );
