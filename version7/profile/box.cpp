...
if ( param.find(current.computer, current.machine, current.part) )
    {
    s.get_profile_name( current.computer, current.machine, current.part, shot_number );
    if ( is_ftii )
        n = read_parameters_from_parmlist( value, s );
    else
        n = read_parameters_from_profile( value, s );
    for ( i=0; i<n; i++ )
        {
        p = &param.parameter[i];
...
if ( p.get() )
    {
    shot_number = p[PLOOKUP_LAST_SHOT_INDEX].int_value();
    cp = date_string;
...
for ( i=0; i<NofStatusBoxes; i++ )
    ...
    if ( PtInRect(StatusBoxes[i].bounds(), p) )
        ...
        copystring( ShotName, int32toasc(StatusBoxes[i].shot_number) );
