...
if ( ParamArray )
    {
    pa = ParamArray;
    s.get_shotparm_dbname( ComputerName, MachineName, PartName );
...
for ( i=0; i<n; i++ )
    ...
    if ( !CurrentMachine.is_ftii )
        {
        s.get_profile_name( ComputerName, MachineName, PartName, ShotNumberArray[i] );
        read_parameters_from_profile( parameter, s );
        }
