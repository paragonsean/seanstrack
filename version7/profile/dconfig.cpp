...
/*
--------------------------------------------------------------
Disable the entries for channels 5 to 8 if this is a FasTrak 1
-------------------------------------------------------------- */
...
xbox     = FIX_FTCHAN_5_XBOX;
min_ebox = MIN_FTCHAN_5_EBOX;
max_ebox = MAX_FTCHAN_5_EBOX;
...
for ( i=0; i<NofCurveTypesToCheck; i++ )
    ...
    if ( i == (NofCurveTypesToCheck-1) )
        {
        s = StringTable.find( TEXT("DIF_PRES_STRING") );
        }
    else if ( channel == CurrentPart.head_pressure_channel )
        {
        s = StringTable.find( TEXT("HEAD_PRES_STRING") );
        if ( pressure_type & HEAD_PRESSURE_CURVE )
            pressure_type |= CurveTypesToCheck[i];
        }
    else if ( channel == CurrentPart.rod_pressure_channel )
        {
        s = StringTable.find( TEXT("ROD_PRES_STRING") );
    ...
    set_text( w, s );
    id++;
    if ( !CurrentMachine.is_ftii && i >= MAX_CONTINUOUS_CHANNELS && i < (NofCurveTypesToCheck-1) )
        {
