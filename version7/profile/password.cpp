...
if ( s.get_exe_dir_file_name(helpfile) )
    WinHelp( EnterWindow, s.text(), helptype, context );
}
...
ini.put_boolean( HaveMultipartRunlistKey, is_checked( w, MULTIPART_RUNTIME_XBOX) );
...
ini.put_string( EditControlButtonKey, s.text() );
...
ini.put_string( Ioex4GrayLevelKey, s.text() );

s.get_text( w, READ_CURRENT_ANALOG_EBOX );
ini.put_string( CurrentFtValuesGrayLevelKey, s.text() );
...
ini.put_string( Ioex4MenuGrayLevelKey, s.text() );
...
ini.put_string( StExitLevelKey, s.text() );
...
for ( i=0; i<MAX_MULTIPART_RUNLIST_WIRES; i++ )
    ...
    s.get_text( w, id );
    ini.put_string( keyname.text(), s.text() );
    id++;
    }
