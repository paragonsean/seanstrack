...
if ( p.get() )
    {
    shotnumber = p.get_last_shot_number();
    p.get_time( shottime );
    }
}
...
s.get_profile_name( pn.computer, pn.machine, pn.part, shotnumber.int_value() );
nof_parms = read_parameters_from_parmlist( pvalue, s );
}
        ...
        for ( col=0; col<NofColumns; col++ )
            ...
            if ( p.x >= r.left && p.x <= r.right )
                ...
                if ( Column[col].type >= DOWNTIME_STATE_TYPE )
                    ...
                    copystring( ShotName, Row[i].shotnumber.text() );
