...
static NAME_CLASS    PrintToClipDestFile;
...
HDC        memory_dc;
//TEXTMETRIC tm;
RECTANGLE_CLASS rmain;
RECTANGLE_CLASS rparam;
RECTANGLE_CLASS rhistory;
...
/*
--------------------------------------------------------------------------------
Get the size of the region from the bottom of the menu to the top of the taskbar
r = my_desktop_client_rect();
r.top = MainMenu.mr.bottom;
-------------------------------------------------------------------------------- */
GetClientRect( MainPlotWindow,     &rmain );
GetClientRect( ParameterWindow,    &rparam );
if ( MyScreenType == ZABISH_SCREEN_TYPE )
    GetClientRect( ParamHistoryWindow, &rhistory );

height = rparam.height();
if ( MyScreenType == ZABISH_SCREEN_TYPE )
    maxint( height, (int) rhistory.height() );
...
//StretchBlt( memory_dc, 0, 0, width, height, screen_dc, r.left, r.top, width, height, SRCCOPY );
x = 0;
y = 0;
sorc_dc = GetDC( MainPlotWindow );
BitBlt( memory_dc, x, y, (int) rmain.width(), (int) rmain.height(), sorc_dc, 0, 0, SRCCOPY );
ReleaseDC( MainPlotWindow, sorc_dc );

y = rmain.height();
sorc_dc = GetDC( ParameterWindow );
BitBlt( memory_dc, x, y, (int) rparam.width(), (int) rparam.height(), sorc_dc, 0, 0, SRCCOPY );
ReleaseDC( ParameterWindow, sorc_dc );

if ( MyScreenType == ZABISH_SCREEN_TYPE )
    {
    x = rparam.right;
    width = rmain.right - x;
    sorc_dc = GetDC( ParamHistoryWindow );
    BitBlt( memory_dc, x, y, width, rhistory.height(), sorc_dc, 0, 0, SRCCOPY );
    ReleaseDC( ParamHistoryWindow, sorc_dc );
    }
/*
------------------------------
...
//"c:\program files\irfanview\i_view32.exe" /clippaste/convert=c:\bozo.jpg
}
...
/***********************************************************************
*                       KILL_PRINT_TO_CLIP_FONT                        *
void kill_print_to_clip_font()
{
if ( PrintToClipFontHandle != INVALID_FONT_HANDLE )
