...
void start_suretrak_monitor_thread();
...
***********************************************************************/
static void update_main_screen_limit_switch_names()
{
HWND w;
int  i;
...
for ( i=0; i<4; i++ )
    LimitSwitch.desc(i).set_text( w, LS1_RADIO+i );
}

/***********************************************************************
*                  UPDATE_MAIN_SCREEN_DISTANCE_UNITS                   *
***********************************************************************/
static void update_main_screen_distance_units()
{
HWND w;
...
for ( i=0; i<4; i++ )
    LimitSwitch.desc(i).set_text( w, i+LS1_DESCRIPTION_EDITBOX );
set_text( w, MAX_VELOCITY_EDITBOX, ascii_float(SureTrakParameters.max_velocity()) );

CheckRadioButton( w, VACUUM_1_RADIO, VACUUM_4_RADIO, VACUUM_1_RADIO + SureTrakSetup.vacuum_limit_switch_number() );
}
...
for ( i=0; i<4; i++ )
    LimitSwitch.desc(i).get_text( w, i+LS1_DESCRIPTION_EDITBOX );
...
SureTrakParameters.save( SureTrakPart.computer, SureTrakPart.machine );
}
...
***********************************************************************/
static void send_vel_dividend_to_board()
{
STRING_CLASS s;
...
***********************************************************************/
static void send_zero_speed_check_pos()
{
BOARD_DATA x;
...
***********************************************************************/
static void send_counts_per_unit()
{
BOARD_DATA x;
...
***********************************************************************/
static void download_parameters_to_board()
{
BOARD_DATA x;
BOARD_DATA gain;
...
send_parameter_to_board( SET_V_LOOP_GAIN,   (BOARD_DATA) SureTrakSetup.velocity_loop_gain_value() );
...
for ( x=0; x<NOF_GAIN_BREAKS; x++ )
    {
    gain     = (BOARD_DATA) (SureTrakSetup.gain_break_gain_value(x) * 1000.0);
    velocity = (BOARD_DATA) (SureTrakSetup.gain_break_velocity_value(x) * 100.0 );
    ...
    case WM_COMMAND:
        switch ( id )
            ...
            case CANCEL_CHANGES_BUTTON:
                SureTrakParameters.find( SureTrakPart.computer, SureTrakPart.machine );
                fill_parameters();
...
*                         WRITE_MASTER_TRACE                           *
***********************************************************************/
void write_master_trace()
{
int32      i;
NAME_CLASS s;
FILE_CLASS f;

s.get_valve_test_master_file_name( SureTrakPart.computer, SureTrakPart.machine );
...
s.get_valve_test_master_file_name( SureTrakPart.computer, SureTrakPart.machine );
if ( !s.file_exists() )
    s.get_valve_test_master_file_name( MyComputer.whoami() );
...
***********************************************************************/
static void get_new_valve_test_data()
{
...
***********************************************************************/
void position_current_analog_window()
{
RECT r;
long x;
long y;
...
s.get_control_program_file_name( MyComputer.whoami() );
save_edit_control_to_file( s.text(), w, CONTROL_PROGRAM_EBOX );

s.get_control_message_file_name( MyComputer.whoami() );
save_edit_control_to_file( s.text(), w, CONTROL_MESSAGE_EBOX );
SureTrakBlockMessage.load( s.text() );
}
...
NAME_CLASS s;
s.get_control_program_file_name( MyComputer.whoami() );
load_edit_control_from_file( w, CONTROL_PROGRAM_EBOX, s.text() );
s.get_control_message_file_name( MyComputer.whoami() );
load_edit_control_from_file( w, CONTROL_MESSAGE_EBOX, s.text() );
}
...
***********************************************************************/
static void set_current_parameters_tab_stops()
{
int tabs[1];
int x;
...
have_step = FALSE;
fn.get_control_program_file_name( MyComputer.whoami() );
...
NAME_CLASS control_file_name;
NAME_CLASS temp_file_name;
int32      this_step;
...
temp_file_name.get_temp_control_program_file_name( MyComputer.whoami() );
control_file_name.get_control_program_file_name( MyComputer.whoami() );

if ( !temp_file_name.isempty() && !control_file_name.isempty() )
    {
    if ( dest.open_for_write(temp_file_name.text()) )
        {
        if ( sorc.open_for_read(control_file_name.text()) )
            {
            while ( TRUE )
                {
                cp = sorc.readline();
...
if ( have_tempfile )
    {
    control_file_name.delete_file();
    temp_file_name.moveto( control_file_name );
    }
...
*                         CLOSE_SERVICE_TOOLS                          *
***********************************************************************/
static void close_service_tools()
{
HWND w;
...
*                         SHOW_PROGRAM_STATUS                          *
***********************************************************************/
static void show_program_status()
{
UINT       id;
...
***********************************************************************/
void download_all_setups_to_board()
{
download_parameters_to_board();
...
if ( HaveRunlist )
    ...
    for ( i=0; i<Runlist.count(); i++ )
        ...
        if ( !part_exists( SureTrakPart.computer, SureTrakPart.machine, Runlist[i].partname) )
            {
            MessageBox( 0, resource_string(NO_WARMUP_PART_STRING), Runlist[i].partname.text(), MB_OK | MB_SYSTEMMODAL );
            }
        }
    }

if ( !SureTrakParameters.find(SureTrakPart.computer, SureTrakPart.machine) )
...
LimitSwitch.get( SureTrakPart.computer, SureTrakPart.machine, SureTrakPart.part );
...
if ( get_dde_data(machine_name, edata) )
    {
    machine_name.strip_crlf();
    if ( machine_name == SureTrakPart.machine )
        reload_suretrak();
    }
}
...
------------------------------------------------------------------------------- */
part_name.acquire( (TCHAR *) lParam );

set_current_part_name( SureTrakPart.computer, SureTrakPart.machine, part_name );

poke_data( DDE_MONITOR_TOPIC, ItemList[MONITOR_SETUP_INDEX].name, SureTrakPart.machine.text() );
}
...
MyComputer.startup();
SureTrakPart.computer = MyComputer.whoami();
s.get_control_message_file_name( MyComputer.whoami() );
SureTrakBlockMessage.load( s.text() );
...
if ( HaveRunlist )
    ...
    if ( Runlist.count() > 1 )
        {
        if ( Runlist[0].partname != SureTrakPart.part )
