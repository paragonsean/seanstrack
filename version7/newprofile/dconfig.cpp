...
for ( i=0; i<NofCurveTypesToCheck; i++ )
    ...
    if ( is_checked(MyWindowHandle, button) )
        ...
        if ( i == (int)CurrentPart.head_pressure_channel )
            dest |= HEAD_PRESSURE_CURVE;

        if ( i == (int)CurrentPart.rod_pressure_channel )
...
if ( IsPressureDisplay )
    ...
    while ( axis_index< NOF_AXES && fixed_index < NofCurveTypesToCheck )
        {
