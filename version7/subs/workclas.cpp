...
s.get_data_dir_file_name( OPERATOR_DB );
t.open( s, OPERATOR_RECLEN, PFL );
...
*                              FULL_NAME                               *
***********************************************************************/
TCHAR * WORKER_LIST_CLASS::full_name()
{
static TCHAR s[OPERATOR_FIRST_NAME_LEN + 1 + OPERATOR_LAST_NAME_LEN + 1];
TCHAR * cp;
...
if ( current_entry )
    {
    cp = copystring( s, current_entry->firstname );
    *cp++ = SpaceChar;
    copystring( cp, current_entry->lastname );
    }
else
    {
    lstrcpy( s, UnknownWorker );
