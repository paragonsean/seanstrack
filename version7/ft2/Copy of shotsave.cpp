...
/*
---------------------------------------------------------
See if I will be writing to a file instead of a com port.
--------------------------------------------------------- */
...
if ( bytes_to_write )
    {
    if ( EosOutputFile )
        write_bytes_to_output_file( buf, bytes_to_write );
    else
        write_bytes_to_serial_port( buf, bytes_to_write );
    }
...
*                            KILL_OUTPUT_FILE                          *
***********************************************************************/
static void kill_output_file()
{
NAME_CLASS s;
if ( EosOutputFile && EosKillOutputFile )
    {
    s = EosOutputFile;
    s.delete_file();
    }
...
***********************************************************************/
static void write_eos_param_file( SHOT_BUFFER_ENTRY * b )
{
...
if ( p.find(b->pn.computer, b->pn.machine, b->pn.part) )
    {
        ...
        for ( i=0; i<n; i++ )
            ...
            while ( tag.get_substring( s, LeftCurlyBracket, RightCurlyBracket ) )
                ...
                if ( tag == param_number_tag )
                    ...
                    {
                    s.replace( tag.text(), p.parameter[i].name );
                    }
                    ...
                    {
                    s.replace( tag.text(), units_name(p.parameter[i].units) );
                    }
                }
            f.writeline( s );
            }
...
***********************************************************************/
static void write_serial_port_info( SHOT_BUFFER_ENTRY * b )
{
...
/*
--------------------------------------
Delete the stamp.txt file if it exists
-------------------------------------- */
if ( EosOutputFile )
    kill_output_file();
...
if ( EosPyramidLines == OneChar )
    ...
    *bp = NullChar;
    ...
    if ( EosNeedsStxEtx )
        {
        if ( EosOutputFile )
            write_bytes_to_output_file( StxBuf, 1 );
        else
...
if ( EosNeedsStxEtx )
    {
    if ( EosOutputFile )
        write_bytes_to_output_file( EtxBuf, 1 );
    else
