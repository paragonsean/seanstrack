...
* it is 0;                                                             *
***********************************************************************/
short first_differential_curve_point()
{
...
***********************************************************************/
static void fill_differential_curve()
{
short i;
float ch;
float cr;
...
for ( channel=1; channel<=MAX_CONTINUOUS_CHANNELS; channel++ )
    ...
    si = channel - 1;
    ai = me->analog_sensor_index[si];
    AnalogSensor.find( ai );
    Ph.curves[pi].units = AnalogSensor.units_index();
    p  = Raw;
    pp = Points[CHAN_1_ANALOG_CURVE_INDEX+si];
    for ( i=1; i<NofPoints; i++ )
        {
        pp++;
        *pp = (float) AnalogSensor.converted_value( (unsigned) *p );
        if ( i == 1 )
            {
            Ph.curves[pi].max = *pp;
            Ph.curves[pi].min = *pp;
...
***********************************************************************/
static MONITOR_ENTRY * find_existing_monitor_entry( STRING_CLASS & machine_to_find )
{
MONITOR_ENTRY * m;
...
while ( TRUE )
    ...
    if ( m->machine.name ==  machine_to_find )
        break;
    }
...
*                           LOAD_MONITOR_ENTRY                         *
***********************************************************************/
static BOOLEAN load_monitor_entry( MONITOR_ENTRY * m, STRING_CLASS & machine_to_find, STRING_CLASS & part_to_find )
{
...
***********************************************************************/
BOOLEAN find_current_monitor( STRING_CLASS & machine_to_find, STRING_CLASS & part_to_find )
{
...
*                          TRUE_EOS_POSITION                           *
***********************************************************************/
static double true_eos_position()
{
int    i1;
int    i2;
double v;
...
***********************************************************************/
static void read_internal_parameters()
{
int       status;
short     channel;
short     csfs_index;
...
***********************************************************************/
static void read_ftanalog_parameters()
{
short      i;
short      j;
short      n;
...
***********************************************************************/
static void read_ftonoff_parameters()
{
short fi;
short p;
MONITOR_ENTRY * m;
...
if ( c >= 0 && c < MAX_MUX_CHANNELS )
    ...
    ----------------------------- */
    copystring( Ph.machine_name, me->machine.text(), OLD_MACHINE_NAME_LEN );
    copystring( Ph.part_name,    me->part.text(), OLD_PART_NAME_LEN );
    ...
    add_to_shotsave( me->machine, me->part, Ph, Points, Parameter, CurrentMonitor->param.count(), HasAlarm );
