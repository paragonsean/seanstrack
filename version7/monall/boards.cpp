...
status = FALSE;
s.get_machset_csvname( ComputerName, mux.machine );
if ( sp.get(s) )
    {
    copystring( rodpitch, sp[MACHSET_RODPITCH_INDEX].value.text() );
    status = TRUE;
    }
...
if ( status )
    {
    status = FALSE;
    s.get_partset_csvname( ComputerName, mux.machine, mux.part );
    if ( sp.get(s) )
        {
        mux.dist_units = sp[PARTSET_DISTANCE_UNITS_INDEX].int_value();
        mux.vel_units  = sp[PARTSET_VELOCITY_UNITS_INDEX].int_value();
        status = TRUE;
        }
    }
...
if ( status )
    ...
    for ( c=0; c<MAX_FT_CHANNELS; c++ )
        mux.analog_sensor_index[c] = DEFAULT_ANALOG_SENSOR;

    /*
    --------------------------------------------------------------
    The list of channels is in the FTCHAN db in the part directory
    -------------------------------------------------------------- */
    s.get_ftchan_dbname( ComputerName, mux.machine, mux.part );
    if ( t.open(s, FTCHAN_RECLEN, PFL) )
        {
        t.reset_search_mode();
        while ( t.get_next_record(NO_LOCK) )
            {
            c = t.get_long( FTCHAN_CHANNEL_NUMBER_OFFSET );
            c--;
            if ( c>=0 && c<MAX_FT_CHANNELS )
                mux.analog_sensor_index[c] = t.get_long( FTCHAN_SENSOR_NUMBER_OFFSET );
            }
        t.close();
        }
    }
...
***********************************************************************/
BOOLEAN BOARD_CLASS::set_pos_sensor_pointer()
{
short c;
...
if ( firmware_version >= V4_FIRMWARE_VERSION )
    ...
    maxdouble( bid.ms_per_time_based_sample, MIN_MS_PER_TIME_SAMPLE );
...
for ( i=0; i<n; i++ )
    {
    gain = 1;
    ai = b->mux[x].analog_sensor_index[i];
    if ( AnalogSensor[ai].max_volts_level != TEN_MAX_VOLTS )
        gain = 2;
    ft_set_gain( i+1, gain );
    }
...
***********************************************************************/
BOOLEAN BOARD_CLASS::init_one_board()
{
...
***********************************************************************/
BOOLEAN BOARD_CLASS::init_all_boards()
{
int i;
BOOLEAN status;
...
*                             LOAD_BOARDS                              *
***********************************************************************/
static void load_boards( STRING_CLASS & computer_name )
{
NAME_CLASS s;
DB_TABLE   t;
short c;
...
for ( i=0; i<MAX_BOARDS; i++ )
    ...
    for ( x=0; x<MAX_MUX_CHANNELS; x++ )
        ...
        for ( c=0; c<MAX_FT_CHANNELS; c++ )
            Board[i].mux[x].analog_sensor_index[c] = DEFAULT_ANALOG_SENSOR;
        }
    }
...
*                          BOARD_CLASS::BOARD                          *
***********************************************************************/
HARDWARE_BOARD_ENTRY * BOARD_CLASS::board()
{
if ( currentboard >= 0 && currentboard < MAX_BOARDS )
...
***********************************************************************/
MUX_ENTRY * BOARD_CLASS::current_mux_entry()
{
short                  c;
MUX_ENTRY            * me;
...
***********************************************************************/
BOOLEAN BOARD_CLASS::monitoring()
{
if ( currentboard >= 0 && currentboard < MAX_BOARDS )
...
*                         CURRENT_MUX_CHANNEL                          *
***********************************************************************/
short BOARD_CLASS::current_mux_channel()
{
if ( currentboard >= 0 && currentboard < MAX_BOARDS )
...
***********************************************************************/
short BOARD_CLASS::read_current_mux_channel()
{
short c;
...
***********************************************************************/
void BOARD_CLASS::set_port_addresses()
{
...
*                       BOARD_CLASS::ADDRESS                           *
***********************************************************************/
BOARD_DATA BOARD_CLASS::address()
{
if ( currentboard >= 0 && currentboard < MAX_BOARDS )
...
***********************************************************************/
void BOARD_CLASS::set_machine( short mux_channel, STRING_CLASS & machine_name, STRING_CLASS & part_name, long new_cycle_timeout_seconds )
{
HARDWARE_BOARD_ENTRY * b;
DB_TABLE    t;
MUX_ENTRY * me;
...
*                       BOARD_CLASS::SET_MACHINE                       *
***********************************************************************/
void BOARD_CLASS::set_machine( short mux_channel, STRING_CLASS & machine_name, STRING_CLASS & part_name )
{
...
***********************************************************************/
BOOLEAN BOARD_CLASS::command_mode()
{
set_port_addresses();
...
***********************************************************************/
BOOLEAN BOARD_CLASS::data_collection_mode()
{
bool need_scan_mode;
short err;
...
***********************************************************************/
BOOLEAN BOARD_CLASS::start_all_boards()
{
short i;
...
***********************************************************************/
BOOLEAN BOARD_CLASS::start_one_board()
{
...
***********************************************************************/
BOOLEAN BOARD_CLASS::stop_all_boards()
{
short i;
...
*                                                                      *
***********************************************************************/
BOOLEAN BOARD_CLASS::locate( short & board_number, short & channel_number, STRING_CLASS & machine_to_locate )
{
short i;
short j;
...
for ( i=0; i<MAX_BOARDS; i++ )
    for ( j=0; j<MAX_MUX_CHANNELS; j++ )
        if ( Board[i].mux[j].machine == machine_to_locate )
            {
            board_number   = i+1;
            channel_number = j+1;
            return TRUE;
...
*                               NEXT_MACHINE                           *
***********************************************************************/
STRING_CLASS & BOARD_CLASS::next_machine()
{
short i;
short c;
...
while ( TRUE )
    ...
    if ( !Board[i].mux[c].machine.isempty() )
        {
        machine_list_board = i;
        machine_list_mux   = c;
        return Board[i].mux[c].machine;
        }
    else
        c++;
    }

return NoPart.machine;
}
