...
slen = b->machine.len() + 1 + b->part.len() + 1 + SHOT_LEN + 1 + ALPHADATE_LEN + 1 + ALPHATIME_LEN + 1 + MAX_PARMS*(SHOTPARM_FLOAT_LEN + 1);
...
if ( ShowMachine )
    cp = copy_w_char( cp, b->machine.text(), DelimitChar );

if ( ShowPart )
    cp = copy_w_char( cp, b->part.text(),    DelimitChar );
...
slen = 1 + b->machine.len() + 1 + b->part.len() + 1 + MAX_INTEGER_LEN + ALPHADATE_LEN + 1 + ALPHATIME_LEN + (MAX_PARMS*MAX_FLOAT_LEN) + MAX_PARMS +1;
    ...
    *bp = NullChar;
    ...
    while ( TRUE )
        ...
        if ( is_numeric(*cp) )
            ...
            {
            lstrcpy( bp, b->machine.text() );
            }
        else if ( *cp == PChar )
            {
            lstrcpy( bp, b->part.text() );
            }
        else if ( *cp == SChar )
            {
            n = 0;
...
if ( HaveEosUdp )
    {
    if ( get_eos_udp_address(sa, ComputerName, b->machine) )
...
nextshot( b );
save_flags = skip_new_shot_notify( b->machine, b->ph.time_of_shot);
...
write_parameters( b, save_flags );
write_profile( b, save_flags );

slen = b->machine.len() + 1 + b->part.len() + 1;
buf = new TCHAR[slen];
if ( buf )
    {
    cp = copy_w_char( buf, b->machine.text(), CommaChar );
    lstrcpy( cp, b->part.text() );
...
if ( i_to_save != NO_INDEX )
    {
    b = BufPtr[i_to_save];
    if ( get_shotsave_semaphore(ComputerName, b->machine) )
        {
        save_shot( b );
