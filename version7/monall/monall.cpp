...
bytes_copied = DdeGetData( edata, NULL, 0, 0 );
if ( bytes_copied == 0 )
    return;
...
bytes_copied *= sizeof( TCHAR );
bytes_copied = DdeGetData( edata, (LPBYTE) NewSetupMachine.text(), bytes_copied, 0 );
if ( bytes_copied > 0 )
    {
    cp = NewSetupMachine.text() + n;
    *cp = NullChar;
    fix_dde_name( NewSetupMachine.text() );
...
***********************************************************************/
static void send_data_server_shot_notification( STRING_CLASS & mach_part, TCHAR * shot )
{
static TCHAR middle[] = TEXT( ",NS," );
...
poke_data( DDE_MONITOR_TOPIC, ItemList[DS_NOTIFY_INDEX].name, s.release() );
}
...
/***********************************************************************
*                              DO_NEWSHOT                              *
* The WM_NEWSHOT message has an allocated string passed in lParam.     *
* I am responsible for deleting this buffer so I let s acquire it.     *
* s will then delete it when the destructor is called                  *
***********************************************************************/
void do_newshot(int shot_number, LPARAM lParam )
{
STRING_CLASS s;
TCHAR      * cp;
...
--------------------------------------- */
int32toasc( shot, shot_number, DECIMAL_RADIX );

send_data_server_shot_notification( s, shot );
...
poke_data( s.text(), ItemList[SHOT_NUMBER_INDEX].name, shot );
}
...
switch (msg)
    ...
    case WM_NEWSETUP:
        MachinesToReset.push( NewSetupMachine.text() );
        return 0;
...
{
if ( free_megabytes(exe_directory()) < MIN_FREE_DISK_MB )
