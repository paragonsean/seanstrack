...
pn.computer.get_text( Ioex4Window.handle(), IOEX4_COMPUTER_TBOX );
pn.machine.get_text(  Ioex4Window.handle(), IOEX4_MACHINE_TBOX  );
pn.part.get_text(     Ioex4Window.handle(), IOEX4_PART_TBOX     );
...
if ( p.find(pn.computer, pn.machine, pn.part) )
    ...
    if ( have_changes )
        {
        poke_data( DDE_MONITOR_TOPIC, ItemList[MONITOR_SETUP_INDEX].name, pn.machine.text() );
...
switch (msg)
    ...
    case WM_INITDIALOG:
        ...
        ComputerName.set_text( hWnd, IOEX4_COMPUTER_TBOX );
        init_total_stroke_globals();
