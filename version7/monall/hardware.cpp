...
static STRING_CLASS AutoShotMachineName;
...
void reset_machine_for_getdata( STRING_CLASS & machine_to_find );
...
*                         GET_EOS_UDP_ADDRESS                          *
***********************************************************************/
BOOLEAN get_eos_udp_address( sockaddr_in & dest, STRING_CLASS & computer, STRING_CLASS & machine )
{
NAME_CLASS s;
FILE_CLASS f;
BOOLEAN    status;
...
s.get_machine_dir_file_name( computer, machine, EOS_UDP_ADDRESS_FILE );
if ( s.file_exists() )
    {
...
***********************************************************************/
static void show_command_mode_message()
{
...
***********************************************************************/
static void show_mux_only_message()
{
...
*                                                                      *
***********************************************************************/
BOOLEAN load_one_machine( HMACHINE_ENTRY & m, STRING_CLASS & machine )
{
NAME_CLASS     s;
SETPOINT_CLASS sp;
DB_TABLE       t;

s.get_machset_csvname( ComputerName, machine );
if ( sp.get(s) )
    {
    m.name = machine;
    m.monitor_flags = (BITSETYPE) sp[MACHSET_MONITOR_FLAGS_INDEX].uint_value();
    m.board_number  = (short) sp[MACHSET_FT_BOARD_NO_INDEX].int_value();
    m.part          = sp[MACHSET_CURRENT_PART_INDEX].value;
    m.mux_channel   = (short) sp[MACHSET_MUX_CHAN_INDEX].int_value();
    m.downtime_timeout_seconds = sp[MACHSET_CYCLE_TIMEOUT_INDEX].int_value();
    ...
    if ( m.monitor_flags & MA_MONITORING_ON )
        {
        s.get_semaphor_dbname( ComputerName, machine );
...
if ( b )
    {
    AutoShotMachineName = b->mux[c].machine;
    if ( AutoShotDisplayWindow )
        {
        SetDlgItemText(AutoShotDisplayWindow, AUTO_SHOT_MACHINE_NUMBER_TEXTBOX, AutoShotMachineName.text() );
...
if ( b )
    {
    if ( AutoShotMachineName == b->mux[c].machine )
        {
        AutoShotMachineName = EmptyString;
        if ( AutoShotDisplayWindow )
            {
...
*                            RESET_MACHINE                             *
***********************************************************************/
void reset_machine( STRING_CLASS & machine_name )
{
...
have_machine = load_one_machine( m, machine_name );
if ( have_machine )
    {
    if ( b.locate(current_board_number, current_mux_channel, machine_name) )
        ...
        if ( current_board_number != m.board_number || current_mux_channel != m.mux_channel || !(m.monitor_flags & MA_MONITORING_ON) )
            ...
            if ( b.find(current_board_number) )
                {
                b.command_mode();
                b.set_machine( current_mux_channel, NoPart.machine, NoPart.part );
                b.init_one_board();
                b.start_one_board();
                }
            }
...
***********************************************************************/
static void check_for_machines_to_reset()
{
STRING_CLASS mymachine;

while ( TRUE )
    {
    mymachine = MachinesToReset.pop();
    if ( mymachine.isempty() )
        break;

    reset_machine( mymachine );
    }
}
...
n = machine.len() + BOARD_DATA_HEX_LEN + BOARD_DATA_HEX_LEN + 3;
s.init( n );
...
PostMessage( MainWindow, WM_POKEMON, (WPARAM) NEW_MACH_STATE_INDEX, (LPARAM) s.release() );
}

/***********************************************************************
*                      ENABLE_BOARD_RADIO_BUTTONS                      *
***********************************************************************/
static void enable_board_radio_buttons()
{
...
if ( hb )
    ...
    hb->mux[c].machine.set_text( w, IOEX4_MACHINE_TBOX );
    hb->mux[c].part.set_text( w, IOEX4_PART_TBOX );

    id  = MUX_ENABLE_1_XBOX;
    xid = MUX_CHANNEL_1_RADIO;
    for ( c=0; c<MAX_MUX_CHANNELS; c++ )
        {
        if ( !hb->mux[c].machine.isempty() )
            is_checked = TRUE;
        else
            is_checked = FALSE;
...
***********************************************************************/
void do_opto_port_test_request()
{
...
***********************************************************************/
static void do_send_command_port()
{
BOARD_DATA x;
...
for ( id=MUX_CHANNEL_1_RADIO; id<=MUX_CHANNEL_8_RADIO; id++ )
    ...
    if ( is_checked( w, id ) )
        ...
        if ( me )
            {
            me->machine.set_text( w, IOEX4_MACHINE_TBOX );
            me->part.set_text( w, IOEX4_PART_TBOX );
            }
        break;
        }
    }
...
***********************************************************************/
static void init_analog_thermometers()
{
int ch;
int id;
...
get_version( &version );
if ( version >= FT_8_CHANEL_VERSION )
    n = MAX_FT_CHANNELS;
else
    n = MAX_CONTINUOUS_CHANNELS;
        ...
        if ( ft_read_parameter(COMMAND[ch], &x) == FT_OK )
            ...
            if ( first_time || x != CurrentAnalog[ch] )
                ...
                if ( me )
                    ...
                    y *= (double) 10.0;
                    y /= (double) 4095.0;
                    ai = me->analog_sensor_index[ch];
                    if ( AnalogSensor[ai].max_volts_level != TEN_MAX_VOLTS )
                        y /= 2.0;
                    set_text( FtCurrentValuesWindow, VOLTS_1_TBOX+ch, fixed_string( y, 3, 2) );

                    y = AnalogSensor[ch].converted_value( (unsigned) x );
...
if ( LastBoard != b.number() || LastMux != b.current_mux_channel() )
    ...
    if ( me )
        ...
        for ( ch=0; ch<n; ch++ )
            {
            ai = me->analog_sensor_index[ch];
            set_text( FtCurrentValuesWindow, id, units_name(AnalogSensor[ai].units_index()) );
            id++;
            }
...
***********************************************************************/
static void start_aquiring_data_thread()
{
DWORD  id;
HANDLE h;
...
p.machine = b.next_machine();
if ( p.machine.isempty() )
    {
...
-------------------------------- */
p.computer = ComputerName;
if ( !current_part_name(p.part, p.computer, p.machine) )
    {
