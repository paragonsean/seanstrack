...
*                         FILL_SKIP_ENTRY                              *
***********************************************************************/
static void fill_skip_entry( SKIP_ENTRY * se, SETPOINT_CLASS & sp, int index )
{
SYSTEMTIME st;

se->acquire_count = sp[index + ACQUIRE_COUNT_OFFSET].int_value();
se->config        = sp[index + FLAGS_OFFSET].int_value();
se->max_saved     = sp[index + MAX_SAVED_OFFSET].int_value();
...
if ( se->config & ACQUIRE_ALL_BIT )
    ...
    if ( se->config & SKIP_TIME_BIT )
        se->skip_count = sp[index + TIME_SKIP_OFFSET].int_value();
    else
        se->skip_count = sp[index].int_value();
...
if ( sp.get(s) )
    {
    fill_skip_entry( &mse->profile, sp, MACHSET_PROFILE_SKIP_COUNT_OFFSET );
    fill_skip_entry( &mse->parm,    sp, MACHSET_PARAM_SKIP_COUNT_OFFSET );
    return TRUE;
    }
