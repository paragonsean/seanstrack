...
double * calculate_ftii_parameters( FTII_PROFILE & f, MACHINE_CLASS & machine, PART_CLASS & part, PARAMETER_CLASS & param, ANALOG_SENSOR & asensor, FTANALOG_CLASS & ftanalog );
...
n = p.get_last_shot_number( DestPart.computer, DestPart.machine, DestPart.part );
n++;
...
if ( get_shotsave_semaphore(DestPart.computer, DestPart.machine) )
    ...
    if ( nextshot() )
        ...
        ---------------------------------------------------------- */
        if ( is_ftii )
            dest.get_profile_ii_name( DestPart.computer, DestPart.machine, DestPart.part, ShotNumber );
        else
            dest.get_profile_name( DestPart.computer, DestPart.machine, DestPart.part, ShotNumber );
            ...
            if ( fh != INVALID_HANDLE_VALUE )
                ...
                if ( status )
                    ...
                    if ( seek_status != 0xFFFFFFFF )
                        {
                        copystring( ph.machine_name, DestPart.machine.text(), OLD_MACHINE_NAME_LEN );
                        copystring( ph.part_name,    DestPart.part.text(),    OLD_PART_NAME_LEN );
        ...
        t.open( dest, GRAPHLST_RECLEN, PWL );
        ...
        if ( status )
            {
            dest.get_shotparm_dbname( DestPart.computer,DestPart.machine, DestPart.part );
            t.open( dest, SHOTPARM_RECLEN, WL, WRITE_PARAM_TIMEOUT );
