...
static STRING_CLASS ComputerName;
...
{
CurrentPart.computer.set_text( BrouseWindow.control(CURRENT_COMPUTER_TBOX) );
CurrentPart.machine.set_text(  BrouseWindow.control(CURRENT_MACHINE_TBOX)  );
CurrentPart.part.set_text(     BrouseWindow.control(CURRENT_PART_TBOX)     );
}
...
while ( mlist.next() )
    ...
    if ( sp.get(s) )
        ...
        if ( MA_MONITORING_ON & sp[MACHSET_MONITOR_FLAGS_INDEX].uint_value() )
            {
            CurrentPart.machine = mlist.name();
            CurrentPart.part    = sp[MACHSET_CURRENT_PART_INDEX].value;
            status = TRUE;
            break;
            }
        }
...
if ( status )
    ...
    ------------------------------------------------- */
    s.get_semaphor_dbname( CurrentPart.computer, CurrentPart.machine );
    if ( s.file_exists() )
        t.create( s.text() );
    show_current_part();
...
switch ( msg )
    ...
    case WM_NEWSHOT:
        ...
        poke_data( CurrentPart.machine.text(), ItemList[SHOT_NUMBER_INDEX].name, shot );
        return 0;
...
CurrentPart.computer = ComputerName;
shifts_startup();
units_startup();
ANALOG_SENSOR::get();
