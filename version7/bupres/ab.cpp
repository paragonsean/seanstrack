...
AutoBackLb.add( resource_string(COMPLETE_STRING) );
...
if ( s.countchars(BackSlashChar) < 1 )
    {
    resource_message_box( MainInstance, NO_ROOT_DIR_STRING, CANT_DO_THAT_STRING );
    return;
    }
...
if ( !backupdir.directory_exists() )
    {
    lb.add( resource_string(CREATING_DIRECTORY_STRING) );
    b = backupdir.create_directory();

    if ( b )
        lb.add( resource_string(COMPLETE_STRING) );
    else
        lb.add( resource_string(DIR_CREATE_FAIL) );
    }

if ( b )
    {
    lb.add( resource_string(COPYING_SETUPS_STRING) );
