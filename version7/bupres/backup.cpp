...
if ( BackupType & PARAMETER_DATA )
    ...
    dest.get_shotparm_dbname(BackupComputer, machine, part );
                ...
                if ( BackupList[i].machine == machine )
                    ...
                    if ( BackupList[i].part == part )
                        ...
                        if ( t.get_next_key_match(1, NO_LOCK) )
                            ...
                            sorc_profile.get_profile_name( computer, machine, part, n );
                            if ( sorc_profile.file_exists() )
                                {
                                dest_profile.get_profile_name( BackupComputer, machine, part, n );
                                }
                            else
                                {
                                sorc_profile.get_profile_ii_name( computer,       machine, part, n );
                                dest_profile.get_profile_ii_name( BackupComputer, machine, part, n );
                                }
    ...
    if ( dir_from_filename(dest.text()) )
        ...
        if ( !dest.directory_exists() )
            ...
            if ( !dest.create_directory() )
                {
                sorc = resource_string( MainInstance, NO_ZIP_DIR_STRING );
...
if ( backing_up && !path_is_drive_only(BackupDir.text()) )
    ...
    if ( !dest.create_directory() )
        {
        sorc = resource_string( MainInstance, DIR_CREATE_FAIL );
...
if ( backing_up )
    ...
    if ( !execute_and_wait(dest.text(), NULL, TEN_MINUTES) )
        {
        dest = resource_string( MainInstance, UNABLE_TO_BACKUP_STRING );
    ...
    if ( !BackupZipFile.file_exists() )
        {
        dest = resource_string( MainInstance, NO_ZIP_FILE_STRING );
        MessageBox( 0, dest.text(), BackupZipFile.text(), MB_OK | MB_SYSTEMMODAL );
        backup_ok = FALSE;
        }
    }
...
sorc = resource_string( MainInstance, PURGING_STRING );
