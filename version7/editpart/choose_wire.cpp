...
static WIRE_MESSAGE_CLASS AlreadyUsedWires[] = {
    ...
    {  VALVE_TEST_IN_OPERATION_WIRE, TEXT( "VALVE_TEST_IN_OPERATION_STRING") },
...
if ( selected_wire_number == NO_WIRE )
    ...
    for ( i=0; i<MAX_FTII_LIMIT_SWITCHES; i++ )
        ...
        if ( CurrentLimitSwitch.wire_number(i) == selected_wire_number )
            {
            s += SpaceString;
    ...
    if ( CurrentOutputType != BINARY_VALVE_PULSE_OUTPUT_TYPE )
        ...
        if ( CurrentMachine.has_binary_valve() )
            {
               if ( CurrentMachine.pulse_wire.int_value() == selected_wire_number )
                {
                s += SpaceString;
...
* ChosenWire is the current opto wire [1,16]. CONTROL_WIRE_OFFSET is   *
* added to ChosenWire when the label is put on the button, if this is  *
* a control output, and subtracted when reading from the button label. *
***********************************************************************/
...
/*
-------------------------------------------------------
Read the wire number from the button.
16 will be subtracted later if this is a control output
-------------------------------------------------------*/
WireSorc = sorc;
s.get_text( sorc );
...
if ( (!ShowMonitorOutputs) || (CurrentMachine.suretrak_controlled && (ChosenWire == 0 || ChosenWire > CONTROL_WIRE_OFFSET)) || ChosenWire > CONTROL_WIRE_OFFSET )
    {
