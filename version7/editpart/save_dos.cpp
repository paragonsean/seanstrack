...
if ( directory_exists(SorcP42Directory) )
    if ( !copy_all_files(DestP42Directory, SorcP42Directory, StarDotStar, WITHOUT_SUBS, w, MSDOS_BU_STATUS_TBOX) )
        status = FALSE;

if ( status )
    if ( directory_exists(SorcProfileDirectory) )
        if ( !copy_all_files(DestProfileDirectory, SorcProfileDirectory, StarDotStar, WITHOUT_SUBS, w, MSDOS_BU_STATUS_TBOX) )
            status = FALSE;
...
if ( status )
    ...
    if ( F.open_for_write(paramsfile) )
        {
        F.writeline( setup.velocity_loop_gain() );

        for ( i=0; i<NOF_GAIN_BREAKS; i++ )
            F.writeline( setup.gain_break_velocity(i) );

        for ( i=0; i<NOF_GAIN_BREAKS; i++ )
            F.writeline( setup.gain_break_gain(i) );
...
if ( status )
    ...
    if ( F.open_for_write(lastprofile) )
        ...
        F.writeline( part.name.text() );
        F.writeline( EmptyString );
        F.writeline( EmptyString );

        for ( i=0; i<MAX_ST_LIMIT_SWITCHES; i++ )
            F.writeline( ls.pos(i) );
...
if ( status )
    {
    s.get_control_program_file_name( part.computer );
    if ( CopyFile( s.text(), destfile, FALSE ) == 0 )
        status = FALSE;
    }
