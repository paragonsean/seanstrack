...
BOOLEAN save_dos_suretrak( PART_CLASS & part, SURETRAK_SETUP_CLASS & setup, SURETRAK_PARAM_CLASS & param, LIMIT_SWITCH_CLASS & ls,  HWND w );
...
*                           SET_STATICS_FONT                           *
***********************************************************************/
static void set_statics_font( WINDOW_CLASS & w )
{
static LPARAM lParam = MAKELPARAM( FALSE, 0 );
...
*                           SET_STATICS_FONT                           *
***********************************************************************/
static void set_statics_font( HWND hWnd, UINT id )
{
WINDOW_CLASS w;
...
*                            UPDATE_STATICS                           *
**********************************************************************/
static void update_statics( HWND hWnd, RESOURCE_TEXT_ENTRY * rlist, int n )
{
int  i;
STRING_CLASS s;
WINDOW_CLASS w;
...
if ( CurrentMachine.has_binary_valve() )
    s = StringTable.find( TEXT("FAST_SHOT_PULSE") );
else
...
if ( is_proportional_valve )
    {
    is_visible = false;
    s = resource_string( PERCENT_STRING );
    }
else
    {
    is_visible = true;
    s = resource_string( PRESSURE_STRING );
    }
...
else if ( !CurrentMachine.is_ftii )
    {
    bw = FALSE;
    }
...
if ( b && !CurrentMachine.is_ftii )
    {
    for ( i=n; i<MAX_FTII_LIMIT_SWITCHES; i++ )
        {
        wc = GetDlgItem( w, LS1_DESC_EB+i );
        wc.disable();
        wc = GetDlgItem( w, LS1_POS_EB+i );
...
for ( i=0; i<n; i++ )
    ...
    if ( CurrentMachine.is_ftii )
        ...
        set_text( w, LS1_WIRE_PB+i, EmptyString );
    }
}
...
static INT SureTrakControlId[] = {
    ...
    LS1_DESC_EB, LS2_DESC_EB, LS3_DESC_EB, LS4_DESC_EB, LS5_DESC_EB,LS6_DESC_EB,LS7_DESC_EB,LS8_DESC_EB,
    LS1_POS_EB, LS2_POS_EB, LS3_POS_EB, LS4_POS_EB, LS5_POS_EB,LS6_POS_EB,LS7_POS_EB,LS8_POS_EB,
    LS1_WIRE_PB, LS2_WIRE_PB, LS3_WIRE_PB, LS4_WIRE_PB, LS5_WIRE_PB,LS6_WIRE_PB,LS7_WIRE_PB,LS8_WIRE_PB
    };
...
if ( HaveSureTrakControl )
    ...
    /*
    ----------------------------------
    Hide the dos button if this is ft2
    ---------------------------------- */
    ...
    for ( i=0; i<MAX_FTII_LIMIT_SWITCHES; i++ )
        ...
        show_window( SuretrakProfileWindow, LS1_WIRE_PB+i, is_visible );
...
static TCHAR * rsn[] = {
    ...
    { TEXT("CHANNELS_TAB") },
    { TEXT("SURETRAK_TAB") },
    { TEXT("PRES_CONTROL_TAB") },
    { TEXT("SETUP_SHEET_TAB") }
    };
const int nof_rsn = sizeof( rsn)/sizeof(TCHAR *);
TC_ITEM tie;
int i;
int tab_item_number;
...
for ( i=0; i<(TAB_COUNT); i++ )
    ...
    copystring( buf, StringTable.find(rsn[i]) );
...
/*
------------------------------------
See if this is an end point setpoint
------------------------------------ */
box = EP_1_EDITBOX;
for ( i=0; i<MAX_ST_STEPS; i++ )
    ...
    if ( box == id )
        ...
        /*
        ----------------------------------
        See if this is the low impact step
        ---------------------------------- */
        box += 2;
        s.get_text( parent, box );
        if ( !s.isempty() )
            {
            /*
            ------------------------------------------------------------------------------------
...
if ( !is_checked(w, id) )
    ...
    i = SureTrakSetup.vacuum_limit_switch_index();
    if ( i != NO_INDEX )
        InvalidateRect( GetDlgItem(w, LS1_POS_EB+i), 0, TRUE );
...
---------------------------------- */
s.get_exe_dir_file_name( MULTIPART_RUNLIST_LABELS_FILE );
...
switch (msg)
    ...
    case WM_COMMAND:
        if ( id == IDOK )
            {
            hourglass_cursor();
            status = save_dos_suretrak( CurrentPart, SureTrakSetup, SureTrakParam, CurrentLimitSwitch, hWnd );
            restore_cursor();
            if ( !status )
...
{
static TCHAR EDITPART_CAPTION[] = TEXT( "CREATE_PART" );
static RESOURCE_TEXT_ENTRY rlist[] =
    {
...
static int LS7_INDEX                 = 9;
static int LS8_INDEX                 = 10;
static int MY_VEL_LOOP_GAIN_INDEX    = 11;
static int MY_FT_STOP_POS_INDEX      = 12;
static int BREAK_VEL_1_INDEX         = 13;
static int BREAK_VEL_2_INDEX         = 14;
static int BREAK_VEL_3_INDEX         = 15;
static int BREAK_GAIN_1_INDEX        = 16;
static int BREAK_GAIN_2_INDEX        = 17;
static int BREAK_GAIN_3_INDEX        = 18;
static int TIME_INTERVAL_MS_INDEX    = 19;
static int NOF_TIME_SAMPLES_INDEX    = 20;
static int BISCUIT_DELAY_MS_INDEX    = 21;
static int PC_CONFIG_WORD_INDEX      = 22;
static int PC_PRES_LOOP_GAIN_INDEX   = 23;
static int PC_RETRACT_VOLTS_INDEX    = 24;
static int PC_PARK_VOLTS_INDEX       = 25;
static int PC_RAMP_1_INDEX           = 26;
static int PC_PRES_1_INDEX           = 27;
static int PC_HOLDTIME_1_INDEX       = 28;
static int PC_RAMP_2_INDEX           = 29;
static int PC_PRES_2_INDEX           = 30;
static int PC_HOLDTIME_2_INDEX       = 31;
static int PC_RAMP_3_INDEX           = 32;
static int PC_PRES_3_INDEX           = 33;
static int PC_HOLDTIME_3_INDEX       = 34;
static int PC_INTEGRAL_GAIN_INDEX    = 35;
static int PC_DERIVATIVE_GAIN_INDEX  = 36;
static int PC_RETCTL_DAC_INDEX       = 37; /* Retctl must be last */
static int PC_RETCTL_ARM_PRES_INDEX  = 38;
static int PC_RETCTL_PRES_INDEX      = 39;

static int vars[] = { TSL_VN, MSL_VN, ESV_VN,
 LS1_VN, LS2_VN, LS3_VN, LS4_VN, LS5_VN, LS6_VN, LS7_VN, LS8_VN,
 VEL_LOOP_GAIN_VN,
 FT_STOP_POS_VN,
...
static int maskvn[] = { LS1_MASK_VN, LS2_MASK_VN, LS3_MASK_VN, LS4_MASK_VN, LS5_MASK_VN, LS6_MASK_VN, LS7_MASK_VN, LS8_MASK_VN };
...
for ( i=0; i<MAX_FTII_LIMIT_SWITCHES; i++ )
    ...
    /*
    -------------------------------------------------------------------------------------------------
    Masks: This used to be a mask but as of version 8 flash this is the wire number. FT2 will convert
    it back to a mask if the flash version is less than 8.
    ------------------------------------------------------------------------------------------------- */
    u = 0;
    if ( setpoint_not_zero )
        u = CurrentLimitSwitch.wire_number( i );
...
Retract Control
--------------- */
va[PC_RETCTL_DAC_INDEX].set( PC_RETCTL_DAC_VN, 4 );  /* DAC channel to be used for retract control */
...
if ( SureTrakSetup.find(CurrentPart.computer, CurrentPart.machine, CurrentPart.name) )
    ...
    if ( have_change )
        ...
        if ( SureTrakSetup.save( CurrentPart.computer, CurrentPart.machine, CurrentPart.name) )
            {
            MessageBox( 0, resource_string(SAVING_VELOCITY_CHANGES_STRING), CurrentPart.name.text(), MB_OK );
            refresh_suretrak_setup();
            update_ftii_control_program();
            send_new_monitor_setup_event();
            }
...
************************************************************************/
static void check_for_hmi_message( LPSTR cmd_line )
{
STRING_CLASS s;
...
***********************************************************************/
BOOLEAN check_for_previous_instance( LPSTR cmd_line )
{
HWND w;
FILE_CLASS f;
NAME_CLASS s;
...
c.startup();
check_for_hmi_message( cmd_line);
if ( check_for_previous_instance(cmd_line) )
    return 0;
