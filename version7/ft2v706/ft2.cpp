...
RESOURCE_TEXT_ENTRY rsn[] = {
    ...
    { CYCLE_START_STRING,               TEXT("CYCLE_START_STRING") },
...
va.get(s);
if ( va.count() < NofMachineVars )
    va.upsize( NofMachineVars );

for ( i=0; i<NofMachineVars; i++ )
    va[i] = CurrentMachineVars[i];
...
if ( i != NO_INDEX && i < NofRows )
    ...
    if ( new_state == NO_FT2_CONNECT_STATE || new_state & NOT_MONITORED_STATE )
        ...
        /*
        ----------------------------------------------------------------------------
        I used to do this, I think, in order to restart the realtime data
        after the board is reset. I'm going to try to move this to board_monitor.cpp
        ---------------------------------------------------------------------------- */
...
{
if ( free_kilobytes(ComputerName.text()) < MIN_FREE_DISK_KB )
