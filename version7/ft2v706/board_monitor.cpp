...
if ( have_runlist )
    ...
    if ( !s.isempty() && (s != part.name) )
        ...
        if ( part_exists( part.computer, part.machine, s) )
            ...
            {
            MessageBox( 0, resource_string(NO_WARMUP_PART_STRING), s.text(), MB_OK | MB_SYSTEMMODAL );
            }
        }
    }
...
if ( f.open_for_read(s.text()) )
    ...
    while ( TRUE )
        ...
        if ( *cp != NullChar && *cp != PoundChar )
            ...
            /*
            ------------------------------------------------------------
            Strip comments again just incase my replacement had comments
            ------------------------------------------------------------ */
            if ( !s.isempty() )
    ...
    if ( dest.m.find(ComputerName, machine_name) )
        ...
        if ( monitoring_is_on )
            ...
            if ( is_different_machine )
                ...
                t.ensure_existance( s );
                s.get_shotparm_dbname( dest.part.computer, dest.part.machine, dest.part.name );
                t.ensure_existance( s );
                }
