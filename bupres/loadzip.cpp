...
s = StringTable.find( TEXT("ZIP_FILES") );
...
s = StringTable.find( TEXT("CHOOSE_ZIP") );
...
if ( GetOpenFileName( &fh ) )
    ...
    if ( newdir.directory_exists() )
        ...
        /*
        -----------------------------------------------------------------------------------
        Check to see if this is V7 data. If so, convert graphlst.txt and shotparm.txt files
        ----------------------------------------------------------------------------------- */
        buf = BackupDir;
        buf.cat_path( DataString );
        v7_results_to_v8( buf, 0, 0 );
        }
