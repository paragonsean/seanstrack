...
***********************************************************************/
bool add_version_to_filename( NAME_CLASS & dest )
{
static TCHAR MinusV[] = TEXT( "-v" );
...
version = ini.get_string( VersionKey );
version.remove_all_occurances( PeriodChar );
if ( !version.isempty() )
    {
    s.get_extension( dest );
    dest.remove_extension();
    dest += MinusV;
    dest += version;
    if ( sizeof(TCHAR) > 1 )
        dest += UCharLow;
        ...
        switch ( id )
            ...
            case IDOK:
                if ( check_zip_name(GetDlgItem(w, TB_ZIP_NAME_EDITBOX)) )
                    ...
                    if ( zipname.get_text(w, TB_ZIP_NAME_EDITBOX) )
                        {
                        add_version_to_filename( zipname );
