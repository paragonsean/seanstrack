...
***********************************************************************/
bool get_external_parameters( FTII_FILE_DATA & dest, PARAMETER_CLASS & param, EXTERNAL_PARAMETER_SETTINGS_CLASS eps )
{
FILE_CLASS   f;
int          first_ep_index;
int          i;
...
for ( i=0; i<NofEpf; i++ )
    ...
    if ( Epf[i].is_enabled && Epf[i].external_parameters_file_name.file_exists() )
        ...
        if ( get_shotsave_semaphore(Epf[i].part.computer, Epf[i].part.machine)  )
            ...
            ------------------------------------------- */
            plookup.get_last_shot_name_only( shot_name, Epf[i].part.computer, Epf[i].part.machine, Epf[i].part.part );
