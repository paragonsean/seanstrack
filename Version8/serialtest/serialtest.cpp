...
static TCHAR EosUseTcpIpKey[]        = TEXT( "EosUseTcpIp" );
static TCHAR EosTcpIpAddressKey[]    = TEXT( "EosTcpIpAddress" );
static TCHAR EosTcpIpPortKey[]       = TEXT( "EosTcpIpPort" );
...
STRING_CLASS EosMachineName;
STRING_CLASS EosStartFile;
STRING_CLASS EosEndFile;
STRING_CLASS EosTcpIpAddress;
STRING_CLASS EosTcpIpPort;

BOOLEAN EosAsciiToSerial = FALSE;
BOOLEAN EosComCts        = FALSE;
BOOLEAN EosComDsr        = FALSE;
BOOLEAN EosComXonXoff    = FALSE;
BOOLEAN EosNeedsStxEtx   = FALSE;
BOOLEAN EosUseTcpIp      = FALSE;

BOOLEAN WindowsSocketsIsStarted = FALSE;
...
*                            Fix ".."                                  *
***********************************************************************/
static void check_tcp_address( HWND w )
{
...
***********************************************************************/
static BOOLEAN startup_windows_sockets()
{
const WORD SocketVersion = MAKEWORD(2,0);
WSADATA wsadata;

if ( !WindowsSocketsIsStarted )
    {
    WSAStartup( SocketVersion, &wsadata );
    WindowsSocketsIsStarted = TRUE;
    }
return WindowsSocketsIsStarted;
}

/***********************************************************************
*                          OPEN_ETHERNET_PORT                          *
***********************************************************************/
static bool open_ethernet_port()
{
char        address[TCP_ADDRESS_LEN+1];
u_short     port;
int         status;
sockaddr_in a;
...
if ( sizeof(TCHAR) > 1 )
    unicode_to_char( address, EosTcpIpAddress.text() );
else
    copystring( address, EosTcpIpAddress.text() );

port = (u_short) EosTcpIpPort.uint_value();
...
a.sin_addr.S_un.S_addr = inet_addr( address );
...
*                         CLOSE_ETHERNET_PORT                          *
***********************************************************************/
static void close_ethernet_port()
{
...
/***********************************************************************
*                     WRITE_BYTES_TO_ETHERNET_PORT                     *
***********************************************************************/
bool write_bytes_to_ethernet_port( TCHAR * sendbuf, int bytes_to_write )
{
send( MySocket, (char *)sendbuf, bytes_to_write, 0 );
...
if ( EosUseTcpIp )
    {
    if ( !open_ethernet_port() )
        {
        s = TEXT( "Unable to open address " );
        s += EosTcpIpAddress;
        s += TEXT( " port " );
        s += EosTcpIpPort;
        MessageBox( MainDialog.handle(), s.text(), TEXT("Message NOT sent."), MB_OK );
        return;
        }
    }
...
if ( EosUseTcpIp )
    ...
    if ( EosAsciiToSerial )
        ...
        if ( EosUseTcpIp )
            (
            write_bytes_to_ethernet_port( cbuf, bytes_to_write );
            close_ethernet_port();
            }
        else
            (
        ...
        if ( EosUseTcpIp )
            (
            write_bytes_to_ethernet_port( s.text(), bytes_to_write );
            close_ethernet_port();
            }
        else
...
TCHAR * tp; /* Used for truncating the shot number */
int     fixed_width;
int     j;
int     n;
bool    firstfield;
...
if ( delimiter == NullCharString || unknown(delimiter.text()) )
...
if ( EosMachineName.isempty() )
    ini.remove_key( EosMachineNameKey );
else
    ini.put_string( EosMachineNameKey, EosMachineName.text()  );

if ( EosTcpIpAddress.isempty() )
    ini.remove_key( EosTcpIpAddressKey );
else
    ini.put_string( EosTcpIpAddressKey, EosTcpIpAddress.text()  );

if ( EosTcpIpPort.isempty() )
    ini.remove_key( EosTcpIpPortKey );
else
    ini.put_string( EosTcpIpPortKey, EosTcpIpPort.text()  );
...
ini.put_boolean( EosUseTcpIpKey,    EosUseTcpIp );
...
EosMachineName.get_text(    MainDialog.control(MACHINE_NAME_EB)  );
EosTcpIpAddress.get_text(   MainDialog.control(TCP_IP_ADDRESS_EB));
EosTcpIpPort.get_text(      MainDialog.control(TCP_IP_PORT_EB)   );

EosAsciiToSerial = is_checked( MainDialog.control(ASCII_TO_SERIAL_XBOX) );
...
EosUseTcpIp      = is_checked( MainDialog.control(USE_TCP_IP_XB)        );
...
if ( ini.find(EosMachineNameKey) )
    EosMachineName = ini.get_string();
else
    EosMachineName.empty();
...
if ( ini.find(EosTcpIpAddressKey) )
    EosTcpIpAddress = ini.get_string();

if ( ini.find(EosTcpIpPortKey) )
    EosTcpIpPort = ini.get_string();

EosAsciiToSerial = ini.get_boolean( EosAsciiToSerialKey );
...
EosTcpIpAddress.set_text(   MainDialog.control(TCP_IP_ADDRESS_EB));
EosTcpIpPort.set_text(      MainDialog.control(TCP_IP_PORT_EB)   );

set_checkbox( MainDialog.control(ASCII_TO_SERIAL_XBOX), EosAsciiToSerial );
...
set_checkbox( MainDialog.control(USE_TCP_IP_XB),        EosUseTcpIp );
...
switch (msg)
    ...
    case WM_COMMAND:
        switch ( id )
            ...
            case TCP_IP_ADDRESS_EB:
                if ( cmd == EN_CHANGE )
                    {
                    check_tcp_address((HWND)lParam);
                    return TRUE;
                    }
                break;
            ...
            case USE_TCP_IP_XB:
                EosUseTcpIp = is_checked( (HWND)lParam );
                return TRUE;
