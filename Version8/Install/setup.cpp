...
STRING_CLASS s;
HWND         w;
StringTable.find( s, id );
w = CurrentDialogWindow.control( STATUS_MSG_TEXTBOX );
if ( w )
    s.set_text( w );
}
...
**********************************************************************/
void copy_resources_directory( bool new_only, int message_textbox )
{
NAME_CLASS s;
NAME_CLASS d;
...
copy_all_files( d.text(), s.text(), WITH_SUBS, CurrentDialogWindow.handle(), message_textbox, new_only );
}
...
if ( !file_exists(s) )
    {
    StringTable.find( s1, TEXT("NO_FILE") );
    MessageBox( 0, s, s1.text(), MB_OK | MB_SYSTEMMODAL );
    return;
    }
...
s.null();
s.set_text( CurrentDialogWindow.handle(), US_MACHINE_STATIC );
s.set_text( CurrentDialogWindow.handle(), US_PART_STATIC    );

/*
----------------------------------------------------------------------------
Copy the resources. If this is an older installation then delete all the old
stringtable files and then copy all the new ones.
---------------------------------------------------------------------------- */
show_update_status( TEXT("COPYING_RESOURCES") );
s = ExeDirectoryName;
s.cat_path( ResourcesString );
s.cat_path( CommonResourceFile
copy_resources_directory( true, STATUS_MACHINE_TB );
...
show_status( TEXT("COPYING_RESOURCES") );
copy_resources_directory( false, FILENAME_TEXTBOX );
...
if ( UsingRestorePoint )
    ...
    if ( !s.find(CurrentString) )
        ...
        StringTable.find( s1, TEXT("AUTOBACK_DIR_SET") );
        MessageBox( 0, s.text(), s1.text(), MB_OK );
        }
...
StringTable.find( s1, TEXT("INSTALL_DIR_EXISTS") );
s += dest;
i = MessageBox( w, s.text(), s1.text(), MB_OKCANCEL | MB_ICONEXCLAMATION | MB_APPLMODAL );
if ( i != IDOK )
    return false;
...
switch (msg)
    ...
    case WM_INITDIALOG:
        ...
        make_sure_i_am_on_screen();
        StringTable.find( s, TEXT("STARTUP_TITLE") );
...
switch (msg)
    ...
    case WM_INITDIALOG:
        CurrentDialogWindow = hWnd;
        MainWindow.shrinkwrap( hWnd );
        StringTable.find( s, TEXT("UPDATE_STARTUP_TITLE") );
        MainWindow.set_title( s.text() );
