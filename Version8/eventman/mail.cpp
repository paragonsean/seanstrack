...
{
static TCHAR ArchiverDirNotFound[] = TEXT( "Archiver DIR NOT FOUND" );
static TCHAR ArchiverIsOffline[]   = TEXT( "Archiver is OFF LINE" );
static TCHAR ArchiverIsOnline[]    = TEXT( "Archiver is ON LINE" );
int i;
BOOLEAN b;
STRING_CLASS msg;
BOOLEAN offline;
...
while ( TRUE )
    ...
    if ( b )
        ...
        if ( !rc.is_online )
            ...
            if ( DisplayingMessages )
                {
                show_message( ArchiverIsOffline );
                show_message( rc.directory.text() );
                }
            ...
            if ( rc.directory.directory_exists() )
                ...
                LeaveCriticalSection( &ComputersCriticalSection );
                if ( DisplayingMessages )
                    show_message( ArchiverIsOnline );
                }
            else
                {
                if ( DisplayingMessages )
                    show_message( ArchiverDirNotFound );
                offline = TRUE;
                }
            }
        }
