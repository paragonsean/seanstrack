...
for ( i=0; i<nof_rsn; i++ )
    ...
    if ( resource_id == rsn[i].id )
        {
        if ( StringTable.find(dest, rsn[i].s) )
            return true;
        break;
        }
    }

dest = resource_string( MainInstance, resource_id );
return false;
}
...
STRING_CLASS m;
STRING_CLASS t;
resource_string( m, msg_id );
resource_string( t, title_id );
...
for ( i=0; i<NofButtons; i++ )
    {
    if ( StringTable.find(s, Button[i].s) )
...
for ( col=0; col<NofCols; col++ )
    ...
    resource_string( s, BOARD_STRING + col );
    w = CreateWindow(
        TEXT("static"),
        s.text(),
        WS_CHILD | WS_VISIBLE | SS_CENTER,
...
for ( module=0; module<4; module++ )
    ...
    for ( led=0; led<4; led++ )
        ...
        if ( wire == 17 )
            {
            StringTable.find( s, TEXT("SLOW_SHOT") );
            s += TwoSpacesString;
            }
        else if ( wire == 18 )
            {
            StringTable.find( s, TEXT("ACCUMULATOR") );
            s += TwoSpacesString;
            }
        ...
        for ( i=0; i<MAX_PARMS; i++ )
            ...
            if ( wire == (int) p.parameter[i].limits[ALARM_MIN].wire_number )
                ...
                s += EqualsChar;
                StringTable.append( s, TEXT("LOW_ALARM") );
                s += TwoSpacesString;
                }
            if ( wire == (int) p.parameter[i].limits[ALARM_MAX].wire_number )
                ...
                s += p.parameter[i].name;
                s += EqualsChar;
                StringTable.append( s, TEXT("HIGH_ALARM") );
                s += TwoSpacesString;
                }
            if ( wire == (int) p.parameter[i].limits[WARNING_MIN].wire_number )
                ...
                s += p.parameter[i].name;
                s += EqualsChar;
                StringTable.append( s, TEXT("LOW_WARNING") );
                s += TwoSpacesString;
                }
            if ( wire == (int) p.parameter[i].limits[WARNING_MAX].wire_number )
                ...
                s += p.parameter[i].name;
                s += EqualsChar;
                StringTable.append( s, TEXT("HIGH_WARNING") );
                s += TwoSpacesString;
                }
            }
...
resource_string( s, AUTO_NULL_BEGIN_STRING );
NullStatusMessage.set_title( s.text() );
ManualInfoTbox.hide();
}
...
Sleep( 100 );
send_command_to_current_machine( TEXT("OC5=1"),  true );
resource_string( s, MANUAL_NULLING_STRING );
NullStatusMessage.set_title( s.text() );
ManualInfoTbox.show();
...
if ( sorc )
    ...
    for( i=0; i<nof_responses; i++ )
        ...
        if ( cp )
            ...
            resource_string( s, response[i].id );
            NullStatusMessage.set_title( s.text() );
            break;
            }
        }
...
if ( f.open_for_write(s.text()) )
    {
    temp_message( SavingString.text(), 500 );
    ...
    {
    resource_string( s1, CANT_OPEN_FILE_STRING );
    MessageBox( 0, s.text(), s1.text(), MB_OK | MB_SYSTEMMODAL );
    }
...
if ( CurrentOpStatus.status_word1 != Control_Status || NeedRefresh )
    ...
    if ( Control_Status & PROGRAM_RUNNING_BIT )
        {
        resource_string( ns, CONTROL_ACTIVE_STRING );
        }
    else if ( !(Control_Status & PROGRAM_LOADED_BIT) )
        {
        resource_string( ns, NO_PROGRAM_STRING );
        }
    else if ( Control_Status & FATAL_ERROR_BIT )
        {
        resource_string( ns, CONTROL_FATAL_STRING );
        }
    else if ( Control_Status & WARNING_BIT )
        {
        resource_string( ns, CONTROL_WARNING_STRING );
        }
    else if ( !(CurrentOpStatus.isw4 & MASTER_ENABLE_BIT) )
        {
        resource_string( ns, NO_MASTER_ENABLE_STRING );
        }
    else
        {
        resource_string( ns, UNKNOWN_STRING );
        }
    ns.set_text( w );
    if ( NeedErrorLog )
        {
...
StringTable.find( s, TEXT("VERT_OPTO") );
StringTable.append( s, TEXT("VERT_TEXT") );
if ( s.isempty() )
    return;
        ...
        switch ( id )
            ...
            case LOG_FILE_SETUP_PB:
                ...
                if ( NeedErrorLog )
                    {
                    NeedErrorLog = FALSE;
                    resource_string( s, OFF_STRING );
                    s.set_text( hWnd, LOG_FILE_SETUP_PB );
                    }
                else
                    {
                    NeedErrorLog = TRUE;
                    resource_string( s, ON_STRING );
                    s.set_text( hWnd, LOG_FILE_SETUP_PB );
                    }
...
for ( module=0; module<8; module++ )
    ...
    for ( led=0; led<4; led++ )
        ...
        if ( module < 4 )
            {
            StringTable.append( s, TEXT("MONITOR_MODULE") );
            s += SpaceChar;
            s += module*4+led+1;
            }
        else
            {
            StringTable.append( s, TEXT("CONTROL_MODULE") );
            s += SpaceChar;
            s += module - 3;
            s += SpaceChar;
            StringTable.append( s, TEXT("LED") );
            s += SpaceChar;
            s += led + 1;
            }
        s += SpaceString;
        ...
        if ( wire == DownTimeAlarmWire )
            {
            StringTable.append( s, TEXT("DOWNTIME_ALARM_WIRE") );
            s += TwoSpacesString;
            }
        if ( wire == MonitorWire )
            {
            StringTable.append( s, TEXT("MONITOR_WIRE") );
            s += TwoSpacesString;
            }
        for ( i=0; i<MAX_FTII_LIMIT_SWITCHES; i++ )
            {
            if ( lsw.wire_number(i) == wire )
                {
                StringTable.append( s, TEXT("LIMIT_SWITCH") );
                s += SpacePoundString;
                s += i+1;
                s += TwoSpacesString;
                }
    ...
    if ( Epf[i].is_enabled && Epf[i].external_parameters_file_name.file_exists() )
        ...
        if ( get_shotsave_semaphore(Epf[i].part.computer, Epf[i].part.machine)  )
            ...
            if ( fd.get(fn) )
                ...
                I don't need the semaphore for this as I'm not reading or
                writing the shotparm file.
                ---------------------------------------------------------- */
                shotparm_index = SHOTPARM_PARAM_1_INDEX;
                for ( pi=0; pi<MAX_PARMS; pi++ )
                    {
...
switch (msg)
            ...
            if ( ShutdownState == SHUTTING_DOWN_MONITORING )
                ...
                if ( !BoardMonitorIsRunning )
                    ...
                    if ( ShotSaveIsRunning )
                        {
                        resource_string( s1, SHUTTING_DOWN_SHOTSAVE_STRING );
                        ShutdownStatusListbox.add( s1.text() );
                        SetEvent( ShotSaveEvent );
                        return 0;
                        }
                    else
                        {
                        resource_string( s1, SHOTSAVE_NOT_RUNNING_STRING );
                        ShutdownStatusListbox.add( s1.text() );
                        }
                    }
            ...
            if ( ShutdownState == SHUTTING_DOWN_SHOTSAVE )
                ...
                if ( !ShotSaveIsRunning )
                    ...
                    if ( IsRemoteMonitor )
                        ...
                        remote_shutdown();
                        resource_string( s1, SHUTTING_DOWN_REMOTE_STRING );
                        ShutdownStatusListbox.add( s1.text() );
                        return 0;
                        }
                    else
    ...
    case WM_NO_DATA:
        ...
        resource_string( s1, INVALID_SHOT_NAME_STRING );
        temp_message( s1.text(), 3000 );
        return 0;
    ...
    case WM_CLOSE:
        ...
        if ( ShutdownState == NOT_SHUTTING_DOWN )
            ...
            resource_string( s1, SHUTTING_DOWN_MONITOR_STRING );
            ShutdownStatusListbox.add( s1.text() );
...
------------------------------- */
resource_string( NoConnectState,   NO_CONNECT_STRING         );
resource_string( NotMonitored,     NOT_MONITORED_STRING      );
resource_string( Connected,        CONNECTED_STRING          );
resource_string( Connecting,       CONNECTING_STRING         );
resource_string( NotConnected,     NOT_CONNECTED_STRING      );
resource_string( ZeroStrokeLength, ZERO_STROKE_LENGTH_STRING );
resource_string( UploadFailed,     UPLOAD_FAIL_STRING        );
resource_string( NoResponse,       NO_RESPONSE_STRING        );

StringTable.find( CycleStartString, TEXT("CYCLE_START")   );
StringTable.find( SavingString,     TEXT("SAVING_STRING") );
