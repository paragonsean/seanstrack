...
if ( m.multipart_runlist_settings.enable )
    {
    StringTable.find( not_found_string, TEXT("NO_MULTIPART_RUNLIST_PART") );
    m.multipart_runlist_settings.part_to_run( s, part.computer, part.machine, u );
    }
else if ( have_runlist )
    {
    StringTable.find( not_found_string, TEXT("NO_WARMUP_PART_STRING") );
...
if ( part.good_shot_wire != NO_WIRE && !part.good_shot_active_low )
...
good_shot_wire = board->part.good_shot_wire;
if ( good_shot_wire != NO_WIRE && !this_is_a_good_shot )
    {
    if ( board->part.good_shot_active_low )
        board->clearwire( good_shot_wire );
    else
        setbits |= wirebit( good_shot_wire );
    }

/*
----------------------------------------------------------------------
If there are external parameters that have alarms I need to wait until
I get the external parameter file before setting summary counts.
---------------------------------------------------------------------- */
...
if ( board->part.alarm_shot_wire != NO_WIRE )
    goodbit = wirebit( board->part.alarm_shot_wire );
...
if ( goodbit && this_is_a_good_shot )
    {
    if ( board->part.good_shot_active_low )
        board->send_bits( goodbit, false );
    else
        setbits |= goodbit;
    }
...
resource_string( s, SAVING_SHOT_STRING );
update_monitor_status( this, s.text() );
...
resource_string( s, READY_STRING );
update_monitor_status( this, s.text() );
if ( part.shot_complete_wire != NO_WIRE )
    {
        ...
        if ( dest.have_runlist )
            ...
            for ( i=0; i<dest.runlist.count(); i++ )
                ...
                if ( !part_exists(ComputerName, machine_name, dest.runlist[i].partname) )
                    {
                    resource_string( s, NO_WARMUP_PART_STRING );
                    MessageBox( 0, s.text(), dest.runlist[i].partname.text(), MB_OK | MB_SYSTEMMODAL );
                    }
                }
            }
        ...
        if ( monitoring_is_on )
            ...
            if ( t.open(s, AUTO_RECLEN, PFL) )
                ...
                if ( !shot_name_is_alpha )
                    {
                    StringTable.find( message, TEXT("UPDATING_GRAPHLST_STRING") );
            ...
            if ( dest.connect_state & CONNECTED_STATE )
                ...
                if ( dest.part.good_shot_active_low && dest.part.good_shot_wire != NO_WIRE )
                    dest.setwire( dest.part.good_shot_wire );
                }
            }
        }
    }
...
for ( i=0; i<NofBoards; i++ )
    ...
    if ( Boards[i].is_monitoring() )
        ...
        if ( Boards[i].connect_state & CONNECTED_STATE )
            ...
            if ( Boards[i].part.good_shot_active_low && Boards[i].part.good_shot_wire != NO_WIRE )
                mask |= wirebit( Boards[i].part.good_shot_wire );
...
{
static TCHAR        POS_AT_IMPACT_DISABLE[]    = TEXT( "OC13=0" );
static TCHAR        POS_AT_IMPACT_ENABLE[]     = TEXT( "OC13=1" );
...
if ( !Boards[i].monitoring_was_stopped )
    ...
    if ( !Boards[i].upload_control_file() )
        {
        resource_string( s, NO_UPLOAD_FILE_STRING );
        update_monitor_status( &Boards[i], s.text() );
        }
    }
...
u = Boards[i].part.good_shot_wire;
if ( Boards[i].part.good_shot_active_low && u != NO_WIRE )
    Boards[i].setwire( u );
}
...
if ( cp )
    ...
    if ( findstring(TIMEOUT, cp) )
        ...
        update_monitor_status( board, s.text() );
        ...
        if ( board->part.good_shot_wire != NO_WIRE && !board->part.good_shot_active_low )
...
if ( !matched_message )
    ...
    if ( cp )
        ...
        if ( board->part.good_shot_active_low && board->part.good_shot_wire != NO_WIRE )
            board->setwire( board->part.good_shot_wire );
        }
    }
            ...
            if ( Boards[i].number == m.ft_board_number )
                ...
                if ( i < NofRows )
                    ...
                    if ( Boards[i].is_monitoring() && !(Boards[i].connect_state & ZERO_STROKE_LENGTH_STATE) )
                        {
                        resource_string( s, NEW_SETUP_STRING );
                ...
                if ( Boards[i].is_monitoring() )
                    ...
                    if ( Boards[i].monitoring_was_stopped )
                        {
                        resource_string( s, RESTARTED_STRING );
                        s.set_text( MonitorStatusTbox[i].w );
                        if ( i == CurrentBoardIndex )
                            s.set_text( CurrentValuesMonitorStatusTb );
                        }
...
if ( mp->s[0] == NakChar )
    ...
    if ( TerminalIsVisible )
        {
        resource_string( s, NAK_RECEIVED_STRING );
            ...
            if ( *cp == 'B' )
                ...
                if ( bh->data_type == START_OF_BINARY_TEXT_DATA )
                    ...
                    if ( int(bh->packet_num) == Boards[i].current_packet_number  )
                        ...
                        if ( TerminalIsVisible )
                            {
                            resource_string( s, PACKET_STRING );
                            s += Boards[i].current_packet_number;
                            resource_string( s1, REPEATED_STRING );
                            s += s1;
                            s += Boards[i].m.name;
                            MainListBox.add( s.text() );
                            }
                        ...
                        if ( TerminalIsVisible )
                            {
                            s = TEXT("**** ");
                            resource_string( s1, PACKET_STRING );
                            s += s1;
                            s += Boards[i].current_packet_number+1;
                            resource_string( s1, MISSING_STRING );
                            s += s1;
                            s += Boards[i].m.name;
                            s += TEXT(" ***" );
                            MainListBox.add( s.text() );
                        ...
                        resource_string( s, PACKET_ERROR_STRING );
                        update_monitor_status( &Boards[i], s.text() );
                    ...
                    while ( mp == 0 )
                        ...
                        if ( start_chars_are_equal(START_OF_UPLOAD, mp->s) )
                            ...
                            if ( Boards[i].do_not_upload )
                                ...
                                SocketMessageStore.push( mp );
                                mp = 0;
                                resource_string( s, BEGIN_UPLOAD_STRING );
                                update_monitor_status( &Boards[i], s.text() );
                                }
                            }
                            ...
                            SocketMessageStore.push( mp );
                            mp = 0;
                            resource_string( s, UPLOAD_COMPLETE_STRING );
                            update_monitor_status( &Boards[i], s.text() );
                            /*
                            --------------------------------------------------
        ...
        if ( m.find(ComputerName, machine_name) )
            ...
            if ( m.monitor_flags & MA_MONITORING_ON )
                ...
                if ( i != NO_INDEX )
                    {
                    resource_string( s, NEW_SETUP_STRING );
                    update_one_board_display( i, s.text() );
                    }
                }
            }
        }
