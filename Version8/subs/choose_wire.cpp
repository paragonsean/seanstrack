...
if ( selected_wire_number == NO_WIRE )
    {
    StringTable.find( s, TEXT("NO_OUTPUT_STRING") );
    WireInfoTbox.set_title( s.text() );
    }
else
    {
    StringTable.find( s, TEXT("WIRE_NUMBER_STRING") );
    s += SpaceString;
    ...
    if ( CurrentOutputModule == CONTROL_OUTPUT_MODULE )
        ...
        s += TwoSpaceString;
        StringTable.append( s, TEXT("OUTPUT_NUMBER_STRING") );
        s += SpaceString;
        s += selected_wire_number;
        s += SpaceString;
        }
    ...
    for ( i=0; i<NofAlreadyUsedWires; i++ )
        ...
        if ( selected_wire_number == AlreadyUsedWires[i].wire_number )
            {
            s += SpaceString;
            StringTable.append( s, TEXT("ALREADY_IN_USE_STRING") );
            StringTable.append( s, AlreadyUsedWires[i].id );
            break;
            }
        }
    ...
    for ( i=0; i<n; i++ )
        ...
        for ( a=0; a<NOF_ALARM_LIMIT_TYPES; a++ )
            ...
            if ( Param->parameter[i].limits[a].wire_number == selected_wire_number )
                ...
                s += SpaceString;
                StringTable.append( s, TEXT("ADVANCED_GRID_1_STRING") );
                break;
                }
            }
        }
    ...
    for ( i=0; i<MAX_FTII_LIMIT_SWITCHES; i++ )
        ...
        if ( LimitSwitch->wire_number(i) == selected_wire_number )
            {
            s += SpaceString;
            StringTable.append( s, TEXT("LIMIT_SWITCH_WIRE_STRING") );
            s += i + 1;
            }
        }
    ...
    if ( CurrentOutputType != GOOD_SHOT_TYPE )
        ...
        if ( CurrentGoodShotWire == selected_wire_number )
            {
            s += SpaceString;
            StringTable.append( s, TEXT("GOOD_SHOT_WIRE_STRING") );
            }
        }
    ...
    if ( CurrentOutputType != BINARY_VALVE_PULSE_OUTPUT_TYPE )
        ...
        if ( Machine->has_binary_valve() )
            ...
            if ( Machine->pulse_wire.int_value() == selected_wire_number )
                {
                s += SpaceString;
                StringTable.append( s, TEXT("BINARY_VALVE_PULSE_WIRE_STRING") );
                }
            }
        }
    ...
    if ( CurrentOutputType != ZERO_SPEED_OUTPUT_TYPE )
        ...
        if ( Part->zero_speed_output_enable )
            ...
            if ( int(Part->zero_speed_output_wire) == selected_wire_number )
                {
                s += SpaceString;
                StringTable.append( s, TEXT("ZERO_SPEED_OUTPUT_STRING") );
                }
            }
        }
...
StringTable.find( s, TEXT("CHOOSE_WIRE") );
w = hWnd;
w.set_title( s.text() );
w.refresh();
...
if ( Machine->suretrak_controlled )
    ...
    for ( i=0; i<NofAlreadyUsedWires; i++ )
        ...
        if ( ChosenWire == AlreadyUsedWires[i].wire_number )
            {
            StringTable.find( title, AlreadyUsedWires[i].id );
            StringTable.find( msg, TEXT("DONT_USE_WIRE_STRING") );
...
StringTable.find( s, TEXT("OUTPUT_WIRE_TITLE") );
w = hWnd;
w.set_title( s.text() );
w.refresh();
}
