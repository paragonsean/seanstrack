...
good_shot_active_low           = sorc.good_shot_active_low;
...
if ( status )
    ...
    if ( s.file_exists() )
        ...
        if ( n > GOOD_SHOT_REQUIRES_NO_WARNINGS_INDEX )
            ...
            if ( n > GOOD_SHOT_ACTIVE_LOW_INDEX )
                good_shot_active_low = sp[GOOD_SHOT_ACTIVE_LOW_INDEX].value.boolean_value();
            }
        }
    else
        {
...
if ( status )
    ...
    gsp[9].desc  = GOOD_SHOT_ACTIVE_LOW_DESC;
    gsp[9].value = boolean_to_ascii( good_shot_active_low );
    gsp.put( s );
    }
