...
resource_string( s, TOTAL_HOURS_STRING );
tmax.check( s.text() );

resource_string( s, TOTAL_HOURS_UP_STRING );
tmax.check( s.text() );

resource_string( s, TOTAL_HOURS_DOWN_STRING );
tmax.check( s.text() );
...
resource_string( s, DOWNBAR_TITLE_STRING );
s += DowntimeMachineName;

if ( CurrentCategory != NoCategory && !Config.show_all_subs )
    {
    resource_string( s1, CATEGORY_TITLE_STRING );
    s += s1;
...
resource_string( s, MAX_DOWNTIME_HOURS_STRING );
HoursX   = LeftX + max_title_pixels(cw) + 2*cw + pixel_width( MyDc, s.text() );

resource_string( s, MAX_DOWNTIME_PERCENT_STRING );
PercentX = HoursX + 2*cw + pixel_width( MyDc, s.text() );

resource_string( s, DOWNTIME_FROM_STRING );
s += Dh.start.text();
...
resource_string( s, HOURS_STRING );
show_text( HoursX, y, s.text() );
SetTextAlign( MyDc, TA_TOP | TA_LEFT );
...
----------- */
resource_string( s, TOTAL_HOURS_STRING );
show_title_and_hours( y, s.text(), Dh.total_seconds(), FALSE );
...
resource_string( s, TOTAL_HOURS_UP_STRING );
show_title_and_hours( y, s.text(), seconds, TRUE );

resource_string( s, MACH_UTILIZATION_STRING );
show_text( PercentX+cw, y, s.text() );
