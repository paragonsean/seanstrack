...
*                           FIXED_PARAM_CLASS                          *
*                                LBLINE                                *
* Note: You can't keep the pointer returned by this function. It can   *
* change the next time someone calles this function.                   *                         *
***********************************************************************/
...
s = parameter_name;
s += TabChar;
s.cat_w_char( min_value, TabChar );
s.cat_w_char( max_value, TabChar );
if ( is_fixed )
    id = YES_STRING;
else
    id = NO_STRING;

resource_string( s1, id );
s += s1;
...
/***********************************************************************
*                              GET_FROM_LINE                           *
*    parameter name  <Tab>  min  <Tab>  max  <Tab>  Yes/No             *
***********************************************************************/
...
if ( s.next_field(TabChar) )
    {
    if ( s.next_field(TabChar) )
        {
        copystring( min_value, s.text(), MAX_DOUBLE_LEN );
        if ( s.next_field(TabChar) )
            {
            copystring( max_value, s.text(), MAX_DOUBLE_LEN );
            is_fixed = false;
            if ( s.next_field(TabChar) )
                {
                resource_string( yes, YES_STRING );
                if ( s == yes )
                    is_fixed = true;
                }
            }
