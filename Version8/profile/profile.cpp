...
resource_string( s, sorc_id );
if ( !s.isempty() )
    TextOut( MyDc, (int) x, (int) y, s.text(), s.len() );
}
...
StringTable.find( s, TEXT("DOWNTIME_ENTRY_CAPTION") );
w = hWnd;
w.set_title( s.text() );
w.refresh();
}
...
if ( my_curve_types & HEAD_PRESSURE_CURVE )
    ...
    if ( i != NO_INDEX )
        ...
        StringTable.find( s, TEXT("HEAD_PRESSURE_LABEL_STRING") );
        copystring( PlotCurve[NofPlotCurves].label, s.text(), MAX_PLOT_LABEL_LEN );
        NofPlotCurves++;
        }
    }
...
if ( my_curve_types & ROD_PRESSURE_CURVE )
    ...
    if ( i != NO_INDEX )
        {
        PlotCurve[NofPlotCurves].curve_index = i;
        StringTable.find( s, TEXT("ROD_PRESSURE_LABEL_STRING") );
        copystring( PlotCurve[NofPlotCurves].label, s.text(), MAX_PLOT_LABEL_LEN );
        NofPlotCurves++;
        }
    }
...
if ( my_curve_types & DIFF_PRESSURE_CURVE )
    ...
    if ( i != NO_INDEX )
        {
        PlotCurve[NofPlotCurves].curve_index = i;
        StringTable.find( s, TEXT("DIFF_PRESSURE_LABEL_STRING") );
        copystring( PlotCurve[NofPlotCurves].label, s.text(), MAX_PLOT_LABEL_LEN );
        NofPlotCurves++;
        }
    }
...
for ( channel=0; channel<2; channel++ )
    ...
    if ( firstime )
        ...
        if ( IsPressureDisplay )
            {
            resource_string( s1, PRESSURE_TYPE_STRING );
            ...
            if ( true_channel != NO_FT_CHANNEL )
                true_channel--;
            if ( i == DIFFERENTIAL_CURVE_INDEX )
                StringTable.find( s1, TEXT("DIF_AXIS_LABEL_STRING") );
            else if ( i >= CHAN_1_ANALOG_CURVE_INDEX )
            ...
            if ( next_channel < NofPlotCurves )
                ...
                if ( true_channel != NO_FT_CHANNEL )
                    true_channel--;
                if ( i == DIFFERENTIAL_CURVE_INDEX )
                    StringTable.find( s2, TEXT("DIF_AXIS_LABEL_STRING") );
                else if ( i >= CHAN_1_ANALOG_CURVE_INDEX )
...
if ( i != NO_INDEX )
    ...
    resource_string( s1, VELOCITY_TYPE_STRING );
    y_axis_label( r, s1.text(), units_name(Ph.curves[i].units), LEFT_AXIS_TYPE );
    }
}
...
if ( i != NO_INDEX )
    ...
    if ( !Axis[x].is_on_hold )
        ...
        resource_string( s1, POSITION_TYPE_STRING );
        y_axis_label( r, s1.text(), units_name(Ph.curves[i].units), RIGHT_AXIS_TYPE );
        }
    }
}
...
for ( i=0; i<MAX_CURVES; i++ )
    ...
    if ( Ph.curves[i].type & POSITION_CURVE )
        ...
        resource_string( pos_x_label, POSITION_TYPE_STRING );
        pos_x_label += TEXT(" (");
        pos_x_label += units_name( Ph.curves[i].units );
        pos_x_label += RightParen;
        ...
        x_axis_label( r, pos_x_label.text() );
        }
...
if ( MyScreenType == ZABISH_SCREEN_TYPE || MyScreenType == STANDARD_SCREEN_TYPE )
    ...
    s += PartName;
    s += TEXT(" : ");
    StringTable.append( s, TEXT("SHOT_LABEL_STRING") );
    s += TEXT(" ");
    s += myshotname;
    s += TEXT(" : ");
    s += DateString;
...
if ( MyScreenType == ZABISH_SCREEN_TYPE || MyScreenType == STANDARD_SCREEN_TYPE )
    ...
    s = MachineName;
    s += TEXT(" : ");
    StringTable.append( s, TEXT("OFF_LINE_STRING") );
...
resource_string( s, SAVING_STRING );
temp_message( s.text() );
...
StringTable.find( s, TEXT("SET_TSL_CAPTION") );
w = hWnd;
w.set_title( s.text() );
w.refresh();
}
...
StringTable.find( s, TEXT("CHOOSE_PARAM_LIST_CAPTION") );
w = CornerParamDialogWindow;
w.set_title( s.text() );
w.refresh();
}
...
StringTable.find( s, TEXT("CHOOSE_DOWNTIME_MACHINE") );
w = hWnd;
w.set_title( s.text() );
w.refresh();
}
...
if ( !have_operator )
    {
    s = lb.selected_text();
    resource_string( s1, OPERATOR_NOT_FOUND );
    MessageBox( NULL, s.text(), s1.text(), MB_OK );
    return;
    }
...
StringTable.find( s, TEXT("OPERATOR_SIGNON") );
w = hWnd;
w.set_title( s.text() );
w.refresh();
}
...
if ( MyScreenType == DOWNBAR_SCREEN_TYPE )
    {
    StringTable.find( s, TEXT("CHOOSE_DOWNTIME_MACHINE_MENU_STRING") );
    PopupMenu.add( s.text(), CHOOSE_DOWNTIME_MACHINE_MENU );

    StringTable.find( s, TEXT("DOWNBAR_CONFIG_MENU_STRING") );
    PopupMenu.add( s.text(), DOWNBAR_CONFIG_MENU );
    }
...
if ( MyScreenType == TREND_SCREEN_TYPE )
    ...
    if ( !RealTime && !GridIsVisible )
        {
        StringTable.find( s, TEXT("TREND_REALTIME_MENU_STRING") );
        PopupMenu.add( s.text(), TREND_REALTIME_MENU );
        }

    StringTable.find( s, TEXT("TREND_ALLSHOTS_MENU_STRING") );
    PopupMenu.add( s.text(), TREND_ALLSHOTS_MENU );

    StringTable.find( s, TEXT("TREND_CONFIG_MENU_STRING") );
    PopupMenu.add( s.text(), TREND_CONFIG_MENU );

    if ( GridIsVisible )
        {
        StringTable.find( s, TEXT("NO_STAT_SUMMARY_MENU_STRING") );
        PopupMenu.add( s.text(), NO_STAT_SUMMARY_MENU );
        }
    else
        {
        StringTable.find( s, TEXT("STAT_SUMMARY_MENU_STRING") );
        PopupMenu.add( s.text(), STAT_SUMMARY_MENU );
        }
    }
...
if ( MyScreenType == STANDARD_SCREEN_TYPE || MyScreenType == ZABISH_SCREEN_TYPE )
    ...
    if ( zoomed() )
        {
        StringTable.find( s, TEXT("UNZOOM_MENU") );
        PopupMenu.add( s.text(), UNZOOM_MENU );
        }
    else
        {
        StringTable.find( s, TEXT("ZOOM_MENU") );
        PopupMenu.add( s.text(), ZOOM_MENU );
        }

    StringTable.find( s, TEXT("TREND_SCREEN_MENU_STRING") );
    PopupMenu.add( s.text(), TREND_SCREEN_MENU );

    StringTable.find( s, TEXT("BOX_SCREEN_MENU_STRING") );
    PopupMenu.add( s.text(), BOX_SCREEN_MENU );

    StringTable.find( s, TEXT("STATUS_SCREEN_MENU_STRING") );
    PopupMenu.add( s.text(), STATUS_SCREEN_MENU );

    StringTable.find( s, TEXT("DAY_PARAM_SCREEN_MENU_STRING") );
    PopupMenu.add( s.text(), DAY_PARAM_SCREEN_MENU );

    if ( InfoBoxType != XDCR_INFO_BOX && !zoomed() )
        {
        StringTable.find( s, TEXT("XDCR_MENU_STRING") );
        PopupMenu.add( s.text(), XDCR_MENU );
        }
    else if ( InfoBoxType == XDCR_INFO_BOX )
        {
        StringTable.find( s, TEXT("STD_SCREEN_MENU") );
        PopupMenu.add( s.text(), PROFILE_SCREEN_MENU );
        }
    if ( CurrentDisplay != POS_TIME_DISPLAY && !zoomed() )
        {
        StringTable.find( s, TEXT("POS_TIME_MENU") );
        PopupMenu.add( s.text(), POS_TIME_MENU );
        }
    if ( CurrentDisplay != POS_ONLY_DISPLAY && !IsHPM ) // && !zoomed() )
        {
        StringTable.find( s, TEXT("POS_ONLY_MENU") );
        PopupMenu.add( s.text(), POS_ONLY_MENU );
        }
    if ( CurrentDisplay != TIME_ONLY_DISPLAY )
        {
        StringTable.find( s, TEXT("TIME_ONLY_MENU") );
        PopupMenu.add( s.text(), TIME_ONLY_MENU );
        }
    }
else
    {
...
if ( MyScreenType == BOX_SCREEN_TYPE )
    {
    StringTable.find( s, TEXT("FONT_STATUS_SCREEN_MENU_STRING") );
    PopupMenu.add( s.text(), BOX_SCREEN_SET_FONT_MENU );
    }
...
if ( MyScreenType == STATUS_SCREEN_TYPE && CurrentPasswordLevel >= EditStatusScreenLevel )
    ...
    if ( Editing_Status_Screen )
        {
        StringTable.find( s, TEXT("FONT_STATUS_SCREEN_MENU_STRING") );
        PopupMenu.add( s.text(), FONT_STATUS_SCREEN_MENU );

        StringTable.find( s, TEXT("SAVE_STATUS_SCREEN_MENU_STRING") );
        PopupMenu.add( s.text(), SAVE_STATUS_SCREEN_MENU );
        }
    else
        {
        StringTable.find( s, TEXT("EDIT_STATUS_SCREEN_MENU_STRING") );
        PopupMenu.add( s.text(), EDIT_STATUS_SCREEN_MENU );
        }
    }
...
resource_string( s, ALARM_LIMITS_STRING );
cell.SetText( s.text() );
...
center_temp_message();
resource_string( s, ADDING_TO_BACKUP_STRING );
temp_message( s.text() );
...
resource_string( s, TIMEOUT_STRING );
s += SpaceString;
s += machine_name;
...
StringTable.find( s, TEXT("CHOOSE_LANG_TITLE") );
w = hWnd;
w.set_title( s.text() );
w.refresh();
}
...
StringTable.find( s, TEXT("LIST_OF_MA") );
w = hWnd;
w.set_title( s.text() );
w.refresh();
}
...
/***********************************************************************
*                      APPEND_DATE_AND_SHIFT_STRING                    *
*                          "MMDDYYYY Shift 10"                         *
***********************************************************************/
static void append_date_and_shift_string( STRING_CLASS & dest, TIME_CLASS & t, int32 shift )
{
dest.cat_w_char( t.mmddyyyy(), SpaceChar );
StringTable.append( dest, TEXT("SHIFT_STRING") );
dest += SpaceChar;
dest += (int) shift;
}
...
resource_string( s, MACHINE_INTRO_STRING );
s += MachineName;
resource_string( s1, PART_INTRO_STRING );
s += s1;
s += PartName;
if ( by_date )
    {
...
if ( by_shift )
    {
    s += TwoSpacesString;
    resource_string( s1, SHIFT_STRING );
    s += s1;
    ...
    as.get_counts();
    s += TwoSpacesString;
    resource_string( s1, DOWNTIME_FROM_STRING );
    s += s1;
    append_date_and_shift_string( s, as.start_time(), as.start_shift_number() );
    resource_string( s1, DOWNTIME_TO_STRING );
    s += s1;
    append_date_and_shift_string( s, as.end_time(), as.end_shift_number() );
    }
...
if ( is_alarm )
    {
    resource_string( s, LOW_ALARM_STRING );
    tmax.check( s.text() );
    resource_string( s, HIGH_ALARM_STRING );
    tmax.check( s.text() );
    }
else
    {
    resource_string( s, LOW_WARNING_STRING );
    tmax.check( s.text() );
    resource_string( s, HIGH_WARNING_STRING );
    tmax.check( s.text() );
    }
resource_string( s, QUANTITY_BELOW_STRING );
tmax.check( s.text() );
resource_string( s, QUANTITY_ABOVE_STRING );
tmax.check( s.text() );
...
resource_string( s, PARAMETER_STRING );
text_out( x, y, s.text() );
...
resource_string( s, rid );
text_out( x_low_limit, y, s.text() );
resource_string( s, QUANTITY_BELOW_STRING );
text_out( x_low_value, y, s.text() );
...
resource_string( s, rid );
text_out( x_high_limit, y, s.text() );

resource_string( s, QUANTITY_ABOVE_STRING );
text_out( x_high_value, y, s.text() );
...
resource_string( s, TOTAL_SHOTS_INTRO_STRING );
...
if ( is_alarm )
    {
    resource_string( s, TOTAL_ALARM_SHOTS_INTRO_STRING );
    i = as.total_alarm_count();
    }
else
    {
    i = as.total_warning_count();
    resource_string( s, TOTAL_WARNING_SHOTS_INTRO_STRING );
    }
...
if ( by_shift )
    {
    resource_string( s, SHIFT_STRING );
    s += int32toasc( CurrentAlarmShift );
    cp = s.text();
    }
else
...
StringTable.find( s, TEXT("ALARM_SUMMARY") );
w = hWnd;
w.set_title( s.text() );
w.refresh();
}
        ...
        switch ( id )
            ...
            case BY_SHIFT_RADIO:
                if ( cmd == BN_CLICKED )
                    ...
                    resource_string( s, PREV_SHIFT_STRING );
                    s.set_text( w );
                    EnableWindow( w, TRUE );

                    w = GetDlgItem( hWnd, NEXT_BUTTON );
                    resource_string( s, NEXT_SHIFT_STRING );
                    s.set_text( w );
                    enable_prev_next_buttons( hWnd );
                    get_alarm_data( hWnd );
                    }
            ...
            case BY_DAY_RADIO:
                if ( cmd == BN_CLICKED )
                    {
                    w = GetDlgItem( hWnd, PREV_BUTTON );
                    resource_string( s, PREV_DAY_STRING );
                    s.set_text( w );
                    EnableWindow( w, TRUE );

                    w = GetDlgItem( hWnd, NEXT_BUTTON );
                    resource_string( s, NEXT_DAY_STRING );
                    s.set_text( w );
                    enable_prev_next_buttons( hWnd );
                    get_alarm_data( hWnd );
                    }
            ...
            case ALL_DATA_RADIO:
                if ( cmd == BN_CLICKED )
                    {
                    w = GetDlgItem( hWnd, PREV_BUTTON );
                    resource_string( s, PREV_STRING );
                    s.set_text( w );

                    w = GetDlgItem( hWnd, NEXT_BUTTON );
                    resource_string( s, NEXT_STRING );
                    s.set_text( w );
                    enable_prev_next_buttons( hWnd );
                    get_alarm_data( hWnd );
                    }
            ...
            case ALARM_RADIO:
                if ( cmd == BN_CLICKED )
                    {
                    resource_string( s, TOTAL_ALARM_SHOTS_STRING );
                    s.set_text( hWnd, ALARM_OUT_TITLE_TBOX );
                    resource_string( s, LOW_ALARM_STRING );
                    s.set_text( hWnd, ALARM_LOW_TITLE_TBOX );
                    resource_string( s, HIGH_ALARM_STRING );
                    s.set_text( hWnd, ALARM_HIGH_TITLE_TBOX );
                    get_alarm_data( hWnd );
                    }
                return TRUE;
            ...
            case WARNING_RADIO:
                if ( cmd == BN_CLICKED )
                    {
                    resource_string( s, TOTAL_WARNING_SHOTS_STRING );
                    s.set_text( hWnd, ALARM_OUT_TITLE_TBOX );
...
StringTable.find( s, TEXT("ALARM_LOG_SETUP") );
w = hWnd;
w.set_title( s.text() );
w.refresh();
}
...
for ( i=0; i<n; i++ )
    ...
    if ( id == helplist[i].id )
        {
        resource_string( s, helplist[i].resource_id );
        s.set_text( MenuHelpWindow );
        return;
        }
    }
