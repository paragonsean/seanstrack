...
SetColWidth( 0, w[0] );
resource_string( s, PW_COL_0_STRING );
cell.SetText( s.text() );
cell.SetBorder( UG_BDR_RTHIN | UG_BDR_BTHIN | UG_BDR_TTHIN );
SetCell( 0, -1, &cell );

SetColWidth( 1, w[1] );
resource_string( s, PW_COL_1_STRING );
cell.SetText( s.text() );
SetCell( 1, -1, &cell );

SetColWidth( 2, w[2] );
resource_string( s, PW_COL_2_STRING );
cell.SetText( s.text() );
SetCell( 2, -1, &cell );
...
while ( c.next() )
    ...
    if ( !c.is_this_computer() )
        ...
        if ( c.is_present() )
            ...
            {
			StringTable.find( s, TEXT("SOME_VERSIONS_TOO_OLD_STRING") );
            StringTable.find( dest, TEXT("CANNOT_COMPLY_STRING") );
...
StringTable.find( s, TEXT("EDIT_PW") );
s.set_text( hWnd );
...
static ID_CONTEXT static_control[] = {
    ...
    { MULTIPART_RUNLIST_CHOOSE_WIRES_METHOD_TB, OPTIONS_MULTIPART_RUNLIST_CHOOSE_WIRES_HELP }
    };
...
static RESOURCE_TEXT_ENTRY rsn[] = {
    ...
    { MULTIPART_RUNLIST_CHOOSE_WIRES_METHOD_TB, TEXT("MULTIPART_RUNLIST_CHOOSE_WIRES_METHOD") }
    };
...
static UINT box[] = {
    ...
    MULTIPART_RUNLIST_CHOOSE_WIRES_METHOD_EB
    };
...
StringTable.find( s, TEXT("EDIT_OPTIONS") );
s.set_text( hWnd );
