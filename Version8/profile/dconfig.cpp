...
for ( tbox=CHANNEL_1_TBOX; tbox<VELOCITY_TBOX; tbox++ )
    ...
    if ( channel == CurrentPart.head_pressure_channel )
        resource_string( s, HEAD_PRES_STRING );
    else if ( channel == CurrentPart.rod_pressure_channel )
        resource_string( s, ROD_PRES_STRING );
    else
...
resource_string( s, DIF_PRES_STRING );
...
if ( id == SAVE_DEFAULT_DISPLAY_CONFIG_PB )
    ...
    if ( !s.directory_exists() )
        ...
        if ( !s.create_directory() )
            {
            resource_string( s1, CANT_CREATE_DEST_DIR_STRING );
            MessageBox( 0, s.text(), s1.text(), MB_OK | MB_SYSTEMMODAL | MB_ICONERROR );
            return;
            }
        is_newdir = TRUE;
        }
