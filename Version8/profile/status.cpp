...
*                            GET_SHOT_INFO                             *
***********************************************************************/
void STATUS_SCREEN_ROW_ENTRY::get_shot_info()
{
COMPUTER_CLASS c;
NAME_CLASS s;
PLOOKUP_CLASS p;
...
p.init( pn.computer, pn.machine, pn.part );
if ( p.get() )
    {
    p.get_last_shot_name( shotname );
    p.get_time( shottime );
    }
}
...
s.get_profile_ii_name( pn.computer, pn.machine, pn.part, shotname );
nof_parms = read_parameters_from_parmlist( pa, s );
}
...
s.get_downtime_dbname( pn.computer, pn.machine );
...
*                               FIND_ROW                               *
***********************************************************************/
static int find_row( STRING_CLASS & machine_name )
{
int i;

for ( i=0; i<NofRows; i++ )
    {
    if ( Row[i].pn.machine == machine_name )
        return i;
    }
...
*                            GET_PART_INFO                             *
***********************************************************************/
static BOOLEAN get_part_info( STATUS_SCREEN_ROW_ENTRY & r )
{
if ( !current_part_name(r.pn.part, r.pn.computer, r.pn.machine) )
    return FALSE;

r.get_shot_info();
if ( NeedAlarms )
    r.get_alarm_summary_info();

r.read_alarm_info();
if ( NeedDowntime )
    {
    r.get_downtime_status();
    r.read_downtimes();
    }
...
for ( i=0; i<NofColumns; i++ )
    ...
    if ( Column[i].type == MACHINE_NAME_TYPE )
        ...
        s = Row[r].pn.part;
        if ( s.isempty() )
            resource_string( s, OFF_LINE_STRING );
        TextOut( dc, x, y, s.text(), s.len() );
        }
    else if ( Column[i].type == SHOT_NUMBER_TYPE )
        {
        TextOut( dc, x, y, Row[r].shotname.text(), Row[r].shotname.len() );
        }
        ...
        if ( Row[r].alarm_state == ALARM_STATE )
            ...
            {
            StringTable.find( s, TEXT("OK_STRING") );
            TextOut( dc, x, y, s.text(), s.len() );
            }
        ...
        if ( s.upsize(MAX_DOUBLE_LEN) )
            ...
            if ( pi >= 0 && pi < Row[r].nof_parms )
                ...
                rounded_double_to_tchar( s.text(), (double) Row[r].pa[pi].real_value() );
                TextOut( dc, x, y, s.text(), s.len() );
...
s.get_status_screen_config_file_name();
...
for ( i=0; i<NofRows; i++ )
    ...
    if ( Row[i].pn.machine == machine_name )
        {
        c.set_online_status( Row[i].pn.computer, TRUE );
        if ( Row[i].pn.part.isempty() )
            get_part_info( Row[i] );
...
for ( r=0; r<NofRows; r++ )
    ...
    if ( c.online_status(Row[r].pn.computer) )
        {
        if ( Row[r].pn.part.isempty() )
            {
            have_data_to_display = get_part_info( Row[r] );
            }
        else if ( this_is_a_new_shift )
            {
            if ( NeedAlarms )
...
***********************************************************************/
void status_screen_down_data_change( STRING_CLASS & machine_name, TCHAR * cat, TCHAR * subcat )
{
COMPUTER_CLASS c;
int32 r;
...
c.set_online_status( Row[r].pn.computer, TRUE );
...
if ( f.open_for_read(s.text()) )
    ...
    while ( NofRows<n )
        ...
        if ( MachList.find(cp) )
            {
            if ( MachList.computer_name().isempty() )
                {
                continue;
                }
            else
                {
                Row[NofRows].pn.computer = MachList.computer_name();
                Row[NofRows].pn.machine  = cp;
                if ( c.find(Row[NofRows].pn.computer) )
                    {
                    if ( c.online_status() )
                        {
                        if ( c.is_present() )
                            get_part_info( Row[NofRows] );
                        }
                    }
                NofRows++;
                }
        ...
        for ( col=0; col<NofColumns; col++ )
            ...
            if ( p.x >= r.left && p.x <= r.right )
                ...
                if ( Column[col].type >= DOWNTIME_STATE_TYPE )
                    {
                    d.write();
                    DowntimeComputerName = Row[i].pn.computer;
                    DowntimeMachineName  = Row[i].pn.machine;
                    update_screen_type( DOWNBAR_SCREEN_TYPE );
                    return;
                    }
                else if ( Column[col].type <= ALARM_STATE_TYPE )
                    {
                    ComputerName = Row[i].pn.computer;
                    MachineName  = Row[i].pn.machine;
                    PartName     = Row[i].pn.part;
                    ShotName     = Row[i].shotname;
