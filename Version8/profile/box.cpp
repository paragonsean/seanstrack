...
{
const int TIPTEXT_STRING_LEN = (2*DOWNCAT_NAME_LEN) + 2;
...
if ( state == HUMAN_DOWN_STATE )
    ...
    {
    resource_string( s, NO_DOWN_TT_STRING + state );
    copystring( buf, s.text(), TIPTEXT_STRING_LEN );
    }
...
for ( i=0; i<NofStatusBoxes; i++ )
    ...
    if ( se->have_part && ToolTipWindow )
        ...
        resource_string( s, MACHINE_TT_STRING );
        ti.lpszText = s.text();
        ti.rect     = *se->machine_rect();
        SendMessage( ToolTipWindow, TTM_SETTOOLINFO, 0, (LPARAM) (LPTOOLINFO) &ti );

        ti.uId++;
        resource_string( s, PART_TT_STRING );
        ti.lpszText = s.text();
        ti.rect     = *se->part_rect();
        SendMessage( ToolTipWindow, TTM_SETTOOLINFO, 0, (LPARAM) (LPTOOLINFO) &ti );

        ti.uId++;
        resource_string( s, SHOT_TT_STRING );
        ti.lpszText = s.text();
        ti.rect     = *se->shot_rect();
        SendMessage( ToolTipWindow, TTM_SETTOOLINFO, 0, (LPARAM) (LPTOOLINFO) &ti );

        ti.uId++;
        resource_string( s, DATE_TT_STRING );
        ti.lpszText = s.text();
        ...
        ti.uId++;
        resource_string( s, ALARM_TT_STRING );
        ti.lpszText = s.text();
