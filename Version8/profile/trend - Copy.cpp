...
short_shotname is the ShotName without the leading zeros. I need this to compare
with the saved names so I don't put all those zeros on the screen.
--------------------------------------------------------------------------------- */
i = XAxis.set_count() - 1;
if ( ShotName > XAxis[i].shot_name )
    ...
    if ( t.open_for_read(s) )
        ...
        if ( t.get_next_key_match(1) )
            ...
            short_shotname = ShotName;
            short_shotname.remove_leading_chars( ZeroChar );
            XAxis.left_shift( short_shotname, st );
            Parameters.rewind();
            for ( i=0; i<Plots.count(); i++ )
                {
                Parameters.next();
