...
while ( i < NofCornerParameters )
    ...
    if ( ci >= 0 && ci < MAX_PARMS )
        ...
        if ( p->input.type != NO_PARAMETER_TYPE )
            ...
            TextOut( dc, x+title_position[3], y, Parms[ci].text(), Parms[ci].len() );
...
resource_string( ParameterListTitle[0], PARAMETER_STRING );
resource_string( ParameterListTitle[1], id[LowLimitType] );
resource_string( ParameterListTitle[2], id[HighLimitType] );
resource_string( ParameterListTitle[3], PARAMETER_VALUE_STRING );
resource_string( ParameterListTitle[4], UNITS_TITLE_STRING );
}
