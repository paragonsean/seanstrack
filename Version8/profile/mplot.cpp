...
*                            NEXT_SET                                  *
***********************************************************************/
BOOLEAN DATA_ARRAY_CLASS::next_set()
{
...
*                              SET_TARGET                              *
***********************************************************************/
void SINGLE_PLOT_CLASS::set_target( TCHAR * new_target )
{
target = new_target;
}
...
*                              SET_TARGET                              *
***********************************************************************/
void SINGLE_PLOT_CLASS::set_target( STRING_CLASS & new_target )
{
target = new_target;
}
...
if ( title.len() > 0 )
    ...
    if ( load_bigfont(dc, ArialFontName.text(), r, title.text()) )
        ...
        xp = (r.left + r.right) / 2;
        yp = r.top + character_height(dc);
        yp += r.bottom;
        yp /= 2;
        TextOut( dc, xp, yp, title.text(), title.len() );
...
if ( alarm_max != alarm_min )
    ...
    if ( mypen )
        ...
        yp = (int32) (a + b*alarm_max);
        draw_line( dc, r.left, yp, r.right, yp );

        yp = (int32) (a + b*alarm_min);
        draw_line( dc, r.left, yp, r.right, yp );
...
if ( warning_max != warning_min )
    ...
    if ( mypen )
        {
        oldpen = (HPEN) SelectObject( dc, mypen );
        yp = (int32) (a + b*warning_max);
        draw_line( dc, r.left, yp, r.right, yp );

        yp = (int32) (a + b*warning_min);
        draw_line( dc, r.left, yp, r.right, yp );
        SelectObject( dc, oldpen );
        DeleteObject( mypen );
        }
...
if ( !target.isempty() )
    {
    mypen = CreatePen( PS_DASH, 1, NoAlarmColor );
    if ( mypen )
        {
        oldpen = (HPEN) SelectObject( dc, mypen );
        yp = (int32) (a + b*target.float_value());
        draw_line( dc, r.left, yp, r.right, yp );
...
if ( mypen )
    ...
    xp = XAxis.index();
    xp = XAxis.x_pos_from_index( xp );

    yp = (int32) ( a + b*average() );
    ...
    while ( next() )
        ...
        yp = (int32) ( a + b*average() );
        if ( yp < r.top )
            MoveToEx( dc, xp, r.top, 0 );
        else if ( yp > r.bottom )
            MoveToEx( dc, xp, r.bottom, 0 );
        else
            LineTo( dc, xp, yp );
        }
...
if ( mypen )
    ...
    yp  = r.top;
    ry = max_value;
    draw_line( dc, xp, yp, r.left, yp );

    yp += (ch*3)/4;
    s = fixed_string( ry, left_places, right_places );
    TextOut( dc, xp, yp, s.text(), s.len() );
    ...
    np = ((r.bottom - ch) - (r.top + ch)) / ( ch + ch/2 );
    if ( np > 0 )
        {
        ry = min_value;
        delta = (max_value - min_value) / float(np+1);
        while ( np > 0 )
            {
            ry += delta;
            yp = (int32) ( a + b*ry );
            draw_line( dc, xp, yp, r.left, yp );
            s = fixed_string( ry, left_places, right_places );
            TextOut( dc, xp, yp+ch/2, s.text(), s.len() );
            np--;
            }
        }
