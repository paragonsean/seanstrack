...
if (  t.open_for_read(s) )
    ...
    if ( t.nof_recs() > 0 )
        ...
        if ( t.get_rec(r1) )
            ...
            resource_string( s, STAT_N_EQUALS_STRING );
            s += int32toasc( NofPointsRead );
            resource_string( s1, N_PER_GROUP_STRING );
            s += s1;
...
tmax.init( MyDc );
resource_string( s, ALARM_SHOT_LABEL_STRING );
tmax.check( s.text() );

resource_string( s, ALARM_DATE_LABEL_STRING );
tmax.check( s.text() );

resource_string( s, ALARM_TIME_LABEL_STRING );
tmax.check( s.text() );
...
resource_string( s, SHOT_NUMBER_STRING );
XAxis.set_title( s.text() );
XAxis.set_rect( r );
...
if ( TcConfig.chart_type == XBAR_CHART_TYPE && TcConfig.nto_average > 1 )
    {
    resource_string( s1, N_EQUALS_STRING );
    s += s1;
    s += int32toasc( TcConfig.nto_average );
    resource_string( s, END_PARENS_STRING );
    s += s1;
    }
