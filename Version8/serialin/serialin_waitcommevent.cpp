...
if (!SetCommMask(CommPortHandle, EV_RXCHAR))
   return;

buf.null();
while ( true )
    {
    if ( !WaitCommEvent(CommPortHandle, &dwCommEvent, NULL) )
        break;

    while ( true )
        {
        if ( !ClearCommError(CommPortHandle, &errors, &stat) )
            {
            MessageBox( MainDialog.handle(), "ClearCommError", "Error", MB_OK );
            return;
            }
        n = (int) stat.cbInQue;
        while (n)
...
switch (msg)
    ...
    case WM_INITDIALOG:
        ...
        MessageLb.init( hWnd, MESSAGE_LB );
        return TRUE;
