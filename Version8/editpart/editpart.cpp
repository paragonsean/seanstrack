...
{
static TCHAR name[PARAMETER_NAME_LEN+1];
STRING_CLASS s;
STRING_CLASS sc;
int          i;
...
if ( type == PARAMETER_MARK )
    ...
    if ( i >= 0 && i < CurrentParam.count() )
        lstrcpy( name, CurrentParam.parameter[i].name );
    }
else
    {
    ...
    sc += (int) number;
    StringTable.find( s, sc );
    copystring( name, s.text(), PARAMETER_NAME_LEN );
    }
...
for ( i=0; i<MAX_FT2_CHANNELS; i++ )
    ...
    if ( !a.exists(mysensor) )
        {
        StringTable.find( s, TEXT("NO_ASENSOR_START") );
        s += i;
        StringTable.find( s1, TEXT("NO_ASENSOR_END") );
        s += s1;
        StringTable.find( s1, TEXT("NO_ASENSOR_LINE2") );
        MessageBox( 0, s1.text(), s.text(), MB_OK | MB_SYSTEMMODAL );
...
switch ( msg )
    ...
    case WM_DBINIT:
        if ( mrl.isempty() )
            StringTable.find( mrl, TEXT("MULTIPART_RUNLIST") );
        ...
        if ( CurrentMachine.multipart_runlist_settings.enable )
            mrl.set_text( hWnd, COPYTO_WARMUP_PART_CHECKBOX );
        return TRUE;
...
set_checkbox( ShotEventsDialog, GOOD_SHOT_ACTIVE_LOW_XBOX,  CurrentPart.good_shot_active_low );
}
    ...
    case WM_COMMAND:
        switch ( id )
            ...
            case GOOD_SHOT_NO_WARNINGS_XBOX:
                ...
                return TRUE;
            case GOOD_SHOT_ACTIVE_LOW_XBOX:
                CurrentPart.good_shot_active_low = is_checked( hWnd, GOOD_SHOT_ACTIVE_LOW_XBOX );
                return TRUE;
            }
        break;

    case WM_CLOSE:
        if ( ShotEventsDialog != 0 )
            {
            ShotEventsDialog = 0;
            EndDialog( hWnd, 0 );
            }
...
static RESOURCE_TEXT_ENTRY rlist[] =
    ...
    { ALARM_LIMITS_STATIC,      TEXT( "ALARM_LIMITS_STRING" ) },
...
static RESOURCE_TEXT_ENTRY rlist[] =
    {
    { EDIT_ANALOG_SENSORS_BUTTON, TEXT( "ANALOG_SENSOR_SETUP" ) },
...
StringTable.find( s, TEXT("SAVING") );
temp_message( s.text() );
...
for ( i=0; i<Nof_Editable_Channels; i++ )
    ...
    if ( NeedToCheckChannels5and7 && (i==4) )
        {
        StringTable.find( s, TEXT("LVDT") );
        label.set( i, s.text() );
        }
    else if ( NeedToCheckChannels5and7 && (i==6) )
        {
        StringTable.find( TEXT("CMD_STRING") );
        label.set( i, s.text() );
        }
...
answer_button = stringtable_okcancel_box( TEXT("WISH_TO_CONTINUE"), TEXT("ABOUT_TO_DELETE_STRING") );
