...
for ( i=0; i<NofMachines; i++ )
    ...
    if ( MachineList[i].has_changed )
        ...
        poke_data( DDE_MONITOR_TOPIC, ItemList[MONITOR_SETUP_INDEX].name, MachineList[i].machine.name.text() );
...
s.set_text( MonitorWindow, base+SKIP_LABEL );
}
...
if ( !ShotsString )
    {
    StringTable.find( s, TEXT("SHOTS_STRING") );
    ShotsString = s.release();
    }

if ( !SecString )
    {
    StringTable.find( s, TEXT("SEC_STRING") );
    SecString = s.release();
    }
}
